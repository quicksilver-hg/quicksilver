...
**When configuring Quicksilver, CMake by default will ignore the depends output.** In
...
created. To use it during configuring Quicksilver:
...
- `NO_VAULT`: Don't download/build/cache libs needed to enable the vault
...
If some packages are not built, for example `make NO_VAULT=1`, the appropriate CMake cache
variables will be set when generating the Quicksilver buildsystem. In this case, `-DENABLE_VAULT=OFF`.
