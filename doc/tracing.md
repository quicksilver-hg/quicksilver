# User-space, Statically Defined Tracing (USDT) for Quicksilver

Quicksilver includes statically defined tracepoints to allow for more
...
**Note**: Quicksilver uses temporary clones of the _main_ UTXO cache
...
changes to a temporary cache. Similarly, relay pool consistency checks, which are
frequent on sandbox, also apply the UTXO set changes to a temporary cache.
...
Arguments passed:
1. Vault name as `pointer to C-style string`
...
3. The position of the change output as an `int32`
...
This provider carried the upstream transaction-pool name until the relay pool
rename, and renaming it is a breaking change for any third-party bpftrace or
bcc script that attaches by name. It was made anyway, before the first public
release, for one reason: the provider was the last place in the system still
using the old vocabulary, and a tracing interface that disagrees with every log
line, RPC method and flag around it teaches the reader something false about
the code they are tracing. The usual counter-argument — that the old provider
name is widely recognised in existing tooling — carries no weight for a chain
with no users yet, which is the same reasoning that justified renaming the
RPC methods. Deciding it later would have meant coordinating across every
operator running a tracing script instead of across nobody. The full old-to-new
mapping is in `doc/release-notes.md`.

`test/functional/interface_usdt_relaypool.py` guards these names. It requires
root for BPF, so it skips on unprivileged runs; the source-level gate that runs
everywhere is `test/lint/lint-quicksilver-relaypool-residue.py`, which fails on
any reintroduced old spelling including a `TRACEPOINT()` site.

#### Tracepoint `relaypool:added`

Is called when a transaction is added to the node's relay pool. Passes information
...
Is called when a transaction is removed from the node's relay pool. Passes information
about the transaction.
...
4. Transaction relay pool entry time (epoch) as `uint64`

#### Tracepoint `relaypool:replaced`

Is called when a transaction in the node's relay pool is getting replaced by another.
...
3. Replaced transaction relay pool entry time (epoch) as `uint64`
4. Replacement transaction ID or package hash as `pointer to unsigned chars` (i.e. 32 bytes in little-endian)
5. Replacement transaction virtual size as `int32`
6. `bool` indicating if the argument 4. is a transaction ID or package hash (true if it's a transaction ID)

Note: In cases where a replacement transaction or package replaces multiple
existing transactions in the relay pool, the tracepoint is called once for each
...
Is called when a transaction is not permitted to enter the relay pool. Passes
...
## Adding tracepoints to Quicksilver
...
The `readelf` tool can be used to display the USDT tracepoints in Quicksilver.
