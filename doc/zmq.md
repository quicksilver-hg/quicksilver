...
The Quicksilver daemon can be configured to act as a trusted "border
router", implementing the Quicksilver wire protocol and relay, making
...
The ZeroMQ feature in Quicksilver requires the ZeroMQ API >= 4.0.0
...
terminator). These options can also be provided in quicksilver.conf.
...
`sequence`: the body is structured as the following based on the type of message:
    ...
    <32-byte hash>R<8-byte LE uint> : Transactionhash removed from relay pool for non-block inclusion reason
    <32-byte hash>A<8-byte LE uint> : Transactionhash added relay pool

Where the 8-byte uints correspond to the relay pool sequence number.

`rawtx`: Notifies about all transactions, both when they are added to the relay pool or when a new block arrives. This means a transaction could be published multiple times. First, when it enters the relay pool and then again in each block that includes it. The messages are ZMQ multipart messages with three parts. The first part is the topic (`rawtx`), the second part is the serialized transaction, and the last part is a sequence number (representing the message count to detect lost messages).

    | rawtx | <serialized transaction> | <uint32 sequence number in Little Endian>

`hashtx`: Notifies about all transactions, both when they are added to the relay pool or when a new block arrives. This means a transaction could be published multiple times. First, when it enters the relay pool and then again in each block that includes it. The messages are ZMQ multipart messages with three parts. The first part is the topic (`hashtx`), the second part is the 32-byte transaction hash, and the last part is a sequence number (representing the message count to detect lost messages).
...
number, which is also published along with all relay pool events. This
is a different sequence value than in ZMQ itself in order to allow a total
ordering of relay pool events to be constructed.
