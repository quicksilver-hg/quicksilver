...
If you do need the vault enabled (`-DENABLE_VAULT=ON`), use vaults backed by SQLite. Vaults from local development builds can be incompatible with release binaries and should be used with caution on mainnet.
...
```sh
cmake --build build --target quicksilver-daemon quicksilver-cli
cmake --build build --target quicksilver
cmake --build build --target bench_quicksilver
```
...
When rebasing, we often want to do a "dummy rebase," whereby we are not rebasing over an updated main branch but rather over the last common commit with main. This might be useful for rearranging commits, `rebase --autosquash`ing, or `rebase --exec`ing without introducing conflicts that arise from an updated main branch. In these situations, we can use `git merge-base` to identify the last common commit with main, and rebase off of that.

To squash in `git commit --fixup` commits without rebasing over an updated main branch, we can do the following:

```sh
git rebase -i --autosquash "$(git merge-base main HEAD)"
```

To execute `cmake --build build && ctest --test-dir build` on every commit since last diverged from main, but without rebasing over an updated main branch, we can do the following:
```sh
git rebase -i --exec "cmake --build build && ctest --test-dir build" "$(git merge-base main HEAD)"
```
...
[remote "upstream-pull"]
        ...
        url = git@github.com:quicksilver-hg/quicksilver.git
```
...
You can do:
```sh
git range-diff main previously-reviewed-head new-head
```
