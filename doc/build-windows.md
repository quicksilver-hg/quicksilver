...
Below are some notes on how to build Quicksilver for Windows.

The options known to work for building Quicksilver on Windows are:
...
Note that for WSL the Quicksilver source path MUST be somewhere in the default mount file system, for
example /usr/src/quicksilver, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\quicksilver`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/quicksilver
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/quicksilver --strip
```
