...
Quicksilver has an internal benchmarking framework, with benchmarks
for cryptographic algorithms (e.g. SHA1, SHA256, SHA512, RIPEMD160, Poly1305, ChaCha20), rolling bloom filter, coins selection,
thread queue, vault balance.

Running
---------------------

For benchmarking, you only need to compile `bench_quicksilver`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_quicksilver

After compiling Quicksilver, the benchmarks can be run with:

    build/bin/bench_quicksilver
...
- Initial block download (Cost: slow IBD results in full node operation being
  less accessible)
- Block template creation (Cost: slow block template creation may delay miners
  building on fresh tips)
...
tests](fuzzing.md) are better suited for this purpose, as they are
