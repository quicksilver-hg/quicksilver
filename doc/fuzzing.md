...
To quickly get started fuzzing Quicksilver using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/quicksilver-hg/quicksilver.git
$ cd quicksilver/
$ cmake --preset=libfuzzer
# macOS users: If you have a problem with this step then make sure to read
...
## Overview of Quicksilver fuzzing
...
[`process_message`](../src/test/fuzz/process_message.cpp) is a fuzzing
harness for the [`ProcessMessage(...)` function
(`net_processing`)](../src/net_processing.cpp). The available fuzzing
harnesses are found in [`src/test/fuzz/`](../src/test/fuzz/).
...
It is possible to specify `quicksilverd` arguments to the `fuzz` executable.
...
an instrumentalized libc++, then using it to build [Quicksilver dependencies
from source](../depends/README.md) and finally the Quicksilver fuzz binary
...
# Fuzzing Quicksilver using afl++

## Quickstart guide

To quickly get started fuzzing Quicksilver using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Quicksilver using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Quicksilver using [Honggfuzz](https://github.com/google/honggfuzz):
...
Quicksilver does not currently document a public OSS-Fuzz project or dashboard
in this repository.

Report Quicksilver security issues using this repository's
