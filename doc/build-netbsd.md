...
This guide describes how to build quicksilver-daemon, command-line utilities, and GUI on NetBSD.
...
It is not necessary to build vault functionality to run quicksilver-daemon or the GUI.

###### Vault Support

`sqlite3` is required to enable support for vaults.
...
There are many ways to configure Quicksilver. Here is an example that
explicitly disables the vault and GUI:

```bash
cmake -B build -DENABLE_VAULT=OFF -DBUILD_GUI=OFF
```
