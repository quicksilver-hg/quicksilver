...
By default, Quicksilver allows up to 125 connections to different peers, 11 of
...
limits are tiny. Keep in mind that Quicksilver's trustless model works best if you are
...
- Vault transaction rebroadcast is disabled unless `-vaultbroadcast` is set explicitly.
- It sets the flag "-vaultbroadcast" to be "0", only if it is currently unset.
  Doing so disables the automatic broadcasting of transactions from vault. Not
  relaying other's transactions could hurt your privacy if used while a vault
