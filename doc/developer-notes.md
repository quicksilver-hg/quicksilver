...
- [Developer Notes](#developer-notes)
    ...
    - [Development tips and tricks](#development-tips-and-tricks)
        ...
        - [`debug.log`](#debuglog)
        - [Public test network and sandbox modes](#public-test-network-and-sandbox-modes)
...
- [Development guidelines](#development-guidelines)
    - [General Quicksilver](#general-quicksilver)
    - [Vault](#vault)
    - [General C++](#general-c)
...
Quicksilver uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
### Public test network and sandbox modes

If you are testing multi-machine code that needs to operate across the internet,
you can run with the `-publictest` config option to test with "play coins" on the
defined Quicksilver public test network. A fixed onion seed ships for publictest;
nodes can use it when Tor proxy access is configured. Operators can still bypass
the fixed seed with an explicit peer configured through `-addnode`, `-connect`,
or `-seednode`.

If you are testing something that can run on one machine, run with the
`-sandbox` option. In sandbox mode, blocks can be created on demand;
see [test/functional/](../test/functional) for tests that run in `-sandbox` mode.

### DEBUG_LOCKORDER

Quicksilver is a multi-threaded application, and deadlocks or other
...
# A coverage report will now be accessible at `./build/test_quicksilver.coverage/index.html`,
...
Quicksilver can be compiled with various "sanitizers" enabled, which add
...
`cs_vault`, while thread 2 locks them in the opposite order: result, deadlock
...
However, in open source software such as Quicksilver, where everyone uses
...
pay attention to for reviewers of Quicksilver code.
...
- Make sure pull requests pass CI before merging.
  ...
  - *Rationale*: Makes sure that they pass thorough testing, and that the tester will keep passing
     on the main branch. Otherwise, all new pull requests will start failing the tests, resulting in
...
logging messages. Every one of them takes a `HgLog::CATEGORY` as its first
argument; the category is mandatory, and it selects the `[tag:level]` prefix
that every line carries. Use `HgLog::ALL` only where no category genuinely
applies. They should be used as follows:

- `LogDebug(HgLog::CATEGORY, fmt, params...)` is what you want
...
- `LogInfo(HgLog::CATEGORY, fmt, params...)` should only be used rarely, e.g.
  for startup messages or for infrequent and important events such as a new
  block tip being found or a new outbound connection being made. These log
  messages are unconditional — the category selects the prefix only, it does
  not gate them — so care must be taken that they can't be used by an attacker
  to fill up storage.

- `LogError(HgLog::CATEGORY, fmt, params...)` should be used in place of
  `LogInfo` for severe problems that require the node (or a subsystem) to shut
  down entirely (e.g., insufficient storage space).

- `LogWarning(HgLog::CATEGORY, fmt, params...)` should be used in place of
  `LogInfo` for severe problems that the node admin should address, but are not
...
- `LogTrace(HgLog::CATEGORY, fmt, params...)` should be used in place of
...
- Make sure that no crashes happen with run-time option `-disablevault`.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: Quicksilver uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
// txrelaypool.cpp
void CTxRelayPool::UpdateTransactionsFromBlock(...)
{
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `QUICKSILVER_FOO_BAR_H`, e.g.

```c++
#ifndef QUICKSILVER_FOO_BAR_H
#define QUICKSILVER_FOO_BAR_H
...
#endif // QUICKSILVER_FOO_BAR_H
```
...
- Avoid adding slow or blocking code in the GUI thread. In particular, do not
  add new `interfaces::Node` and `interfaces::Vault` method calls, even if they
...
Some of these are maintained by active developers of Quicksilver, in which case
...
a Quicksilver subtree, so that they can be integrated quickly. Cosmetic changes
should be taken upstream.

These are vendored copies rather than git subtrees, so there is no subtree-consistency
tool to run against them; compare directly with the upstream repository instead.

Current vendored trees include:

- src/leveldb
  - Subtree maintained by Core contributors.
...
file descriptors, it will cause problems with Quicksilver's `select()` loop, because
...
compatibility between nodes. When upgrading LevelDB, you should review the upstream changes
to check for issues affecting consensus compatibility.

For example, if LevelDB had a bug that accidentally prevented a key from being
returned in an edge case, and that bug was fixed upstream, the bug "fix" would
be an incompatible consensus change. In this situation, the correct behavior
would be to revert the upstream fix before applying the updates to Quicksilver's
...
```bash
test/lint/commit-script-check.sh origin/main..HEAD
```
...
- Argument and field naming: please consider whether there is already a naming
  ...
  `time_delta` (and not, e.g. `timedelta` or camel case `timeDelta`).
...
- Use the JSON parser for parsing, don't manually parse integers or strings from
  ...
  - *Exception*: AmountFromValue can parse amounts as string. This was introduced because many JSON
    ...
    inputting or outputting monetary values.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `quicksilver-cli`, and can be surprising to users.

- For new RPC methods, if implementing a `verbosity` argument, use integer verbosity rather than boolean.
  `ParseVerbosity()` in [util.h](../src/rpc/util.h) rejects booleans. Integer verbosity allows
  multiple values without maintaining an ambiguous boolean alias.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `quicksilver-cli` and the GUI debug console use this table to determine how to
    convert a plaintext command line to JSON. If the types don't match, the method can be unusable
    from there.

- A RPC method must either be a vault method or a non-vault method. Do not
  introduce new methods that differ in behavior based on the presence of a vault.

  - *Rationale*: As well as complicating the implementation and interfering
    with the introduction of multi-vault, vault and non-vault code should be
...
- Vault RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  `getblockchaininfo`'s state immediately prior to the call's execution. Vault
  RPCs whose behavior does *not* depend on the current chainstate may omit this
  call.

  - *Rationale*: In previous versions of Quicksilver, the vault was always
    in-sync with the chainstate (by virtue of them all being updated in the
    same cs_main lock). In order to maintain the behavior that vault RPCs
    return results as of at least the highest best-known block an RPC
    client may be aware of prior to entering a vault RPC call, we must block
    until the vault is caught up to the chainstate as of the RPC call's entry.
...
independent (node, vault, GUI), are defined in
[`src/interfaces/`](../src/interfaces/). The main interface classes defined
there are [`interfaces::Chain`](../src/interfaces/chain.h), used by vault to
access the node's latest chain state,
[`interfaces::Node`](../src/interfaces/node.h), used by the GUI to control the
node, [`interfaces::Vault`](../src/interfaces/vault.h), used by the GUI
to control an individual vault and [`interfaces::Mining`](../src/interfaces/mining.h),
...
Interface classes are written in a particular style so node, vault, and GUI
...
- Interface method definitions should wrap existing functionality instead of
  implementing new functionality. Any substantial new node or vault
  functionality should be implemented in [`src/node/`](../src/node/) or
  [`src/vault/`](../src/vault/) and just exposed in
...
- Interface method parameter and return types should either be serializable or
  ...
  virtual unique_ptr<interfaces::Vault> loadVault(std::string filename) = 0;

  // Bad: returns CVault reference that can't be used from another process
  virtual CVault& loadVault(std::string filename) = 0;
  ```
