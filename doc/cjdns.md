# CJDNS support in Quicksilver

It is possible to run Quicksilver over CJDNS, an encrypted IPv6 network that
...
done outside of Quicksilver, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quicksilver network and individual nodes.
...
before it will work with your Quicksilver node. This is described in steps
...
## Run Quicksilver with CJDNS

Once you are connected to the CJDNS network, the following Quicksilver
...
When enabled, this option tells Quicksilver that it is running in an
...
IPv6 local network. This helps Quicksilver perform better address management:
...
There may be fewer
CJDNS peers than Tor or IP ones. You can use `quicksilver-cli -addrinfo` to see the
...
[tor.md](tor.md) for details.

## CJDNS-related information in Quicksilver

There are several ways to see your CJDNS address in Quicksilver:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `quicksilver-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `quicksilver-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `quicksilver-cli help getnodeaddresses` for details.
