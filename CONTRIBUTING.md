...
The Quicksilver project operates an open contributor model where anyone is
welcome to contribute towards development in the form of peer review, testing
and patches. This document explains the practical process and guidelines for
contributing.

First, in terms of structure, there is no particular concept of "Quicksilver
developers" in the sense of privileged people. Open source often naturally
revolves around a meritocracy where contributors earn trust from the developer
community over time. Nevertheless, some hierarchy is necessary for practical
purposes. As such, there are repository maintainers who are responsible for
merging pull requests and
moderation.
...
Before you start contributing, familiarize yourself with the Quicksilver build
system and tests. Refer to the documentation in the repository on how to build
Quicksilver and how to run the unit tests, functional tests, and fuzz tests.

Quicksilver is newly published, so there is not yet a backlog of triaged issues
to pick from. Until one accumulates, the most useful places to start are
building the tree on your own platform and reporting what the instructions get
wrong, running the test suites, and reviewing open pull requests. Issues
suitable for a first contribution are labelled `good first issue` as they are
identified. If you are interested in an issue but unsure whether it is still
applicable, leave a comment on it first.

Reviewing Quicksilver pull requests is also a valuable way to contribute,
especially for build, vault, mining, Qt, and test changes.
...
Discussion about codebase improvements happens in this repository's GitHub
issues and pull requests.

Complicated or controversial consensus or P2P protocol changes should be
raised in a GitHub issue before working on a patch set, so the design gets
visibility ahead of code review.
...
Open Quicksilver GUI, node, vault, mining, and build-system issues or pull
requests in this repository.

As a rule of thumb, changes that modify `src/qt` should still explain their GUI
impact clearly. Changes that cross `src/qt`, `src/interfaces`, vault, node, or
build-system code should include a test plan for every affected surface.

For large GUI changes that include build-system and interface changes, describe
the GUI, interface, and build impacts in the same pull request so reviewers can
evaluate the complete change.
...
the pull request affects. Valid areas as:
  ...
  - `qt` or `gui` for changes to quicksilver-qt
  ...
  - `vault` for changes to the vault code
  - `build` for changes to CMake

Examples:

    consensus: Add new opcode for QIP-XXXX OP_CHECKAWESOMESIG
    net: Automatically create onion service, listen on Tor
    qt: Fix transaction list sorting
...
You should include references to any discussions (for example, related issues,
pull requests, design documents, or mailing list threads).

The description for a new pull request should not contain any `@` mentions. The
PR description will be included in the commit message when the PR is merged and
any users mentioned in the description will be annoyingly notified each time a
fork of Quicksilver copies the merge. Instead, make any username mentions in a
subsequent comment to the PR.

### Translation changes

Translation updates should be coordinated with the current maintainers before
opening a pull request so source-string changes, generated catalog files, and
release timing stay consistent.

### Design documents

Design decisions that affect Quicksilver's public behavior belong in
`doc/design/`. A change must not depend on unpublished planning material to
explain its consensus, security, or operational behavior.
...
When a pull request conflicts with the target branch, you may be asked to rebase it on top of the current target branch.
    ...
    git fetch origin  # Fetch the latest target branch from this repository
    git rebase origin/main  # Rebuild commits on top of the new base
...
The following applies to code changes to the Quicksilver project (and related
projects such as libsecp256k1), and is not to be confused with overall Quicksilver
Network Protocol consensus changes.

Whether a pull request is merged into Quicksilver rests with the project merge
maintainers.
...
Patches that change Quicksilver consensus rules are considerably more involved than
normal because they affect the entire ecosystem and so must be preceded by
extensive public design discussion and have a numbered Quicksilver improvement
proposal. While each case will be different, one should be prepared to expend
more time and effort than for other kinds of patches because of increased peer
review and consensus building requirements.
...
spread out over GitHub issues, pull requests, and design documents).
...
Where a patch set proposes to change the Quicksilver consensus, it must have been
discussed extensively in public project channels, be accompanied by a widely
discussed Quicksilver improvement proposal, and have a generally widely
perceived technical consensus of being a worthwhile change based on the
judgement of the maintainers.
...
about:
  ...
  - It may be because the changes you are suggesting do not appeal to people. Rather than
    ...
    Identify and address any of the issues you find. Then ask in the pull request
    or a related GitHub issue if someone could give their opinion on the concept
    itself.
  ...
  - Finally, if all else fails, ask in a related GitHub issue or project
    discussion for someone to give your pull request a look. If you think you've
    been waiting for an unreasonably long time (say,
...
Existing Quicksilver backport pull requests should be used as examples once
release branches are published.
