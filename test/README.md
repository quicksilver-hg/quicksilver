This directory contains integration tests that test quicksilverd and its
...
quicksilverd and quicksilver-qt by interacting with them through the RPC and P2P
interfaces.
- [util](util) which tests the utilities (quicksilver-util, quicksilver-tx, ...).
...
The P2P and RPC ports used by the quicksilverd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another quicksilverd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its quicksilverd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other quicksilverd processes are running.

On linux, the test framework will warn if there is another
quicksilverd process running when the tests are started.

If there are zombie quicksilverd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
quicksilverd processes running on the system, so should not be used if any non-test
quicksilverd processes are being run.**

```bash
killall quicksilverd
```

or

```bash
pkill -9 quicksilverd
```
...
sure quicksilverd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](../ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and quicksilverd `debug.log`s will all be dumped
...
`test_framework.log` and quicksilverd `debug.log`s can be combined into a single
...
call methods that interact with the quicksilverd nodes-under-test.

If further introspection of the quicksilverd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/sandbox/quicksilverd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/quicksilverd <pid>
```
