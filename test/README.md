This directory contains integration tests that test quicksilver-daemon and its
utilities in their entirety. It does not contain unit tests, which
can be found in [/src/test](../src/test), [/src/vault/test](../src/vault/test),
etc.
...
quicksilver-daemon and quicksilver by interacting with them through the RPC and P2P
interfaces.
- [util](util) which tests quicksilver-tx.
...
Before tests can be run locally, Quicksilver must be built.  See the [building instructions](../doc#building) for help.
...
```
build/test/functional/feature_quicksilver_replace.py
```

or can be run through the test_runner harness, eg:

```
build/test/functional/test_runner.py feature_quicksilver_replace.py
```
...
to run all the vault tests:

```
build/test/functional/test_runner.py test/functional/vault*
functional/test_runner.py functional/vault*  # (called from the build/test/ directory)
test_runner.py vault*  # (called from the build/test/functional/ directory)
```

but not

```
build/test/functional/test_runner.py vault*
```

Combinations of wildcards can be passed:

```
build/test/functional/test_runner.py ./test/functional/tool* test/functional/relaypool*
...
The P2P and RPC ports used by the quicksilver-daemon nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another quicksilver-daemon
process running on the system (perhaps from a previous test which hasn't successfully
killed all its quicksilver-daemon nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other quicksilver-daemon processes are running.

On linux, the test framework will warn if there is another
quicksilver-daemon process running when the tests are started.

If there are zombie quicksilver-daemon processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
quicksilver-daemon processes running on the system, so should not be used if any non-test
quicksilver-daemon processes are being run.**

```bash
killall quicksilver-daemon
```

or

```bash
pkill -9 quicksilver-daemon
```
...
sure quicksilver-daemon processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](../ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and quicksilver-daemon `debug.log`s will all be dumped
...
printed in the first line of test output):
  ...
  - `<test data directory>/node<node number>/sandbox/debug.log`.
...
`test_framework.log` and quicksilver-daemon `debug.log`s can be combined into a single
...
call methods that interact with the quicksilver-daemon nodes-under-test.

If further introspection of the quicksilver-daemon instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/sandbox/quicksilver.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/quicksilver-daemon <pid>
```
...
functional test. Ex: `build/test/functional/vault_hd.py --timeout-factor 0`.
...
See the README in [test/lint](lint).

# Writing functional tests

You are encouraged to write functional tests for new or existing features.
Further information about the functional test framework and individual
tests is found in [test/functional](functional).
