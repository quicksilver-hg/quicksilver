...
from test_framework.relaypool_util import (
...
class OrphanRPCsTest(QuicksilverTestFramework):
    ...
    def test_orphan_activity(self):
        ...
        tx_parent_2 = self.vault.create_self_transfer()
        tx_child_2 = self.vault.create_self_transfer(utxo_to_spend=tx_parent_2["new_utxo"])
        peer = node.add_p2p_connection(P2PInterface())
        peer.send_and_ping(msg_tx(tx_child_1["tx"]))
        peer.send_and_ping(msg_tx(tx_child_2["tx"]))

        self.log.info("Check that neither parent is in the relaypool")
        ...
        self.log.info("Check that parent 1 and child 1 are in the relaypool")
        raw_relaypool = node.getrawrelaypool()
        assert_equal(len(raw_relaypool), 2)
        assert tx_parent_1["txid"] in raw_relaypool
        assert tx_child_1["txid"] in raw_relaypool
        ...
        self.log.info("Check that all parents and children are now in the relaypool")
        raw_relaypool = node.getrawrelaypool()
        assert_equal(len(raw_relaypool), 4)
        assert tx_parent_1["txid"] in raw_relaypool
        assert tx_child_1["txid"] in raw_relaypool
        assert tx_parent_2["txid"] in raw_relaypool
        assert tx_child_2["txid"] in raw_relaypool
        self.log.info("Check that the orphanage is empty")
        assert_equal(len(node.getorphantxs()), 0)

        self.log.info("Confirm the transactions (clears relaypool)")
    ...
    def test_misc(self):
        node = self.nodes[0]
        assert_raises_rpc_error(-3, "JSON value of type bool is not of expected type number", node.getorphantxs, verbosity=True)
        assert_raises_rpc_error(-3, "JSON value of type bool is not of expected type number", node.getorphantxs, verbosity=False)
