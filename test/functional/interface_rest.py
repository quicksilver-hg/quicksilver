...
class RESTTest (QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-rest", "-blockfilterindex=1", "-txpownocycle=1"], ["-txpownocycle=1"]]
    ...
    def run_test(self):
        ...
        txid = self.vault.send_to(from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=int(0.1 * COIN))["txid"]
        self.sync_all()
        ...
        self.log.info("Test the /getutxos URI with and without /checkrelaypool")
        # Create a transaction, check that it's found with /checkrelaypool, but
        # not found without. Then confirm the transaction and check that it's
        # found with or without /checkrelaypool.
        ...
        json_obj = self.test_rest_request(f"/getutxos/checkrelaypool/{spending[0]}-{spending[1]}")
        assert_equal(len(json_obj['utxos']), 1)

        json_obj = self.test_rest_request(f"/getutxos/{spent[0]}-{spent[1]}")
        assert_equal(len(json_obj['utxos']), 1)

        json_obj = self.test_rest_request(f"/getutxos/checkrelaypool/{spent[0]}-{spent[1]}")
        assert_equal(len(json_obj['utxos']), 0)
        ...
        self.test_rest_request("/getutxos", http_method='POST', req_type=ReqType.JSON, body='{"checkrelaypool', status=400, ret_type=RetType.OBJ)
        self.test_rest_request("/getutxos", http_method='POST', req_type=ReqType.BIN, body='{"checkrelaypool', status=400, ret_type=RetType.OBJ)
        self.test_rest_request("/getutxos/checkrelaypool", http_method='POST', req_type=ReqType.JSON, status=400, ret_type=RetType.OBJ)
        ...
        self.test_rest_request(f"/getutxos/checkrelaypool/{long_uri}", http_method='POST', status=400, ret_type=RetType.OBJ)

        long_uri = '/'.join([f'{txid}-{n_}' for n_ in range(15)])
        self.test_rest_request(f"/getutxos/checkrelaypool/{long_uri}", http_method='POST', status=200)
        ...
        for invalid_uri in [f"/headers/{bb_hash}%", f"/blockfilterheaders/basic/{bb_hash}%", "/relaypool/contents.json?%"]:
        ...
        self.log.info("Test tx inclusion in the /relaypool and /block URIs")

        # Make 3 chained txs and mine them on node 1
        txs = []
        input_txid = txid
        for _ in range(3):
            utxo_to_spend = self.vault.get_utxo(txid=input_txid)
            txs.append(self.vault.send_self_transfer(from_node=self.nodes[0], utxo_to_spend=utxo_to_spend)['txid'])
            input_txid = txs[-1]
        self.sync_all()

        # Check that there are exactly 3 transactions in the TX memory pool before generating the block
        json_obj = self.test_rest_request("/relaypool/info")
        assert_equal(json_obj['size'], 3)
        # the size of the memory pool should be greater than 3x ~100 bytes
        assert_greater_than(json_obj['bytes'], 300)

        relaypool_info = self.nodes[0].getrelaypoolinfo()
        # pop unstable unbroadcastcount before check
        for obj in [json_obj, relaypool_info]:
            obj.pop("unbroadcastcount")
        assert_equal(json_obj, relaypool_info)

        # Check that there are our submitted transactions in the TX memory pool
        json_obj = self.test_rest_request("/relaypool/contents")
        raw_relaypool_verbose = self.nodes[0].getrawrelaypool(verbose=True)

        assert_equal(json_obj, raw_relaypool_verbose)
        ...
        # Check the relaypool response for explicit parameters
        json_obj = self.test_rest_request("/relaypool/contents", query_params={"verbose": "true", "relaypool_sequence": "false"})
        assert_equal(json_obj, raw_relaypool_verbose)

        # Check the relaypool response for not verbose
        json_obj = self.test_rest_request("/relaypool/contents", query_params={"verbose": "false"})
        raw_relaypool = self.nodes[0].getrawrelaypool(verbose=False)

        assert_equal(json_obj, raw_relaypool)

        # Check the relaypool response for sequence
        json_obj = self.test_rest_request("/relaypool/contents", query_params={"verbose": "false", "relaypool_sequence": "true"})
        raw_relaypool = self.nodes[0].getrawrelaypool(verbose=False, relaypool_sequence=True)

        assert_equal(json_obj, raw_relaypool)

        # Check for error response if verbose=true and relaypool_sequence=true
        resp = self.test_rest_request("/relaypool/contents", ret_type=RetType.OBJ, status=400, query_params={"verbose": "true", "relaypool_sequence": "true"})
        assert_equal(resp.read().decode('utf-8').strip(), 'Verbose results cannot contain relay pool sequence values. (hint: set "verbose=false")')

        # Check for error response if verbose is not "true" or "false"
        resp = self.test_rest_request("/relaypool/contents", ret_type=RetType.OBJ, status=400, query_params={"verbose": "TRUE"})
        assert_equal(resp.read().decode('utf-8').strip(), 'The "verbose" query parameter must be either "true" or "false".')

        # Check for error response if relaypool_sequence is not "true" or "false"
        resp = self.test_rest_request("/relaypool/contents", ret_type=RetType.OBJ, status=400, query_params={"verbose": "false", "relaypool_sequence": "TRUE"})
        assert_equal(resp.read().decode('utf-8').strip(), 'The "relaypool_sequence" query parameter must be either "true" or "false".')
        ...
        # Removed count-in-path forms must not remain part of the public REST surface.
        self.test_rest_request(f"/headers/1/{bb_hash}", status=400, ret_type=RetType.OBJ)
        self.test_rest_request(f"/blockfilterheaders/basic/5/{bb_hash}", status=400, ret_type=RetType.OBJ)
