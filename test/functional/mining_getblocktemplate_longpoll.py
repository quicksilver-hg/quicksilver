...
class GetBlockTemplateLPTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        self.generate(self.minivault, COINBASE_MATURITY + 1)
        ...
        # check that thread will exit now that new transaction entered relaypool
        thr.join(5)  # wait 5 seconds or until thread exits
        ...
        self.log.info("Test that introducing a new transaction into the relaypool will terminate the longpoll")
        thr = LongpollThread(self.nodes[0])
        with self.nodes[0].assert_debug_log(["ThreadRPCServer method=getblocktemplate"], timeout=3):
            thr.start()
        # generate a transaction and submit it
        self.minivault.send_self_transfer(from_node=random.choice(self.nodes))
        # after one minute, every 10 seconds the relaypool is probed, so in 80 seconds it should have returned
        thr.join(60 + 20)
        assert not thr.is_alive()
