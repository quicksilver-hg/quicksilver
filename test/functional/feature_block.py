...
SANDBOX_TX_POW_MINT = COIN
FILLER_TX_WEIGHT_OVERHEAD = 276 + 176 * 4


class FullBlockTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        self.block_heights = {}
        # Quicksilver (#5c-1 Phase 2): nCongestion is a consensus-checked header field
        # derived from the parent's multiplier and this block's weight, so a block builder
        # has to track it per branch exactly as it tracks heights.
        self.block_congestion = {}
        self.coinbase_key, self.coinbase_pubkey = generate_keypair()
        self.tip = None
        self.blocks = {}
        self.genesis_hash = int(self.nodes[0].getbestblockhash(), 16)
        self.block_heights[self.genesis_hash] = 0
        self.block_congestion[self.genesis_hash] = CONGESTION_ONE
        self.spendable_outputs = []

        # Create a new block
        b_dup_cb = self.next_block('dup_cb')
        b_dup_cb.vtx[0].vin[0].scriptSig = DUPLICATE_COINBASE_SCRIPT_SIG
        b_dup_cb.vtx[0].vout[0].nValue = 0
        ...
        # Try to create a block that pays too much coinbase reward
        #     genesis -> b1 (0) -> b2 (1) -> b5 (2) -> b6 (3)
        #                                                    \-> b9 (4)
        #                      \-> b3 (1) -> b4 (2)
        self.log.info("Reject a block where the miner creates too much coinbase reward")
        self.move_tip(6)
        b9 = self.next_block(9, spend=out[4], additional_coinbase_value=SANDBOX_TX_POW_MINT + 1)
        self.send_blocks([b9], success=False, reject_reason='bad-cb-amount', reconnect=True)

        # Create a fork that ends in a block with too much coinbase reward (the one that causes the reorg)
        #     genesis -> b1 (0) -> b2 (1) -> b5 (2) -> b6  (3)
        #                                          \-> b10 (3) -> b11 (4)
        #                      \-> b3 (1) -> b4 (2)
        ...
        b11 = self.next_block(11, spend=out[4], additional_coinbase_value=SANDBOX_TX_POW_MINT + 1)
        ...
        self.save_spendable_output()
        b14 = self.next_block(14, spend=out[5], additional_coinbase_value=SANDBOX_TX_POW_MINT + 1)
        ...
        tx = CTransaction()
        script_length = (MAX_BLOCK_WEIGHT - b23.get_weight() - FILLER_TX_WEIGHT_OVERHEAD) // 4
        script_output = CScript([b'\x00' * script_length])
        tx.vin.append(CTxIn(COutPoint(b23.vtx[1].sha256, 0)))
        tx.vout.append(CTxOut(b23.vtx[1].vout[0].nValue, script_output))
        ...
        script_length = (MAX_BLOCK_WEIGHT - b24.get_weight() - FILLER_TX_WEIGHT_OVERHEAD) // 4
        script_output = CScript([b'\x00' * (script_length + 1)])
        tx.vout = [CTxOut(b24.vtx[1].vout[0].nValue, script_output)]
        ...
        # Extend the b26 chain to make sure quicksilver-daemon isn't accepting b26
        ...
        # Extend the b28 chain to make sure quicksilver-daemon isn't accepting b28
        ...
        # Create a transaction that spends one cinnabar to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 cinnabar to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        #
        # b40 spends a bounded number of 6-sigop P2SH outputs from b39. The
        # remaining sigops are added in the final transaction so the test still
        # reaches the sigop boundary without exceeding block weight after the
        # Quicksilver tx-PoW tail on every transaction.
        #
        ...
        numTxes = min(numTxes, 100)
        assert_equal(numTxes <= b39_outputs, True)

        lastOutpoint = COutPoint(b40.vtx[1].sha256, 0)
        last_value = b40.vtx[1].vout[0].nValue
        new_txs = []
        for i in range(1, numTxes + 1):
            tx = CTransaction()
            p2sh_value = b39.vtx[i].vout[0].nValue
            tx.vout.append(CTxOut(last_value + p2sh_value, CScript([OP_TRUE])))
            ...
            prove_tx_pow(tx, anchor_height=self.current_tx_anchor_height())
            new_txs.append(tx)
            last_value += p2sh_value
            lastOutpoint = COutPoint(tx.sha256, 0)

        b40_sigops_to_fill = MAX_BLOCK_SIGOPS - (numTxes * b39_sigops_per_output + sigops) + 1
        tx = CTransaction()
        tx.vin.append(CTxIn(lastOutpoint, b''))
        tx.vout.append(CTxOut(last_value, CScript([OP_CHECKSIG] * b40_sigops_to_fill)))
        tx.rehash()
        new_txs.append(tx)
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(lastOutpoint, b''))
        tx.vout.append(CTxOut(last_value, CScript([OP_CHECKSIG] * b41_sigops_to_fill)))
        tx.rehash()
        ...
        b44.nBits = SANDBOX_N_BITS
        b44.vtx.append(coinbase)
        tx = self.create_and_sign_transaction(out[14], 1)
        b44.vtx.append(tx)
        b44.hashMerkleRoot = b44.calc_merkle_root()
        # Hand-built blocks skip create_block, so the congestion multiplier has to be
        # derived here too — this one is meant to CONNECT, and a zero would be rejected
        # as bad-congestion instead of testing what it is here to test.
        b44.nCongestion = next_congestion_multiplier(
            self.block_congestion[b44.hashPrevBlock], b44.get_weight())
        b44.solve()
        self.tip = b44
        self.block_heights[b44.sha256] = height
        self.block_congestion[b44.sha256] = b44.nCongestion
        self.blocks[44] = b44
        ...
        b45.nBits = SANDBOX_N_BITS
        b45.vtx.append(non_coinbase)
        b45.hashMerkleRoot = b45.calc_merkle_root()
        # nCongestion is left unset: this block is rejected by CheckBlock (bad-cb-missing)
        # before it ever reaches the ConnectBlock congestion check.
        b45.solve()
        ...
        b46.nBits = SANDBOX_N_BITS
        b46.vtx = []
        b46.hashMerkleRoot = 0
        # As with b45: rejected by CheckBlock (bad-blk-length) before the congestion check.
        b46.solve()
        ...
        while proof_hash_int(b47.nCycle) <= target:
            # Rehash cycles until the Cuckatoo proof hash exceeds the claimed target.
            b47.nCycle[0] = (b47.nCycle[0] + 42) & 0xffffffff
            b47.rehash()
        ...
        # b48's nTime was rewritten after next_block(), so it re-keyed under a new hash and
        # its congestion record has to move with it — same reason as the height above. The
        # value itself is unchanged: only the header moved, not the body.
        self.block_congestion[b48.sha256] = b48.nCongestion
        ...
        # Keep b57's header verbatim — see update_block's recompute_congestion note. An
        # attacker mounting CVE-2012-2459 reuses the victim's header; recomputing
        # nCongestion off the larger body would change the hash and dissolve the collision
        # this case exists to test.
        b56 = self.update_block(56, [tx1], recompute_congestion=False)
        ...
        b56p2 = self.update_block("b56p2", [tx3, tx4], recompute_congestion=False)
        ...
        # Reject a transaction that would overwrite an earlier unspent output.
        #
        ...
        #
        self.log.info("Reject a block with a transaction whose hash duplicates a previous transaction")
        self.move_tip(60)
        ...
        self.send_blocks([b61], success=False, reject_reason='bad-txns-overwrite', reconnect=True)

        # A duplicate is allowed after every earlier output has been spent.
        #
        # -> b57 (16) -> b60 ()
        ...
        tx.vout.append(CTxOut(out[18].vout[0].nValue, CScript([OP_TRUE])))
        ...
        script_length = (MAX_BLOCK_WEIGHT - 4 * len(b64a.normal_serialize()) - FILLER_TX_WEIGHT_OVERHEAD) // 4
        script_output = CScript([b'\x00' * script_length])
        tx.vin.append(CTxIn(COutPoint(b64a.vtx[1].sha256, 0)))
        tx.vout.append(CTxOut(b64a.vtx[1].vout[0].nValue, script_output))
        b64a = self.update_block("64a", [tx])
        assert_equal(b64a.get_weight(), MAX_BLOCK_WEIGHT + 8 * 4)
        self.send_blocks([b64a], success=False, reject_reason='non-canonical ReadCompactSize()')

        # quicksilver-daemon doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                        \-> b68 (20)
        #
        # b68 - coinbase claims one cinnabar above the single tx-PoW mint allowance
        #
        # b69 - coinbase claims the exact single tx-PoW mint allowance
        #       this succeeds.
        #
        self.log.info("Reject a block trying to claim too much subsidy in the coinbase transaction")
        self.move_tip(65)
        self.next_block(68, additional_coinbase_value=SANDBOX_TX_POW_MINT + 1)
        tx = self.create_and_sign_transaction(out[20], out[20].vout[0].nValue)
        b68 = self.update_block(68, [tx])
        self.send_blocks([b68], success=False, reject_reason='bad-cb-amount', reconnect=True)

        self.log.info("Accept a block claiming the correct subsidy in the coinbase transaction")
        self.move_tip(65)
        b69 = self.next_block(69, additional_coinbase_value=SANDBOX_TX_POW_MINT)
        tx = self.create_and_sign_transaction(out[20], out[20].vout[0].nValue)
        ...
        #
        #    b78 creates a tx, which is spent in b79. After b82, both should be in relaypool
        #
        #    The tx'es must be unsigned and pass the node's relaypool policy.  It is unsigned for the
        #    rather obscure reason that the Python signature code does not distinguish between
        #    Low-S and High-S values (whereas the Quicksilver code has custom code which does so);
        #    as a result of which, the odds are 50% that the python code will use the right
        #    value and the transaction will be accepted into the relaypool. Until we modify the
        ...
        # now check that tx78 and tx79 have been put back into the peer's relaypool
        relaypool = self.nodes[0].getrawrelaypool()
        assert_equal(len(relaypool), 2)
        assert tx78.hash in relaypool
        assert tx79.hash in relaypool
        ...
        for i in range(89, LARGE_REORG_SIZE + 89):
            ...
            tx = CTransaction()
            script_length = (MAX_BLOCK_WEIGHT - b.get_weight() - FILLER_TX_WEIGHT_OVERHEAD) // 4
            script_output = CScript([b'\x00' * script_length])
            tx.vin.append(CTxIn(COutPoint(b.vtx[1].sha256, 0)))
            tx.vout.append(CTxOut(b.vtx[1].vout[0].nValue, script_output))
    ...
    def current_tx_anchor_height(self):
        return self.block_heights[self.tip.hashPrevBlock]

    # this is a little handier to use than the version in blocktools.py
    def create_tx(self, spend_tx, n, value, output_script=None):
        if output_script is None:
            output_script = CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])
        tx = create_tx_with_script(spend_tx, n, amount=value, output_script=output_script)
        change = spend_tx.vout[n].nValue - value
        if change > 0:
            tx.vout.append(CTxOut(change, CScript([OP_TRUE])))
        prove_tx_pow(tx, anchor_height=self.current_tx_anchor_height())
        return tx
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, *, script=None, version=4):
        ...
        coinbase.rehash()
        prev_congestion = self.block_congestion[base_block_hash]
        if spend is None:
            block = create_block(base_block_hash, coinbase, block_time, version=version,
                                 prev_congestion=prev_congestion)
        else:
            tx = self.create_tx(spend, 0, spend.vout[0].nValue, output_script=script)
            self.sign_tx(tx, spend)
        ...
        self.block_congestion[block.sha256] = block.nCongestion
    ...
    def update_block(self, block_number, new_transactions, *, recompute_congestion=True):
        """Add transactions to a block and re-derive the body-dependent header fields.

        recompute_congestion=False models an ATTACKER who reuses a victim block's header
        verbatim (the CVE-2012-2459 merkle-malleability shape): recomputing nCongestion
        would change the header, and the whole point of those cases is that the malleated
        block collides with a valid block's hash.
        """
        block = self.blocks[block_number]
        anchor_height = self.block_heights[block.hashPrevBlock]
        for tx in new_transactions:
            if not self.is_coinbase_tx(tx):
                prove_tx_pow(tx, anchor_height=anchor_height)
        self.add_transactions_to_block(block, new_transactions)
        old_sha256 = block.sha256
        block.hashMerkleRoot = block.calc_merkle_root()
        # The added transactions changed the weight, so the multiplier the parent implies
        # changed with it — the same re-derivation node::RegenerateCommitments does on the
        # C++ side. Must happen BEFORE solve(): nCongestion is inside the pre-pow.
        if recompute_congestion:
            block.nCongestion = next_congestion_multiplier(
                self.block_congestion[block.hashPrevBlock], block.get_weight())
        block.solve()
        ...
        if block.sha256 != old_sha256:
            ...
            self.block_congestion.pop(old_sha256, None)
        self.block_congestion[block.sha256] = block.nCongestion
