...
class TxDownloadTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.setup_clean_chain = True
        self.extra_args = [['-datacarriersize=100000', '-maxrelaypool=5', '-persistrelaypool=0', '-txpownocycle=1']] * self.num_nodes
    ...
    def test_inv_block(self):
        ...
        self.log.info("Put the tx in node 0's relaypool")
        self.vault.sendrawtransaction(from_node=self.nodes[0], tx_hex=tx['hex'])
    ...
    def test_preferred_inv(self, connection_type: ConnectionType):
        if connection_type == ConnectionType.WHITELIST:
            ...
            self.restart_node(0, extra_args=['-whitelist=noban@127.0.0.1', '-txpownocycle=1'])
    ...
    def test_large_inv_batch(self):
        ...
        self.restart_node(0, extra_args=['-whitelist=relay@127.0.0.1', '-txpownocycle=1'])
        peer = self.nodes[0].add_p2p_connection(TestP2PConn())
    ...
    def test_inv_wtxidrelay_mismatch(self):
        ...
        random_tx = self.vault.create_self_transfer(utxo_to_spend=self.vault.get_utxo(mark_as_spent=False))
