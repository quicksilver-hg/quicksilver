...
def serialize_addrman(
    *,
    ...
    r += ser_uint256(0)  # Empty asmap checksum.
    checksum = hash256(r)
...
class AddrmanTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that a pre-Quicksilver addrman format is rejected")
        self.stop_node(0)
        write_addrman(peers_dat, format=3)
        with self.nodes[0].assert_debug_log([
                f'Creating new peers.dat because the file version was not compatible ("{peers_dat}"). Original backed up to peers.dat.bak',
        ]):
            self.start_node(0)
        assert_equal(self.nodes[0].getnodeaddresses(), [])
        os.remove(peers_dat + ".bak")
        ...
        self.stop_node(0)
        write_addrman(peers_dat, net_magic="publictest")
        self.nodes[0].assert_start_raises_init_error(
