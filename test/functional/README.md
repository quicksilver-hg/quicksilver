...
- See [the python lint script](../lint/lint-python.py) that checks for violations that
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the QuicksilverTestFramework, place overrides for the
...
- Use `platform.system()` for detecting the running operating system and `os.name` to
  ...
  methods in the `QuicksilverTestFramework` class, which can be used to skip a whole test
  depending on the platform).

#### Naming guidelines

- Name the test `<area>_test.py`, where area can be one of the following:
    - `feature` for tests for full features that aren't vault/mining/relay pool, eg `feature_quicksilver_replace.py`
    - `interface` for tests for other interfaces (REST, ZMQ, etc), eg `interface_rest.py`
    - `mining` for tests for mining features, eg `mining_basic.py`
    - `p2p` for tests that explicitly test the p2p interface, eg `p2p_disconnect_ban.py`
    - `relaypool` for tests for relay pool behaviour, eg `relaypool_reorg.py`
    - `rpc` for tests for individual RPC methods or features, eg `rpc_deriveaddresses.py`
    - `tool` for tests for tools, eg `tool_vault.py`
    - `vault` for tests for vault features, eg `vault_keypool.py`
...
- Instead of inline comments or no test documentation at all, log the comments to the test log, e.g.
  ...
  logic easier [to debug](../README.md#test-logging).
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 Hg) in its vault. Using them is much more
...
with the quicksilver-daemon(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a quicksilver-daemon.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Quicksilver node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the QuicksilverTestFramework
...
Adapted from the upstream authproxy implementation.
...
Test objects for interacting with a quicksilver-daemon node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts.
