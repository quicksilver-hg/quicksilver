...
class BIP65Test(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        vault = MiniVault(self.nodes[0], mode=MiniVaultMode.RAW_OP_TRUE)
        ...
        invalid_cltv_txs = []
        for i in range(5):
            spendtx = vault.create_self_transfer()['tx']
            cltv_invalidate(spendtx, i)
            invalid_cltv_txs.append(spendtx)

        tip = self.nodes[0].getbestblockhash()
        block_time = self.nodes[0].getblockheader(tip)['mediantime'] + 1
        block = create_block(int(tip, 16), create_coinbase(CLTV_HEIGHT - 1, nValue=0), block_time, version=3, txlist=invalid_cltv_txs)
        block.solve()
        ...
        tip = block.sha256
        block_time += 1
        block = create_block(tip, create_coinbase(CLTV_HEIGHT, nValue=0), block_time, version=3)
        block.solve()
