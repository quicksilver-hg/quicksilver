...
class LoggingTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].assert_start_raises_init_error(
            extra_args=["-loglevel=mesh:abc"],
            expected_msg="Error: Unsupported category-specific logging level -loglevel=mesh:abc.",
            match=ErrorMatch.PARTIAL_REGEX,
        )
        self.nodes[0].assert_start_raises_init_error(
            extra_args=["-loglevel=mesh:info:abc"],
            expected_msg="Error: Unsupported category-specific logging level -loglevel=mesh:info:abc.",
            match=ErrorMatch.PARTIAL_REGEX,
        )
        ...
        for disable_debug_opt in disable_debug_options:
            ...
            self.restart_node(0, ['-debug=http', '-debug=abc', disable_debug_opt, '-debug=rpc', '-debug=mesh'])
