...
class P2PBlocksOnly(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        self.extra_args = [["-blocksonly", "-txpownocycle=1"]]

    def run_test(self):
        self.minivault = MiniVault(self.nodes[0])
    ...
    def blocksonly_mode_tests(self):
        ...
        assert_equal(self.nodes[0].testrelaypoolaccept([tx_hex])[0]['allowed'], True)
        with self.nodes[0].assert_debug_log(['received getdata for: wtx {} peer'.format(wtxid)]):
            self.nodes[0].sendrawtransaction(tx_hex)
            tx_relay_peer.wait_for_tx(txid)
            assert_equal(self.nodes[0].getrelaypoolinfo()['size'], 1)

        self.log.info("Restarting node 0 with relay permission and blocksonly")
        self.restart_node(0, ["-persistrelaypool=0", "-whitelist=relay@127.0.0.1", "-blocksonly", "-txpownocycle=1"])
        assert_equal(self.nodes[0].getrawrelaypool(), [])
        ...
        with self.nodes[0].assert_debug_log(["received getdata"]):
            ...
            # and Quicksilver respects that choice and will not send transactions.
    ...
    def blocks_relay_conn_tests(self):
        ...
        self.restart_node(0, ["-noblocksonly", "-txpownocycle=1"])  # disables blocks only mode
