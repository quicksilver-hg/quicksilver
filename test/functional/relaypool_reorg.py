...
"""Test relaypool re-org scenarios.

Test re-org scenarios with a relaypool that contains transactions
...
class RelayPoolCoinbaseTest(QuicksilverTestFramework):
    ...
    def test_reorg_relay(self):
        ...
        tx_disconnected = self.vault.send_self_transfer(from_node=self.nodes[1])
        self.generate(self.nodes[1], 1, sync_fun=self.no_op)

        # Create a transaction and submit it to node1's relaypool.
        tx_before_reorg = self.vault.send_self_transfer(from_node=self.nodes[1], confirmed_only=True)

        # Create a child of that transaction and submit it to node1's relaypool.
        tx_child = self.vault.send_self_transfer(utxo_to_spend=tx_disconnected["new_utxo"], from_node=self.nodes[1])
        assert_equal(self.nodes[1].getrelaypoolentry(tx_child["txid"])["ancestorcount"], 1)
        ...
        assert_equal(self.nodes[1].getrelaypoolentry(tx_child["txid"])["ancestorcount"], 2)
        assert_equal(self.nodes[1].getrelaypoolentry(tx_before_reorg["txid"])["ancestorcount"], 1)
        ...
        # The tx from the disconnected block was never announced, and it entered the relaypool later
        ...
        tx_after_reorg = self.vault.send_self_transfer(from_node=self.nodes[1])
        request_after_reorg = msg_getdata([CInv(t=MSG_WTX, h=int(tx_after_reorg["tx"].getwtxid(), 16))])
        assert tx_after_reorg["txid"] in self.nodes[1].getrawrelaypool()
    ...
    def run_test(self):
        ...
        self.log.info("Add 4 coinbase utxos to the minivault")
        # Block 76 contains the first spendable coinbase txs.
        first_block = 76

        # Three scenarios for re-orging coinbase spends in the memory pool:
        # 1. Direct coinbase spend  :  spend_1
        # 2. Indirect (coinbase spend in chain, child in relaypool) : spend_2 and spend_2_1
        ...
        # and make sure the relaypool code behaves correctly.
        b = [self.nodes[0].getblockhash(n) for n in range(first_block, first_block+4)]
        coinbase_txids = [self.nodes[0].getblock(h)['tx'][0] for h in b]
        utxo_1 = vault.get_utxo(txid=coinbase_txids[1])
        utxo_2 = vault.get_utxo(txid=coinbase_txids[2])
        utxo_3 = vault.get_utxo(txid=coinbase_txids[3])
        self.log.info("Create three transactions spending from coinbase utxos: spend_1, spend_2, spend_3")
        spend_1 = vault.create_self_transfer(utxo_to_spend=utxo_1)
        spend_2 = vault.create_self_transfer(utxo_to_spend=utxo_2)
        spend_3 = vault.create_self_transfer(utxo_to_spend=utxo_3)

        self.log.info("Create another transaction which is time-locked to two blocks in the future")
        utxo = vault.get_utxo(txid=coinbase_txids[0])
        timelock_tx = vault.create_self_transfer(
            utxo_to_spend=utxo,
        ...
        vault.sendrawtransaction(from_node=self.nodes[0], tx_hex=spend_2['hex'])
        vault.sendrawtransaction(from_node=self.nodes[0], tx_hex=spend_3['hex'])
        ...
        spend_2_1 = vault.create_self_transfer(utxo_to_spend=spend_2["new_utxo"])
        spend_3_1 = vault.create_self_transfer(utxo_to_spend=spend_3["new_utxo"])
        ...
        self.log.info("Add spend_1 and spend_2_1 to the relaypool")
        spend_1_id = self.nodes[0].sendrawtransaction(spend_1['hex'])
        spend_2_1_id = self.nodes[0].sendrawtransaction(spend_2_1['hex'])

        assert_equal(set(self.nodes[0].getrawrelaypool()), {spend_1_id, spend_2_1_id, timelock_tx_id})
        self.sync_all()

        self.log.info("invalidate the last block")
        for node in self.nodes:
            node.invalidateblock(last_block[0])
        self.log.info("The time-locked transaction is now too immature and has been removed from the relaypool")
        self.log.info("spend_3_1 has been re-orged out of the chain and is back in the relaypool")
        assert_equal(set(self.nodes[0].getrawrelaypool()), {spend_1_id, spend_2_1_id, spend_3_1_id})
        ...
        self.log.info("Check that the relaypool is empty")
