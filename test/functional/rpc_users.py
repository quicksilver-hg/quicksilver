...
def call_with_auth(node, user, password, method="getbestblockhash"):
    ...
    conn.connect()
    conn.request('POST', '/', json.dumps({"jsonrpc": "2.0", "method": method, "id": 1}), headers)
...
class HTTPBasicsTest(QuicksilverTestFramework):
    ...
    def conf_setup(self):
        #Append rpcauth to quicksilver.conf before initialization
        ...
        with open(self.nodes[0].datadir_path / "quicksilver.conf", "a", encoding="utf8") as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(self.nodes[1].datadir_path / "quicksilver.conf", "a", encoding="utf8") as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
    ...
    def test_norpccookiefile(self, node0_cookie_path):
        ...
        with self.nodes[0].busy_wait_for_debug_log([b'init message: Node online']):
    ...
    def run_test(self):
        ...
        assert_equal(401, call_with_auth(self.nodes[0], 'user1', 'quicksilver').status)
