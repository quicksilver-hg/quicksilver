...
class P2PIBDStallingTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        # Need to wait until the node has requested the 1023 non-withheld
        # blocks. P2PStaller sends each requested block immediately.
        expected_requested_blocks = {block.sha256 for block in blocks[1:NUM_BLOCKS-1]}
        self.wait_until(lambda: self.requested_blocks(peers) >= expected_requested_blocks)
    ...
    def requested_blocks(self, peers):
        return {block_hash for p in peers for block_hash in p.getdata_requests}
