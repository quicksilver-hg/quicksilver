...
"""Test vault group functionality."""
...
class VaultGroupTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        # - node[1] should pick one UTXO and leave the rest
        ...
        # one output should be 0.2, the other should be ~0.8
        ...
        self.log.info("Confirm balances before explicit avoidpartialspends coverage continues")
        self.sync_all()
        self.generate(self.nodes[0], 1)
        ...
        # Node 2 enforces avoidpartialspends. The older auto-APS threshold for
        # disabled vaults was fee-cost based and is not meaningful on Quicksilver.

        # Empty out node2's vault
        self.nodes[2].sendall(recipients=[self.nodes[0].getnewaddress()])
        self.sync_all()
        self.generate(self.nodes[0], 1)

        self.log.info("Fill a vault with 10,000 outputs corresponding to the same scriptPubKey")
        for _ in range(5):
            raw_tx = self.nodes[0].createrawtransaction([{"txid":"0"*64, "vout":0}], [{addr2[0]: 0.004}])
        ...
        assert self.nodes[2].sendtoaddress(address=addr2[0], amount=0.4)


if __name__ == '__main__':
    VaultGroupTest(__file__).main()
