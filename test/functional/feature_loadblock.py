...
Test starting a node with -loadblock, which loads a linearized chain file.
To generate that file this test uses the helper scripts in contrib/linearize.
"""
...
class LoadblockTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        linearized_file = Path(self.options.tmpdir) / "quicksilver-linear.dat"
        ...
        with open(cfg_file, "a", encoding="utf-8") as cfg:
            ...
            cfg.write(f"output_file={linearized_file}\n")
            cfg.write("max_height=100\n")
            # Quicksilver sandbox message start (chainparams.cpp: 'H','G','Q',0x52).
            # linearize-data.py scans the block files for this magic, so a wrong
            # value does not fail the test -- it never finds a block and spins
            # forever.
            cfg.write("netmagic=48475152\n")
        ...
        self.log.info("Restart second, unsynced node with linearized chain file")
        self.restart_node(1, extra_args=[f"-loadblock={linearized_file}"])
