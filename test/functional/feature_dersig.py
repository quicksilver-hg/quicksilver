...
Test the DERSIG soft-fork activation on sandbox.
"""
...
class BIP66Test(QuicksilverTestFramework):
    ...
    def create_tx(self, input_txid):
        utxo_to_spend = self.minivault.get_utxo(txid=input_txid, mark_as_spent=False)
        return self.minivault.create_self_transfer(utxo_to_spend=utxo_to_spend)['tx']
    ...
    def run_test(self):
        ...
        self.coinbase_txids = [self.nodes[0].getblock(b)['tx'][0] for b in self.generate(self.minivault, DERSIG_HEIGHT - 2)]
        ...
        block = create_block(int(tip, 16), create_coinbase(DERSIG_HEIGHT - 1, nValue=0), block_time, txlist=[spendtx])
        block.solve()
        ...
        tip = block.sha256
        block_time += 1
        block = create_block(tip, create_coinbase(DERSIG_HEIGHT, nValue=0), block_time, version=2)
        block.solve()
        ...
        # rejected from the relaypool for exactly that reason.
        ...
        assert_equal(
            ...
            self.nodes[0].testrelaypoolaccept(rawtxs=[spendtx.serialize().hex()]),
        )
