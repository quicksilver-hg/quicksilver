...
class BlocksXORTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        self.extra_args = [[
            '-blocksxor=1',
            '-fastprune=1',             # use smaller block files
            '-datacarriersize=100000',  # needed to pad transaction with MiniVault
            '-txpownocycle=1',
        ]]
    ...
    def run_test(self):
        ...
        for _ in range(5):
            vault.send_self_transfer(from_node=node, target_vsize=20000)
            self.generate(vault, 1)
        ...
        self.start_node(0, extra_args=['-blocksxor=0', '-txpownocycle=1'])
