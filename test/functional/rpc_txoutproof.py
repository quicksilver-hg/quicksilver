...
class MerkleBlockTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [
            ["-txpownocycle=1"],
            ["-txindex", "-txpownocycle=1"],
        ]

    def run_test(self):
        minivault = MiniVault(self.nodes[0])

        chain_height = self.nodes[1].getblockcount()
        assert_equal(chain_height, 200)

        txid1 = minivault.send_self_transfer(from_node=self.nodes[0])['txid']
        txid2 = minivault.send_self_transfer(from_node=self.nodes[0], confirmed_only=True)['txid']
        ...
        txin_spent = minivault.get_utxo(txid=txid2)  # Get the change from txid2
        tx3 = minivault.send_self_transfer(from_node=self.nodes[0], utxo_to_spend=txin_spent)
        txid3 = tx3['txid']
        self.generate(self.nodes[0], 1)
