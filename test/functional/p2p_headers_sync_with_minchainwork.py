...
class RejectLowDifficultyHeadersTest(QuicksilverTestFramework):
    ...
    def test_chains_sync_when_long_enough(self):
        ...
        with self.nodes[1].assert_debug_log(expected_msgs=["[mesh:debug] Ignoring low-work chain (height=14)"]), self.nodes[2].assert_debug_log(expected_msgs=["[mesh:debug] Ignoring low-work chain (height=14)"]), self.nodes[3].assert_debug_log(expected_msgs=["syncing stage=headers height=14"]):
        ...
        with self.nodes[2].assert_debug_log(expected_msgs=["[mesh:debug] Ignoring low-work chain (height=15)"]), self.nodes[3].assert_debug_log(expected_msgs=["syncing stage=headers height=15"]):
    ...
    def test_large_reorgs_can_succeed(self):
        self.log.info("Test that a large reorg after headers presync can succeed")

        self.sync_all() # Ensure all nodes are synced.
        self.disconnect_all()

        # Keep this large enough to exercise headers presync and reorg handling,
        # but below the old 4000+ block fixture that makes Quicksilver
        # sandbox block download dominate this focused P2P test.
        BLOCKS_TO_MINE = 512
