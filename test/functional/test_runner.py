...
`test/functional/test_framework/test_framework.py:QuicksilverTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    # feature_pruning.py wants ~4GB of scratch disk, which is why upstream keeps it
    # out of the default run. It covers the block filter index and scanblocks against
    # pruned data, and a green default run that skips it says nothing about either.
    'feature_pruning.py',
    'feature_quicksilver_bringup.py',
    'feature_quicksilver_feeless.py',
    'feature_quicksilver_no_fee_rpcs.py',
    'feature_quicksilver_txpow_anchor.py',
    'p2p_txpow_anchor_no_ban.py',
    'feature_quicksilver_txwork_pricing.py',
    'feature_quicksilver_congestion.py',
    'feature_quicksilver_replace.py',
    'feature_quicksilver_mine.py',
    'feature_quicksilver_gbt.py',
    'feature_quicksilver_agent.py',
    'feature_block.py',
    'vault_conflicts.py',
    ...
    'relaypool_updatefromblock.py',
    'relaypool_persist.py',
    # vv Tests less than 60s vv
    'rpc_psqt.py',
    'vault_fundrawtransaction.py',
    'vault_archive.py',
    'vault_backup.py',
    'p2p_tx_download.py',
    'vault_avoidreuse.py',
    ...
    'rpc_txoutproof.py',
    'rpc_orphans.py',
    'feature_minivault_utxo.py',
    'vault_listreceivedby.py',
    'feature_reindex.py',
    'feature_blockfilter_agreement.py',
    ...
    'vault_hd.py',
    'vault_blank.py',
    'vault_keypool_topup.py',
    'vault_fast_rescan.py',
    'vault_gethdkeys.py',
    'vault_createvaultdescriptor.py',
    'interface_zmq.py',
    'rpc_invalid_address_message.py',
    'rpc_validateaddress.py',
    'interface_quicksilver_cli.py',
    'feature_bind_extra.py',
    'relaypool_resurrect.py',
    'vault_txn_doublespend.py --mineblock',
    'tool_vault.py',
    'rpc_getchaintips.py',
    'rpc_misc.py',
    'interface_rest.py',
    'relaypool_spend_coinbase.py',
    'relaypool_reorg.py',
    'p2p_block_sync.py --v1transport',
    'p2p_block_sync.py --v2transport',
    'vault_createvault.py --usecli',
    'vault_createvault.py',
    'vault_reindex.py',
    'vault_reorgsrestore.py',
    'interface_http.py',
    'interface_rpc.py',
    'interface_usdt_net.py',
    'interface_usdt_relaypool.py',
    'interface_usdt_utxocache.py',
    'interface_usdt_validation.py',
    'rpc_users.py',
    'rpc_whitelist.py',
    'feature_proxy.py',
    'vault_signrawtransactionwithvault.py',
    'rpc_signrawtransactionwithkey.py',
    'rpc_signmessage.py',
    'vault_transactiontime_rescan.py',
    'p2p_addrv2_relay.py',
    ...
    'example_test.py',
    'vault_multisig_descriptor_psqt.py',
    'vault_miniscript_decaying_multisig_descriptor_psqt.py',
    'vault_txn_doublespend.py',
    'feature_notifications.py',
    'rpc_getblockfilter.py',
    'rpc_getblockfrompeer.py',
    'rpc_invalidateblock.py',
    'feature_utxo_set_hash.py',
    'relaypool_packages.py',
    'relaypool_package_onemore.py',
    'relaypool_package_limits.py',
    'tool_qscalibrate.py',
    'feature_versionbits_warning.py',
    'feature_blocksxor.py',
    'rpc_preciousblock.py',
    'vault_importprunedfunds.py',
    ...
    'vault_simulaterawtx.py',
    'vault_listsinceblock.py',
    'vault_listdescriptors.py',
    'p2p_leak.py',
    'vault_encryption.py',
    'feature_dersig.py',
    'feature_cltv.py',
    'rpc_uptime.py',
    'feature_discover.py',
    'vault_resendvaulttransactions.py',
    ...
    'vault_sendmany.py',
    'vault_create_tx.py',
    'vault_rescan_unconfirmed.py',
    'p2p_fingerprint.py',
    'feature_uacomment.py',
    'feature_init.py',
    'vault_coinbase_category.py',
    'feature_filelock.py',
    ...
    'feature_framework_minivault.py',
    'relaypool_unbroadcast.py',
    'relaypool_accept_wtxid.py',
    'relaypool_sigoplimit.py',
    ...
    'feature_logging.py',
    'feature_anchors.py',
    'relaypool_datacarrier.py',
    'feature_coinstatsindex.py',
    'vault_orphanedreward.py',
    'vault_timelock.py',
    'p2p_permissions.py',
    ...
    'rpc_relaypool_info.py',
    'rpc_relaypool_names.py',
    'rpc_help.py',
    'p2p_handshake.py',
    'p2p_handshake.py --v2transport',
    'feature_dirsymlinks.py',
    'feature_generated_docs.py',
    'feature_help.py',
    'feature_shutdown.py',
    'p2p_seednode.py',
...
NON_SCRIPTS = [
    ...
    "create_cache.py",
    "test_runner.py",
    # Measurement harnesses for the transaction-cost and chain-growth calibration
    # (tools/calibration/m4-tx-sizes, m5-disk-cost). They assert nothing, take
    # minutes to hours, and are run by hand with parameters -- their READMEs record
    # the exact command. Listing them as tests aborted the WHOLE runner on the
...
# dust, and ephemeral-dust functional tests are intentionally removed
# from the tree. Replacement coverage lives in the Quicksilver
# no-fee-RPC, feeless, vault, PSQT, and relaypool tests.
...
def main():
    ...
    if tests:
        ...
        #   `test/functional/test_runner.py test/functional/vault*`
        #   `test/functional/test_runner.py ./test/functional/vault*`
        #   `test_runner.py vault*`
        #   but not:
        #   `test/functional/test_runner.py vault*`
        # Multiple wildcards can be passed:
        #   `test_runner.py tool* relaypool*`
        for test in tests:
            script = test.split("/")[-1]
    ...
    if args.exclude:
        ...
        for exclude_test in exclude_tests:
            # A space in the name indicates it has arguments such as "vault_basic.py"
            if ' ' in exclude_test:
    ...
    check_build_test_tree(src_dir=config["environment"]["SRCDIR"], build_dir=config["environment"]["BUILDDIR"])
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Quicksilver functional test suite. "
    ...
    # a hard link or a copy on any platform.
...
def check_script_prefixes():
    ...
    good_prefixes_re = re.compile("^(example|feature|interface|mining|p2p|relaypool|rpc|vault|tool)_")
...
def check_build_test_tree(*, src_dir, build_dir):
    """Check that the build tree's copy of the functional suite matches the source.

    The suite is launched out of the build tree, not the source tree, so the
    mirror the build system creates there is the code that actually runs. When it
    drifts, the suite reports on files nobody edited: a deleted test lingers, an
    added test is missing, or a stale copy shadows the source. Bytecode caches are
    ignored; only the mirrored files themselves are compared."""

    src_root = os.path.join(src_dir, 'test', 'functional')
    build_root = os.path.join(build_dir, 'test', 'functional')
    if os.path.realpath(src_root) == os.path.realpath(build_root):
        return  # Run straight out of the source tree; there is no mirror to drift.
    ...
    problems = []
    src_files = relative_files(src_root)
    build_files = relative_files(build_root)

    for rel in sorted(src_files - build_files):
        problems.append(f"{rel}: in the source tree but not in the build tree")
    for rel in sorted(build_files - src_files):
        problems.append(f"{rel}: in the build tree but not in the source tree")
    for rel in sorted(src_files & build_files):
        build_path = os.path.join(build_root, rel)
        if not os.path.exists(build_path):
            problems.append(f"{rel}: dangling symlink in the build tree")
            continue
        with open(build_path, 'rb') as built, open(os.path.join(src_root, rel), 'rb') as source:
            if built.read() != source.read():
                problems.append(f"{rel}: build tree copy differs from the source")

    if problems:
        print("%sERROR:%s the build tree's functional suite does not match %s:" % (BOLD[1], BOLD[0], src_root))
        print("  %s" % ("\n  ".join(problems)))
        print("Re-run the build to refresh %s, then run the suite again." % build_root)
        sys.exit(1)
...
class RPCCoverage():
    ...
    commands per `quicksilver-cli help` (`rpc_interface.txt`).
