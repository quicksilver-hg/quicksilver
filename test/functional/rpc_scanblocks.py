...
class ScanblocksTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-blockfilterindex=1", "-txpownocycle=1"], []]
    ...
    def run_test(self):
        ...
        # send 1.0, relaypool only
        _, spk_1, addr_1 = getnewdestination()
        vault.send_to(from_node=node, scriptPubKey=spk_1, amount=1 * COIN)

        parent_key = "squb6UShJgvLuWbXhZQZ1fML1Lg1yCsWwqoXY4vG1H3hFXrcUJ3kMeYf22JbXHi8iWoZQ8d1QbA5SBPxQw68h2mUNLdXNVQ4F5kgSzf8F3ox7o6"
        # send 1.0, relaypool only
        # childkey 5 of `parent_key`
        vault.send_to(from_node=node,
                       scriptPubKey=address_to_scriptpubkey("SSD72KVdTrBQQ4vB6SV9zaphFL9o6PXxiD"),
                       amount=1 * COIN)
        ...
        # Mine another block without waiting for the validation-interface queue.
        # scanblocks must synchronize the filter index to its snapshotted stop
        # height instead of silently treating a missing range as an empty one.
        blockhash_new = self.generate(node, 1, sync_fun=self.no_op)[0]
        ...
        out = node.scanblocks("start", [f"addr({addr_1})"], height)
        assert blockhash in out['relevant_blocks']
        assert_equal(height_new, out['to_height'])
        assert_equal(True, out['completed'])
        for v in [False, True]:
        ...
        # use a pre-calculated one that collides with the sandbox genesis block's
        ...
        if not genesis_spks:
            self.log.info("Skip genesis false-positive fixture: Quicksilver sandbox genesis has no basic filter elements")
        else:
            assert_equal(len(genesis_spks), 1)
            genesis_coinbase_spk = list(genesis_spks)[0]
            false_positive_spk = bytes.fromhex("0014000000000000000000000000000000000000047153")
