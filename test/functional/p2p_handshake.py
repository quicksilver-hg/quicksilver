...
#  2. the local chain is close to the tip (<144 blocks; about 12h on Quicksilver)
DESIRABLE_SERVICE_FLAGS_FULL = NODE_NETWORK | NODE_WITNESS
DESIRABLE_SERVICE_FLAGS_PRUNED = NODE_NETWORK_LIMITED | NODE_WITNESS


class P2PHandshakeTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        self.generate_at_mocktime(int(time.time()) - 13 * 3600)  # tip outside the 144-block window, should fail
        self.test_desirable_service_flags(node, [NODE_NETWORK_LIMITED | NODE_WITNESS],
                                          DESIRABLE_SERVICE_FLAGS_FULL, expect_disconnect=True)
        self.generate_at_mocktime(int(time.time()) - 11 * 3600)  # tip inside the 144-block window, should succeed
        self.test_desirable_service_flags(node, [NODE_NETWORK_LIMITED | NODE_WITNESS],
        ...
        with node.assert_debug_log(["unlinked reason=connected-to-self"]):
