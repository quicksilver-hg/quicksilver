...
class VaultLabelsTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        # the same address, so we call twice to get two coinbase labels.
        ...
        spendable_balance = node.getbalance()
        assert spendable_balance > 0

        # there should be 2 address groups
        # each with 1 coinbase address
        ...
        # Send the full spendable balance to a third address not in this vault.
        common_address = self.nodes[1].getnewaddress()
        node.sendmany(
            amounts={common_address: spendable_balance},
        )
        ...
        # Reset so that the "" label has what's expected.
        amount_to_send = 1.0
        ...
        self.log.info('Check labels on a key-disabled tracking vault')
        tracked_addresses = [node.getnewaddress() for _ in range(3)]
        node.createvault(vault_name='tracking', disable_private_keys=True)
        tracking_vault = node.get_vault_rpc('tracking')
        BECH32_VALID = {
            'tracked_0': tracked_addresses[0],
            'tracked_1': tracked_addresses[1],
            'tracked_2': tracked_addresses[2],
        }
        BECH32_INVALID = {
            '❌_VER15_PROG41': 'shg1sqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqajlxj8',
            '❌_VER16_PROB01': 'shg1sqq5r4036',
        }
        for l in BECH32_VALID:
            ad = BECH32_VALID[l]
            tracking_vault.importaddress(label=l, rescan=False, address=ad)
            self.generatetoaddress(node, 1, ad)
            assert_equal(tracking_vault.getaddressesbylabel(label=l), {ad: {'purpose': 'receive'}})
            assert_equal(tracking_vault.getreceivedbylabel(label=l), 0)
        for l in BECH32_INVALID:
            ad = BECH32_INVALID[l]
            assert_raises_rpc_error(
                -5,
                "Address is not valid",
                lambda: tracking_vault.importaddress(label=l, rescan=False, address=ad),
            )
