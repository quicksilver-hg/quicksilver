...
"""Useful util functions for testing the vault"""
...
def get_multisig(node):
    """Generate a fresh 2-of-3 multisig
    ...
    return Multisig(privkeys=privkeys,
                    pubkeys=pubkeys,
...
def bytes_to_wif(b, compressed=True):
    if compressed:
        b += b'\x01'
    return byte_to_base58(b, QCKS_SECRET_KEY)

def generate_keypair(compressed=True, wif=False):
    """Generate a new random keypair and return the corresponding ECKey /
    bytes objects. The private key can also be provided as WIF (vault
    import format) string instead, which is often useful for vault RPC
    interaction."""
    privkey = ECKey()
...
class VaultUnlock():
    """
    A context manager for unlocking a vault with a passphrase and automatically locking it afterward.
    """

    MAXIMUM_TIMEOUT = 999000

    def __init__(self, vault, passphrase, timeout=MAXIMUM_TIMEOUT):
        self.vault = vault
        self.passphrase = passphrase
        self.timeout = timeout

    def __enter__(self):
        self.vault.vaultpassphrase(self.passphrase, self.timeout)

    def __exit__(self, *args):
        _ = args
        self.vault.vaultlock()


class TestFrameworkVaultUtil(unittest.TestCase):
