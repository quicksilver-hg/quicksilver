...
"""Encode and decode Quicksilver addresses.
...
ADDRESS_SHG1_UNSPENDABLE = 'shg1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq55gtdc'
ADDRESS_SHG1_UNSPENDABLE_DESCRIPTOR = 'addr(shg1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq55gtdc)#44rkkqf5'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_SHG1_P2WSH_OP_TRUE = 'shg1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsq4tf2w7'
...
QCKS_PUBKEY_ADDRESS = 63
QCKS_SCRIPT_ADDRESS = 125
QCKS_SECRET_KEY = 170
QCKS_BECH32_HRP = "shg"
QCKS_MAIN_BECH32_HRP = "hg"


def create_deterministic_address_shg1_p2tr_op_true(explicit_internal_key=None):
    """
    ...
    if explicit_internal_key is None:
        assert_equal(address, 'shg1p9yfmy5h72durp7zrhlw9lf7jpwjgvwdg0jr0lqmmjtgg83266lqsuyfjlh')
...
def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    return byte_to_base58(hash, QCKS_PUBKEY_ADDRESS)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    return byte_to_base58(hash, QCKS_SCRIPT_ADDRESS)
...
def program_to_witness(version, program, main=False):
    ...
    return encode_segwit_address(QCKS_MAIN_BECH32_HRP if main else QCKS_BECH32_HRP, version, program)
...
def bech32_to_bytes(address):
    ...
    if hrp not in ['hg', 'phg', 'shg', 'bc', 'tb', 'bcrt']:
        return (None, None)
...
def address_to_scriptpubkey(address):
    ...
    if version is not None:
        return program_to_witness_script(version, payload) # witness scriptPubKey
    payload, version = base58_to_byte(address)
    if version == QCKS_PUBKEY_ADDRESS:
        return keyhash_to_p2pkh_script(payload)
    elif version == QCKS_SCRIPT_ADDRESS:
