...
"""Quicksilver test framework primitive and message structures
...
COIN = 100000000  # 1 coin in base units
MAX_MONEY = 1000000000 * COIN

SEQUENCE_NONFINAL = 0xfffffffe  # Non-final sequence: nLockTime binds, BIP68 relative locktime disabled
...
DEFAULT_ANCESTOR_LIMIT = 25    # default max number of in-relaypool ancestors
DEFAULT_DESCENDANT_LIMIT = 25  # default max number of in-relaypool descendants

# Default setting for -datacarriersize. 80 bytes of data, +1 for OP_RETURN, +2 for the pushdata opcodes.
MAX_OP_RETURN_RELAY = 83

DEFAULT_RELAYPOOL_EXPIRY_HOURS = 336  # hours

MAGIC_BYTES = {
    # Quicksilver rebranded the network magic (kernel/chainparams.cpp pchMessageStart);
    # the p2p mock must match or every connection is dropped with "Wrong MessageStart".
    "mainnet": b"\x48\x47\x51\x53",   # mainnet ("HGQS")
    "publictest": b"\x48\x47\x51\x56",  # publictest (F-147 re-mint; was \x55)
    "sandbox": b"\x48\x47\x51\x52",   # sandbox ("HGQR")
}
...
class CTransaction:
    # Quicksilver: every tx carries a per-tx PoW tail after nLockTime —
    # nAnchorHeight (u32) + nPowNonce (u32) + nCycle (42 x u32) = 176 bytes
    # (primitives/transaction.h Serialize/UnserializeTransaction). It is part of
    # the non-witness serialization, so the txid binds it.
    __slots__ = ("hash", "nAnchorHeight", "nCycle", "nLockTime", "nPowNonce",
                 "version", "sha256", "vin", "vout", "wit")
    ...
    def __init__(self, tx=None):
        if tx is None:
            ...
            self.nAnchorHeight = tx.nAnchorHeight
            self.nPowNonce = tx.nPowNonce
            self.nCycle = list(tx.nCycle)
    ...
    def deserialize(self, f):
        ...
        self.nAnchorHeight = int.from_bytes(f.read(4), "little")
        self.nPowNonce = int.from_bytes(f.read(4), "little")
        self.nCycle = [int.from_bytes(f.read(4), "little") for _ in range(42)]
        self.sha256 = None
        self.hash = None

    def _pow_tail(self):
        r = self.nAnchorHeight.to_bytes(4, "little")
        r += self.nPowNonce.to_bytes(4, "little")
        for e in self.nCycle:
            r += int(e).to_bytes(4, "little")
        return r
    ...
    def serialize_legacy_sighash(self):
        r = b""
        r += self.version.to_bytes(4, "little")
        r += ser_vector(self.vin)
        r += ser_vector(self.vout)
...
class CBlockHeader:
    __slots__ = ("hash", "hashMerkleRoot", "hashPrevBlock", "nBits", "nCongestion",
                 "nCycle", "nNonce", "nTime", "nVersion", "sha256")
    ...
    def __init__(self, header=None):
        if header is None:
            ...
            self.nCongestion = header.nCongestion
            self.nNonce = header.nNonce
            self.nCycle = list(header.nCycle)
    ...
    def deserialize(self, f):
        ...
        self.nCongestion = int.from_bytes(f.read(4), "little")
    ...
    def serialize(self):
        ...
        r += self.nCongestion.to_bytes(4, "little")
    ...
    def __repr__(self):
        return "CBlockHeader(nVersion=%i hashPrevBlock=%064x hashMerkleRoot=%064x nTime=%s nBits=%08x nCongestion=%i nNonce=%08x)" \
            % (self.nVersion, self.hashPrevBlock, self.hashMerkleRoot,
               time.ctime(self.nTime), self.nBits, self.nCongestion, self.nNonce)

BLOCK_HEADER_SIZE = len(CBlockHeader().serialize())
# Quicksilver: the pre-pow runs through nNonce (nVersion 4 + hashPrevBlock 32 +
# hashMerkleRoot 32 + nTime 4 + nBits 4 + nCongestion 4 + nNonce 4 = 84), and the
# header carries the 42-edge Cuckatoo block proof (nCycle) after it, so the serialized
# header is 84 + 42*4 = 252 bytes (matches C++ GetHash).
assert_equal(BLOCK_HEADER_SIZE, 252)
...
class CBlock(CBlockHeader):
    ...
    def is_valid(self):
        self.calc_sha256()
        # Quicksilver targets the Cuckatoo cycle's proof hash, not the header's
        # SHA256d (pow.cpp CheckProofOfWorkImpl). Miners never grind nNonce to
        # clear nBits, so testing self.sha256 here is a ~50% coin flip.
        target = uint256_from_compact(self.nBits)
        if proof_hash_int(self.nCycle) > target:
            return False
        for tx in self.vtx:
            if not tx.is_valid():
                return False
    ...
    def solve(self):
        """Satisfy the block proof of work.

        Quicksilver's block PoW is a Cuckatoo cycle in nCycle, not a nonce grind
        over the header hash: CheckProofOfWorkImpl thresholds the cycle's proof
        hash against nBits and never looks at the header's SHA256d. On the sandbox
        network create_block leaves the default all-zero cycle, whose proof hash
        already clears the target, so there is nothing to search. The inherited
        nNonce loop ground a value consensus does not read.
        """
        self.rehash()
