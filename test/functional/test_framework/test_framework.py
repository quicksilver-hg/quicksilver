...
from .address import create_deterministic_address_shg1_p2tr_op_true
...
TMPDIR_PREFIX = "quicksilver_func_test_"
...
class QuicksilverTestMetaClass(type):
    """Metaclass for QuicksilverTestFramework.

    Ensures that any attempt to register a subclass of `QuicksilverTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'QuicksilverTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("QuicksilverTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("QuicksilverTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class QuicksilverTestFramework(metaclass=QuicksilverTestMetaClass):
    """Base class for a Quicksilver test script.

    Individual Quicksilver test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def __init__(self, test_file) -> None:
        ...
        self.default_vault_name = "default_vault"
        self.vault_data_filename = "vault.dat"
        # Optional list of vault names that can be set in set_test_params to
        # create and import keys to. If unset, default is len(nodes) *
        # [default_vault_name]. If vault names are None, vault creation is
        # skipped. If list is truncated, vault creation is skipped and keys
        # are not imported.
        self.vault_names = None
        # By default the vault is not required. Set to true by skip_if_no_vault().
        # When False, we ignore vault_names regardless of what it is.
        self._requires_vault = False
        # Disable ThreadOpenConnections by default, so that adding entries to
        # addrman will not result in automatic connections to them.
        self.disable_autoconnect = True
        self.set_test_params()
        assert self.vault_names is None or len(self.vault_names) <= self.num_nodes
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use quicksilver-cli instead of RPC for all commands")
        ...
        # Quicksilver: every spend grinds a per-tx proof of work, and since the Stage 2
        # flag day the work required scales with the transaction's serialized bytes. A
        # heavy script building large transactions can hold a core for minutes, so under
        # -j8 whichever script loses the CPU race exceeds the RPC timeout and fails --
        # a different one each run, which reads as flakiness rather than as the timeout
        # being wrong. Upstream's default of 1 assumes transactions are cheap to build.
        #
        # 12 is measured, not guessed: at 4, vault_send and vault_fundrawtransaction
        # fail even at -j2; at 12 they pass, vault_send in 99s against the 360s the
        # factor allows. Raising it costs nothing on a passing run, because the factor
        # scales limits rather than waits.
        self.options.timeout_factor = self.options.timeout_factor or (48 if self.options.valgrind else 12)
        ...
        if not getattr(self.options, "enable_vault", False):
            # Vault is not required by the test. Start nodes with -disablevault.
            self.options.enable_vault = False
        elif not self.is_sqlite_compiled():
            self.options.enable_vault = False
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("QuicksilverRPC")
    ...
    def import_deterministic_coinbase_privkeys(self):
        for i in range(self.num_nodes):
            self.init_vault(node=i)

    def init_vault(self, *, node):
        vault_name = self.default_vault_name if self.vault_names is None else self.vault_names[node] if node < len(self.vault_names) else False
        if vault_name is not False:
            n = self.nodes[node]
            if vault_name is not None:
                n.createvault(vault_name=vault_name, load_on_startup=True)
            n.importprivkey(privkey=n.get_deterministic_priv_key().key, label='coinbase', rescan=True)

    # Only enables vault support when the module is available
    def enable_vault_if_possible(self):
        self._requires_vault = self.is_vault_compiled()
    ...
    def add_vault_options(self, parser):
        parser.set_defaults(enable_vault=True)
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None):
        ...
        # Whitelist peers to speed up tx relay / relaypool sync. Don't use it if testing tx relay or timing.
        ...
        for i in range(num_nodes):
            ...
            test_node_i = TestNode(
                ...
                enable_vault=self.options.enable_vault,
    ...
    def sync_relaypools(self, nodes=None, wait=1, timeout=60, flush_scheduler=True):
        """
        ...
        stop_time = time.time() + timeout
        while time.time() <= stop_time:
            pool = [set(r.getrawrelaypool()) for r in rpc_connections]
            ...
            assert (all([len(x.getpeerinfo()) for x in rpc_connections]))
            time.sleep(wait)
        raise AssertionError("RelayPool sync timed out after {}s:{}".format(
            timeout,
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            gen_addresses = [k.address for k in TestNode.PRIV_KEYS][:3] + [create_deterministic_address_shg1_p2tr_op_true()[0]]
            ...
            os.rmdir(cache_path('vaults'))  # Remove empty vaults dir
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in quicksilver.conf

    def _initialize_chain_clean(self):
        """Initialize empty blockchain for use by the test.

        Create an empty blockchain and num_nodes vaults.
    ...
    def skip_if_no_vault(self):
        ...
        self._requires_vault = True
        if not self.is_vault_compiled():
            raise SkipTest("vault has not been compiled.")
    ...
    def skip_if_no_vault_tool(self):
    ...
    def skip_if_no_cli(self):
        ...
        if not self.is_cli_compiled():
            raise SkipTest("quicksilver-cli has not been compiled.")

    def skip_if_no_agent(self):
        """Skip the running test if quicksilver-agent has not been compiled."""
        if not self.is_agent_compiled():
            raise SkipTest("quicksilver-agent has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether quicksilver-cli was compiled."""
        return self.config["components"].getboolean("ENABLE_CLI")

    def is_agent_compiled(self):
        """Checks whether quicksilver-agent was compiled."""
        return self.config["components"].getboolean("ENABLE_AGENT")
    ...
    def is_vault_compiled(self):
        """Checks whether the vault module was compiled."""
        return self.config["components"].getboolean("ENABLE_VAULT")

    def is_specified_vault_compiled(self):
        """Checks whether vault support was compiled."""
    ...
    def is_sqlite_compiled(self):
        """Checks whether the vault module was compiled with Sqlite support."""
