...
def check_json_precision():
    """Make sure json library being used does not lose precision converting Hg values"""
    n = Decimal("20000000.00000003")
    cinnabar = int(json.loads(json.dumps(float(n))) * 1.0e8)
    if cinnabar != 2000000000000003:
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `QuicksilverTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "quicksilver.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def write_config(config_path, *, n, chain, extra_config="", disable_autoconnect=True):
    ...
    with open(config_path, 'w', encoding='utf8') as f:
        ...
        # To improve SQLite vault performance so that the tests don't timeout, use -unsafesqlitesync
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Quicksilver"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Quicksilver"
        else:
            datadir = temp_dir / ".quicksilver"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "quicksilver.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "quicksilver.conf")):
        with open(os.path.join(datadir, "quicksilver.conf"), 'r', encoding='utf8') as f:
            for line in f:
...
def create_lots_of_big_transactions(mini_vault, node, tx_batch_size, txouts, utxos=None):
    from .txpow import prove_tx_pow
    txids = []
    use_internal_utxos = utxos is None
    for _ in range(tx_batch_size):
        tx = mini_vault.create_self_transfer(
            utxo_to_spend=None if use_internal_utxos else utxos.pop(),
        )["tx"]
        tx.vout.extend(txouts)
        prove_tx_pow(tx, anchor_height=node.getblockcount(), target_int=(1 << 240) - 1)
        res = node.testrelaypoolaccept([tx.serialize().hex()])[0]
        assert "fees" not in res
        assert res["allowed"], res
        txids.append(node.sendrawtransaction(tx.serialize().hex()))
    return txids


def mine_large_block(test_framework, mini_vault, node):
    # generate a 66k transaction,
    # and 14 of them is close to the 1MB block limit
    txouts = gen_return_txouts()
    create_lots_of_big_transactions(mini_vault, node, 14, txouts)
...
def find_vout_for_address(node, txid, addr):
    ...
    for i in range(len(tx["vout"])):
        if addr == tx["vout"][i]["output_script"]["address"]:
            return i
