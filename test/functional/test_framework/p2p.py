...
"""Test objects for interacting with a quicksilver-daemon node over the p2p protocol.

The P2PInterface objects interact with the quicksilver-daemon nodes under test using the
...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor, supports_v2_p2p):
        ...
        logger.debug('Connecting to Quicksilver node: %s:%d' % (self.dstaddr, self.dstport))
    ...
    def peer_accept_connection(self, connect_id, connect_cb=lambda: None, *, net, timeout_factor, supports_v2_p2p, reconnect):
        ...
        logger.debug('Listening for Quicksilver node with id: {}'.format(connect_id))
...
class P2PInterface(P2PConnection):
    """A high-level P2P interface class for communicating with a Quicksilver node.
    ...
    def on_version(self, message):
        assert message.nVersion >= P2P_VERSION, "Node advertises unsupported protocol version {}".format(message.nVersion)
...
class P2PDataStore(P2PInterface):
    ...
    def send_txs_and_test(self, txs, node, *, success=True, expect_disconnect=False, reject_reason=None):
        """Send txs to test node and test whether they're accepted to the relaypool.

         - add all txs to our tx_store
         - send tx messages for all txs
         - if success is True/False: assert that the txs are/are not accepted to the relaypool
        ...
        with node.assert_debug_log(expected_msgs=reject_reason):
            ...
            if success:
                # Check that all txs are now in the relaypool
                for tx in txs:
                    assert tx.hash in raw_relaypool, "{} not found in relaypool".format(tx.hash)
            else:
                # Check that none of the txs are now in the relaypool
                for tx in txs:
                    assert tx.hash not in raw_relaypool, "{} tx found in relaypool".format(tx.hash)
...
class P2PTxInvStore(P2PInterface):
    ...
    def wait_for_broadcast(self, txns, *, timeout=60):
        ...
        The relaypool should mark unbroadcast=False for these transactions.
        """
