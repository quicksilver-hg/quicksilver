...
"""Class for quicksilver-daemon node under test"""
...
QUICKSILVERDAEMON_PROC_WAIT_TIMEOUT = 60
# The size of the blocks xor key
# from InitBlocksdirXorKey::xor_key.size()
NUM_XOR_BYTES = 8
# The null blocks key (all 0s)
NULL_BLK_XOR_KEY = bytes([0] * NUM_XOR_BYTES)
QUICKSILVER_PID_FILENAME_DEFAULT = "quicksilver.pid"
...
class TestNode():
    """A class for representing a quicksilver-daemon node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, quicksilverdaemon, quicksilver_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, enable_vault=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the quicksilver.conf file.
        # This means that starting a quicksilver-daemon using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        if not self.enable_vault:
            self.args.append("-disablevault")

        # Run under valgrind when requested.
        if use_valgrind:
        ...
        self.args.append(f"-v2transport={int(v2transport)}")

        self.cli = TestNodeCLI(quicksilver_cli, self.datadir_path)
    ...
    PRIV_KEYS = [
            # address , privkey
            AddressKeyPair('SREoEpuucKKZVFdSYHvaHrmhUNBBrF1yiJ', 'SHojgtCAoDACtcpYiSU4c8aAbz4SVLyUHEUYkYTD2L6xGTvMzF7U'),
            AddressKeyPair('SZJ9UCE6t48ehEhvrFLM4NnwedC8Vx6gUU', 'SGxN4qTuUY8gAkfAYFAtNkezGwRMLa4kgbAL5cVxEnZJ3CaeZXrr'),
            AddressKeyPair('SUapw9fT4oX7ce2tAw9vbaDN4sgBviy1dT', 'SFrBfVtA6368rt8MGHuydJc3FnWptKe7orTV8t96ZEDsZth459qd'),
            AddressKeyPair('SX5xZNZJ9DnrQ5CF3nVj6FyfFvYHWokuZy', 'SHtosSFcX3UkBDapwmt5cc4cyEymEEc4wqrX5Ax1JZ25tVC9iqJV'),
            AddressKeyPair('SZKdLu86YRjtWJ85bckgggXS17UEof1Ja8', 'SCCgjmTYeyaLHWuCgMNkWj24nfYiXPP2wQJ1cXqWjWyiuXV5KmsD'),
            AddressKeyPair('SidqeGV7G2HnQM4arckdDk4eryC2wT6fVS', 'SCaFKXXNFqcq4GSF7hwDPH9uJjwehKsXFniiZiQfhhnUDMknMe5Z'),
            AddressKeyPair('Sfmx8k87nzosfUJ5jrQN38JtpDJfPhXomy', 'SCMJmCSs5WzhQSTcFNQBWaUaXnPwtro3rCSzEzeg1ou8iiyEpDNK'),
            AddressKeyPair('SbYzCN44XHzLE5FcA3C4muHPb44cBUEQ5E', 'SEXFyBfRYCbRTfTVfP8hKfSGHtbnwD7kwfEWFurLMzEGUwoBSwge'),
            AddressKeyPair('SWGAK48sH649JsuLw77RHfoAhAvTX9eTgP', 'SEo2Ry2gyDdUpLZMRNJUiTwAF4nRsfYiwYjNnsN9i2VNMbB9GgR8'),
            AddressKeyPair('SWqhvAKnfcesHE5ZZVAHV69ffP3tsvsKK9', 'SA4aBSnwDCgSwTmAAHG4HcHDencMd1hHS78XjZyvTdAa4mKsS5n5'),
            AddressKeyPair('SW2D21Rh2fcbRYMhKfuLFyM1YPacS4Jt4J', 'SF7Kryows9Kk8DQonPnwvWUcugEHoEkBna7V4VgfqxF1Kj37Ds9N'),
            AddressKeyPair('SgCgsCFXacU1sXHM2jejNwtU3ebkjpkX5p', 'SBhv46GFH8Ewia6MMXA2vh9C7Gv57QgMFq6anVpyFMxfD6tMbBpe'),
    ]
    ...
    def __del__(self):
        # Ensure that we don't leave any quicksilver-daemon processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then quicksilver-daemon would bind P2P ports on
        ...
        # quicksilver.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time quicksilver-daemon is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by quicksilver-daemon, and
        ...
        self.running = True
        self.log.debug("quicksilver-daemon started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the quicksilver-daemon process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'quicksilver-daemon exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                if wait_for_import:
                    self.wait_until(lambda: rpc.getrelaypoolinfo()['loaded'])
                    # Wait for the node to finish reindex, block import, and
                    # loading the relaypool. Usually importing happens fast or
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; quicksilver-daemon is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to quicksilver-daemon after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; quicksilver-daemon is still starting
    ...
    def get_vault_rpc(self, vault_name):
        if self.use_cli:
            return RPCOverloadWrapper(self.cli("-rpcvault={}".format(vault_name)), True)
        else:
            assert self.rpc_connected and self.rpc, self._node_msg("RPC not connected")
            vault_path = "vault/{}".format(urllib.parse.quote(vault_name))
            return RPCOverloadWrapper(self.rpc / vault_path)
    ...
    def wait_until_stopped(self, *, timeout=QUICKSILVERDAEMON_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def replace_in_config(self, replacements):
        ...
        The substitutions are passed as a list of search-replace-tuples, e.g.
            [("old", "new"), ("foo", "bar"), ...]
        """
        with open(self.quicksilverconf, 'r', encoding='utf8') as conf:
            conf_data = conf.read()
        for replacement in replacements:
            assert_equal(len(replacement), 2)
            old, new = replacement[0], replacement[1]
            conf_data = conf_data.replace(old, new)
        with open(self.quicksilverconf, 'w', encoding='utf8') as conf:
    ...
    @property
    def vaults_path(self) -> Path:
        return self.chain_path / "vaults"
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into quicksilver-daemon")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to quicksilver-daemon
        expected_msg: regex that stderr should match when quicksilver-daemon fails

        Will throw if quicksilver-daemon starts without an error.
        Will throw if an expected_msg is provided and it does not match quicksilver-daemon's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'quicksilver-daemon exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'quicksilver-daemon should have exited within {self.rpc_timeout}s '
    ...
    def add_p2p_connection(self, p2p_conn, *, wait_for_verack=True, send_version=True, supports_v2_p2p=None, wait_for_v2_handshake=True, expect_success=True, **kwargs):
        ...
        if wait_for_verack:
            ...
            # transaction that will be added to the relaypool as soon as we return here.
            #
...
class TestNodeCLI():
    """Interface to quicksilver-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.quicksilvercli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with quicksilver-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run quicksilver-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running quicksilver-cli {}".format(p_args[2:]))
...
class RPCOverloadWrapper():
    ...
    def createvault(self, vault_name, disable_private_keys=None, blank=None, passphrase='', avoid_reuse=None, load_on_startup=None, external_signer=None):
        return self.__getattr__('createvault')(vault_name, disable_private_keys, blank, passphrase, avoid_reuse, load_on_startup, external_signer)
