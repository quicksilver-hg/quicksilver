...
# 2 hashes required per sandbox block (with no difficulty adjustment)
SANDBOX_WORK_PER_BLOCK = 2

class MinimumChainWorkTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        starting_chain_work = SANDBOX_WORK_PER_BLOCK # Genesis block's work
        self.log.info(f"Testing relay across node 1 (minChainWork = {self.node_min_work[1]})")

        starting_blockcount = self.nodes[2].getblockcount()

        num_blocks_to_generate = int((self.node_min_work[1] - starting_chain_work) / SANDBOX_WORK_PER_BLOCK)
        ...
        # Because nodes in sandbox are all manual connections (eg using
