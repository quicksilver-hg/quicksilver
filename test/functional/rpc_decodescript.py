...
def with_dummy_txpow_tail(tx_hex):
    """Append a zeroed Quicksilver tx-PoW tail to static decode-only vectors."""
    return tx_hex + "00" * (4 + 4 + 42 * 4)


class DecodeScriptTest(QuicksilverTestFramework):
    ...
    def decodescript_script_pub_key(self):
        ...
        assert_equal('0 ' + public_key_hash, rpc_result['witness']['asm'])
        ...
        assert_equal('witness_v0_keyhash', rpc_result['witness']['type'])
        assert_equal('0 ' + public_key_hash, rpc_result['witness']['asm'])
        ...
        assert_equal('witness_v0_scripthash', rpc_result['witness']['type'])
        assert_equal('0 ' + multisig_script_hash, rpc_result['witness']['asm'])
        ...
        # P2SH does not work in witness scripts. decodescript should not return a result for it.
        assert 'witness' not in rpc_result
        ...
        assert_equal('0 ' + cltv_script_hash, rpc_result['witness']['asm'])
        ...
        # uncompressed pubkeys are invalid for checksigs in witness scripts.
        ...
        # just imagine that the pub keys used below are different.
        # the purpose of this test is to check that a witness script is not returned for bare multisig scripts
        ...
        # Witness programs cannot be wrapped.
        ...
        assert_equal('shg1pfeesctsvuq', rpc_result['address'])
    ...
    def decoderawtransaction_asm_sighashtype(self):
        ...
        tx = with_dummy_txpow_tail('')
        rpc_result = self.nodes[0].decoderawtransaction(tx)
        assert_equal('304402207174775824bec6c2700023309a168231ec80b82c6069282f5133e6f11cbb04460220570edc55c7c5da2ca687ebd0372d3546ebc3f810516a002350cac72dfe192dfb[ALL] 04d3f898e6487787910a690410b7a917ef198905c27fb9d3b0a42da12aceae0544fc7088d239d9a48f2828a15a09e84043001f27cc80d162cb95404e1210161536', rpc_result['vin'][0]['input_script']['asm'])

        # this test case uses a mainnet transaction that has a P2SH input and both P2PKH and P2SH outputs.
        # it's from James D'Angelo's awesome introductory videos about multisig: https://www.youtube.com/watch?v=zIbUSaZBJgU and https://www.youtube.com/watch?v=OSA1pwlaypc
        # verify that we have not altered scriptPubKey decoding.
        tx = with_dummy_txpow_tail('')
        rpc_result = self.nodes[0].decoderawtransaction(tx)
        assert_equal('0f6dce7daf3498259ef60d4ab4a9f95ec1f60f1553364ee712ff3e7da742c7b0', rpc_result['txid'])
        assert_equal('0 3045022100ae3b4e589dfc9d48cb82d41008dc5fa6a86f94d5c54f9935531924602730ab8002202f88cf464414c4ed9fa11b773c5ee944f66e9b05cc1e51d97abc22ce098937ea[ALL] 3045022100b44883be035600e9328a01b66c7d8439b74db64187e76b99a68f7893b701d5380220225bf286493e4c4adcf928c40f785422572eb232f84a0b83b0dea823c3a19c75[ALL] 5221020743d44be989540d27b1b4bbbcfd17721c337cb6bc9af20eb8a32520b393532f2102c0120a1dda9e51a938d39ddd9fe0ebc45ea97e1d27a7cbd671d5431416d3dd87210213820eb3d5f509d7438c9eeecb4157b2f595105e7cd564b3cdbb9ead3da41eed53ae', rpc_result['vin'][0]['input_script']['asm'])
        assert_equal('OP_DUP OP_HASH160 dc863734a218bfe83ef770ee9d41a27f824a6e56 OP_EQUALVERIFY OP_CHECKSIG', rpc_result['vout'][0]['output_script']['asm'])
        assert_equal('OP_HASH160 2a5edea39971049a540474c6a99edf0aa4074c58 OP_EQUAL', rpc_result['vout'][1]['output_script']['asm'])
        txSave = tx_from_hex(tx)

        self.log.info("- tx not passing DER signature checks")
        # make sure that a specifically crafted op_return value will not pass all the IsDERSignature checks and then get decoded as a sighash type
        tx = with_dummy_txpow_tail('')
        rpc_result = self.nodes[0].decoderawtransaction(tx)
        assert_equal('OP_RETURN 300602010002010001', rpc_result['vout'][0]['output_script']['asm'])

        self.log.info("- tx passing DER signature checks")
        # verify that we have not altered scriptPubKey processing even of a specially crafted P2PKH pubkeyhash and P2SH redeem script hash that is made to pass the der signature checks
        tx = with_dummy_txpow_tail('')
        rpc_result = self.nodes[0].decoderawtransaction(tx)
        assert_equal('OP_DUP OP_HASH160 3011020701010101010101020601010101010101 OP_EQUALVERIFY OP_CHECKSIG', rpc_result['vout'][0]['output_script']['asm'])
        assert_equal('OP_HASH160 3011020701010101010101020601010101010101 OP_EQUAL', rpc_result['vout'][1]['output_script']['asm'])
        ...
        assert_equal(signature_sighash_decoded, rpc_result['vin'][0]['input_script']['asm'])

        # make sure that the sighash decodes come out correctly for a more complex / lesser used case.
        txSave.vin[0].scriptSig = bytes.fromhex(push_signature_2)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal(signature_2_sighash_decoded, rpc_result['vin'][0]['input_script']['asm'])

        self.log.info("- multisig scriptSig")
        txSave.vin[0].scriptSig = bytes.fromhex('00' + push_signature + push_signature_2)
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal('0 ' + signature_sighash_decoded + ' ' + signature_2_sighash_decoded, rpc_result['vin'][0]['input_script']['asm'])
        ...
        rpc_result = self.nodes[0].decoderawtransaction(txSave.serialize().hex())
        assert_equal('OP_RETURN 3011020701010101010101020601010101010101', rpc_result['vin'][0]['input_script']['asm'])
    ...
    def decodescript_miniscript(self):
        ...
        assert res["witness"]["desc"] == "wsh(and_v(and_v(v:hash160(ffffffffffffffffffffffffffffffffffffffff),v:pk(0250929b74c1a04954b78b4b6035e97a5e078a5a0f28ec96d547bfee9ace803ac0)),older(1)))#gm8xz4fl"
        ...
        assert res["witness"]["desc"] == "addr(shg1q73qyfypp47hvgnkjqnav0j3k2lq3v76wg22dk8tmwuz5sfgv66xsf5ug4k)#nuxcetu6"
