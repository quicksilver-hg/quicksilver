...
class ChainstateWriteCrashTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 4
        self.rpc_timeout = 480
        self.supports_cli = False

        # Set -maxrelaypool=0 to turn off relaypool memory sharing with dbcache
        self.base_args = [
        ...
        self.node3_args = ["-blockmaxweight=4000000", "-txpownocycle=1"]
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                utxo_hash = self.nodes[node_index].gettxoutsetinfo()['muhash']
                return utxo_hash
            except Exception:
                # An exception here should mean the node is about to crash.
                # If quicksilver-daemon exits, then try again.  wait_for_node_exit()
                # should raise an exception if quicksilver-daemon doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, quicksilver-daemon isn't coming back up on restart.  Could be a
        # bug in quicksilver-daemon, or we've gotten unlucky with our dbcrash ratio --
    ...
    def sync_node3blocks(self, block_hashes):
        ...
        node3_utxo_hash = self.nodes[3].gettxoutsetinfo()['muhash']
    ...
    def verify_utxo_hash(self):
        ...
        for i in range(3):
            try:
                nodei_utxo_hash = self.nodes[i].gettxoutsetinfo()['muhash']
            except OSError:
    ...
    def run_test(self):
        self.vault = MiniVault(self.nodes[3])
        ...
        utxo_list = []
        for _ in range(5):
            utxo_list.extend(self.vault.send_self_transfer_multi(from_node=self.nodes[3], num_outputs=1000)['new_utxos'])
            self.generate(self.nodes[3], 1, sync_fun=self.no_op)
        assert_equal(len(self.nodes[3].getrawrelaypool()), 0)
        ...
        for i in range(12):
            ...
            utxo_list = self.vault.get_utxos()
            self.log.debug(f"MiniVault utxo count: {len(utxo_list)}")
            if self.restart_counts != [0, 0, 0]:
                self.log.info("Required dbcrash restart coverage reached")
                break
        ...
        if self.crashed_on_restart == 0:
            self.log.info("No crash-during-recovery event observed in this bounded run")
