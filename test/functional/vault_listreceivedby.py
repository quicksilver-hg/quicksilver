...
class ReceivedByTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        # Trying to getreceivedby for an address the vault doesn't own should return an error
        assert_raises_rpc_error(-4, "Address not found in vault", self.nodes[0].getreceivedbyaddress, addr)
        ...
        # getreceivedbylabel returns an error if the vault doesn't own the label
        assert_raises_rpc_error(-4, "Label not found in vault", self.nodes[0].getreceivedbylabel, "dummy")
