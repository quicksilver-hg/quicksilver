...
def mine_large_blocks(node, n):
    ...
    # These blocks are ~950 KB against a sandbox fullness target of 20,000 weight, so the
    # congestion multiplier climbs steeply and compounds down the chain. Seed it from the
    # real tip and carry it forward: the floor default would be wrong from the second
    # block on, and every submit would be rejected as bad-congestion.
    prev_congestion = best_block["congestion"]

    for _ in range(n):
        block = create_block(hashprev=previousblockhash, ntime=mine_large_blocks.nTime,
                             coinbase=create_coinbase(height, script_pubkey=big_script),
                             prev_congestion=prev_congestion)
        block.solve()

        # Submit to the node. Check the result: submitblock reports rejection as a return
        # value rather than raising, so ignoring it turns a block that never lands into a
        # far-away timeout in whatever the caller was waiting for.
        result = node.submitblock(block.serialize().hex())
        assert result is None, f"submitblock rejected a large block: {result}"

        previousblockhash = block.sha256
        prev_congestion = block.nCongestion
        height += 1
...
class PruneTest(QuicksilverTestFramework):
    ...
    def set_test_params(self):
        ...
        # Create nodes 5 to test vault in prune mode, but do not connect
        self.extra_args = [
    ...
    def reorg_back(self):
        ...
        with self.nodes[2].assert_debug_log(expected_msgs=['verifying halted height=', 'reason=no-block-data cause=prune']):
    ...
    def vault_test(self):
        # check that the pruning node's vault is still in good shape
        self.log.info("Stop and start pruning node to trigger vault rescan")
        self.restart_node(2, extra_args=["-prune=550"])
        self.log.info("Success")

        # check that vault loads successfully when restarting a pruned node after IBD.
        # this was reported to fail in #7494.
        self.log.info("Syncing node 5 to test vault")
    ...
    def run_test(self):
        ...
        if self.is_vault_compiled():
            self.log.info("Test vault re-scan")
            self.vault_test()
    ...
    def test_scanblocks_pruned(self):
        # Upstream scanned genesis with the hard-coded scriptPubKey
        # 001400000000000000000000000000000000000cadcb, picked because it
        # false-positives against *Bitcoin's* genesis BIP158 filter. That cannot be
        # made to work here, and not merely because genesis was re-minted: Quicksilver
        # pays genesis to an OP_RETURN mark, and BIP158 skips scripts that begin with
        # OP_RETURN (blockfilter.cpp:194), so the genesis filter is the empty GCS "00"
        # and matches nothing. No constant, hard-coded or derived, can collide with an
        # empty filter.
        #
        # Scan a pruned block carrying a real output instead, and take its
        # scriptPubKey from a node that has not pruned it. The match is then a true
        # positive that no future change to genesis can invalidate. What is under test
        # is unchanged: a filter match names the block, and asking scanblocks to
        # confirm that match against pruned block data fails.
        node = self.nodes[5]
        full_node = self.nodes[1]

        # The first blocks were mined to a real address, before the 645 large blocks
        # whose OP_RETURN coinbases are likewise absent from their filters. They are
        # also the oldest blocks, so a pruning node has certainly discarded them.
        height = 1
        pruneheight = node.getblockchaininfo()["pruneheight"]
        assert_greater_than(pruneheight, height)

        blockhash = node.getblockhash(height)
        assert_equal(blockhash, full_node.getblockhash(height))
        spk = full_node.getblock(blockhash, 2)["tx"][0]["vout"][0]["output_script"]["hex"]
        assert not spk.startswith("6a"), "an OP_RETURN output would be excluded from the filter"

        assert blockhash in node.scanblocks(
            "start", [{"desc": f"raw({spk})"}], height, height)['relevant_blocks']

        assert_raises_rpc_error(-1, "Block not available (pruned data)", node.scanblocks,
            "start", [{"desc": f"raw({spk})"}], height, height, "basic", {"filter_false_positives": True})
