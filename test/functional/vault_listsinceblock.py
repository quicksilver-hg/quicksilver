...
class ListSinceBlockTest(QuicksilverTestFramework):
    ...
    def test_double_spend(self):
        ...
        1. User 1 receives Hg in tx1 from utxo1 in block aa1.
        2. User 2 receives Hg in tx2 from utxo1 (same) in block bb1
        ...
        change = '%.8f' % (float(utxo['amount']) - 1)
        recipient_dict = [{self.nodes[0].getnewaddress(): 1}, {self.nodes[1].getnewaddress(): change}]
        utxo_dicts = [{
            'txid': utxo['txid'],
            'vout': utxo['vout'],
        }]
        txid1 = self.nodes[1].sendrawtransaction(
            prove_raw_tx_pow(self.nodes[1].signrawtransactionwithvault(
                self.nodes[1].createrawtransaction(utxo_dicts, recipient_dict))['hex'], self.nodes[1]))

        # send from nodes[2] using utxo to nodes[3]
        recipient_dict2 = [{self.nodes[3].getnewaddress(): 1}, {self.nodes[2].getnewaddress(): change}]
        self.nodes[2].sendrawtransaction(
            prove_raw_tx_pow(self.nodes[2].signrawtransactionwithvault(
                self.nodes[2].createrawtransaction(utxo_dicts, recipient_dict2))['hex'], self.nodes[2]))
    ...
    def test_double_send(self):
        ...
        utxo = utxos[0]
        change = '%.8f' % (float(utxo['amount']) - 1)
        recipient_dict = [{self.nodes[0].getnewaddress(): 1}, {self.nodes[2].getnewaddress(): change}]
        utxo_dicts = [{
            'txid': utxo['txid'],
            'vout': utxo['vout'],
        }]
        signedtxres = self.nodes[2].signrawtransactionwithvault(
            self.nodes[2].createrawtransaction(utxo_dicts, recipient_dict))
        assert signedtxres['complete']

        signedtx = prove_raw_tx_pow(signedtxres['hex'], self.nodes[2])
    ...
    def test_desc(self):
        ...
        # Create a key-disabled vault tracking two multisig descriptors.
        ...
        assert_equal(len(wo_vault.listsinceblock()["transactions"]), 0)
        ...
        coins = wo_vault.listsinceblock()["transactions"]
    ...
    def test_op_return(self):
        ...
        raw_tx = self.nodes[2].createrawtransaction([], [{self.nodes[2].getnewaddress(): Decimal("0.001")}, {'data': 'aa'}])
        funded_tx = self.nodes[2].fundrawtransaction(raw_tx)
        signed_tx = self.nodes[2].signrawtransactionwithvault(funded_tx['hex'])
        tx_id = self.nodes[2].sendrawtransaction(prove_raw_tx_pow(signed_tx['hex'], self.nodes[2]))

        op_ret_tx = [tx for tx in self.nodes[2].listsinceblock(blockhash=block_hash)["transactions"] if tx['txid'] == tx_id and 'address' not in tx][0]
