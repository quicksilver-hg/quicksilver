...
class V2TransportTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        V1_PREFIX = MAGIC_BYTES["sandbox"] + b"version\x00\x00\x00\x00\x00"
        ...
        wrong_network_magic_prefix = MAGIC_BYTES["publictest"] + V1_PREFIX[4:]
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            with self.nodes[0].wait_for_new_peer():
                s.connect(("127.0.0.1", p2p_port(0)))
