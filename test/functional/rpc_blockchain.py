...
class BlockchainTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        self._test_y2106()
        # This test runs MiniVault transactions under -txpownocycle=1, so level
        # 4's full script/tx-PoW replay would reject the deliberately cheap
        # test cycles. Level 3 still verifies block and coin database consistency.
        assert self.nodes[0].verifychain(3, 0)
    ...
    def _test_prune_disk_space(self):
        ...
        self.restart_node(0, extra_args=self.node_args("-prune=1"))
        self.log.info("Avoid warning when assumed chain size is enough")
        self.restart_node(0, extra_args=self.node_args("-prune=123456789"))
    ...
    def _test_max_future_block_time(self):
        ...
        self.start_node(0, extra_args=self.node_args(f"-mocktime={TIME_RANGE_TIP - MAX_FUTURE_BLOCK_TIME}"))
    ...
    def _test_getblockchaininfo(self):
        ...
        self.restart_node(0, self.node_args('-stopatheight=207'))
        res = self.nodes[0].getblockchaininfo()
        ...
        assert_equal(res['bits'], nbits_str(SANDBOX_N_BITS))
        assert_equal(res['target'], target_str(SANDBOX_TARGET))
    ...
    def _test_getdeploymentinfo(self):
        ...
        self.generate(self.vault, 287 - gbci207["blocks"])
    ...
    def _test_gettxoutsetinfo(self):
        ...
        assert_equal(res['total_amount'], Decimal('3850.50000050'))
        ...
        # Explicitly selecting the default MuHash should not change the result.
        ...
        assert_raises_rpc_error(-8, "is not a valid hash_type", node.gettxoutsetinfo, "hash_serialized_3")
    ...
    def _test_gettxout(self):
        ...
        assert_equal(txout['value'], Decimal('1.00000000'))
        assert_equal(txout['output_script']['address'], self.vault.get_address())
        assert_equal(txout['output_script']['hex'], self.vault.get_output_script().hex())
        decoded_script = node.decodescript(self.vault.get_output_script().hex())
        assert_equal(txout['output_script']['asm'], decoded_script['asm'])
        assert_equal(txout['output_script']['desc'], decoded_script['desc'])
        assert_equal(txout['output_script']['type'], decoded_script['type'])
    ...
    def _test_getblockheader(self):
        ...
        assert_equal(header['bits'], nbits_str(SANDBOX_N_BITS))
        assert_equal(header['target'], target_str(SANDBOX_TARGET))
    ...
    def _test_getnetworkworkps(self):
        ...
        assert_raises_rpc_error(
            ...
            """).strip(),
            lambda: self.nodes[0].getnetworkworkps("a", []),
        )
        assert_raises_rpc_error(
            -8,
            "Block does not exist at specified height",
            lambda: self.nodes[0].getnetworkworkps(100, self.nodes[0].getblockcount() + 1),
        )
        assert_raises_rpc_error(
            -8,
            "Block does not exist at specified height",
            lambda: self.nodes[0].getnetworkworkps(100, -10),
        )
        assert_raises_rpc_error(
            -8,
            "Invalid nblocks. Must be a positive number or -1.",
            lambda: self.nodes[0].getnetworkworkps(-100),
        )
        assert_raises_rpc_error(
            -8,
            "Invalid nblocks. Must be a positive number or -1.",
            lambda: self.nodes[0].getnetworkworkps(0),
        )

        # Genesis block height estimate should return 0
        work_per_second = self.nodes[0].getnetworkworkps(100, 0)
        assert_equal(work_per_second, 0)

        # This should be 2 work units every 10 minutes or 1/300
        work_per_second = self.nodes[0].getnetworkworkps()
        assert abs(work_per_second * 300 - 1) < 0.0001

        # Test setting the first param of getnetworkworkps to -1 returns the average network
        # work per second from the last difficulty change.
        ...
        expected_work_per_second_since_diff_change = self.nodes[0].getnetworkworkps(blocks_since_last_diff_change)

        assert abs(self.nodes[0].getnetworkworkps(-1) - expected_work_per_second_since_diff_change) < 0.0001

        # Ensure long lookups get truncated to chain length
        work_per_second = self.nodes[0].getnetworkworkps(self.nodes[0].getblockcount() + 1000)
        assert work_per_second > 0.003

    def _test_stopatheight(self):
        self.log.info("Test stopping at height")
        assert_equal(self.nodes[0].getblockcount(), HEIGHT)
        self.generate(self.vault, 6)
        assert_equal(self.nodes[0].getblockcount(), HEIGHT + 6)
        self.log.debug('Node should not stop at this height')
        assert_raises(subprocess.TimeoutExpired, lambda: self.nodes[0].process.wait(timeout=3))
        try:
            self.generatetoaddress(self.nodes[0], 1, self.vault.get_address(), sync_fun=self.no_op)
    ...
    def _test_getblock(self):
        ...
        def assert_hexblock_hashes(verbosity):
            block_hex = node.getblock(blockhash, verbosity)
        ...
        def assert_vin_contains_prevout(verbosity):
            ...
            for vin in tx["vin"]:
                ...
                assert_equal(set(vin["prevout"].keys()), set(("value", "height", "generated", "output_script")))
                assert_equal(vin["prevout"]["generated"], True)
                total_vin += vin["prevout"]["value"]
            for vout in tx["vout"]:
                total_vout += vout["value"]
            assert "fee" not in tx
            assert_equal(total_vin, total_vout)
        ...
        self.log.info("Test that getblock doesn't include fee")
        assert_fee_not_in_block(blockhash, 1)

        self.log.info('Test that getblock with verbosity 2 and 3 does not include fee')
        assert_fee_not_in_block(blockhash, 2)
        assert_fee_not_in_block(blockhash, 3)
        ...
        assert_raises_rpc_error(-3, "JSON value of type bool is not of expected type number", node.getblock, blockhash, True)
