...
class UTXOCacheChange(ctypes.Structure):
    ...
    def __repr__(self):
        return f"UTXOCacheChange(outpoint={bytes(self.txid[::-1]).hex()}:{self.index}, height={self.height}, value={self.value}cinnabar, is_coinbase={self.is_coinbase})"
...
class UTXOCacheTracepointTest(QuicksilverTestFramework):
    ...
    def test_uncache(self):
        ...
        invalid_tx = self.vault.create_self_transfer()["tx"]
        ...
        self.log.info(
            "testrelaypoolaccept the invalid transaction to trigger an UTXO-cache uncache")
        result = self.nodes[0].testrelaypoolaccept(
    ...
    def test_add_spent(self):
        ...
        self.vault.send_self_transfer(from_node=self.nodes[0])
        ...
        block_hash = self.generate(self.vault, 1)[0]
        ...
        for (block_index, tx) in enumerate(block["tx"]):
            ...
            for (i, vout) in enumerate(tx["vout"]):
                if vout["output_script"]["type"] != "nulldata":
    ...
    def test_flush(self):
        ...
        self.generate(self.vault, BLOCKS_TO_MINE)
