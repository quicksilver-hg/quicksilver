...
# TestP2PConn: A peer we use to send messages to quicksilver-daemon, and store responses.
...
class CompactBlocksTest(QuicksilverTestFramework):
    ...
    def build_block_on_tip(self, node):
        ...
        # The template's own "congestion" describes the template's transaction set; this
        # block carries only a coinbase, so derive from the parent instead.
        rederive_block_congestion(block, node)
        block.solve()
        return block
    ...
    def make_utxos(self):
        ...
        self.generate(self.vault, COINBASE_MATURITY)
        ...
        for i in range(10):
            value = out_value if i < 9 else total_value - out_value * 9
            tx.vout.append(CTxOut(value, CScript([OP_TRUE])))
        prove_tx_pow(tx, anchor_height=self.nodes[0].getblockcount())

        block2 = self.build_block_on_tip(self.nodes[0])
        block2.vtx.append(tx)
        block2.hashMerkleRoot = block2.calc_merkle_root()
        rederive_block_congestion(block2, self.nodes[0])
        block2.solve()
        self.segwit_node.send_and_ping(msg_no_witness_block(block2))
        assert_equal(int(self.nodes[0].getbestblockhash(), 16), block2.sha256)
        self.utxos.extend([[tx.sha256, i, tx.vout[i].nValue] for i in range(10)])
    ...
    # This test actually causes quicksilver-daemon to (reasonably!) disconnect us, so do this last.
    ...
    # quicksilver-daemon's choice of nonce.
    def test_compactblock_construction(self, test_node):
        ...
        for _ in range(num_transactions):
            hex_tx = self.vault.send_self_transfer(from_node=self.nodes[0])['hex']
    ...
    # Test that quicksilver-daemon requests compact blocks when we announce new blocks
    ...
    def test_getblocktxn_requests(self, test_node):
        ...
        assert block.vtx[1].hash in node.getrawrelaypool()

        # Prefill 4 out of the 6 transactions, and verify that only the one
        # that was not in the relaypool is requested.
        ...
        relaypool = node.getrawrelaypool()
        for tx in block.vtx[1:]:
            assert tx.hash in relaypool
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for quicksilver-daemon to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # quicksilver-daemon will not send blocktxn responses for blocks whose height is
    ...
    def test_low_work_compactblocks(self, test_node):
        ...
        with self.nodes[0].assert_debug_log(['[mesh:debug] Ignoring low-work compact block from peer 0']):
            test_node.send_and_ping(msg_cmpctblock(comp_block.to_p2p()))
