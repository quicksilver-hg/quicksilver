...
def _txid_from_uint256(txid):
    return ser_uint256(txid)[::-1].hex()


def _amount_to_sats(amount):
    return int(Decimal(str(amount)) * COIN)


def balance_and_prove_tx(node, tx, *, anchor_height=None, available_outputs=None, target_int=(1 << 248) - 1):
    """Make hand-built tx fixtures feeless and attach cheap sandbox tx-PoW."""
    if not tx.vin:
        tx.rehash()
        return
    ...
    for txin in tx.vin:
        ...
        if key in available_outputs:
            total_in += available_outputs[key]
            continue
        prevout = node.gettxout(_txid_from_uint256(txin.prevout.hash), txin.prevout.n, True)
        if prevout is None:
            tx.rehash()
            prove_tx_pow(tx, anchor_height=anchor_height if anchor_height is not None else node.getblockcount(), target_int=target_int)
            return
        total_in += _amount_to_sats(prevout["value"])

    total_out = sum(out.nValue for out in tx.vout)
    if total_in > total_out:
        tx.vout.append(CTxOut(total_in - total_out, script_to_p2wsh_script(CScript([OP_TRUE]))))
    tx.rehash()
    prove_tx_pow(tx, anchor_height=anchor_height if anchor_height is not None else node.getblockcount(), target_int=target_int)
...
def test_transaction_acceptance(node, p2p, tx, with_witness, accepted, reason=None, available_outputs=None, target_int=(1 << 248) - 1):
    """Send a transaction to the node and check that it's accepted to the relaypool

    - Submit the transaction over the p2p interface
    - use the getrawrelaypool rpc to check for acceptance."""
    reason = [reason] if reason else []
    balance_and_prove_tx(node, tx, available_outputs=available_outputs, target_int=target_int)
    with node.assert_debug_log(expected_msgs=reason):
        p2p.send_and_ping(msg_tx(tx) if with_witness else msg_no_witness_tx(tx))
        assert_equal(tx.hash in node.getrawrelaypool(), accepted)
...
class SegWitTest(QuicksilverTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            # validation).
            ["-acceptnonstdtxn=1", f"-testactivationheight=segwit@{SEGWIT_HEIGHT}", "-par=1", "-txpownocycle=1"],
            ["-acceptnonstdtxn=0", f"-testactivationheight=segwit@{SEGWIT_HEIGHT}", "-txpownocycle=1"],
        ]
    ...
    def build_next_block(self):
        ...
        block_time = tip_header["mediantime"] + 1
        # Seed the congestion recurrence from the real parent, not from the floor: this
        # test mines a MAX_BLOCK_WEIGHT block partway through, which drives the multiplier
        # well above CONGESTION_ONE for every block after it.
        block = create_block(int(tip, 16), create_coinbase(height), block_time,
                             prev_congestion=tip_header["congestion"])
        block.rehash()
        return block
    ...
    def update_witness_block_with_transactions(self, block, tx_list, nonce=0):
        ...
        available_outputs = {}
        anchor_height = self.nodes[0].getblockcount()
        for tx in tx_list:
            balance_and_prove_tx(self.nodes[0], tx, anchor_height=anchor_height, available_outputs=available_outputs)
            for n, txout in enumerate(tx.vout):
                available_outputs[(tx.sha256, n)] = txout.nValue
        block.vtx.extend(tx_list)
        add_witness_commitment(block, nonce)
        # Every block here is mutated after create_block -- witness commitments,
        # inflated witnesses, swapped coinbases -- and all of that moves the weight
        # that nCongestion folds in. Re-derive before the grind; it sits in the pre-pow.
        rederive_block_congestion(block, self.nodes[0])
        block.solve()
    ...
    def run_test(self):
        ...
        self.test_wrapped_witness_rejected()
    ...
    def test_non_witness_transaction(self):
        ...
        self.generate(self.vault, 99)  # let the block mature
        ...
        balance_and_prove_tx(self.nodes[0], tx)
        self.test_node.send_and_ping(msg_tx(tx))  # make sure the block was processed
        assert tx.hash in self.nodes[0].getrawrelaypool()
    ...
    def test_block_relay(self):
        ...
        if not self.segwit_active:
            ...
            rpc_block = self.nodes[0].getblock(block.hash, 0)
            ...
            rpc_details = self.nodes[0].getblock(block.hash, 1)
    ...
    def test_v0_outputs_arent_spendable(self):
        ...
        # Create a native P2WSH output.
        witness_script = CScript([OP_TRUE])
        script_pubkey = script_to_p2wsh_script(witness_script)

        value = self.utxo[0].nValue // 2

        tx = CTransaction()
        tx.vin = [CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b'')]
        tx.vout = [CTxOut(value, script_pubkey), CTxOut(value, CScript([OP_TRUE]))]
        tx.rehash()
        ...
        block = self.build_next_block()
        self.update_witness_block_with_transactions(block, [p2wsh_tx])
    ...
    def test_standardness_v0(self):
        ...
        tx.vin[0].nSequence = SEQUENCE_NONFINAL  # Non-final, so nLockTime would bind; replaceability does not depend on it
        tx.rehash()

        # This is always accepted, since the relaypool policy is to consider segwit as always active
        ...
        self.sync_relaypools([self.nodes[0], self.nodes[1]])
        ...
        tx3.rehash()
        if not self.segwit_active:
            # Just check relaypool acceptance, but don't add the transaction to the relaypool, since witness is disallowed
            # in blocks and the tx is impossible to mine right now.
            balance_and_prove_tx(self.nodes[0], tx3)
            testres3 = self.nodes[0].testrelaypoolaccept([tx3.serialize_with_witness().hex()])
            ...
            tx3 = tx
            tx3.vout = [tx3_out]
            balance_and_prove_tx(self.nodes[0], tx3, available_outputs={(p2sh_tx.sha256, 0): p2sh_tx.vout[0].nValue}, target_int=(1 << 244) - 1)
            testres3_replaced = self.nodes[0].testrelaypoolaccept([tx3.serialize_with_witness().hex()])
        ...
        available_outputs = {(p2sh_tx.sha256, 0): p2sh_tx.vout[0].nValue} if not self.segwit_active else None
        test_transaction_acceptance(self.nodes[0], self.test_node, tx3, with_witness=True, accepted=True, available_outputs=available_outputs, target_int=(1 << 244) - 1)
    ...
    @subtest
    def test_wrapped_witness_rejected(self):
        """Test that P2SH-wrapped witness programs are rejected by consensus."""

        # Prepare a P2SH output whose redeem script is a witness program.
        ...
        tx.vin.append(CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b""))
        value = self.utxo[0].nValue // 2
        tx.vout.append(CTxOut(value, script_pubkey))
        tx.vout.append(CTxOut(value, CScript([OP_TRUE])))
        tx.rehash()

        # Verify relaypool acceptance and block validity
        test_transaction_acceptance(self.nodes[0], self.test_node, tx, with_witness=False, accepted=True)
        block = self.build_next_block()
        self.update_witness_block_with_transactions(block, [tx])
        ...
        # A witness cannot make the wrapped output spendable.
        spend_tx = CTransaction()
        spend_tx.vin.append(CTxIn(COutPoint(tx.sha256, 0), script_sig))
        spend_tx.vout.append(CTxOut(tx.vout[0].nValue, CScript([OP_TRUE])))
        spend_tx.wit.vtxinwit.append(CTxInWitness())
        spend_tx.wit.vtxinwit[0].scriptWitness.stack = [b'a', witness_script]
        spend_tx.rehash()

        balance_and_prove_tx(self.nodes[0], spend_tx)
        with self.nodes[0].assert_debug_log(
                expected_msgs=[spend_tx.hash, 'was not accepted: mandatory-script-verify-flag-failed (P2SH-wrapped witness programs are not supported)']):
            test_transaction_acceptance(self.nodes[0], self.test_node, spend_tx, with_witness=True, accepted=False)

        # The same rule is mandatory in blocks.
        block = self.build_next_block()
        self.update_witness_block_with_transactions(block, [spend_tx])
        test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                           reason='mandatory-script-verify-flag-failed (P2SH-wrapped witness programs are not supported)')
    ...
    def test_witness_commitments(self):
        ...
        tx2.vout.append(CTxOut(tx.vout[0].nValue, witness_script))
        tx2.wit.vtxinwit.append(CTxInWitness())
        tx2.wit.vtxinwit[0].scriptWitness.stack = [witness_script]
        balance_and_prove_tx(self.nodes[0], tx2, available_outputs={(tx.sha256, 0): tx.vout[0].nValue})
        ...
        tx3.vout.append(CTxOut(tx2.vout[0].nValue, witness_script))
        balance_and_prove_tx(self.nodes[0], tx3, available_outputs={(tx2.sha256, 0): tx2.vout[0].nValue})
    ...
    def test_witness_block_size(self):
        ...
        balance_and_prove_tx(self.nodes[0], parent_tx)

        child_tx = CTransaction()
        child_input_value = 0
        for i in range(NUM_OUTPUTS):
            child_tx.vin.append(CTxIn(COutPoint(parent_tx.sha256, i), b""))
            child_input_value += parent_tx.vout[i].nValue
        child_tx.vout = [CTxOut(child_input_value, CScript([OP_TRUE]))]
        for _ in range(NUM_OUTPUTS):
            child_tx.wit.vtxinwit.append(CTxInWitness())
            child_tx.wit.vtxinwit[-1].scriptWitness.stack = [b'a' * 195] * (2 * NUM_DROPS) + [witness_script]
        balance_and_prove_tx(self.nodes[0], child_tx, available_outputs={(parent_tx.sha256, i): parent_tx.vout[i].nValue for i in range(NUM_OUTPUTS)})
        ...
        # The witness was inflated by megabytes after child_tx was proved, and
        # required tx-PoW scales with serialized WITH-witness bytes, so the tail
        # attached earlier no longer covers this body. Re-prove before committing,
        # or the block is rejected as tx-pow-invalid and never reaches the weight
        # check this subtest is about.
        prove_tx_pow(block.vtx[-1], anchor_height=self.nodes[0].getblockcount())
    ...
    def test_submit_block(self):
        """Test that submitblock validates the block it was handed, nonce included."""
        block = self.build_next_block()

        # A commitment computed over a custom nonce, with the nonce then dropped.
        # The node must not repair the coinbase on the miner's behalf: it rejects the
        # block and names the missing reserved value, rather than a merkle mismatch
        # against a nonce it substituted itself.
        add_witness_commitment(block, nonce=1)
        block.vtx[0].wit = CTxWitness()  # drop the nonce
        # The node weighs exactly the bytes submitted, so the multiplier is derived
        # last, after every change to the body.
        rederive_block_congestion(block, self.nodes[0])
        block.solve()
        assert_equal('bad-witness-nonce-size', self.nodes[0].submitblock(block.serialize().hex()))
        assert self.nodes[0].getbestblockhash() != block.hash

        # The standard nonce, supplied by the miner rather than filled in by the node.
        # This is the one documented line an external miner owes, and it makes what is
        # weighed and ground identical to what is validated.
    ...
    def test_witness_input_length(self):
        ...
        tx2.vout.append(CTxOut(sum(tx.vout[i].nValue for i in range(10)), CScript([OP_TRUE])))
    ...
    def test_tx_relay_after_segwit_activation(self):
        ...
        After segwit activates, verify that relaypool:
        ...
        tx2.vout.append(CTxOut(tx.vout[0].nValue, script_pubkey))
        balance_and_prove_tx(self.nodes[0], tx2, available_outputs={(tx_hash, 0): tx.vout[0].nValue})
        ...
        tx3.vout.append(CTxOut(tx2.vout[0].nValue, script_to_p2sh_script(p2sh_script)))
        ...
        tx3.vout[0] = CTxOut(tx2.vout[0].nValue, CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE]))
    ...
    def test_segwit_versions(self):
        ...
        assert len(self.nodes[0].getrawrelaypool()) == 0
    ...
    def test_uncompressed_pubkey(self):
        ...
        tx.vout.append(CTxOut(utxo.nValue, script_pkh))
        balance_and_prove_tx(self.nodes[0], tx, available_outputs={(utxo.sha256, utxo.n): utxo.nValue})
        ...
        tx2 = CTransaction()
        tx2.vin.append(CTxIn(COutPoint(tx.sha256, 0), b""))
        tx2.vout.append(CTxOut(tx.vout[0].nValue, script_wsh))
        ...
        # Test 2: spend the native P2WSH output to P2PKH.
        script_pubkey = keyhash_to_p2pkh_script(pubkeyhash)
        tx3 = CTransaction()
        tx3.vin.append(CTxIn(COutPoint(tx2.sha256, 0), b""))
        tx3.vout.append(CTxOut(tx2.vout[0].nValue, script_pubkey))
        tx3.wit.vtxinwit.append(CTxInWitness())
        balance_and_prove_tx(self.nodes[0], tx3, available_outputs={(tx2.sha256, 0): tx2.vout[0].nValue})
        ...
        # Test 3: Uncompressed pubkeys remain valid in non-witness transactions.
        tx4 = CTransaction()
        tx4.vin.append(CTxIn(COutPoint(tx3.sha256, 0), b""))
        tx4.vout.append(CTxOut(tx3.vout[0].nValue, CScript([OP_TRUE])))
        balance_and_prove_tx(self.nodes[0], tx4, available_outputs={(tx3.sha256, 0): tx3.vout[0].nValue})
        tx4.vin[0].scriptSig = CScript([pubkey])
        sign_input_legacy(tx4, 0, script_pubkey, key)
    ...
    def test_signature_version_1(self):
        ...
        for sigflag in [0, SIGHASH_ANYONECANPAY]:
            for hashtype in [SIGHASH_ALL, SIGHASH_NONE, SIGHASH_SINGLE]:
                ...
                def signed_spend(input_value):
                    tx = CTransaction()
                    tx.vin.append(CTxIn(COutPoint(prev_utxo.sha256, prev_utxo.n), b""))
                    tx.vout.append(CTxOut(prev_utxo.nValue, script_pubkey))
                    tx.wit.vtxinwit.append(CTxInWitness())
                    balance_and_prove_tx(self.nodes[0], tx, available_outputs={(prev_utxo.sha256, prev_utxo.n): prev_utxo.nValue})
                    sign_p2pk_witness_input(witness_script, tx, 0, hashtype, input_value, key)
                    return tx

                # Too-large input value
                tx = signed_spend(prev_utxo.nValue + 1)
                self.update_witness_block_with_transactions(block, [tx])
                test_witness_block(self.nodes[0], self.test_node, block, accepted=False,
                ...
                tx = signed_spend(prev_utxo.nValue)
                block.vtx.pop()
                self.update_witness_block_with_transactions(block, [tx])
                test_witness_block(self.nodes[0], self.test_node, block, accepted=True)
        ...
        split_values = [split_value] * NUM_SIGHASH_TESTS
        split_values[0] += prev_utxo.nValue - split_value * NUM_SIGHASH_TESTS
        ...
        for i in range(NUM_SIGHASH_TESTS):
            ...
            output_values = [split_value] * num_outputs
            output_values[0] += total_value - split_value * num_outputs
            for value in output_values:
                tx.vout.append(CTxOut(value, script_pubkey))
            balance_and_prove_tx(self.nodes[0], tx, available_outputs={(temp_utxos[i].sha256, temp_utxos[i].n): temp_utxos[i].nValue for i in range(num_inputs)})
            for i in range(num_inputs):
        ...
        tx.wit.vtxinwit.append(CTxInWitness())
        balance_and_prove_tx(self.nodes[0], tx, available_outputs={(temp_utxos[0].sha256, temp_utxos[0].n): temp_utxos[0].nValue})
        ...
        total_value = sum(i.nValue for i in temp_utxos)
        output_value = total_value // 2

        tx = CTransaction()
        index = 0
        # Just spend to our usual anyone-can-spend output
        tx.vout = [CTxOut(output_value, CScript([OP_TRUE])), CTxOut(total_value - output_value, CScript([OP_TRUE]))]
        for i in temp_utxos:
            # Use SIGHASH_ALL|SIGHASH_ANYONECANPAY so we can build up
            # the signatures as we go.
            tx.vin.append(CTxIn(COutPoint(i.sha256, i.n), b""))
            tx.wit.vtxinwit.append(CTxInWitness())
        balance_and_prove_tx(self.nodes[0], tx, available_outputs={(i.sha256, i.n): i.nValue for i in temp_utxos})
        for i in temp_utxos:
    ...
    def test_non_standard_witness(self):
        ...
        # Generate one native P2WSH output for each script.
        outputvalue = (self.utxo[0].nValue - 1000) // len(scripts)
        for i in scripts:
            tx.vout.append(CTxOut(outputvalue, script_to_p2wsh_script(i)))
        tx.rehash()
        test_transaction_acceptance(self.nodes[0], self.test_node, tx, with_witness=False, accepted=True)
        txid = tx.sha256
        ...
        for i in range(len(scripts)):
            p2wsh_tx = CTransaction()
            p2wsh_tx.vin.append(CTxIn(COutPoint(txid, i)))
        ...
        self.generate(self.nodes[0], 1)  # Mine and clean up the relaypool of non-standard node
    ...
    def test_wtxid_relay(self):
        ...
        tx = CTransaction()
        tx.vin.append(CTxIn(COutPoint(self.utxo[0].sha256, self.utxo[0].n), b""))
        tx.vout.append(CTxOut(self.utxo[0].nValue, script_pubkey))
        balance_and_prove_tx(self.nodes[0], tx, available_outputs={(self.utxo[0].sha256, self.utxo[0].n): self.utxo[0].nValue})
        ...
        assert_equal(tx2.hash in self.nodes[0].getrawrelaypool(), True)
