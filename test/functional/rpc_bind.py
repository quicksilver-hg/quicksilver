...
class RPCBindTest(QuicksilverTestFramework):
    ...
    def run_bind_test(self, allow_ips, connect_to, addresses, expected):
        ...
        base_args = ['-disablevault', '-nolisten']
        if allow_ips:
    ...
    def run_test(self):
        ...
        if not self.options.run_nonloopback:
            ...
            if self.options.run_ipv4:
                self.run_invalid_bind_test(['127.0.0.1'], ['127.0.0.1:notaport', '127.0.0.1:-1', '127.0.0.1:0', '127.0.0.1:65536'])
            if self.options.run_ipv6:
                self.run_invalid_bind_test(['[::1]'], ['[::1]:notaport', '[::1]:-1', '[::1]:0', '[::1]:65536'])
