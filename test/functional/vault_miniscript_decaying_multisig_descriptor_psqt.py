...
This is similar to `test/functional/vault_multisig_descriptor_psqt.py`.
"""
...
class VaultMiniscriptDecayingMultisigDescriptorPSQTTest(QuicksilverTestFramework):
    ...
    @staticmethod
    def _get_xpub(vault, internal):
        """Extract the vault's xpubs using `listdescriptors` and pick the one from the `pkh` descriptor since it is least likely to be accidentally reused."""
    ...
    def create_multisig(self, external_xpubs, internal_xpubs):
        """The multisig is created by importing the following descriptors. The resulting vault only tracks the scripts, and every signer can do this."""
        self.node.createvault(vault_name=f"{self.name}", blank=True, disable_private_keys=True)
        multisig = self.node.get_vault_rpc(f"{self.name}")
        # spending policy: `thresh(4,pk(key_1),pk(key_2),pk(key_3),pk(key_4),after(t1),after(t2),after(t3))`
    ...
    def run_test(self):
        ...
        self.log.info("Create the signer vaults and get their xpubs...")
        signers = [self.node.get_vault_rpc(self.node.createvault(vault_name=f"signer_{i}")["name"]) for i in range(self.N)]
        external_xpubs, internal_xpubs = [[self._get_xpub(signer, internal) for signer in signers] for internal in [False, True]]

        self.log.info("Create the tracking-only decaying multisig using signers' xpubs...")
        multisig = self.create_multisig(external_xpubs, internal_xpubs)

        self.log.info("Get a mature utxo to send to the multisig...")
        coordinator_vault = self.node.get_vault_rpc(self.node.createvault(vault_name="coordinator")["name"])
        self.generatetoaddress(self.node, 101, coordinator_vault.getnewaddress())

        self.log.info("Send funds to the multisig's receiving address...")
        deposit_amount = 6.15
        coordinator_vault.sendtoaddress(multisig.getnewaddress(), deposit_amount)
        ...
        for locktime in [0] + self.locktimes:
            ...
            # in this test each signer signs the same psqt "in series" one after the other.
            # Another option is for each signer to sign the original psqt, and then combine
            # and finalize these. In some cases this may be more optimal for coordination.
            psqt = multisig.vaultcreatefundedpsqt(inputs=[], outputs=[{receiver.getnewaddress(): amount}], locktime=locktime)
            ...
            # it is a critical property of this vault.
            for i, m in enumerate(random.sample(range(self.M), self.M)):
                psqt = signers[m].vaultprocesspsqt(psqt["psqt"])
                assert_equal(psqt["complete"], i == self.M - 1)
            ...
            if self.M < self.N:
                ...
                assert_raises_rpc_error(-26, "non-final", multisig.sendrawtransaction, psqt["hex"])
            ...
            multisig.sendrawtransaction(psqt["hex"])
            sent += amount
...
if __name__ == "__main__":
    VaultMiniscriptDecayingMultisigDescriptorPSQTTest(__file__).main()
