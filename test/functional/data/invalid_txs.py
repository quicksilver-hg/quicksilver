...
validation checks through different interfaces (AcceptBlock, AcceptToRelayPool,
...
class BadTxTemplate:
    ...
    # The expected error code given by quicksilver-daemon upon submission of the tx.
    reject_reason: Optional[str] = ""

    # Only specified if it differs from relaypool acceptance error.
    block_reject_reason = ""
    ...
    # the relaypool (i.e. does it violate policy but not consensus)?
...
def getDisabledOpcodeTemplate(opcode):
    ...
    def get_tx(self):
        ...
        tx.vin.append(vin)
        tx.vout.append(CTxOut(self.spend_avail, basic_p2sh))
        tx.calc_sha256()
        return tx
...
def iter_all_templates():
    ...
    Upstream's 64-byte "tx-size-small" template (guarding CVE-2017-12842) has no
    Quicksilver equivalent: the mandatory tx-PoW tail adds 176 bytes to every
    transaction, so a 64-byte transaction cannot be constructed at all.
    """
    return list(BadTxTemplate.__subclasses__())
