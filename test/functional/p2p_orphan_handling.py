...
from test_framework.relaypool_util import tx_in_orphanage
...
def cleanup(func):
    ...
    def wrapper(self):
        try:
            ...
            # Check that relaypool and orphanage have been cleared
            self.wait_until(lambda: len(self.nodes[0].getorphantxs()) == 0)
            assert_equal(0, len(self.nodes[0].getrawrelaypool()))
            self.vault.rescan_utxos(include_relaypool=True)
    return wrapper
...
class OrphanHandlingTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        self.extra_args = [["-txpownocycle=1"]]

    def create_parent_and_child(self):
        """Create package with 1 parent and 1 child."""
        parent = self.vault.create_self_transfer()
        child = self.vault.create_self_transfer(utxo_to_spend=parent['new_utxo'])
    ...
    def test_arrival_timing_orphan(self):
        ...
        node = self.nodes[0]
        tx_parent_arrives = self.vault.create_self_transfer()
        tx_parent_doesnt_arrive = self.vault.create_self_transfer()
        # Fake orphan spends nonexistent outputs of the two parents
        tx_fake_orphan = self.vault.create_self_transfer_multi(utxos_to_spend=[
        ...
        # transaction entered its relaypool. Send unsolicited because otherwise we need to wait for
        # request delays.
        peer_normal.send_and_ping(msg_tx(tx_parent_arrives["tx"]))
        assert tx_parent_arrives["txid"] in node.getrawrelaypool()
    ...
    def test_orphan_multiple_parents(self):
        ...
        self.log.info("Test orphan parent requests with a mixture of confirmed, in-relaypool and missing parents")
        # Independent parents: reserve first. A feeless self-transfer keeps its unique
        # subsidy, so implicit get_utxo re-picks that same unique-largest coin.
        parent_utxos = [self.vault.get_utxo() for _ in range(4)]

        # This UTXO confirmed a long time ago.
        utxo_conf_old = self.vault.send_self_transfer(from_node=node, utxo_to_spend=parent_utxos[0])["new_utxo"]
        txid_conf_old = utxo_conf_old["txid"]
        self.generate(self.vault, 10)
        ...
        utxo_conf_recent = self.vault.send_self_transfer(from_node=node, utxo_to_spend=parent_utxos[1])["new_utxo"]
        self.generate(node, 1)

        # This UTXO is unconfirmed and in the relaypool.
        assert_equal(len(node.getrawrelaypool()), 0)
        relaypool_tx = self.vault.send_self_transfer(from_node=node, utxo_to_spend=parent_utxos[2])
        utxo_unconf_relaypool = relaypool_tx["new_utxo"]

        # This UTXO is unconfirmed and missing.
        missing_tx = self.vault.create_self_transfer(utxo_to_spend=parent_utxos[3])
        utxo_unconf_missing = missing_tx["new_utxo"]
        assert missing_tx["txid"] not in node.getrawrelaypool()

        orphan = self.vault.create_self_transfer_multi(utxos_to_spend=[utxo_conf_old,
            utxo_conf_recent, utxo_unconf_relaypool, utxo_unconf_missing])
        ...
        assert_equal(node.getrelaypoolentry(orphan["txid"])["ancestorcount"], 3)
    ...
    def test_orphans_overlapping_parents(self):
        ...
        confirmed_utxos = [self.vault_nonsegwit.get_utxo() for _ in range(4)]
        ...
        missing_parent_A = self.vault_nonsegwit.create_self_transfer(utxo_to_spend=confirmed_utxos[0])
        # Parents of child_A and child_B
        missing_parent_AB = self.vault_nonsegwit.create_self_transfer(utxo_to_spend=confirmed_utxos[1])
        inflight_parent_AB = self.vault_nonsegwit.create_self_transfer(utxo_to_spend=confirmed_utxos[2])
        # Parent of child_B only
        missing_parent_B = self.vault_nonsegwit.create_self_transfer(utxo_to_spend=confirmed_utxos[3])
        child_A = self.vault_nonsegwit.create_self_transfer_multi(
            utxos_to_spend=[missing_parent_A["new_utxo"], missing_parent_AB["new_utxo"], inflight_parent_AB["new_utxo"]]
        )
        child_B = self.vault_nonsegwit.create_self_transfer_multi(
    ...
    def test_orphan_of_orphan(self):
        ...
        missing_grandparent = self.vault_nonsegwit.create_self_transfer()
        missing_parent_orphan = self.vault_nonsegwit.create_self_transfer(utxo_to_spend=missing_grandparent["new_utxo"])
        missing_parent = self.vault_nonsegwit.create_self_transfer()
        orphan = self.vault_nonsegwit.create_self_transfer_multi(utxos_to_spend=[missing_parent["new_utxo"], missing_parent_orphan["new_utxo"]])
    ...
    def test_same_txid_orphan(self):
        ...
        tx_parent = self.vault.create_self_transfer()

        # Create the real child
        tx_child = self.vault.create_self_transfer(utxo_to_spend=tx_parent["new_utxo"])
        ...
        node_relaypool = node.getrawrelaypool()
        assert tx_parent["txid"] in node_relaypool
        assert tx_child["txid"] in node_relaypool
        assert_equal(node.getrelaypoolentry(tx_child["txid"])["wtxid"], tx_child["wtxid"])
    ...
    def test_same_txid_orphan_of_orphan(self):
        ...
        tx_grandparent = self.vault.create_self_transfer()

        # Create middle tx (both parent and child) which will be in orphanage.
        tx_middle = self.vault.create_self_transfer(utxo_to_spend=tx_grandparent["new_utxo"])
        ...
        # have the same txid).
        tx_grandchild = self.vault.create_self_transfer(utxo_to_spend=tx_middle["new_utxo"])
        ...
        # The real child should be accepted and the fake one rejected.
        node_relaypool = node.getrawrelaypool()
        assert tx_grandparent["txid"] in node_relaypool
        assert tx_middle["txid"] in node_relaypool
        assert tx_grandchild["txid"] in node_relaypool
        assert_equal(node.getrelaypoolentry(tx_middle["txid"])["wtxid"], tx_middle["wtxid"])
    ...
    def test_orphan_txid_inv(self):
        ...
        # tx_orphan_bad_wit is rejected for txn-same-nonwitness-data-in-relaypool (no punishment).
        node_relaypool = node.getrawrelaypool()
        assert tx_parent["txid"] in node_relaypool
        assert tx_child["txid"] in node_relaypool
        assert_equal(node.getrelaypoolentry(tx_child["txid"])["wtxid"], tx_child["wtxid"])
    ...
    def test_max_orphan_amount(self):
        ...
        for _ in range(DEFAULT_MAX_ORPHAN_TRANSACTIONS):
            tx_parent_1 = self.vault.create_self_transfer()
            tx_child_1 = self.vault.create_self_transfer(utxo_to_spend=tx_parent_1["new_utxo"])
    ...
    def run_test(self):
        ...
        self.vault_nonsegwit = MiniVault(self.nodes[0], mode=MiniVaultMode.RAW_P2PK)
        self.generate(self.vault_nonsegwit, 10)
        self.vault = MiniVault(self.nodes[0])
        self.generate(self.vault, 160)

        self.test_arrival_timing_orphan()
        self.log.info("Skip zero-fee rejected-parent orphan checks: Quicksilver transactions are feeless")
        self.test_orphan_multiple_parents()
        self.test_orphans_overlapping_parents()
        self.test_orphan_of_orphan()
        self.log.info("Skip rejected-parent inheritance checks: Quicksilver accepts zero-fee parents")
        ...
        self.log.info("Skip orphan parent replacement-cycle checks: Quicksilver replacement is surplus-work based")
