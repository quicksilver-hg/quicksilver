...
class BIP68_112_113Test(QuicksilverTestFramework):
    ...
    def create_self_transfer_from_utxo(self, input_tx):
        utxo = self.minivault.get_utxo(txid=input_tx.rehash(), mark_as_spent=False)
        tx = self.minivault.create_self_transfer(utxo_to_spend=utxo)['tx']
        return tx

    def create_bip112special(self, input, txversion):
        tx = self.create_self_transfer_from_utxo(input)
        tx.version = txversion
        self.minivault.sign_tx(tx)
    ...
    def send_generic_input_tx(self, coinbases):
        ...
        utxo_to_spend = self.minivault.get_utxo(txid=input_txid)
        return self.minivault.send_self_transfer(from_node=self.nodes[0], utxo_to_spend=utxo_to_spend)['tx']
    ...
    def generate_blocks(self, number):
        ...
        for _ in range(number):
            ...
            self.tipheight += 1
            self.tipcongestion = block.nCongestion
        return test_blocks

    def create_test_block(self, txs):
        for tx in txs:
            prove_tx_pow(tx, anchor_height=self.tipheight)
        block = create_block(self.tip, create_coinbase(self.tipheight + 1), self.last_block_time + 600, txlist=txs,
                             prev_congestion=self.tipcongestion)
        block.solve()
        return block
    ...
    def run_test(self):
        ...
        self.minivault = MiniVault(self.nodes[0], mode=MiniVaultMode.RAW_P2PK)

        self.log.info("Generate blocks in the past for coinbase outputs.")
        long_past_time = int(time.time()) - 600 * 1000  # enough to build up to 1000 blocks 10 minutes apart without worrying about getting into the future
        self.nodes[0].setmocktime(long_past_time - 100)  # enough so that the generated blocks will still all be before long_past_time
        self.coinbase_blocks = self.generate(self.minivault, COINBASE_BLOCK_COUNT)  # blocks generated for inputs
        self.nodes[0].setmocktime(0)  # set time back to present so yielded blocks aren't in the future as we advance last_block_time
        self.tipheight = COINBASE_BLOCK_COUNT  # height of the next block to build
        self.last_block_time = long_past_time
        self.tip = int(self.nodes[0].getbestblockhash(), 16)
        # Quicksilver (#5c-1 Phase 2): nCongestion is consensus-checked and derived from the
        # parent's value, so it has to be tracked alongside tip/tipheight. This test builds
        # runs of blocks before sending them, so the parent is often a block the node has
        # not seen yet and cannot be asked about.
        self.tipcongestion = self.nodes[0].getblock(self.nodes[0].getbestblockhash())["congestion"]
        ...
        self.tipheight += 1
        self.last_block_time += 600
        self.tipcongestion = self.nodes[0].getblock(inputblockhash)["congestion"]
        ...
        self.minivault.sign_tx(bip113tx_v1)
        ...
        self.minivault.sign_tx(bip113tx_v2)
