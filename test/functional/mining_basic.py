...
class MiningTest(QuicksilverTestFramework):
    def set_test_params(self):
        self.num_nodes = 3
        self.extra_args = [
            ["-txpownocycle=1"],
            ["-txpownocycle=1"],
            ["-fastprune", "-prune=1", "-txpownocycle=1"]
        ]
    ...
    def mine_chain(self):
        ...
        self.restart_node(0, extra_args=[f'-mocktime={t}', '-blockversion=1337', '-txpownocycle=1'])
        self.connect_nodes(0, 1)
        assert_equal(1337, self.nodes[0].getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)['version'])
        self.restart_node(0, extra_args=[f'-mocktime={t}', '-txpownocycle=1'])
        self.connect_nodes(0, 1)
    ...
    def run_test(self):
        ...
        txid = int(self.vault.send_self_transfer(from_node=node)['wtxid'], 16)
        ...
        # Mine a block to leave initial block download and clear the relaypool
        self.generatetoaddress(node, 1, node.get_deterministic_priv_key().address)
        tmpl = node.getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)
        self.log.info("getblocktemplate: Test capability advertised")
        assert 'proposal' in tmpl['capabilities']
        assert 'coinbasetxn' not in tmpl
        assert 'coinbaseaux' not in tmpl
        assert 'vbrequired' not in tmpl
        ...
        # Quicksilver (#5c-1 Phase 2): the template publishes the congestion multiplier
        # for its own transaction set; this block uses that set unchanged, so it is the
        # value consensus will demand. It sits inside the pre-pow, so set it before solve().
        ...
        self.vault._prove_tx_pow(bad_tx)
        bad_tx.rehash()
        ...
        self.log.info("Zero-peer getblocktemplate names the bootstrap path")
        # The gate itself is main-only, so a sandbox node cannot exercise it.
        # What is testable here is that the string a bootstrapping operator will
        # read exists and names startmining. Keep this assertion in step with
        # src/rpc/mining.cpp; the gate is walked for real on rehearsal-main.
        help_text = self.nodes[0].help("startmining")
        assert "startmining" in help_text
        gbt_help = self.nodes[0].help("getblocktemplate")
        assert "startmining" in gbt_help, (
            "getblocktemplate help must point a bootstrapping operator at startmining")
