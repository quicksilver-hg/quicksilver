...
VB_PERIOD = 144           # versionbits period length for sandbox
VB_THRESHOLD = 108        # versionbits activation threshold for sandbox
...
class VersionBitsWarningTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        for rpc in [node.getblockchaininfo, node.getmininginfo, node.getnetworkinfo]:
            assert not VB_PATTERN.match(",".join(rpc()["warnings"]))
        ...
        # Stop-start the node. This is required because quicksilver-daemon will only warn once about unknown versions or unknown rules activating.
        self.restart_node(0)
        ...
        for rpc in [node.getblockchaininfo, node.getmininginfo, node.getnetworkinfo]:
            assert WARN_UNKNOWN_RULES_ACTIVE in ",".join(rpc()["warnings"])
