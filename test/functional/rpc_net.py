...
def seed_addrman(node):
    ...
    assert_equal(node.addpeeraddress(address="1.2.3.4", tried=True, port=9555), success)
    assert_equal(node.addpeeraddress(address="2.0.0.0", port=9555), success)
    assert_equal(node.addpeeraddress(address="1233:3432:2434:2343:3234:2345:6546:4534", tried=True, port=9555), success)
    assert_equal(node.addpeeraddress(address="2803:0:1234:abcd::1", port=45324), success)
    assert_equal(node.addpeeraddress(address="fc00:1:2:3:4:5:6:7", port=9555), success)
    assert_equal(node.addpeeraddress(address="pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion", tried=True, port=9555), success)
    ...
    assert_equal(node.addpeeraddress(address="c4gfnttsuwqomiygupdqqqyy5y5emnk5c73hrfvatri67prd7vyq.b32.i2p", port=9555), success)


class NetTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        # We need minivault to make a transaction
    ...
    def test_getpeerinfo(self):
        ...
        self.vault.send_self_transfer(from_node=self.nodes[0]) # Make a transaction so we can see it in the getpeerinfo results
        ...
        assert "minfeefilter" not in peer_info[0][0]
        assert "minfeefilter" not in peer_info[1][0]
        assert "feefilter" not in peer_info[0][0]["bytessent_per_msg"]
        assert "feefilter" not in peer_info[1][0]["bytessent_per_msg"]
        assert "feefilter" not in peer_info[0][0]["bytesrecv_per_msg"]
        assert "feefilter" not in peer_info[1][0]["bytesrecv_per_msg"]
    ...
    def test_getnodeaddresses(self):
        ...
        self.nodes[0].addpeeraddress(address=ipv6_addr, port=9555)
        ...
        for i in range(10000):
            ...
            self.nodes[0].addpeeraddress(a, 9555)
    ...
    def test_addpeeraddress(self):
        ...
        assert_equal(node.addpeeraddress(address="", port=9555), {"success": False})
        ...
        assert_equal(node.addpeeraddress(address="1.0.0.0", tried=False, port=9555), {"success": True})
        ...
        assert_equal(new_table[0]["port"], 9555)

        self.log.debug("Test that adding an already-present new address to the new and tried tables fails")
        for value in [True, False]:
            assert_equal(node.addpeeraddress(address="1.0.0.0", tried=value, port=9555), {"success": False, "error": "failed-adding-to-new"})
        assert_equal(len(node.getnodeaddresses(count=0)), 1)

        self.log.debug("Test that adding a valid address to the tried table succeeds")
        assert_equal(node.addpeeraddress(address="1.2.3.4", tried=True, port=9555), {"success": True})
        addrman = node.getrawaddrman()
        ...
        assert_equal(tried_table[0]["port"], 9555)
        ...
        for value in [True, False]:
            assert_equal(node.addpeeraddress(address="1.2.3.4", tried=value, port=9555), {"success": False, "error": "failed-adding-to-new"})
        ...
        colliding_address = "1.2.2.199"  # grinded address that produces a tried-table collision
        assert_equal(node.addpeeraddress(address=colliding_address, tried=True, port=9555), {"success": False, "error": "failed-adding-to-tried"})
        ...
        assert_equal(node.addpeeraddress(address="2.0.0.0", port=9555), {"success": True})
        addrman_info = node.getaddrmaninfo()
        assert_equal(addrman_info["all_networks"]["tried"], 1)
