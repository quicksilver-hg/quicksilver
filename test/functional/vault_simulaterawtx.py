...
class SimulateTxTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        node.createvault(vault_name='w0')
        node.createvault(vault_name='w1')
        node.createvault(vault_name='w2', disable_private_keys=True)
        ...
        # w0 funds transaction; it should now see the payment decrease, and w1 should see the same as above
        funding = w0.fundrawtransaction(tx1)
        tx1 = funding["hex"]
        tx1changepos = funding["changepos"]
        assert "fee" not in funding

        # w0 sees a 5 Hg decrease, w2 sees +5 Hg
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -Decimal("5"))
        ...
        #       w0                      -5 + 4.9999                     -5
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5"))
        ...
        node.sendrawtransaction(w0.signrawtransactionwithvault(tx1)["hex"])
        self.generate(node, 1, sync_fun=self.no_op) # Confirm tx to trigger error below
        self.sync_all()

        # w0 funds transaction 2; it should now see a decrease from the payment, and w1 should see the same as above
        funding = w0.fundrawtransaction(tx2)
        tx2 = funding["hex"]
        assert "fee" not in funding
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -Decimal("10"))
