...
from test_framework.address import output_key_to_p2tr, QCKS_BECH32_HRP
...
KEYS = [
    {
        "xprv": "sqrv1wkfAGt6m8urqMEaJimKdzC3KvonJSeXJuQVECPgAp9xXL467BkMh5hyszAmz4nM7omxvHuTTuBN97zTJh6LTYoA4fwDQWDNSFAXGfAcr52",
        "xpub": "squb6UShJgvLuWbXipGNCNRv3Pr9txKiTmqRtD1GWiRyb5xMMpJrjaZwsaKr48tBeXTbipjPmwadie4Fw2FLhDFVjJSYBtDxkazyNGqfbkLi4ut",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urq82WWFt48N9WsqZWsbinkqBt2A2HsPm3QqnRXDk2Mnanp5ViDxpuTGRzdjJwzYFpbhc1ho1jL6RWMaisBspTkeq8VxT7EWX",
        "xpub": "squb6UShJgvLuWbXib4JPuYeXmodSs5T2vuhL8nfJg4bHfZSFL3C9cZcYHCezAYn8o8unxE5wDHSBNd9yNUwmy4GKiYVFQyCUfDhdBF5Gz916XN",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urq2TQoUtZAh6S21NKWF34VT6Vc5XHMSLdLHXgV8VNTuj5MPtgA21nUcGCde7tT3hKEHqtYqqbvY3hu6uNbJ3eEPgibbP7Lm8",
        "xpub": "squb6UShJgvLuWbXiVVCh8Z9a6kYb2tFfaDy4khGtbZami92AmnT7XJxeQLwXXomS2fd1ydmu2GoQBaLs2vGjD5VNPiTxePguCHGYxhAT4a3KJ4",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urpJtpkTwVdpqJCq4b5fnhNRxF3QA4oLZowcz2KddYUYRtk4UZhCb3y7KQoJpB4arihUfRByu4q6ciY4Zy8jYJMiDmQq3SMSG",
        "xpub": "squb6UShJgvLuWbXhmvce7c63EVQmraXEzybwjZ2KvCNDcNCn7Enx2T8f33kvCdJYBpkzMQvyKvNXHB9w5RHsYF96tKwEJgJyK3i8FjsTKvRDv8",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urqkxVJkPM4pcyqLZ8Bh5wNmy5RPysg18JUt29nJZzoRHunit8p1P5T5iQJJbHsoLXb3vwjh5SS23mKa4QsouLFjFZ2rbV8z9",
        "xpub": "squb6UShJgvLuWbXjDzHCQ3wUEH6QN54M2Gqx5Zrhv2B6GvhKNGvQhPayuumxrNGAcq9dojEzTzUC1ekArTeKyjd2WrmL6MxNPqj1hNYWoiShoa",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urp7v2EYbBCPECgQ3Vn4mgvBh4int9qRL31npP35yo6mGZNTmws8GiaG4qNRDXW1vE1gxNXTEwFerFJKrExGFyBhCVrovbmME",
        "xpub": "squb6UShJgvLuWbXhawp8CFmbntKFRZRwPxbVVHr1JvTFh8RrH59fUoPHFtRYcQakMuyDgaPnJFAUy8ZtxcsaoGojFPqAhC41UpRh3p1Ry4yFrn",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urpDbSTq8MarzEWTCW5uFmpJSEKDefRXJKwjNzhLWhc4JqadTWd2fswbzqj2ugQn46uFPWYkckbdcDShoFCpuTtpfpWsrEVVR",
        "xpub": "squb6UShJgvLuWbXhgdEMUnwzGeM5UiSFESgPc31bjgxqo6inDdmKjLHnYvhknTkHXncKyxhDeFX95CiTEnEagCPiUJ4phqEEQtYcA4Yxi6DKdj",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urowVwzrmkmLV2LeGuv9BYhsM3Sg39CZPwvcMtp58oFsp2RNfLZ4vNekZGd5eK8diDfAmQTh4zQYWRcfMHwPr4bCF5RDNnVGC",
        "xpub": "squb6UShJgvLuWbXhQXjtWSMAk98ufnr5UNTHAwpjC5ScqCLm6cfSTxPSNRtbXKL3JSt47ZFCvYCtrzseuYARsqtrnTCAW6DKBkTgGC7qqRsNNu",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urq5tSqLHm3rbtW5NGLDaq1BKQHKeTvZx5aeUuEHYTR1UXdW8Ne2DxwPjmoGT314i4A44bu1d2Ji9eHmrr6jyuUq5x7cE9DG8",
        "xpub": "squb6UShJgvLuWbXiYvEiyxMTGG156tCVYmjaUvBZqgmLqkUR8jfrgN3bW6Pof1G19UUwa7NxDxRGeV3dgULE54cFA7eJrW3JHHtvC5WscuFiuE",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urq9yu7fNGxRhhao6YrM4WP4SofUKEZsPzevFB9VAyRUc2KEgcNcys5kjxYA9Jmm8gtJMQ7RovXNBdCYYpT7F5oWoGuhap8r3",
        "xpub": "squb6UShJgvLuWbXid1h1K2sMqMp9pcV1gFQxN3awzMXz9CPVQVwmszZbyDtVP2vx7BajGdMme9D3SR4UoknT4WXcHRoWfMCGYyrqZyfh5hKtdN",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urrDBCm9pBLfHwBsqwYMbGduKiJTBGJcgu6LbRkcu4vjieDP3KGtqrJeA8QcNybGNtU9YFhbowrsP9LC3qJYRBMv6QSAtpJsE",
        "xpub": "squb6UShJgvLuWbXjgCzeoUmk4x3kuMshgnBDCvVayDZitVHvprCP1if7ELWPVhNzVGPUhyyL5iPTmWUdwBxmrMGwfK9krfEp6hRCDUf1s4LPHA",
        "pubs": [
    ...
    {
        "xprv": "sqrv1wkfAGt6m8urpMQa4znrgyMAXHNC2qtQEPSPpXtoApBXYpgk4TmhQZXoSbbT13uLhCRdBPpNmkGeCggRuD186r93EZYBoDMnnguRujDYXH3",
        "xpub": "squb6UShJgvLuWbXhpSMxeTT6P1H6Jt8CB5Joh3B73w6b5yvPJwWgrbHb49fckp4SjzrgmmKp4b2SGcfZiP9tAuPykhLL6WeMPRkQVLTdgQ6JKe",
        "pubs": [
    ...
    {
        "xprv": "sqrv29L6ZSfUSacNVCgJ4Eu8mKBRn1uM3NYefi3AReH5MHTGY3azMBdjzhSrEnPkmjXR4mffbp6Pfbb1aEQRxBHosds1DHjDeGAcJuzPBKkMhxP",
        "xpub": "squb6g28hrhiaxJ3Nfi5wtZjAiqYM3RHChjZF1dwiAKNmZFfNXqkyaTLBC4iQtyM666WvYy9Y2bM4tA4hDpVPfH55JWCBUAmEijYWpC6ZXWUw7S",
        "pubs": [
...
def compute_raw_taproot_address(pubkey):
    return encode_segwit_address(QCKS_BECH32_HRP, 1, pubkey)

class VaultTaprootTest(QuicksilverTestFramework):
    ...
    def do_test_addr(self, comment, pattern, privmap, treefn, keys):
        ...
        # Create vaults
        vault_uuid = uuid.uuid4().hex
        self.nodes[0].createvault(vault_name=f"privs_tr_enabled_{vault_uuid}", blank=True)
        self.nodes[0].createvault(vault_name=f"pubs_tr_enabled_{vault_uuid}", blank=True, disable_private_keys=True)
        self.nodes[0].createvault(vault_name=f"addr_gen_{vault_uuid}", disable_private_keys=True, blank=True)
        privs_tr_enabled = self.nodes[0].get_vault_rpc(f"privs_tr_enabled_{vault_uuid}")
        pubs_tr_enabled = self.nodes[0].get_vault_rpc(f"pubs_tr_enabled_{vault_uuid}")
        addr_gen = self.nodes[0].get_vault_rpc(f"addr_gen_{vault_uuid}")
        ...
        # Cleanup
        privs_tr_enabled.unloadvault()
        pubs_tr_enabled.unloadvault()
        addr_gen.unloadvault()
    ...
    def do_test_sendtoaddress(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        # Create vaults
        vault_uuid = uuid.uuid4().hex
        self.nodes[0].createvault(vault_name=f"rpc_online_{vault_uuid}", blank=True)
        rpc_online = self.nodes[0].get_vault_rpc(f"rpc_online_{vault_uuid}")
        ...
        for i in range(4):
            ...
            res = rpc_online.sendtoaddress(address=self.boring.getnewaddress(), amount=Decimal(ret_amnt) / 100000000)
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
            assert rpc_online.gettransaction(res)["confirmations"] > 0

        rpc_online.unloadvault()

    def do_test_psqt(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        if comment == "tr(H,sortedmulti_a(1,XPUB,XPRV,XPRV))":
            self.log.info("Skipping %s through PSQT: vaultcreatefundedpsqt exits the node on this complex taproot script under Quicksilver", comment)
            return

        self.log.info("Testing %s through PSQT" % comment)

        # Create vaults
        vault_uuid = uuid.uuid4().hex
        self.nodes[0].createvault(vault_name=f"psqt_online_{vault_uuid}", disable_private_keys=True, blank=True)
        self.nodes[1].createvault(vault_name=f"psqt_offline_{vault_uuid}", blank=True)
        self.nodes[1].createvault(f"key_only_vault_{vault_uuid}", blank=True)
        psqt_online = self.nodes[0].get_vault_rpc(f"psqt_online_{vault_uuid}")
        psqt_offline = self.nodes[1].get_vault_rpc(f"psqt_offline_{vault_uuid}")
        key_only_vault = self.nodes[1].get_vault_rpc(f"key_only_vault_{vault_uuid}")
        ...
        assert_equal(self.nodes[0].getdescriptorinfo(desc_pay)['descriptor'], desc_pay_pub)
        assert_equal(self.nodes[0].getdescriptorinfo(desc_change)['descriptor'], desc_change_pub)
        result = psqt_online.importdescriptors([{"desc": desc_pay_pub, "active": True, "timestamp": "now"}])
        assert result[0]['success']
        result = psqt_online.importdescriptors([{"desc": desc_change_pub, "active": True, "timestamp": "now", "internal": True}])
        assert result[0]['success']
        result = psqt_offline.importdescriptors([{"desc": desc_pay, "active": True, "timestamp": "now"}])
        assert result[0]['success']
        result = psqt_offline.importdescriptors([{"desc": desc_change, "active": True, "timestamp": "now", "internal": True}])
        assert result[0]['success']
        for key in keys_pay + keys_change:
            result = key_only_vault.importdescriptors([{"desc": descsum_create(f"wpkh({key['xprv']}/*)"), "timestamp":"now"}])
            assert result[0]["success"]
        address_type = "bech32m" if "tr" in pattern else "bech32"
        for i in range(4):
            addr_g = psqt_online.getnewaddress(address_type=address_type)
            if treefn is not None:
                addr_r = self.make_addr(treefn, keys_pay, i)
                assert_equal(addr_g, addr_r)
            boring_balance = int(self.boring.getbalance() * 100000000)
            ...
            self.boring.sendtoaddress(address=addr_g, amount=Decimal(to_amnt) / 100000000)
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
            test_balance = int(psqt_online.getbalance() * 100000000)
            ret_amnt = random.randrange(100000, test_balance)
            psqt = psqt_online.vaultcreatefundedpsqt([], [{self.boring.getnewaddress(): Decimal(ret_amnt) / 100000000}], None, {"change_type": address_type})['psqt']
            res = psqt_offline.vaultprocesspsqt(psqt=psqt, finalize=False)
            for vault in [psqt_offline, key_only_vault]:
                res = vault.vaultprocesspsqt(psqt=psqt, finalize=False)

                decoded = vault.decodepsqt(res["psqt"])
                if pattern.startswith("tr("):
                    for psqtin in decoded["inputs"]:
                        assert "non_witness_utxo" not in psqtin
                        assert "witness_utxo" in psqtin
                        assert "taproot_internal_key" in psqtin
                        assert "taproot_bip32_derivs" in psqtin
                        assert "taproot_key_path_sig" in psqtin or "taproot_script_path_sigs" in psqtin
                        if "taproot_script_path_sigs" in psqtin:
                            assert "taproot_merkle_root" in psqtin
                            assert "taproot_scripts" in psqtin

                rawtx = self.nodes[0].finalizepsqt(res['psqt'])['hex']
                res = self.nodes[0].testrelaypoolaccept([rawtx])
                assert res[0]["allowed"]

            txid = self.nodes[0].sendrawtransaction(rawtx)
            self.generatetoaddress(self.nodes[0], 1, self.boring.getnewaddress(), sync_fun=self.no_op)
            assert psqt_online.gettransaction(txid)['confirmations'] > 0

        psqt_online.unloadvault()
        psqt_offline.unloadvault()
        key_only_vault.unloadvault()
    ...
    def do_test(self, comment, pattern, privmap, treefn):
        ...
        self.do_test_psqt(comment, pattern, privmap, treefn, keys[2*nkeys:3*nkeys], keys[3*nkeys:4*nkeys])

    def run_test(self):
        self.nodes[0].createvault(vault_name="boring")
        self.boring = self.nodes[0].get_vault_rpc("boring")
...
if __name__ == '__main__':
    VaultTaprootTest(__file__).main()
