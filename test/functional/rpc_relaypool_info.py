...
"""Test RPCs that retrieve information from the relaypool."""
...
class RPCRelayPoolInfoTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        confirmed_utxo = self.vault.get_utxo()

        # Create a tree of unconfirmed transactions in the relaypool:
        #             txA
        #             / \
        #            /   \
        #           /     \
        ...
        assert_equal(len(relaypool), 8)
        for txid in [txidA, txidB, txidC, txidD, txidE, txidF, txidG, txidH]:
            assert_equal(txid in relaypool, True)
...
if __name__ == '__main__':
    RPCRelayPoolInfoTest(__file__).main()
