...
The `TestShell` submodule extends the `QuicksilverTestFramework` functionality to
...
to extend the object lifetime of the underlying `QuicksilverTestFramework` between
...
We can import the `TestShell` by adding the path of the configured Quicksilver
...
```
>>> import sys
>>> sys.path.insert(0, "/path/to/quicksilver/build/test/functional")
...
The `TestShell` inherits all `QuicksilverTestFramework` members and methods, such
as:
...
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Initializing test directory /path/to/quicksilver_func_test_XXXXXXX
```
The `TestShell` forwards all functional test parameters of the parent
`QuicksilverTestFramework` object. The full set of argument keywords which can be
...
**Example: Mining a sandbox chain**
...
We now let the first node generate 101 sandbox blocks, and direct the coinbase
rewards to a vault address owned by the mining node.

```
>>> test.nodes[0].createvault('default')
{'name': 'default', 'warning': 'Empty string given as passphrase, vault will not be encrypted.'}
...
101
```
The block rewards from the first block are now spendable by the vault of the
first node.
...
```
>>> test.nodes[0].log.info("Successfully mined sandbox chain!")
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework.node0 (INFO): Successfully mined sandbox chain!
```
...
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Cleaning up /path/to/quicksilver_func_test_XXXXXXX on exit
...
>>> test.shutdown()
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Stopping nodes
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Not cleaning up dir /path/to/quicksilver_func_test_XXXXXXX on exit
20XX-XX-XXTXX:XX:XX.XXXXXXX TestFramework (INFO): Tests successful
```
...
* `/path/to/quicksilver/build/test/functional/combine_logs.py
  '/path/to/quicksilver_func_test_XXXXXXX'`

## 6. Custom `TestShell` parameters

The `TestShell` object initializes with the default settings inherited from the
`QuicksilverTestFramework` class. The user can override these in
