...
class HelpRpcTest(QuicksilverTestFramework):
    ...
    def test_client_conversion_table(self):
        ...
        # Only check if all RPC methods have been compiled (i.e. vault is enabled)
        if self.is_vault_compiled() and sorted(mapping_client) != sorted(mapping_server_conversion):
        ...
        for argname, convert in converts_by_argname.items():
            if all(convert) != any(convert):
                # Only allow dummy and psqt to fail consistency check
                assert argname in ['dummy', "psqt"], ('WARNING: conversion mismatch for argument named %s (%s)' % (argname, list(zip(all_methods_by_argname[argname], converts_by_argname[argname]))))
    ...
    def vault_help(self):
        ...
        self.restart_node(0, extra_args=['-novault=1'])
        assert 'getnewaddress ( "label" "address_type" )' in self.nodes[0].help('getnewaddress')
