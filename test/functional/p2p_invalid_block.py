...
class InvalidBlockRequestTest(QuicksilverTestFramework):
    ...
    def run_test(self):
        ...
        tx1 = create_tx_with_script(block1.vtx[0], 0, script_sig=bytes([OP_TRUE]), amount=block1_value)
        prove_tx_pow(tx1, anchor_height=anchor_height)
        tx2 = create_tx_with_script(tx1, 0, script_sig=bytes([OP_TRUE]), amount=block1_value)
        prove_tx_pow(tx2, anchor_height=anchor_height)
        ...
        tx3 = create_tx_with_script(tx2, 0, script_sig=bytes([OP_TRUE]), amount=block1_value)
        tx3.vin.append(tx3.vin[0])  # Duplicates input
        prove_tx_pow(tx3, anchor_height=height - 1)
