...
RET=0
# symbolic-ref, NOT name-rev. name-rev NAMES A COMMIT, it does not report the
# checked-out ref: on a tagged HEAD it answers "tags/v0.1.0^0", and checking
# that back out at the end leaves the caller DETACHED. The bug is latent until
# a release is tagged, which is the one moment it is certain to fire, and the
# `||` fallback below never catches it because the wrong checkout SUCCEEDS.
# symbolic-ref reports the branch or fails cleanly when HEAD really is detached.
...
if test -n "$PREV_BRANCH"; then
    git checkout --quiet "$PREV_BRANCH"
else
    git checkout --quiet "$PREV_HEAD"
fi
exit $RET
