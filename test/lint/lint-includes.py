...
EXCLUDED_DIRS = ["research/tier0-pow/",
...
def find_included_cpps():
    ...
    return [
        ...
        if not any(included_cpp.startswith(expected) for expected in EXPECTED_CUCKATOO_CPP_INCLUDES)
    ]
...
def find_quote_syntax_inclusions():
    ...
    return [
        inclusion
        for inclusion in quote_syntax_inclusions
        if not any(inclusion.startswith(expected) for expected in EXPECTED_CUCKATOO_QUOTE_INCLUDES)
    ]
