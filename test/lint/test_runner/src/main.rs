...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from quicksilver-build-config.h is used without the header being included",
        ...
        &Linter {
            description: "Check that Qt's %n numerus plural idiom is not used",
            name: "qt_numerus_plurals",
            lint_fn: lint_qt_numerus_plurals
        },
        &Linter {
...
test_runner --lint=doc --lint=trailing_whitespace
...
///
/// The range is not allowed to be empty: `main()` rejects an empty one, because every
/// commit-range lint check would then inspect nothing and report success.
...
/// Explain where `commit_range()` took its range from, for the empty-range failure message
fn commit_range_origin() -> String {
    if env::var("COMMIT_RANGE").is_ok() {
        "It came from the COMMIT_RANGE environment variable, which is likely stale: a range \
         such as 'master..HEAD' is empty once the branch has been merged."
            .to_string()
    } else {
        "It was derived from the most recent merge commit, which is HEAD itself -- or this \
         history holds no merge commit at all."
            .to_string()
    }
}

/// Return the number of commits in the range, or a LintError explaining why it could not be counted
fn commit_range_count(commit_range: &str) -> Result<u64, LintError> {
    let count = check_output(git().args(["rev-list", "--count", commit_range]))
        .map_err(|e| format!("Could not count the commits in '{commit_range}':\n{e}"))?;
    count.parse::<u64>().map_err(|e| {
        format!("Could not read a commit count for '{commit_range}' from '{count}': {e}")
    })
}

/// Return all vendored third-party tree paths.
///
/// These are plain vendored copies, not git subtrees: Quicksilver does not use the
/// upstream subtree workflow. They are listed solely so linters can exclude them.
fn get_vendored_trees() -> Vec<&'static str> {
    vec![
        "src/crc32c",
        "src/crypto/ctaes",
        "src/leveldb",
...
/// Return the pathspecs to exclude all vendored trees
fn get_pathspecs_exclude_vendored() -> Vec<String> {
    get_vendored_trees()
        .iter()
...
fn lint_py_lint() -> LintResult {
    ...
    let files = check_output(
        git()
            .args(["ls-files", "--", "*.py"])
            .args(get_pathspecs_exclude_vendored()),
    )?;
...
fn lint_qt_numerus_plurals() -> LintResult {
    let found = git()
        .args([
            "grep",
            "--line-number",
            "--extended-regexp",
            r"\b(tr|translate|QT_TRANSLATE_NOOP)\(.*%n",
            "--",
            "src/",
        ])
        .status()
    ...
    if found {
        Err(r#"
Qt's "%n unit(s)" numerus idiom must not be used. Write the singular and the plural as
two separate translatable strings instead, and select between them on the count:

    count == 1 ? tr("1 block") : tr("%1 blocks").arg(count)

%n is only substituted by a translation catalogue, and this application installs no
catalogue of its own -- initTranslations() in src/qt/quicksilver.cpp loads only Qt's own
base translations, so that native dialog buttons stay localized. With nothing to resolve
the numerus form, Qt fills in the number and leaves the literal "(s)" behind, and every
user in every locale reads "3 minute(s)". See src/qt/maturity.cpp for the worked
precedent.

This check is line-based, so it cannot see a tr() whose string sits on the next line.
            "#
        .trim()
        .to_string())
    } else {
...
fn get_pathspecs_exclude_whitespace() -> Vec<String> {
    let mut list = get_pathspecs_exclude_vendored();
    list.extend(
        [
            // Permanent excludes
            "*.patch",
            "doc/README_windows.txt",
            // Vendored upstream cuckatoo. Reformatting it would destroy the ability to
            // diff against the upstream solver for zero correctness gain.
            "src/crypto/cuckatoo/vendor",
            // Generated calibration data. These are solver measurement dumps, not source.
            "research/*.csv",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/quicksilver-build-config.h.in";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args(get_pathspecs_exclude_vendored())
                .args([
                    // These are exceptions which don't use quicksilver-build-config.h, rather CMakeLists.txt adds
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <quicksilver-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the quicksilver-build-config.h header. However, they are not
...
even though quicksilver-build-config.h indicates that a faster feature is available and should be used.
...
#include <quicksilver-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the quicksilver-build-config.h header. However, they are including
...
fn lint_markdown() -> LintResult {
    if Command::new("python3")
        .arg("test/lint/check-doc-links.py")
        .status()
        .expect("command error")
        .success()
    {
...
fn main() -> ExitCode {
    ...
    match commit_range_count(&commit_range) {
        Err(err) => {
            println!("{err}\n^---- ⚠️ Failure: the commit range could not be inspected!\n");
            return ExitCode::FAILURE;
        }
        Ok(0) => {
            println!(
                "The commit range ({commit_range}) holds no commits, so every commit-range lint \
                 check would inspect nothing and report success.\n{origin}\nPass an explicit \
                 non-empty range instead, for example COMMIT_RANGE=<base>..HEAD.\n\
                 ^---- ⚠️ Failure: refusing to lint an empty commit range!\n",
                origin = commit_range_origin(),
            );
            return ExitCode::FAILURE;
        }
        Ok(_) => {}
