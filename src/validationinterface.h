...
#ifndef QUICKSILVER_VALIDATIONINTERFACE_H
#define QUICKSILVER_VALIDATIONINTERFACE_H
...
/**
 * Implement this to subscribe to events generated in validation and relaypool
 *
 * Each CValidationInterface() subscriber will receive event callbacks
 * in the order in which the events were generated by validation and relaypool.
...
protected:
    ...
    /**
     * Notifies listeners of a transaction having been added to relaypool.
     *
     * Called on a background thread.
     */
    virtual void TransactionAddedToRelayPool(const CTransactionRef& tx, uint64_t relaypool_sequence) {}

    /**
     * Notifies listeners of a transaction leaving relaypool.
     *
     * This notification fires for transactions that are removed from the
     * relaypool for the following reasons:
     *
     * - EXPIRY (expired from relaypool after -relaypoolexpiry hours)
     * - SIZELIMIT (removed in size limiting if the relaypool exceeds -maxrelaypool megabytes)
     * - REORG (removed during a reorg)
     * - CONFLICT (removed because it conflicts with in-block transaction)
     * - REPLACED (removed due to replacement)
     *
     * This does not fire for transactions that are removed from the relaypool
     * because they have been included in a block. Clients learn about those
     * transactions from the following BlockConnected notification.
     *
     * Transactions that are removed from the relaypool because they conflict
     * with a transaction in the new block will have
     * TransactionRemovedFromRelayPool events fired *before* the BlockConnected
     * event is fired. If multiple blocks are connected in one step, then the
     * ordering could be:
     *
     * - TransactionRemovedFromRelayPool(tx1 from block A)
     * - TransactionRemovedFromRelayPool(tx2 from block A)
     * - TransactionRemovedFromRelayPool(tx1 from block B)
     * - TransactionRemovedFromRelayPool(tx2 from block B)
     * - BlockConnected(A)
     * - BlockConnected(B)
     *
     * Called on a background thread.
     */
    virtual void TransactionRemovedFromRelayPool(const CTransactionRef& tx, RelayPoolRemovalReason reason, uint64_t relaypool_sequence) {}
    /**
     * Notifies listeners of a block being connected.
     * Provides a vector of transactions evicted from the relaypool as a result.
     *
     * Called on a background thread.
     */
...
public:
    ...
    void TransactionAddedToRelayPool(const CTransactionRef&, uint64_t relaypool_sequence);
    void TransactionRemovedFromRelayPool(const CTransactionRef&, RelayPoolRemovalReason, uint64_t relaypool_sequence);
...
#endif // QUICKSILVER_VALIDATIONINTERFACE_H
