...
void SignVerifyMessageDialog::setModel(VaultModel *_model)
{
    this->model = _model;
}
...
void SignVerifyMessageDialog::on_addressBookButton_SM_clicked()
{
    if (model && model->getAddressTableModel())
    {
        ...
        auto dlg = new AddressBookPage(platformStyle, AddressBookPage::ForSelection, AddressBookPage::ReceivingTab, this);
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    if (!pkhash) {
        ...
        ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_SM->setText(tr("Message signing is only supported for Base58 addresses. Please check the address and try again."));
        return;
    }

    const PKHash pkhash_copy = *pkhash;
    const std::string message = ui->messageIn_SM->document()->toPlainText().toStdString();
    QPointer<SignVerifyMessageDialog> self(this);
    model->requestUnlock([this, self, pkhash_copy, message](std::shared_ptr<VaultModel::UnlockContext> ctx) {
        if (!self) return;
        if (!ctx || !ctx->isValid()) {
            ui->statusLabel_SM->setStyleSheet("QLabel { color: red; }");
            ui->statusLabel_SM->setText(tr("Vault unlock was cancelled."));
            return;
        }

        std::string signature;
        SigningResult res = model->vault().signMessage(message, pkhash_copy, signature);
...
void SignVerifyMessageDialog::on_verifyMessageButton_VM_clicked()
{
    ...
    case MessageVerificationResult::ERR_ADDRESS_NO_KEY:
        ...
        ui->statusLabel_VM->setText(tr("Message signing is only supported for Base58 addresses. Please check the address and try again."));
        return;
