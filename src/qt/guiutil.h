...
#ifndef QUICKSILVER_QT_GUIUTIL_H
#define QUICKSILVER_QT_GUIUTIL_H
...
namespace GUIUtil
{
    ...
    /** Show a save-file picker asynchronously. `done` runs on a queued turn with
        a native-separator path, or an empty string if the user cancels. A default
        suffix from `filter` is appended when the chosen name has none.
     */
    ...
    /** Show an open-file picker asynchronously. `done` runs on a queued turn with
        a native-separator path, or an empty string if the user cancels.
     */
    ...
    */
    ...
    /**
     * Shows a QMessageBox asynchronously. `done` runs on a queued turn after the
     * box finishes, with `clicked` snapshotted before WA_DeleteOnClose destroys it.
     */
...
#endif // QUICKSILVER_QT_GUIUTIL_H
