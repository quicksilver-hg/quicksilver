...
void RPCNestedTests::rpcNestedTests()
{
    ...
    // getblockheader, not getblock: getblock takes a numeric verbosity, so a bool
    // argument is a type error there. getblockheader still takes a real BOOL, which
    // is what keeps a boolean parameter under test in the nested parser.
    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblockheader(getblock(getbestblockhash())[hash], true)"); //3 level nesting with boolean parameter
    QVERIFY(result.substr(0, 1) == "{");

    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblockheader( getblockheader( getblock(getbestblockhash())[hash] )[hash], true)"); //4 level nesting with whitespace, filtering path and boolean parameter
    QVERIFY(result.substr(0, 1) == "{");

    RPCConsole::RPCExecuteCommandLine(m_node, result, "getblockheader(getblock(getbestblockhash())[hash], false)"); //boolean parameter is really parsed as a bool, not echoed as a string
    QVERIFY(result.substr(0, 1) != "{");
    ...
    RPCConsole::RPCParseCommandLine(nullptr, result, "vaultpassphrase", false, &filtered);
    QVERIFY(filtered == "vaultpassphrase(…)");
    ...
    RPCConsole::RPCParseCommandLine(nullptr, result, "vaultpassphrase(help())", false, &filtered);
    QVERIFY(filtered == "vaultpassphrase(…)");
    RPCConsole::RPCParseCommandLine(nullptr, result, "vaultpassphrasechange(help(vaultpassphrasechange(abc)))", false, &filtered);
    QVERIFY(filtered == "vaultpassphrasechange(…)");
    RPCConsole::RPCParseCommandLine(nullptr, result, "help(encryptvault(abc, def))", false, &filtered);
    QVERIFY(filtered == "help(encryptvault(…))");
    RPCConsole::RPCParseCommandLine(nullptr, result, "help(signmessagewithprivkey())", false, &filtered);
    QVERIFY(filtered == "help(signmessagewithprivkey(…))");
    RPCConsole::RPCParseCommandLine(nullptr, result, "help(signmessagewithprivkey(help()))", false, &filtered);
    QVERIFY(filtered == "help(signmessagewithprivkey(…))");
    RPCConsole::RPCParseCommandLine(nullptr, result, "help(signmessagewithprivkey(abc), vaultpassphrase(def))", false, &filtered);
    QVERIFY(filtered == "help(signmessagewithprivkey(…), vaultpassphrase(…))");
