...
CoinControlDialog::CoinControlDialog(CCoinControl& coin_control, VaultModel* _model, const PlatformStyle *_platformStyle, QWidget *parent) :
...
void CoinControlDialog::showMenu(const QPoint &point)
{
    ...
    if(item)
    {
        ...
        if (txid) { // a valid txid means this is a child node, and not a parent node in tree mode
            ...
            if (model->vault().isLockedCoin(COutPoint(*txid, item->data(COLUMN_ADDRESS, VOutRole).toUInt()))) {
...
{
    GUIUtil::setClipboard(QuicksilverUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::lockCoin()
{
    ...
    model->vault().lockCoin(outpt, /* write_to_db = */ true);
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    ...
    if (txid) { // a valid txid means this is a child node, and not a parent node in tree mode
        ...
        if (ui->treeWidget->isEnabled()) // do not update on every click for select all
            CoinControlDialog::updateLabels(m_coin_control, model, this);
    }
}
...
void CoinControlDialog::updateLabelLocked()
{
    ...
    model->vault().listLockedCoins(vOutpts);
    if (vOutpts.size() > 0)
    {
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, VaultModel *model, QDialog* dialog)
{
    if (!model)
        return;
    ...
    if (model->tryGetCoins(vCoinControl, selected_coins)) {
        for (const auto& out : selected_coins) {
            ...
            else if(ExtractDestination(out.txout.scriptPubKey, address))
            {
                ...
                if (pkhash && model->vault().getPubKey(out.txout.scriptPubKey, ToKeyID(*pkhash), pubkey))
                {
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            // This mirrors the old virtual-size estimate closely enough for the coin-control byte summary.
    ...
    QuicksilverUnit nDisplayUnit = QuicksilverUnit::HG;
    ...
    l2->setText(QuicksilverUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(QuicksilverUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
...
void CoinControlDialog::updateView()
{
    ...
    QuicksilverUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();

    interfaces::Vault::CoinsList coins_by_address;
    if (!model->tryListCoins(coins_by_address)) {
        updateLabels(m_coin_control, model, this);
        return;
    }

    for (const auto& coins : coins_by_address) {
        CCoinControlWidgetItem* itemVaultAddress{nullptr};
        QString sVaultAddress = QString::fromStdString(EncodeDestination(coins.first));
        QString sVaultLabel = model->getAddressTableModel()->labelForAddress(sVaultAddress);
        if (sVaultLabel.isEmpty())
            sVaultLabel = tr("(no label)");

        if (treeMode)
        {
            // vault address
            itemVaultAddress = new CCoinControlWidgetItem(ui->treeWidget);

            itemVaultAddress->setFlags(flgTristate);
            itemVaultAddress->setCheckState(COLUMN_CHECKBOX, Qt::Unchecked);

            // label
            itemVaultAddress->setText(COLUMN_LABEL, sVaultLabel);

            // address
            itemVaultAddress->setText(COLUMN_ADDRESS, sVaultAddress);
        }
        ...
        for (const auto& outpair : coins.second) {
            ...
            const interfaces::VaultTxOut& out = std::get<1>(outpair);
            nSum += out.txout.nValue;
            nChildren++;

            CCoinControlWidgetItem *itemOutput;
            if (treeMode)    itemOutput = new CCoinControlWidgetItem(itemVaultAddress);
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show Quicksilver address. In tree mode, address is not shown again for direct vault address outputs
                if (!treeMode || (!(sAddress == sVaultAddress)))
                    itemOutput->setText(COLUMN_ADDRESS, sAddress);
            }

            // label
            if (!(sAddress == sVaultAddress)) // change
            {
                // tooltip from where the change comes from
                itemOutput->setToolTip(COLUMN_LABEL, tr("change from %1 (%2)").arg(sVaultLabel).arg(sVaultAddress));
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, QuicksilverUnits::format(nDisplayUnit, out.txout.nValue));
            ...
            if (model->vault().isLockedCoin(output))
            {
        ...
        if (treeMode)
        {
            itemVaultAddress->setText(COLUMN_CHECKBOX, "(" + QString::number(nChildren) + ")");
            itemVaultAddress->setText(COLUMN_AMOUNT, QuicksilverUnits::format(nDisplayUnit, nSum));
            itemVaultAddress->setData(COLUMN_AMOUNT, Qt::UserRole, QVariant((qlonglong)nSum));
        }
    }
