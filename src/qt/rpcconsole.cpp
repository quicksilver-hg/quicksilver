...
public Q_SLOTS:
    void request(const QString &command, const VaultModel* vault_model);
 ...
 * (used for e.g. re-locking the vault after a timeout)
 */
...
bool RPCConsole::RPCParseCommandLine(interfaces::Node* node, std::string &strResult, const std::string &strCommand, const bool fExecute, std::string * const pstrFilteredOut, const VaultModel* vault_model)
{
...
#ifdef ENABLE_VAULT
                            if (vault_model) {
                                QByteArray encodedName = QUrl::toPercentEncoding(vault_model->getVaultName());
                                uri = "/vault/"+std::string(encodedName.constData(), encodedName.length());
                            }
#endif
                            assert(node);
...
void RPCExecutor::request(const QString &command, const VaultModel* vault_model)
{
    try
    {
        std::string result;
        ...
        if (!RPCConsole::RPCExecuteCommandLine(m_node, result, executableCommand, nullptr, vault_model)) {
...
{
    ui->setupUi(this);
    setProperty("class", QStringLiteral("quicksilverWindow"));
    ui->verticalLayout_2->setContentsMargins(18, 16, 18, 18);
    ...
    // disable the vault selector by default
    ui->VaultSelector->setVisible(false);
    ui->VaultSelectorLabel->setVisible(false);
...
void RPCConsole::setClientModel(ClientModel *model, int bestblock_height, int64_t bestblock_date, double verification_progress)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        connect(model, &ClientModel::relaypoolSizeChanged, this, &RPCConsole::setRelayPoolSize);
        ...
        // Paired with the values below: the banner exists to explain the N/A placeholders,
        // so it must go exactly when they are replaced, and not in any other branch.
        ui->consensusOffBanner->setVisible(false);
...
#ifdef ENABLE_VAULT
void RPCConsole::addVault(VaultModel * const vaultModel)
{
    // use name for text and vault model for internal data object (to allow to move to a vault id later)
    ui->VaultSelector->addItem(vaultModel->getDisplayName(), QVariant::fromValue(vaultModel));
    if (ui->VaultSelector->count() == 2) {
        // First vault added, set to default to match vault RPC behavior
        ui->VaultSelector->setCurrentIndex(1);
    }
    if (ui->VaultSelector->count() > 2) {
        ui->VaultSelector->setVisible(true);
        ui->VaultSelectorLabel->setVisible(true);
    }
}

void RPCConsole::removeVault(VaultModel * const vaultModel)
{
    ui->VaultSelector->removeItem(ui->VaultSelector->findData(QVariant::fromValue(vaultModel)));
    if (ui->VaultSelector->count() == 2) {
        ui->VaultSelector->setVisible(false);
        ui->VaultSelectorLabel->setVisible(false);
    }
...
void RPCConsole::setCurrentVault(VaultModel* const vault_model)
{
    QVariant data = QVariant::fromValue(vault_model);
    ui->VaultSelector->setCurrentIndex(ui->VaultSelector->findData(data));
}
#endif
...
void RPCConsole::clear(bool keep_prompt)
{
    ...
    static const QString welcome_message =
        ...
        tr("Welcome to the %1 RPC console.\n"
           ...
           "\n"
           "%7WARNING: Malicious actors on other networks have tricked users into"
           " typing commands here that drained their vaults. Do not use this console"
...
void RPCConsole::setRelayPoolSize(long numberOfTxs, size_t dynUsage, size_t maxUsage)
{
    ui->relaypoolNumberTxs->setText(QString::number(numberOfTxs));
    ...
    ui->relaypoolSize->setText(cur_usage_str + " / " + max_usage_str);
}
...
void RPCConsole::on_lineEdit_returnPressed()
{
    ...
    try {
        ...
        auto* box = new QMessageBox(QMessageBox::Critical, "Error",
            QString("Error: ") + QString::fromStdString(e.what()), QMessageBox::Ok, this);
        box->setObjectName(QStringLiteral("rpcParseError"));
    ...
    VaultModel* vault_model{nullptr};
#ifdef ENABLE_VAULT
    vault_model = ui->VaultSelector->currentData().value<VaultModel*>();

    if (m_last_vault_model != vault_model) {
        if (vault_model) {
            message(CMD_REQUEST, tr("Executing command using \"%1\" vault").arg(vault_model->getVaultName()));
        } else {
            message(CMD_REQUEST, tr("Executing command without any vault"));
        }
        m_last_vault_model = vault_model;
    }
#endif // ENABLE_VAULT
    ...
    QMetaObject::invokeMethod(m_executor, [this, cmd, vault_model] {
        m_executor->request(cmd, vault_model);
    });
