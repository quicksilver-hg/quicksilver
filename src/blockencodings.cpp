...
CBlockHeaderAndShortTxIDs::CBlockHeaderAndShortTxIDs(const CBlock& block, const uint64_t nonce) :
    ...
    //TODO: Use our relaypool prior to block acceptance to predictively fill more than just the coinbase
...
ReadStatus PartiallyDownloadedBlock::InitData(const CBlockHeaderAndShortTxIDs& cmpctblock, const std::vector<CTransactionRef>& extra_txn) {
    ...
    // Calculate map of txids -> positions and check relaypool to see what we have (or don't)
    ...
    for (const auto& tx : pool->txns_randomized) {
        ...
        if (idit != shorttxids.end()) {
            if (!have_txn[idit->second]) {
                ...
                // If we find two relaypool txn that match the short id, just request it.
        ...
        // the extra risk.
        if (relaypool_count == shorttxids.size())
            break;
    }
    }
    ...
    for (size_t i = 0; i < extra_txn.size(); i++) {
        ...
        if (idit != shorttxids.end()) {
            if (!have_txn[idit->second]) {
                ...
                // If we find two relaypool/extra txn that match the short id, just
                // request it.
                // This should be rare enough that the extra bandwidth doesn't matter,
                // but eating a round-trip due to FillBlock failure would be annoying
                // Note that we don't want duplication between extra_txn and relaypool to
    ...
    LogDebug(HgLog::CMPCTBLOCK, "Initialized PartiallyDownloadedBlock for block %s using a cmpctblock of size %lu\n", cmpctblock.header.GetHash().ToString(), GetSerializeSize(cmpctblock));
...
ReadStatus PartiallyDownloadedBlock::FillBlock(CBlock& block, const std::vector<CTransactionRef>& vtx_missing)
{
    ...
    LogDebug(HgLog::CMPCTBLOCK, "Successfully reconstructed block %s with %lu txn prefilled, %lu txn from the relay pool (incl at least %lu from extra pool) and %lu txn requested\n", hash.ToString(), prefilled_count, relaypool_count, extra_count, vtx_missing.size());
    if (vtx_missing.size() < 5) {
        for (const auto& tx : vtx_missing) {
            LogDebug(HgLog::CMPCTBLOCK, "Reconstructed block %s required tx %s\n", hash.ToString(), tx->GetHash().ToString());
        }
    }
