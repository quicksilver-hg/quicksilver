...
static void AddTx(const CTransactionRef& tx, CTxRelayPool& pool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    ...
    LockPoints lp;
    AddToRelayPool(pool, CTxRelayPoolEntry(
        tx, nTime, nHeight, sequence,
        spendsCoinbase, sigOpCost, lp));
}

// Right now this is only testing eviction performance in an extremely small
// relaypool. Code needs to be written to generate a much wider variety of
// unique transactions for a more meaningful performance measurement.
static void RelayPoolEviction(benchmark::Bench& bench)
{
...
BENCHMARK(RelayPoolEviction, benchmark::PriorityLevel::HIGH);
