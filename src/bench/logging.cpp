...
{
    Logging(bench, {"-logthreadnames=0", "-debug=mesh"}, [] { LogDebug(HgLog::MESH, "%s\n", "test"); });
}

static void LogWithoutDebug(benchmark::Bench& bench)
{
    Logging(bench, {"-logthreadnames=0", "-debug=0"}, [] { LogDebug(HgLog::MESH, "%s\n", "test"); });
}

static void LogWithThreadNames(benchmark::Bench& bench)
{
    Logging(bench, {"-logthreadnames=1"}, [] { LogInfo(HgLog::BENCH, "%s\n", "test"); });
}

static void LogWithoutThreadNames(benchmark::Bench& bench)
{
    Logging(bench, {"-logthreadnames=0"}, [] { LogInfo(HgLog::BENCH, "%s\n", "test"); });
}
