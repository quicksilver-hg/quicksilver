...
{
    LogInfo(HgLog::ALL, "POTENTIAL DEADLOCK DETECTED\n");
    LogInfo(HgLog::ALL, "Previous lock order was:\n");
    for (const LockStackItem& i : s1) {
        ...
        LogInfo(HgLog::ALL, "%s %s\n", prefix, i.second.ToString());
    }

    std::string mutex_a, mutex_b;
    LogInfo(HgLog::ALL, "Current lock order is:\n");
...
{
    LogInfo(HgLog::ALL, "DOUBLE LOCK DETECTED\n");
    LogInfo(HgLog::ALL, "Lock order:\n");
...
void CheckLastCritical(void* cs, std::string& lockname, const char* guardname, const char* file, int line)
{
    ...
    LogInfo(HgLog::ALL, "INCONSISTENT LOCK ORDER DETECTED\n");
    LogInfo(HgLog::ALL, "Current lock order (least recent first) is:\n");
    for (const LockStackItem& i : lock_stack) {
        LogInfo(HgLog::ALL, " %s\n", i.second.ToString());
    }
    if (g_debug_lockorder_abort) {
