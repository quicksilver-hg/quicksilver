...
bool ExternalSigner::SignTransaction(PartiallySignedQuicksilverTransaction& psqtx, std::string& error)
{
    // Serialize the PSQT
    DataStream ssTx{};
    ssTx << psqtx;
    ...
    auto matches_signer_fingerprint = [&](const PSQTInput& input) {
    ...
    if (!std::any_of(psqtx.inputs.begin(), psqtx.inputs.end(), matches_signer_fingerprint)) {
    ...
    if (!signer_result.find_value("psqt").isStr()) {
        error = "Unexpected result from signer";
        return false;
    }

    PartiallySignedQuicksilverTransaction signer_psqtx;
    std::string signer_psqt_error;
    if (!DecodeBase64PSQT(signer_psqtx, signer_result.find_value("psqt").get_str(), signer_psqt_error)) {
        error = strprintf("TX decode failed %s", signer_psqt_error);
        return false;
    }
