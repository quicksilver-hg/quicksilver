...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    // The HRP alone cannot decide the encoding on Quicksilver. Upstream's HRPs
    // ("bc", "tb", "bcrt") cannot prefix a base58 address because those start
    // '1', '3', 'm', 'n' or '2' -- but Quicksilver's do: sandbox encodes P2SH as
    // 's...' against HRP "shg", and publictest as 'p...' against "phg". Roughly
    // one address in 841 has the next two characters spell out the rest of the
    // HRP, and was then handed to the Bech32 decoder and rejected as invalid.
    //
    // A valid Base58Check checksum settles it. Misrouting a real address is a
    // ~1-in-841 event; a Bech32 string passing a 4-byte Base58Check checksum by
    // accident is a 2^-32 one, so deferring to the checksum is strictly safer
    // than deferring to the prefix.
    if (is_bech32 && DecodeBase58Check(str, data, 21)) is_bech32 = false;
    data.clear();

    if (!is_bech32 && DecodeBase58Check(str, data, 21)) {
        // Base58Check-encoded address payloads.
        // Public-key-hash addresses use the network's PUBKEY_ADDRESS prefix.
        ...
        // Script-hash addresses use the network's SCRIPT_ADDRESS prefix.
