...
public:
    ...
    HgLog::CategoryMask getLogCategories() override { return LogInstance().GetCategoryMask(); }
    ...
    void startShutdown() override
    {
        ...
        if (!(Assert(ctx.shutdown_request))()) {
            LogError(HgLog::INIT, "Failed to send shutdown signal\n");
        }
    ...
    std::vector<CService> getNodeAddresses(size_t count) override
    {
        std::vector<CService> services;
        if (!m_context->connman) return services;

        const std::vector<CAddress> addresses{m_context->connman->GetAddresses(count, /*max_pct=*/0, std::nullopt)};
        services.reserve(addresses.size());
        for (const CAddress& address : addresses) {
            const CService service{address};
            if (!service.IsValid() || service.GetPort() == 0 || !service.IsRelayable()) continue;
    ...
    bool addNode(const std::string& address) override
    {
        ...
        // v2transport chosen the way the addnode RPC chooses it when the caller does
        // not say: from what this node actually offers.
        return m_context->connman->AddNode(
            {address, static_cast<bool>(m_context->connman->GetLocalServices() & NODE_P2P_V2)});
    }
    ...
    std::vector<std::unique_ptr<interfaces::ExternalSigner>> listExternalSigners() override
    {
        ...
        // no signers. For the current GUI this doesn't matter, because the vault
        // creation dialog hides the external signer checkbox entirely when
        // compiled out. The return type could be changed to
        // std::optional<std::vector> (or something that also includes error
        // messages) if this distinction becomes important.
        return {};
#endif // ENABLE_EXTERNAL_SIGNER
    }
    int64_t getTotalBytesRecv() override { return m_context->connman ? m_context->connman->GetTotalBytesRecv() : 0; }
    int64_t getTotalBytesSent() override { return m_context->connman ? m_context->connman->GetTotalBytesSent() : 0; }
    size_t getRelayPoolSize() override { return m_context->relaypool ? m_context->relaypool->size() : 0; }
    size_t getRelayPoolDynamicUsage() override { return m_context->relaypool ? m_context->relaypool->DynamicMemoryUsage() : 0; }
    size_t getRelayPoolMaxUsage() override { return m_context->relaypool ? m_context->relaypool->m_opts.max_size_bytes : 0; }
    ...
    util::Result<void> startMining(const std::string& payout_address) override
    {
        if (!m_context->mining_service) return util::Error{Untranslated("Mining service unavailable")};
        return node::StartMining(*m_context->mining_service, payout_address);
    }
    void stopMining() override
    {
        if (m_context->mining_service) m_context->mining_service->Stop();
    }
    interfaces::MiningStatus miningStatus() override
    {
        if (!m_context->mining_service) return interfaces::MiningStatus{};
        return node::BuildMiningStatus(*m_context->mining_service, chainman());
    }
    ...
    VaultLoader& vaultLoader() override
    {
        return *Assert(m_context->vault_loader);
    }
    ...
    std::unique_ptr<Handler> handleInitVault(InitVaultFn fn) override
    {
        return MakeSignalHandler(::uiInterface.InitVault_connect(fn));
    }
...
public:
    ...
    {
        m_notifications->transactionAddedToRelayPool(tx);
    }
    ...
    {
        m_notifications->transactionRemovedFromRelayPool(tx, reason);
    }
    ...
    explicit RpcHandlerImpl(const CRPCCommand& command) : m_command(command), m_wrapped_command(&command)
    {
        m_command.actor = [this](const JSONRPCRequest& request, UniValue& result, bool last_handler) {
            ...
            try {
                ...
                // If this is not the last handler and a vault not found
                // exception was thrown, return false so the next handler can
                // try to handle the request. Otherwise, reraise the exception.
                if (!last_handler) {
                    const UniValue& code = e["code"];
                    if (code.isNum() && code.getInt<int>() == RPC_VAULT_NOT_FOUND) {
                        return false;
                    }
                }
                throw;
...
public:
    ...
    {
        if (!hasChainstate()) return std::nullopt;
        const int height{WITH_LOCK(::cs_main, return chainman().ActiveChain().Height())};
        return height >= 0 ? std::optional{height} : std::nullopt;
    }
    uint256 getBlockHash(int height) override
    {
        if (!hasChainstate()) return {};
        LOCK(::cs_main);
        return Assert(chainman().ActiveChain()[height])->GetBlockHash();
    }
    uint256 txPowTarget(int anchor_height, uint64_t bytes, size_t nout, size_t nin) override
    {
        if (!hasChainstate()) return {};
        LOCK(::cs_main);
        const CBlockIndex* anchor{Assert(chainman().ActiveChain()[anchor_height])};
        const Consensus::Params& consensus{chainman().GetConsensus()};
        const arith_uint256 required{RequiredTxWorkForBytes(bytes, nout, nin, anchor, consensus)};
        // Mirrors GetTxPowTarget, which cannot be reused directly because the sender
        // has no finished transaction to hand it -- only a bound on the one it will
        // build. The work itself comes from the same function consensus calls.
        const arith_uint256 target{(~arith_uint256(0)) / required};
        const arith_uint256 limit{UintToArith256(consensus.txPowLimit)};
        return ArithToUint256(target < limit ? target : limit);
    }
    bool haveBlockOnDisk(int height) override
    {
        if (!hasChainstate()) return false;
        LOCK(::cs_main);
    ...
    bool isInRelayPool(const uint256& txid) override
    {
        if (!m_node.relaypool) return false;
        LOCK(m_node.relaypool->cs);
        return m_node.relaypool->exists(GenTxid::Txid(txid));
    }
    TransactionError broadcastTransaction(const CTransactionRef& tx,
                                          bool relay,
                                          std::string& err_string) override
    {
        if (!hasChainstate()) {
            err_string = "Consensus is not running";
            return TransactionError::RELAYPOOL_ERROR;
        }
        return BroadcastTransaction(m_node, tx, err_string, relay, /*wait_callback=*/false);
    }
    void getTransactionAncestry(const uint256& txid, size_t& ancestors, size_t& descendants, size_t* ancestorsize) override
    {
        ancestors = descendants = 0;
        if (!m_node.relaypool) return;
        m_node.relaypool->GetTransactionAncestry(txid, ancestors, descendants, ancestorsize);
    }

    void getPackageLimits(unsigned int& limit_ancestor_count, unsigned int& limit_descendant_count) override
    {
        const CTxRelayPool::Limits default_limits{};

        const CTxRelayPool::Limits& limits{m_node.relaypool ? m_node.relaypool->m_opts.limits : default_limits};
    ...
    util::Result<void> checkChainLimits(const CTransactionRef& tx) override
    {
        if (!m_node.relaypool) return {};
        LockPoints lp;
        CTxRelayPoolEntry entry(tx, 0, 0, 0, false, 0, lp);
        LOCK(m_node.relaypool->cs);
        return m_node.relaypool->CheckPackageLimits({tx}, entry.GetTxSize());
    }
    ...
    bool isReadyToBroadcast() override { return hasChainstate() && !chainman().m_blockman.LoadingBlocks() && !isInitialBlockDownload(); }
    bool isInitialBlockDownload() override
    {
        if (!hasChainstate()) return true;
        return chainman().IsInitialBlockDownload();
    }
    ...
    {
        if (!hasChainstate()) return interfaces::MakeCleanupHandler([] {});
    ...
    void requestRelayPoolTransactions(Notifications& notifications) override
    {
        if (!m_node.relaypool) return;
        LOCK2(::cs_main, m_node.relaypool->cs);
        for (const CTxRelayPoolEntry& entry : m_node.relaypool->entryAll()) {
            notifications.transactionAddedToRelayPool(entry.GetSharedTx());
        }
    }
    bool hasChainstate() override
    {
        // Deliberately not the chainman() helper: that is *Assert(m_node.chainman)
        // and would abort on exactly the case this is being asked about.
...
public:
    ...
    bool submitSolution(uint32_t version, uint32_t timestamp, uint32_t congestion, uint32_t nonce, const std::array<uint32_t, 42>& cycle, CTransactionRef coinbase) override
    {
        ...
        // Quicksilver: taken from the submission, not the template — the caller's
        // coinbase above may have changed the block weight that consensus recomputes
        // this from, and it is inside the pre-pow the caller ground against.
        block.nCongestion = congestion;
        block.nNonce = nonce;
        block.nCycle = cycle; // Quicksilver: the Cuckatoo proof that satisfies CheckProofOfWork
...
public:
    ...
    {
        return m_node.relaypool ? m_node.relaypool->GetTransactionsUpdated() : 0;
    }
    ...
    std::unique_ptr<BlockTemplate> createNewBlock(const BlockCreateOptions& options) override
    {
        ...
        return std::make_unique<BlockTemplateImpl>(BlockAssembler{chainman().ActiveChainstate(), context()->relaypool.get(), assemble_options}.CreateNewBlock(), m_node);
    }
