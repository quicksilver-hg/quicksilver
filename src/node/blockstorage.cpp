    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // Quicksilver: the Cuckatoo 42-cycle is part of the header and is required to
                // reconstruct a valid block header for the PoW re-check below. Without it the
                // reloaded header carries a zero cycle and CheckProofOfWork fails — so a node
                // could never restart after mining. (Pre-existing: nCycle was persisted in
                // CDiskBlockIndex but never restored here; surfaced by the #5c-1 restart test.)
                pindexNew->nCycle         = diskindex.nCycle;
                pindexNew->nStatus        = diskindex.nStatus;
                pindexNew->nTx            = diskindex.nTx;
                // Quicksilver (#5c-1 Phase 2): the congestion multiplier is a header field, so
                // like nCycle above it must be restored before the CheckProofOfWork below —
                // it is inside the pre-pow and therefore inside the block hash.
                pindexNew->m_congestion   = diskindex.m_congestion;

                if (!CheckProofOfWork(pindexNew->GetBlockHeader(), consensusParams)) {
                    LogError(HgLog::BLOCKSTORE, "%s: CheckProofOfWork failed: %s\n", __func__, pindexNew->ToString());
                    return false;
                }

                pcursor->Next();
            } else {
                LogError(HgLog::BLOCKSTORE, "%s: failed to read value\n", __func__);
                return false;
            }
        } else {
            break;
    ...
    ChainstateManager& chainman)
{
    ...
    LogInfo(HgLog::BLOCKSTORE, "pruned mode=manual prune_height=%d pairs=%d\n",
    ...
    ChainstateManager& chainman)
{
    ...
    LogDebug(HgLog::PRUNE, "target=%dMiB actual=%dMiB diff=%dMiB min_height=%d max_prune_height=%d removed %d blk/rev pairs\n",
             target / 1024 / 1024, nCurrentUsage / 1024 / 1024,
...
bool BlockManager::LoadBlockIndex()
{
    ...
    for (CBlockIndex* pindex : vSortedByHeight) {
        ...
        if (previous_index && pindex->nHeight > previous_index->nHeight + 1) {
            LogError(HgLog::BLOCKSTORE, "%s: block index is non-contiguous, index of height %d missing\n", __func__, previous_index->nHeight + 1);
            return false;
        }
...
bool BlockManager::LoadBlockIndexDB()
{
    ...
    LogInfo(HgLog::BLOCKSTORE, "loaded last_blockfile=%i\n", max_blockfile_num);
    for (int nFile = 0; nFile <= max_blockfile_num; nFile++) {
        m_block_tree_db->ReadBlockFileInfo(nFile, m_blockfile_info[nFile]);
    }
    LogInfo(HgLog::BLOCKSTORE, "loaded last_blockfile_info=%s\n", m_blockfile_info[max_blockfile_num].ToString());
    ...
    LogInfo(HgLog::BLOCKSTORE, "checking blkfiles=all\n");
    ...
    {
        // Initialize the blockfile cursor to the last file.
        LOCK(cs_LastBlockFile);
    ...
    if (m_have_pruned) {
        LogInfo(HgLog::BLOCKSTORE, "loaded pruned_previously=1\n");
    }
...
void BlockManager::CleanupBlockRevFiles() const
{
    ...
    LogInfo(HgLog::BLOCKSTORE, "pruned files=blk+rev reason=reindex-with-prune\n");
...
bool BlockManager::ReadBlockUndo(CBlockUndo& blockundo, const CBlockIndex& index) const
{
    ...
    if (filein.IsNull()) {
        LogError(HgLog::BLOCKSTORE, "OpenUndoFile failed for %s", pos.ToString());
        return false;
    }
    ...
    try {
        ...
        LogError(HgLog::BLOCKSTORE, "%s: Deserialize or I/O error - %s at %s\n", __func__, e.what(), pos.ToString());
        return false;
    }

    // Verify checksum
    if (hashChecksum != verifier.GetHash()) {
        LogError(HgLog::BLOCKSTORE, "%s: Checksum mismatch at %s\n", __func__, pos.ToString());
        return false;
    }
...
void BlockManager::UnlinkPrunedFiles(const std::set<int>& setFilesToPrune) const
{
    ...
    for (std::set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        if (removed_blockfile || removed_undofile) {
            LogDebug(HgLog::BLOCKSTORE, "Prune: %s deleted blk/rev (%05u)\n", __func__, *it);
        }
    }
}
...
FlatFilePos BlockManager::FindNextBlockPos(unsigned int nAddSize, unsigned int nHeight, uint64_t nTime)
{
    ...
    if (!m_blockfile_cursor) {
        ...
        m_blockfile_cursor = new_cursor;
        LogDebug(HgLog::BLOCKSTORE, "initializing blockfile cursor to %s\n", new_cursor);
    }
    ...
    while (m_blockfile_info[nFile].nSize + nAddSize >= max_blockfile_size) {
        ...
        finalize_undo = (static_cast<int>(m_blockfile_info[nFile].nHeightLast) ==
                         Assert(m_blockfile_cursor)->undo_height);
...
bool BlockManager::WriteBlockUndo(const CBlockUndo& blockundo, BlockValidationState& state, CBlockIndex& block)
{
    ...
    if (block.GetUndoPos().IsNull()) {
        ...
        if (!FindUndoPos(state, block.nFile, pos, blockundo_size + UNDO_DATA_DISK_OVERHEAD)) {
            LogError(HgLog::BLOCKSTORE, "FindUndoPos failed");
            return false;
        }
        // Open history file to append
        AutoFile fileout{OpenUndoFile(pos)};
        if (fileout.IsNull()) {
            LogError(HgLog::BLOCKSTORE, "OpenUndoFile failed");
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos) const
{
    ...
    if (filein.IsNull()) {
        LogError(HgLog::BLOCKSTORE, "%s: OpenBlockFile failed for %s\n", __func__, pos.ToString());
        return false;
    }
    ...
    if (!CheckProofOfWork(block, GetConsensus())) {
        LogError(HgLog::BLOCKSTORE, "%s: Errors in block header at %s\n", __func__, pos.ToString());
        return false;
    }
...
bool BlockManager::ReadBlock(CBlock& block, const CBlockIndex& index) const
{
    ...
    if (block.GetHash() != index.GetBlockHash()) {
        LogError(HgLog::BLOCKSTORE, "%s: GetHash() doesn't match index for %s at %s\n", __func__, index.ToString(), block_pos.ToString());
        return false;
    }
    return true;
}
...
bool BlockManager::ReadRawBlock(std::vector<uint8_t>& block, const FlatFilePos& pos) const
{
    ...
    try {
        ...
        if (blk_start != GetParams().MessageStart()) {
            LogError(HgLog::BLOCKSTORE, "%s: Block magic mismatch for %s: %s versus expected %s\n", __func__, pos.ToString(),
                         HexStr(blk_start),
        ...
        if (blk_size > MAX_SIZE) {
            LogError(HgLog::BLOCKSTORE, "%s: Block data is larger than maximum deserialization size for %s: %s versus %s\n", __func__, pos.ToString(),
                         blk_size, MAX_SIZE);
            return false;
        }
        ...
        LogError(HgLog::BLOCKSTORE, "%s: Read from block file failed: %s for %s\n", __func__, e.what(), pos.ToString());
        return false;
    }
...
FlatFilePos BlockManager::WriteBlock(const CBlock& block, int nHeight)
{
    ...
    if (pos.IsNull()) {
        LogError(HgLog::BLOCKSTORE, "FindNextBlockPos failed");
        return FlatFilePos();
    }
    AutoFile fileout{OpenBlockFile(pos)};
    if (fileout.IsNull()) {
        LogError(HgLog::BLOCKSTORE, "OpenBlockFile failed");
...
static auto InitBlocksdirXorKey(const BlockManager::Options& opts)
{
    ...
    LogInfo(HgLog::BLOCKSTORE, "loaded blocksdir=%s obfuscation_key=%s\n", fs::PathToString(opts.blocks_dir), HexStr(xor_key));
...
void ImportBlocks(ChainstateManager& chainman, std::span<const fs::path> import_paths)
{
    ...
    if (!chainman.m_blockman.m_blockfiles_indexed) {
        ...
        while (true) {
            ...
            LogInfo(HgLog::BLOCKSTORE, "reindexing blkfile=blk%05u.dat\n", (unsigned int)nFile);
            chainman.LoadExternalBlockFile(file, &pos, &blocks_with_unknown_parent);
            if (chainman.m_interrupt) {
                LogInfo(HgLog::BLOCKSTORE, "halted reason=interrupt stage=%s\n", __func__);
                return;
            }
            nFile++;
        }
        ...
        LogInfo(HgLog::BLOCKSTORE, "reindexed ok=1\n");
    ...
    for (const fs::path& path : import_paths) {
        ...
        if (!file.IsNull()) {
            LogInfo(HgLog::BLOCKSTORE, "importing blkfile=%s\n", fs::PathToString(path));
            ...
            LogWarning(HgLog::BLOCKSTORE, "importing reason=open-failed blkfile=%s\n", fs::PathToString(path));
        }
    }
