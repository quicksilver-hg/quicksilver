...
void TxDownloadManager::RelayPoolAcceptedTx(const CTransactionRef& tx)
{
    m_impl->RelayPoolAcceptedTx(tx);
}
RejectedTxTodo TxDownloadManager::RelayPoolRejectedTx(const CTransactionRef& ptx, const TxValidationState& state, NodeId nodeid, bool first_time_failure)
{
    return m_impl->RelayPoolRejectedTx(ptx, state, nodeid, first_time_failure);
}
void TxDownloadManager::RelayPoolRejectedPackage(const Package& package)
{
    m_impl->RelayPoolRejectedPackage(package);
}
...
bool TxDownloadManagerImpl::AlreadyHaveTx(const GenTxid& gtxid, bool include_reconsiderable)
{
    ...
    return RecentRejectsFilter().contains(hash) || m_opts.m_relaypool.exists(gtxid);
}
...
bool TxDownloadManagerImpl::MaybeAddOrphanResolutionCandidate(const std::vector<Txid>& unique_parents, const Wtxid& wtxid, NodeId nodeid, std::chrono::microseconds now)
{
    ...
    LogDebug(HgLog::TXPACKAGES, "added peer=%d as a candidate for resolving orphan %s\n", nodeid, wtxid.ToString());
    return true;
}
...
std::vector<GenTxid> TxDownloadManagerImpl::GetRequestsToSend(NodeId nodeid, std::chrono::microseconds current_time)
{
    ...
    for (const auto& entry : expired) {
        LogDebug(HgLog::MESH, "timeout of inflight %s %s from peer=%d\n", entry.second.IsWtxid() ? "wtx" : "tx",
            entry.second.GetHash().ToString(), entry.first);
    }
    for (const GenTxid& gtxid : requestable) {
        if (!AlreadyHaveTx(gtxid, /*include_reconsiderable=*/false)) {
            LogDebug(HgLog::MESH, "Requesting %s %s peer=%d\n", gtxid.IsWtxid() ? "wtx" : "tx",
...
std::optional<PackageToValidate> TxDownloadManagerImpl::Find1P1CPackage(const CTransactionRef& ptx, NodeId nodeid)
{
    ...
    // different peers.
    const auto package_candidates_same_peer{m_orphanage.GetChildrenFromSamePeer(ptx, nodeid)};

    // These children should be sorted from newest to oldest. In the (probably uncommon) case
    // of children that replace each other, this helps us accept the highest surplus-work rate
    // (probably the most recent) one efficiently.
    for (const auto& child : package_candidates_same_peer) {
        Package maybe_package{ptx, child};
        if (!RecentRejectsReconsiderableFilter().contains(GetPackageHash(maybe_package)) &&
...
void TxDownloadManagerImpl::RelayPoolAcceptedTx(const CTransactionRef& tx)
{
...
node::RejectedTxTodo TxDownloadManagerImpl::RelayPoolRejectedTx(const CTransactionRef& ptx, const TxValidationState& state, NodeId nodeid, bool first_time_failure)
{
    ...
    if (state.GetResult() == TxValidationResult::TX_MISSING_INPUTS) {
        ...
        if (first_time_failure && !RecentRejectsFilter().contains(ptx->GetWitnessHash().ToUint256())) {
            ...
            for (const uint256& parent_txid : unique_parents) {
                if (RecentRejectsFilter().contains(parent_txid)) {
                    ...
                    break;
                           ...
                           !m_opts.m_relaypool.exists(GenTxid::Txid(parent_txid))) {
            ...
            if (!fRejectedParents) {
                ...
                // previously rejected for a reason a package context can change: too
                // little surplus work against its own conflicts.
                // A child never supplies work for its parent (see package_no_work_subsidy_tests).
                // Full-pool rejection is TX_RELAYPOOL_POLICY, not reconsiderable.
                std::erase_if(unique_parents, [&](const auto& txid){
                    return AlreadyHaveTx(GenTxid::Txid(txid), /*include_reconsiderable=*/false);
                });
                ...
                LogDebug(HgLog::RELAYPOOL, "not keeping orphan with rejected parents %s (wtxid=%s)\n",
                         tx.GetHash().ToString(),
        ...
        if (state.GetResult() == TxValidationResult::TX_RECONSIDERABLE) {
            ...
            if (first_time_failure) {
                ...
                LogDebug(HgLog::TXPACKAGES, "tx %s (wtxid=%s) failed but reconsiderable, looking for child in orphanage\n",
    ...
    if (state.GetResult() != TxValidationResult::TX_MISSING_INPUTS && m_orphanage.EraseTx(ptx->GetWitnessHash()) > 0) {
        LogDebug(HgLog::TXPACKAGES, "   removed orphan tx %s (wtxid=%s)\n", ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString());
    }
...
void TxDownloadManagerImpl::RelayPoolRejectedPackage(const Package& package)
{
...
std::pair<bool, std::optional<PackageToValidate>> TxDownloadManagerImpl::ReceivedTx(NodeId nodeid, const CTransactionRef& ptx)
{
    ...
    // was confirmed with a different witness, or exists in our relaypool
    // with a different witness, but this has limited downside:
    // relaypool validation does its own lookup of whether we have the txid
    ...
    if (AlreadyHaveTx(GenTxid::Wtxid(wtxid), /*include_reconsiderable=*/false)) {
        ...
        // submitted the tx to our relaypool, we won't have computed a DoS
        ...
        // transactions, but any tx not accepted by the relaypool, which may be
        ...
        LogDebug(HgLog::TXPACKAGES, "found tx %s (wtxid=%s) in reconsiderable rejects, looking for child in orphanage\n",
