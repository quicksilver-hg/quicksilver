...
#ifndef QUICKSILVER_NODE_TXDOWNLOADMAN_H
#define QUICKSILVER_NODE_TXDOWNLOADMAN_H
...
struct TxDownloadOptions {
    /** Read-only reference to relaypool. */
    const CTxRelayPool& m_relaypool;
 ...
 * - When a transaction or package is accepted or rejected from relaypool
...
public:
    ...
    /** Respond to successful transaction submission to relaypool */
    void RelayPoolAcceptedTx(const CTransactionRef& tx);

    /** Respond to transaction rejected from relaypool */
    RejectedTxTodo RelayPoolRejectedTx(const CTransactionRef& ptx, const TxValidationState& state, NodeId nodeid, bool first_time_failure);

    /** Respond to package rejected from relaypool */
    void RelayPoolRejectedPackage(const Package& package);
...
#endif // QUICKSILVER_NODE_TXDOWNLOADMAN_H
