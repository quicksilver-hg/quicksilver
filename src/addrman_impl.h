...
#ifndef QUICKSILVER_ADDRMAN_IMPL_H
#define QUICKSILVER_ADDRMAN_IMPL_H
...
private:
    ...
    //! The peer database format Quicksilver writes and the oldest it accepts.
    //! Earlier numbered formats predate Quicksilver and were never shipped by it.
    //! The format (first byte in the serialized stream) can be higher than this and
    //! still this software may be able to unserialize the file - if the second byte
    //! (see `lowest_compatible` in `Unserialize()`) is less or equal to this.
    static constexpr uint8_t FILE_FORMAT{4};
...
#endif // QUICKSILVER_ADDRMAN_IMPL_H
