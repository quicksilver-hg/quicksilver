...
#ifndef QUICKSILVER_CLUSTER_LINEARIZE_H
#define QUICKSILVER_CLUSTER_LINEARIZE_H
...
/** Data structure that holds a transaction graph's preprocessed data (work, size, ancestors,
 *  descendants). */
...
class DepGraph
{
    ...
    struct Entry
    {
        /** Work and size of transaction itself. */
        WorkFrac workrate;
        ...
        /** Construct an entry with a given workrate, ancestor set, descendant set. */
        Entry(const WorkFrac& f, const SetType& a, const SetType& d) noexcept : workrate(f), ancestors(a), descendants(d) {}
    };
...
public:
    ...
    DepGraph(const DepGraph<SetType>& depgraph, Span<const ClusterIndex> mapping, ClusterIndex pos_range) noexcept : entries(pos_range)
    {
        ...
        for (ClusterIndex i : depgraph.Positions()) {
            ...
            m_used.Set(new_idx);
            // Fill in work and size.
            entries[new_idx].workrate = depgraph.entries[i].workrate;
        }
        for (ClusterIndex i : depgraph.Positions()) {
    ...
    /** Get the workrate of a given transaction i. Complexity: O(1). */
    const WorkFrac& WorkRate(ClusterIndex i) const noexcept { return entries[i].workrate; }
    /** Get the mutable workrate of a given transaction i. Complexity: O(1). */
    WorkFrac& WorkRate(ClusterIndex i) noexcept { return entries[i].workrate; }
     ...
     */
    ClusterIndex AddTransaction(const WorkFrac& workfrac) noexcept
    {
        ...
        if (new_idx == entries.size()) {
            entries.emplace_back(workfrac, SetType::Singleton(new_idx), SetType::Singleton(new_idx));
        } else {
            entries[new_idx] = Entry(workfrac, SetType::Singleton(new_idx), SetType::Singleton(new_idx));
        }
        m_used.Set(new_idx);
        return new_idx;
    }
    ...
    void RemoveTransactions(const SetType& del) noexcept
    {
        ...
        // that the deleted positions will retain old workrate and dependency information. This does
    ...
    /** Compute the aggregate workrate of a set of nodes in this graph.
     *
     * Complexity: O(N) where N=elems.Count().
     **/
    WorkFrac WorkRate(const SetType& elems) const noexcept
    {
        WorkFrac ret;
        for (auto pos : elems) ret += entries[pos].workrate;
        return ret;
    }
...
/** A set of transactions together with their aggregate workrate. */
template<typename SetType>
struct SetInfo
{
    /** The transactions in the set. */
    SetType transactions;
    /** Their combined work and size. */
    WorkFrac workrate;

    /** Construct a SetInfo for the empty set. */
    SetInfo() noexcept = default;

    /** Construct a SetInfo for a specified set and workrate. */
    SetInfo(const SetType& txn, const WorkFrac& fr) noexcept : transactions(txn), workrate(fr) {}

    /** Construct a SetInfo for a given transaction in a depgraph. */
    explicit SetInfo(const DepGraph<SetType>& depgraph, ClusterIndex pos) noexcept :
        transactions(SetType::Singleton(pos)), workrate(depgraph.WorkRate(pos)) {}

    /** Construct a SetInfo for a set of transactions in a depgraph. */
    explicit SetInfo(const DepGraph<SetType>& depgraph, const SetType& txn) noexcept :
        transactions(txn), workrate(depgraph.WorkRate(txn)) {}
    ...
    void Set(const DepGraph<SetType>& depgraph, ClusterIndex pos) noexcept
    {
        ...
        workrate += depgraph.WorkRate(pos);
    }
    ...
    {
        return {transactions | txn, workrate + depgraph.WorkRate(txn - transactions)};
    }
...
/** Compute the workrates of the chunks of linearization. */
template<typename SetType>
std::vector<WorkFrac> ChunkLinearization(const DepGraph<SetType>& depgraph, Span<const ClusterIndex> linearization) noexcept
{
    std::vector<WorkFrac> ret;
    for (ClusterIndex i : linearization) {
        /** The new chunk to be added, initially a singleton. */
        auto new_chunk = depgraph.WorkRate(i);
        // As long as the new chunk has a higher workrate than the last chunk so far, absorb it.
...
class LinearizationChunking
{
    ...
    /** Chunk sets and their workrates, of what remains of the linearization. */
    ...
    void BuildChunks() noexcept
    {
        ...
        // keeps track of the sets themselves instead of just their workrates.
        for (auto idx : m_linearization) {
            if (!m_todo[idx]) continue;
            // Start with an initial chunk containing just element idx.
            SetInfo add(m_depgraph, idx);
            // Absorb existing final chunks into add while they have lower workrate.
            while (!m_chunks.empty() && add.workrate >> m_chunks.back().workrate) {
                add |= m_chunks.back();
                m_chunks.pop_back();
            }
            // Remember new chunk.
...
public:
    ...
    /** Access a chunk. Chunk 0 is the highest-workrate prefix of what remains. */
     ...
     *  of the linearization that has a workrate not below subset's.
     *
     * This is a crucial operation in guaranteeing improvements to linearizations. If subset has
     * a workrate not below GetChunk(0)'s, then moving IntersectPrefixes(subset) to the front of
    ...
    SetInfo<SetType> IntersectPrefixes(const SetInfo<SetType>& subset) const noexcept
    {
        ...
        for (ClusterIndex i = 0; i < NumChunksLeft(); ++i) {
            ...
            if (to_add.Any()) {
                ...
                // shorter intersection with higher/equal workrate exists.
                accumulator.transactions |= to_add;
                if (accumulator.transactions == subset.transactions) break;
                // Otherwise update the accumulator workrate.
                accumulator.workrate += m_depgraph.WorkRate(to_add);
                // If that does result in something better, or something with the same workrate but
                // smaller, return that. Even if a longer, higher-workrate intersection exists, it
                ...
                if (!(accumulator.workrate << subset.workrate)) return accumulator;
            }
        }
        return subset;
    }
 ...
 * SetInfo with the highest-workrate ancestor set that remains (an ancestor set is a single
...
class AncestorCandidateFinder
{
    ...
    SetType m_todo;
    /** Precomputed ancestor-set workrates (only kept up-to-date for indices in m_todo). */
    std::vector<WorkFrac> m_ancestor_set_workrates;
...
public:
    ...
    AncestorCandidateFinder(const DepGraph<SetType>& depgraph LIFETIMEBOUND) noexcept :
        ...
        m_ancestor_set_workrates(depgraph.PositionRange())
    {
        // Precompute ancestor-set workrates.
        for (ClusterIndex i : m_depgraph.Positions()) {
            ...
            WorkFrac anc_workrate;
            // Reuse accumulated workrate from first ancestor, if usable.
            Assume(anc_to_add.Any());
            ClusterIndex first = anc_to_add.First();
            if (first < i) {
                anc_workrate = m_ancestor_set_workrates[first];
                Assume(!anc_workrate.IsEmpty());
                anc_to_add -= m_depgraph.Ancestors(first);
            }
            // Add in other ancestors (which necessarily include i itself).
            Assume(anc_to_add[i]);
            anc_workrate += m_depgraph.WorkRate(anc_to_add);
            // Store the result.
            m_ancestor_set_workrates[i] = anc_workrate;
        }
    }
    ...
    void MarkDone(SetType select) noexcept
    {
        ...
        m_todo -= select;
        for (auto i : select) {
            auto workrate = m_depgraph.WorkRate(i);
            for (auto j : m_depgraph.Descendants(i) & m_todo) {
                m_ancestor_set_workrates[j] -= workrate;
            }
        }
    }
    ...
    /** Find the best (highest-workrate, smallest among those in case of a tie) ancestor set
    ...
    SetInfo<SetType> FindCandidateSet() const noexcept
    {
        ...
        for (auto i : m_todo) {
            if (best.has_value()) {
                Assume(!m_ancestor_set_workrates[i].IsEmpty());
                if (!(m_ancestor_set_workrates[i] > m_ancestor_set_workrates[*best])) continue;
            }
            best = i;
        }
        Assume(best.has_value());
        return {m_depgraph.Ancestors(*best) & m_todo, m_ancestor_set_workrates[*best]};
    }
};
...
public:
    ...
    {
        // Determine reordering mapping, by sorting by decreasing workrate. Unused positions are
        ...
        std::sort(m_sorted_to_original.begin(), m_sorted_to_original.end(), [&](auto a, auto b) {
            auto workrate_cmp = depgraph.WorkRate(a) <=> depgraph.WorkRate(b);
            if (workrate_cmp == 0) return a < b;
            return workrate_cmp > 0;
        });
    ...
    /** Find a high-workrate topologically-valid subset of what remains of the cluster.
     *  Requires !AllDone().
     *
     * @param[in] max_iterations  The maximum number of optimization steps that will be performed.
     * @param[in] best            A set/workrate pair with an already-known good candidate. This may
     *                            be empty.
     * @return                    A pair of:
     *                            - The best (highest workrate, smallest size as tiebreaker)
     *                              topologically valid subset (and its workrate) that was
    ...
    std::pair<SetInfo<SetType>, uint64_t> FindCandidateSet(uint64_t max_iterations, SetInfo<SetType> best) noexcept
    {
        ...
        struct WorkItem
        {
            /** Set of transactions definitely included (and its workrate). This must be a subset
            ...
            SetType und;
            /** (Only when inc is not empty) The best workrate of any superset of inc that is also a
             ...
             *  Transactions whose workrate is below best's are ignored when determining this value,
            ...
            WorkFrac pot_workrate;

            /** Construct a new work item. */
            WorkItem(SetInfo<SetType>&& i, SetType&& u, WorkFrac&& p_f) noexcept :
                inc(std::move(i)), und(std::move(u)), pot_workrate(std::move(p_f))
            {
                Assume(pot_workrate.IsEmpty() == inc.workrate.IsEmpty());
            }
            ...
            void Swap(WorkItem& other) noexcept
            {
                swap(inc, other.inc);
                swap(und, other.und);
                swap(pot_workrate, other.pot_workrate);
            }
        };
        ...
        do {
            ...
            if (best.workrate.IsEmpty()) best = SetInfo(m_sorted_depgraph, component);
            queue.emplace_back(/*inc=*/SetInfo<SetType>{},
                               /*und=*/std::move(component),
                               /*pot_workrate=*/WorkFrac{});
        } while (to_cover.Any());

        /** Local copy of the iteration limit. */
        uint64_t iterations_left = max_iterations;

        /** The set of transactions in m_todo which have workrate > best's. */
        SetType imp = m_todo;
        while (imp.Any()) {
            ClusterIndex check = imp.Last();
            if (m_sorted_depgraph.WorkRate(check) >> best.workrate) break;
            imp.Reset(check);
        }
        ...
        auto add_fn = [&](SetInfo<SetType> inc, SetType und) noexcept {
            /** SetInfo object with the set whose workrate will become the new work item's
             *  pot_workrate. It starts off equal to inc. */
            auto pot = inc;
            if (!inc.workrate.IsEmpty()) {
                // Add entries to pot. We iterate over all undecided transactions whose workrate is
                // higher than best. While undecided transactions of lower workrate may improve pot,
                // the resulting pot workrate cannot possibly exceed best's (and this item will be
                ...
                for (auto pos : imp & und) {
                    ...
                    // individual workrate order.
                    if (!(m_sorted_depgraph.WorkRate(pos) >> pot.workrate)) break;
                ...
                // its workrate exceeds that of any set compatible with this work item (superset of
                ...
                for (auto pos : pot.transactions - inc.transactions) {
                    ...
                    // with its ancestors to inc. Just update the transactions here; the workrate
                ...
                // Finally update und and inc's workrate to account for the added transactions.
                und -= inc.transactions;
                inc.workrate += m_sorted_depgraph.WorkRate(inc.transactions - init_inc);

                // If inc's workrate is better than best's, remember it as our new best.
                if (inc.workrate > best.workrate) {
                    best = inc;
                ...
                if (pot.workrate.size == inc.workrate.size) return;
            ...
            queue.emplace_back(/*inc=*/std::move(inc),
                               /*und=*/std::move(und),
                               /*pot_workrate=*/std::move(pot.workrate));
        };
        ...
        auto split_fn = [&](WorkItem&& elem) noexcept {
            ...
            Assume(elem.inc.workrate.IsEmpty() == elem.pot_workrate.IsEmpty());

            const ClusterIndex first = elem.und.First();
            if (!elem.inc.workrate.IsEmpty()) {
                // If no undecided transactions remain with workrate higher than best, this entry
                // cannot be improved beyond best.
                if (!elem.und.Overlaps(imp)) return;
                // We can ignore any queue item whose potential workrate isn't better than the best
                // seen so far.
                if (elem.pot_workrate <= best.workrate) return;
            } else {
                // In case inc is empty use a simpler alternative check.
                if (m_sorted_depgraph.WorkRate(first) <= best.workrate) return;
            }
            ...
            // individual workrate undecided transaction. Pick the one which reduces the search space
            ...
            // that 2^I(t) + 2^E(t) is minimal, tie-breaking by highest individual workrate for t.
 ...
 *                                  good (in the workrate diagram sense) as old_linearization.
...
std::pair<std::vector<ClusterIndex>, bool> Linearize(const DepGraph<SetType>& depgraph, uint64_t max_iterations, uint64_t rng_seed, Span<const ClusterIndex> old_linearization = {}) noexcept
{
    ...
    // (largely due to the cost of constructing the internal sorted-by-workrate DepGraph inside
    ...
    while (true) {
        // Find the highest-workrate prefix of the remainder of old_linearization.
        ...
        if (!best_prefix.workrate.IsEmpty() && best_prefix.workrate >= best.workrate) best = best_prefix;
 ...
 *   optionally with its work increased. Let L3 be the postlinearization of L2. L3 will be at least
 *   as good as L1. This means that replacing transactions with same-size higher-work transactions
...
void PostLinearize(const DepGraph<SetType>& depgraph, Span<ClusterIndex> linearization)
{
    ...
    //   - While L has at least one group before C, and the group immediately before C has workrate
    //     lower than C:
    //     - If C depends on P:
    ...
    // parent/child, and of high/low workrate are reversed, and the final concatenation is reversed
    // on output.
    //
    ...
    struct TxEntry
    {
        ...
        SetType deps;
        /** The combined work/size of transactions in the group. Work is negated in even passes. */
        WorkFrac workrate;
    };
    ...
    for (int pass = 0; pass < 2; ++pass) {
        ...
        Assume(entries[SENTINEL].workrate.IsEmpty());
        ...
        for (ClusterIndex i = 0; i < linearization.size(); ++i) {
            ...
            // parent/child and high/low workrate are swapped.
            ...
            entries[cur_group].workrate = depgraph.WorkRate(idx);
            if (rev) entries[cur_group].workrate.work = -entries[cur_group].workrate.work;
            ...
            // Continue as long as the current group has higher workrate than the previous one.
            while (entries[cur_group].workrate >> entries[prev_group].workrate) {
                ...
                // The sentinel has empty workrate, which is neither higher or lower than other
                // workrates. Thus, the while loop we are in here guarantees that cur_group and
                ...
                if (entries[cur_group].deps.Overlaps(entries[prev_group].group)) {
                    ...
                    // into cur_group. The group/deps/workrate fields of prev_group remain unchanged
                    // but become unused.
                    entries[cur_group].group |= entries[prev_group].group;
                    entries[cur_group].deps |= entries[prev_group].deps;
                    entries[cur_group].workrate += entries[prev_group].workrate;
...
std::vector<ClusterIndex> MergeLinearizations(const DepGraph<SetType>& depgraph, Span<const ClusterIndex> lin1, Span<const ClusterIndex> lin2)
{
    ...
    while (true) {
        ...
        if (lin2_firstchunk.workrate >> lin1_firstchunk.workrate) {
...
#endif // QUICKSILVER_CLUSTER_LINEARIZE_H
