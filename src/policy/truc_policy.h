...
#ifndef QUICKSILVER_POLICY_TRUC_POLICY_H
#define QUICKSILVER_POLICY_TRUC_POLICY_H
...
// replacement abilities more robust. A transaction with version=3 is treated as TRUC.
...
/** Maximum number of transactions including a TRUC tx and all its relaypool ancestors. */
 ...
 *
 *
 * @param[in]   relaypool_ancestors       The in-relaypool ancestors of ptx.
 * @param[in]   direct_conflicts        In-relaypool transactions this tx conflicts with. These conflicts
 *                                      are used to more accurately calculate the resulting descendant
 *                                      count of in-relaypool ancestors.
 ...
 * - debug string + pointer to a relaypool sibling if this transaction would be the second child in a
 ...
 * If it's not a TRUC transaction, verify it has no direct TRUC parents in the relaypool or the package.

 * If it is a TRUC transaction, verify that any direct parents in the relaypool or the package are TRUC.
 * If such a parent exists, verify that parent has no other children in the package or the relaypool,
 ...
 * - if a TRUC transaction T has a parent in the relaypool and a child in the package, then PTRUCC(T) will fail
 * - if a TRUC transaction T has a parent in the package and a child in the package, then PTRUCC(T) will fail
 * - if a TRUC transaction T and a TRUC (sibling) transaction U have some parent in the relaypool,
...
#endif // QUICKSILVER_POLICY_TRUC_POLICY_H
