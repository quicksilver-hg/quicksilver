...
/** Helper for PackageTRUCChecks, storing info for a relaypool or package parent. */
struct ParentInfo {
    ...
    /** If parent is in relaypool, whether it has any descendants in relaypool. */
    bool m_has_relaypool_descendant;

    ParentInfo() = delete;
    ParentInfo(const Txid& txid, const Wtxid& wtxid, decltype(CTransaction::version) version, bool has_relaypool_descendant) :
        m_txid{txid}, m_wtxid{wtxid}, m_version{version},
        m_has_relaypool_descendant{has_relaypool_descendant}
    {}
};

std::optional<std::string> PackageTRUCChecks(const CTransactionRef& ptx, int64_t vsize,
                                           const Package& package,
                                           const CTxRelayPool::setEntries& relaypool_ancestors)
{
    ...
    if (ptx->version == TRUC_VERSION) {
        ...
        if (relaypool_ancestors.size() + in_package_parents.size() + 1 > TRUC_ANCESTOR_LIMIT) {
            return strprintf("tx %s (wtxid=%s) would have too many ancestors",
                             ptx->GetHash().ToString(), ptx->GetWitnessHash().ToString());
        }

        const bool has_parent{relaypool_ancestors.size() + in_package_parents.size() > 0};
        if (has_parent) {
            ...
            // Exactly 1 parent exists, either in relaypool or package. Find it.
            const auto parent_info = [&] {
                if (relaypool_ancestors.size() > 0) {
                    auto& relaypool_parent = *relaypool_ancestors.begin();
                    return ParentInfo{relaypool_parent->GetTx().GetHash(),
                                      relaypool_parent->GetTx().GetWitnessHash(),
                                      relaypool_parent->GetTx().version,
                                      /*has_relaypool_descendant=*/relaypool_parent->GetCountWithDescendants() > 1};
                } else {
                    ...
                    return ParentInfo{package_parent->GetHash(),
                                      ...
                                      /*has_relaypool_descendant=*/false};
                }
            }();
            ...
            if (parent_info.m_has_relaypool_descendant) {
                return strprintf("tx %s (wtxid=%s) would exceed descendant count limit",
                                parent_info.m_txid.ToString(), parent_info.m_wtxid.ToString());
            }
        }
        ...
        for (auto it : relaypool_ancestors) {
                                          ...
                                          int64_t vsize)
{
    ...
    for (const auto& entry : relaypool_ancestors) {
    ...
    if (relaypool_ancestors.size() + 1 > TRUC_ANCESTOR_LIMIT) {
    ...
    if (relaypool_ancestors.size() > 0) {
        ...
        // Check the descendant counts of in-relaypool ancestors.
        const auto& parent_entry = *relaypool_ancestors.begin();
        ...
        const auto& children = parent_entry->GetRelayPoolChildrenConst();
        ...
        const bool child_will_be_replaced = !children.empty() &&
            std::any_of(children.cbegin(), children.cend(),
                [&direct_conflicts](const CTxRelayPoolEntry& child){return direct_conflicts.count(child.GetTx().GetHash()) > 0;});
        if (parent_entry->GetCountWithDescendants() + 1 > TRUC_DESCENDANT_LIMIT && !child_will_be_replaced) {
            ...
            // we don't conflict inputs with it, consider evicting it under replacement rules. We rely on TRUC rules
