...
// Quicksilver has no dust threshold; any positive-value standard output is relayable.
...
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes, bool permit_bare_multisig, std::string& reason)
{
    ...
    // significant validation CPU, because computing signature hashes is
    // O(ninputs*txsize). Limiting transactions to MAX_STANDARD_TX_WEIGHT
...
bool AreInputsStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (whichType == TxoutType::NONSTANDARD || whichType == TxoutType::WITNESS_UNKNOWN) {
            ...
            if (subscript.IsWitnessProgram(witness_version, witness_program)) {
                return false;
            }
...
bool IsWitnessStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (witnessversion == 1 && witnessprogram.size() == WITNESS_V1_TAPROOT_SIZE) {
            // Taproot spend (version 1, witness program size 32; see BIP 341)
