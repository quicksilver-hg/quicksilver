...
namespace vault {
bool ExternalSignerScriptPubKeyMan::SetupDescriptor(VaultBatch& batch, std::unique_ptr<Descriptor> desc)
{
    LOCK(cs_desc_man);
    assert(m_storage.IsVaultFlagSet(VAULT_FLAG_DESCRIPTORS));
    assert(m_storage.IsVaultFlagSet(VAULT_FLAG_EXTERNAL_SIGNER));

    int64_t creation_time = GetTime();

    // Make the descriptor
    VaultDescriptor w_desc(std::move(desc), creation_time, 0, 0, 0);
    m_vault_descriptor = w_desc;

    // Store the descriptor
    if (!batch.WriteDescriptor(GetID(), m_vault_descriptor)) {
    ...
    m_storage.UnsetBlankVaultFlag(batch);
    return true;
}
...
{
    // TODO: avoid the need to infer a descriptor from inside a vault
...
std::optional<PSQTError> ExternalSignerScriptPubKeyMan::FillPSQT(PartiallySignedQuicksilverTransaction& psqt, const PrecomputedTransactionData& txdata, int sighash_type, bool sign, bool bip32derivs, int* n_signed, bool finalize) const
{
    if (!sign) {
        return DescriptorScriptPubKeyMan::FillPSQT(psqt, txdata, sighash_type, false, bip32derivs, n_signed, finalize);
    }

    // Already complete if every input is now signed
    bool complete = true;
    for (const auto& input : psqt.inputs) {
        // TODO: for multisig vaults, we should only care if all _our_ inputs are signed
        complete &= PSQTInputSigned(input);
    }
    if (complete) return {};

    std::string strFailReason;
    if(!GetExternalSigner().SignTransaction(psqt, strFailReason)) {
        tfm::format(std::cerr, "Failed to sign: %s\n", strFailReason);
        return PSQTError::EXTERNAL_SIGNER_FAILED;
    }
    if (finalize) FinalizePSQT(psqt); // This won't work in a multisig setup
    return {};
}
} // namespace vault
