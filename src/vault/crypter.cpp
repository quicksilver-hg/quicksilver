...
int CCrypter::BytesToKeySHA512AES(const std::span<const unsigned char> salt, const SecureString& key_data, int count, unsigned char* key, unsigned char* iv) const
{
    ...
    memcpy(key, buf, VAULT_CRYPTO_KEY_SIZE);
    memcpy(iv, buf + VAULT_CRYPTO_KEY_SIZE, VAULT_CRYPTO_IV_SIZE);
    memory_cleanse(buf, sizeof(buf));
    return VAULT_CRYPTO_KEY_SIZE;
}

bool CCrypter::SetKeyFromPassphrase(const SecureString& key_data, const std::span<const unsigned char> salt, const unsigned int rounds)
{
    if (rounds < 1 || salt.size() != VAULT_CRYPTO_SALT_SIZE) {
        return false;
    }

    int i = BytesToKeySHA512AES(salt, key_data, rounds, vchKey.data(), vchIV.data());

    if (i != (int)VAULT_CRYPTO_KEY_SIZE)
    {
...
{
    if (new_key.size() != VAULT_CRYPTO_KEY_SIZE || new_iv.size() != VAULT_CRYPTO_IV_SIZE) {
        return false;
    }
...
bool EncryptSecret(const CKeyingMaterial& vMasterKey, const CKeyingMaterial &vchPlaintext, const uint256& nIV, std::vector<unsigned char> &vchCiphertext)
{
    ...
    std::vector<unsigned char> chIV(VAULT_CRYPTO_IV_SIZE);
    memcpy(chIV.data(), &nIV, VAULT_CRYPTO_IV_SIZE);
...
bool DecryptSecret(const CKeyingMaterial& master_key, const std::span<const unsigned char> ciphertext, const uint256& iv, CKeyingMaterial& plaintext)
{
    ...
    static_assert(VAULT_CRYPTO_IV_SIZE <= std::remove_reference_t<decltype(iv)>::size());
    const std::span iv_prefix{iv.data(), VAULT_CRYPTO_IV_SIZE};
