...
#include <vault/vaultutil.h> // for VAULT_FLAG_DESCRIPTORS
...
static std::vector<std::unique_ptr<VaultDatabase>> TestDatabases(const fs::path& path_root)
{
    std::vector<std::unique_ptr<VaultDatabase>> dbs;
...
#endif
    dbs.emplace_back(CreateMockableVaultDatabase());
    return dbs;
}
...
// Verify 'ErasePrefix' functionality using db keys similar to the ones used by the vault.
