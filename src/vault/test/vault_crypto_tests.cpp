...
namespace vault {
BOOST_FIXTURE_TEST_SUITE(vault_crypto_tests, BasicTestingSetup)
...
{
    CCrypter crypt;
    BOOST_REQUIRE(crypt.SetKeyFromPassphrase(passphrase, salt, rounds));
                        ...
                        const std::span<const unsigned char> correct_plaintext = {})
{
    ...
    if (ok) {
        std::vector<unsigned char> recrypted;
        BOOST_REQUIRE(crypt.Encrypt(decrypted, recrypted));
        BOOST_CHECK_EQUAL_COLLECTIONS(recrypted.begin(), recrypted.end(), ciphertext.begin(), ciphertext.end());
    }
}

static void TestDecryptRejects(const CCrypter& crypt, const std::span<const unsigned char> ciphertext)
{
    CKeyingMaterial decrypted;
...
BOOST_AUTO_TEST_CASE(encrypt) {
    constexpr std::array<uint8_t, VAULT_CRYPTO_SALT_SIZE> salt{"0000deadbeef0000"_hex_u8};
    CCrypter crypt;
    crypt.SetKeyFromPassphrase("passphrase", salt, 25000);
...
BOOST_AUTO_TEST_CASE(decrypt) {
    ...
    // Corner cases from testing. Independent oracle (python hashlib SHA-512
    // EVP_BytesToKey equivalent, which reproduces the committed passphrase
    // key/IV for "test"; then openssl enc -aes-256-cbc -d -nopad and PKCS#7
    // unpad) decrypts all six to padsize 0, so Decrypt must return false.
    TestCrypter::TestDecryptRejects(crypt,"795643ce39d736088367822cdc50535ec6f103715e3e48f4f3b1a60a08ef59ca"_hex_u8);
    TestCrypter::TestDecryptRejects(crypt,"de096f4a8f9bd97db012aa9d90d74de8cdea779c3ee8bc7633d8b5d6da703486"_hex_u8);
    TestCrypter::TestDecryptRejects(crypt,"32d0a8974e3afd9c6c3ebf4d66aa4e6419f8c173de25947f98cf8b7ace49449c"_hex_u8);
    TestCrypter::TestDecryptRejects(crypt,"e7c055cca2faa78cb9ac22c9357a90b4778ded9b2cc220a14cea49f931e596ea"_hex_u8);
    TestCrypter::TestDecryptRejects(crypt,"b88efddd668a6801d19516d6830da4ae9811988ccbaf40df8fbb72f3f4d335fd"_hex_u8);
    TestCrypter::TestDecryptRejects(crypt,"8cae76aa6a43694e961ebcb28c8ca8f8540b84153d72865e8561ddd93fa7bfa9"_hex_u8);
