...
static std::optional<std::pair<VaultDescriptor, FlatSigningProvider>> CreateVaultDescriptor(FuzzedDataProvider& fuzzed_data_provider)
{
    ...
    VaultDescriptor w_desc{std::move(parsed_descs.at(0)), /*creation_time=*/0, /*range_start=*/0, /*range_end=*/1, /*next_index=*/1};
    return std::make_pair(w_desc, keys);
}

static DescriptorScriptPubKeyMan* CreateDescriptor(VaultDescriptor& vault_desc, FlatSigningProvider& keys, CVault& keystore)
{
    LOCK(keystore.cs_vault);
    keystore.AddVaultDescriptor(vault_desc, keys, /*label=*/"", /*internal=*/false);
    return keystore.GetDescriptorScriptPubKeyMan(vault_desc);
};
...
FUZZ_TARGET(scriptpubkeyman, .init = initialize_spkm)
{
    ...
    std::unique_ptr<CVault> vault_ptr{std::make_unique<CVault>(node.chain.get(), "", CreateMockableVaultDatabase())};
    CVault& vault{*vault_ptr};
    {
        LOCK(vault.cs_vault);
        vault.SetVaultFlag(VAULT_FLAG_DESCRIPTORS);
        vault.SetLastBlockProcessed(chainstate.m_chain.Height(), chainstate.m_chain.Tip()->GetBlockHash());
        vault.m_keypool_size = 1;
    }

    auto vault_desc{CreateVaultDescriptor(fuzzed_data_provider)};
    if (!vault_desc.has_value()) return;
    auto spk_manager{CreateDescriptor(vault_desc->first, vault_desc->second, vault)};
    if (spk_manager == nullptr) return;

    if (fuzzed_data_provider.ConsumeBool()) {
        auto vault_desc{CreateVaultDescriptor(fuzzed_data_provider)};
        if (!vault_desc.has_value()) {
            return;
        }
        std::string error;
        if (spk_manager->CanUpdateToVaultDescriptor(vault_desc->first, error)) {
            auto new_spk_manager{CreateDescriptor(vault_desc->first, vault_desc->second, vault)};
    ...
    LIMITED_WHILE(good_data && fuzzed_data_provider.ConsumeBool(), 20) {
        CallOneOf(
            ...
            [&] {
                ...
                auto vault_desc{spk_manager->GetVaultDescriptor()};
                if (vault_desc.descriptor->IsSingleType()) {
                    auto output_type{vault_desc.descriptor->GetOutputType()};
            ...
            [&] {
                std::optional<PartiallySignedQuicksilverTransaction> opt_psqt{ConsumeDeserializable<PartiallySignedQuicksilverTransaction>(fuzzed_data_provider)};
                if (!opt_psqt) {
                    good_data = false;
                    return;
                }
                auto psqt{*opt_psqt};
                const PrecomputedTransactionData txdata{PrecomputePSQTData(psqt)};
                ...
                (void)spk_manager->FillPSQT(psqt, txdata, sighash_type, sign, bip32derivs, nullptr, finalize);
            }
        );
    }
