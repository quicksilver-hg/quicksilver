...
namespace vault {
static constexpr int32_t VAULT_SCHEMA_VERSION = 0;
...
static void ErrorLogCallback(void* arg, int code, const char* msg)
{
    ...
    assert(arg == nullptr);
    LogInfo(HgLog::VAULTDB, "SQLite Error. Code: %d. Message: %s\n", code, msg);
}
...
static int TraceSqlCallback(unsigned code, void* context, void* param1, void* param2)
{
    ...
    if (code == SQLITE_TRACE_STMT) {
        ...
        LogTrace(HgLog::VAULTDB, "[%s] SQLite Statement: %s\n", db->Filename(), expanded ? expanded : sqlite3_sql(stmt));
                                ...
                                const std::string& description)
{
    ...
    if (res != SQLITE_OK) {
        LogInfo(HgLog::VAULTDB, "Unable to bind %s to statement: %s\n", description, sqlite3_errstr(res));
...
SQLiteDatabase::SQLiteDatabase(const fs::path& dir_path, const fs::path& file_path, const DatabaseOptions& options, bool mock)
    : VaultDatabase(), m_mock(mock), m_dir_path(fs::PathToString(dir_path)), m_file_path(fs::PathToString(file_path)), m_write_semaphore(1), m_use_unsafe_sync(options.use_unsafe_sync), m_read_only(options.read_only)
{
    {
        LOCK(g_sqlite_mutex);
        LogInfo(HgLog::VAULTDB, "ready sqlite_version=%s\n", SQLiteDatabaseVersion());
        LogInfo(HgLog::VAULTDB, "loading dir=%s\n", m_dir_path);
...
void SQLiteDatabase::Cleanup() noexcept
{
    ...
    if (--g_sqlite_count == 0) {
        ...
        if (ret != SQLITE_OK) {
            LogInfo(HgLog::VAULTDB, "SQLiteDatabase: Failed to shutdown SQLite: %s\n", sqlite3_errstr(ret));
        }
    }
}
...
bool SQLiteDatabase::Verify(bilingual_str& error)
{
    ...
    read_result = ReadPragmaInteger(m_db, "user_version", "sqlite vault schema version", error);
    if (!read_result.has_value()) return false;
    int32_t user_ver = read_result.value();
    if (user_ver != VAULT_SCHEMA_VERSION) {
        error = strprintf(_("SQLiteDatabase: Unknown sqlite vault schema version %d. Only version %d is supported"), user_ver, VAULT_SCHEMA_VERSION);
        return false;
    }
...
{
    int flags = SQLITE_OPEN_FULLMUTEX | (m_read_only ? SQLITE_OPEN_READONLY : SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE);
    if (m_mock) {
        flags |= SQLITE_OPEN_MEMORY; // In memory database for mock db
    }

    if (m_db == nullptr) {
        if (!m_mock && !m_read_only) {
        ...
        // Trace SQL statements if tracing is enabled with -debug=vaultdb -loglevel=vaultdb:trace
        if (LogAcceptCategory(HgLog::VAULTDB, HgLog::Level::Trace)) {
           ret = sqlite3_trace_v2(m_db, SQLITE_TRACE_STMT, TraceSqlCallback, this);
           if (ret != SQLITE_OK) {
               LogInfo(HgLog::VAULTDB, "Failed to enable SQL tracing for %s\n", Filename());
           }
        }
    }
    ...
    if (m_use_unsafe_sync) {
        ...
        LogInfo(HgLog::VAULTDB, "WARNING SQLite is configured to not wait for data to be flushed to disk. Data loss and corruption may occur.\n");
    ...
    // Do the db setup things because the table doesn't exist only when we are creating a new vault
    if (!table_exists) {
        ...
        SetPragma(m_db, "user_version", strprintf("%d", VAULT_SCHEMA_VERSION),
                  "Failed to set the vault schema version");
    }
}
...
bool SQLiteDatabase::Backup(const std::string& dest) const
{
    ...
    if (!backup) {
        LogInfo(HgLog::VAULTDB, "%s: Unable to begin backup: %s\n", __func__, sqlite3_errmsg(m_db));
        sqlite3_close(db_copy);
        return false;
    }
    ...
    if (res != SQLITE_DONE) {
        LogInfo(HgLog::VAULTDB, "%s: Unable to backup: %s\n", __func__, sqlite3_errstr(res));
...
void SQLiteBatch::Close()
{
    ...
    if (m_txn) {
        if (TxnAbort()) {
            LogInfo(HgLog::VAULTDB, "SQLiteBatch: Batch closed unexpectedly without the transaction being explicitly committed or aborted\n");
        } else {
            ...
            LogInfo(HgLog::VAULTDB, "SQLiteBatch: Batch closed and failed to abort transaction, resetting db connection..\n");
        }
    }
    ...
    for (const auto& [stmt_prepared, stmt_description] : statements) {
        ...
        if (res != SQLITE_OK) {
            LogInfo(HgLog::VAULTDB, "SQLiteBatch: Batch closed but could not finalize %s statement: %s\n",
...
bool SQLiteBatch::ReadKey(DataStream&& key, DataStream& value)
{
    ...
    if (res != SQLITE_ROW) {
        if (res != SQLITE_DONE) {
            ...
            LogInfo(HgLog::VAULTDB, "%s: Unable to execute statement: %s\n", __func__, sqlite3_errstr(res));
        }
...
DatabaseCursor::Status SQLiteCursor::Next(DataStream& key, DataStream& value)
{
    ...
    if (res != SQLITE_ROW) {
        LogInfo(HgLog::VAULTDB, "%s: Unable to execute cursor step: %s\n", __func__, sqlite3_errstr(res));
        return Status::FAIL;
    }
...
SQLiteCursor::~SQLiteCursor()
{
    ...
    if (res != SQLITE_OK) {
        LogInfo(HgLog::VAULTDB, "%s: cursor closed but could not finalize cursor statement: %s\n",
                  __func__, sqlite3_errstr(res));
    }
}
...
bool SQLiteBatch::TxnBegin()
{
    ...
    if (res != SQLITE_OK) {
        LogInfo(HgLog::VAULTDB, "SQLiteBatch: Failed to begin the transaction\n");
        m_database.m_write_semaphore.post();
    } else {
        m_txn = true;
    }
...
bool SQLiteBatch::TxnCommit()
{
    ...
    if (res != SQLITE_OK) {
        LogInfo(HgLog::VAULTDB, "SQLiteBatch: Failed to commit the transaction\n");
    } else {
        m_txn = false;
        m_database.m_write_semaphore.post();
    }
...
bool SQLiteBatch::TxnAbort()
{
    ...
    if (res != SQLITE_OK) {
        LogInfo(HgLog::VAULTDB, "SQLiteBatch: Failed to abort the transaction\n");
    } else {
        m_txn = false;
        m_database.m_write_semaphore.post();
    }
