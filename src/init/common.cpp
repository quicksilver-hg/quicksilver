...
void AddLoggingArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-loglevel=<level>|<category>:<level>", strprintf("Set the global or per-category severity level for logging categories enabled with the -debug configuration option or the logging RPC. Possible values are %s (default=%s). The following levels are always logged: error, warning, info. If <category>:<level> is supplied, the setting will override the global one and may be specified multiple times to set multiple category-specific levels. <category> can be: %s.", LogInstance().LogLevelsString(), LogInstance().LogLevelToStr(HgLog::DEFAULT_LOG_LEVEL), LogInstance().LogCategoriesString()), ArgsManager::DISALLOW_NEGATION | ArgsManager::DISALLOW_ELISION | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-logratelimit", strprintf("Apply rate limiting to unconditional logging to mitigate disk-filling attacks (default: %u)", HgLog::DEFAULT_LOGRATELIMIT), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
...
bool StartLogging(const ArgsManager& args)
{
    ...
    if (!LogInstance().m_log_timestamps)
        LogInfo(HgLog::INIT, "ready startup_time=%s\n", FormatISO8601DateTime(GetTime()));
    LogInfo(HgLog::INIT, "ready datadir_default=%s\n", fs::PathToString(GetDefaultDataDir()));
    LogInfo(HgLog::INIT, "ready datadir=%s\n", fs::PathToString(gArgs.GetDataDirNet()));

    // Only log conf file usage message if conf file actually exists.
    fs::path config_file_path = args.GetConfigFilePath();
    if (args.IsArgNegated("-conf")) {
        LogInfo(HgLog::INIT, "loaded config=none reason=disabled\n");
    } else if (fs::is_directory(config_file_path)) {
        LogWarning(HgLog::INIT, "loaded ok=0 config=%s reason=is-directory\n", fs::PathToString(config_file_path));
    } else if (fs::exists(config_file_path)) {
        LogInfo(HgLog::INIT, "loaded config=%s\n", fs::PathToString(config_file_path));
    } else if (args.IsArgSet("-conf")) {
        InitWarning(strprintf(_("The specified config file %s does not exist"), fs::PathToString(config_file_path)));
    } else {
        // Not categorizing as "Warning" because it's the default behavior
        LogInfo(HgLog::INIT, "loaded config=%s reason=not-found\n", fs::PathToString(config_file_path));
    }
...
#endif
    LogInfo(HgLog::INIT, "ready version=%s build=%s\n", FormatFullVersion(), build_type);
}
} // namespace init
