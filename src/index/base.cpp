...
void BaseIndex::Sync()
{
    ...
    if (!m_synced) {
        ...
        while (true) {
            if (m_interrupt) {
                LogInfo(HgLog::SCAN, "%s: m_interrupt set; exiting ThreadSync\n", GetName());
            ...
            if (last_log_time + SYNC_LOG_INTERVAL < current_time) {
                LogInfo(HgLog::SCAN, "Syncing %s with block chain from height %d\n",
    ...
    if (pindex) {
        LogInfo(HgLog::SCAN, "%s is enabled at height %d\n", GetName(), pindex->nHeight);
    } else {
        LogInfo(HgLog::SCAN, "%s is enabled\n", GetName());
    }
}
...
bool BaseIndex::Commit()
{
    ...
    if (!ok) {
        LogError(HgLog::SCAN, "%s: Failed to commit latest %s state\n", __func__, GetName());
        return false;
    }
    return true;
}
...
void BaseIndex::BlockConnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* pindex)
{
    ...
    if (!best_block_index) {
        ...
        if (best_block_index->GetAncestor(pindex->nHeight - 1) != pindex->pprev) {
            LogInfo(HgLog::SCAN, "%s: WARNING: Block %s does not connect to an ancestor of "
...
void BaseIndex::ChainStateFlushed(const CBlockLocator& locator)
{
    ...
    if (best_block_index->GetAncestor(locator_tip_index->nHeight) != locator_tip_index) {
        LogInfo(HgLog::SCAN, "%s: WARNING: Locator contains block (hash=%s) not on known best "
...
bool BaseIndex::BlockUntilSyncedToCurrentChain() const
{
    ...
    LogInfo(HgLog::SCAN, "%s: %s is catching up on block notifications\n", __func__, GetName());
