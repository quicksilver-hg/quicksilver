...
bool BlockFilterIndex::CustomInit(const std::optional<interfaces::BlockRef>& block)
{
    ...
    if (block) {
        ...
        if (!op_last_header) {
            LogError(HgLog::SCAN, "Cannot read last block filter header; index may be corrupted\n");
            return false;
        }
...
bool BlockFilterIndex::CustomCommit(CDBBatch& batch)
{
    ...
    if (file.IsNull()) {
        LogError(HgLog::SCAN, "%s: Failed to open filter file %d\n", __func__, pos.nFile);
        return false;
    }
    if (!file.Commit()) {
        LogError(HgLog::SCAN, "%s: Failed to commit filter file %d\n", __func__, pos.nFile);
        return false;
    }
...
bool BlockFilterIndex::ReadFilterFromDisk(const FlatFilePos& pos, const uint256& hash, BlockFilter& filter) const
{
    ...
    try {
        ...
        if (Hash(encoded_filter) != hash) {
            LogError(HgLog::SCAN, "Checksum mismatch in filter decode.\n");
            return false;
        }
        filter = BlockFilter(GetFilterType(), block_hash, std::move(encoded_filter), /*skip_decode_check=*/true);
    }
    catch (const std::exception& e) {
        LogError(HgLog::SCAN, "%s: Failed to deserialize block filter from disk: %s\n", __func__, e.what());
        return false;
    }
...
size_t BlockFilterIndex::WriteFilterToDisk(FlatFilePos& pos, const BlockFilter& filter)
{
    ...
    if (pos.nPos + data_size > MAX_FLTR_FILE_SIZE) {
        ...
        if (last_file.IsNull()) {
            LogInfo(HgLog::SCAN, "%s: Failed to open filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!last_file.Truncate(pos.nPos)) {
            LogInfo(HgLog::SCAN, "%s: Failed to truncate filter file %d\n", __func__, pos.nFile);
            return 0;
        }
        if (!last_file.Commit()) {
            LogInfo(HgLog::SCAN, "%s: Failed to commit filter file %d\n", __func__, pos.nFile);
            return 0;
        }
    ...
    if (out_of_space) {
        LogInfo(HgLog::SCAN, "%s: out of disk space\n", __func__);
        return 0;
    }
...
std::optional<uint256> BlockFilterIndex::ReadFilterHeader(int height, const uint256& expected_block_hash)
{
    ...
    if (read_out.first != expected_block_hash) {
        LogError(HgLog::SCAN, "%s: previous block header belongs to unexpected block %s; expected %s\n",
...
bool BlockFilterIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    if (block.height > 0) {
        ...
        // reconstruct the compact filter for spent outputs.
...
{
    if (start_height < 0) {
        LogError(HgLog::SCAN, "%s: start height (%d) is negative\n", __func__, start_height);
        return false;
    }
    if (start_height > stop_index->nHeight) {
        LogError(HgLog::SCAN, "%s: start height (%d) is greater than stop height (%d)\n",
    ...
    for (const CBlockIndex* block_index = stop_index;
        ...
        if (!db.Read(DBHashKey(block_hash), results[i])) {
            LogError(HgLog::SCAN, "%s: unable to read value in %s at key (%c, %s)\n",
