...
static void HandleError(const leveldb::Status& status)
{
    ...
    LogInfo(HgLog::LEVELDB, "%s\n", errmsg);
    LogInfo(HgLog::LEVELDB, "You can use -debug=leveldb to get more complete diagnostic messages\n");
    throw dbwrapper_error(errmsg);
}

class CQuicksilverLevelDBLogger : public leveldb::Logger {
public:
    // This code is adapted from posix_logger.h, which is why it is using vsprintf.
    // Please do not do this in normal code
    void Logv(const char * format, va_list ap) override {
            if (!LogAcceptCategory(HgLog::LEVELDB, HgLog::Level::Debug)) {
                return;
            }
            char buffer[500];
            for (int iter = 0; iter < 2; iter++) {
                ...
                if (p < limit) {
                    ...
                    // Do not use vsnprintf elsewhere in Quicksilver source code, see above.
...
#endif
    LogDebug(HgLog::LEVELDB, "LevelDB using max_open_files=%d (default=%d)\n",
...
static leveldb::Options GetOptions(size_t nCacheSize)
{
    ...
    options.info_log = new CQuicksilverLevelDBLogger();
    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
    ...
    if (params.memory_only) {
        ...
        if (params.wipe_data) {
            LogInfo(HgLog::LEVELDB, "Wiping LevelDB in %s\n", fs::PathToString(params.path));
            leveldb::Status result = leveldb::DestroyDB(fs::PathToString(params.path), DBContext().options);
            HandleError(result);
        }
        TryCreateDirectories(params.path);
        LogInfo(HgLog::LEVELDB, "Opening LevelDB in %s\n", fs::PathToString(params.path));
    }
    ...
    HandleError(status);
    LogInfo(HgLog::LEVELDB, "Opened LevelDB successfully\n");

    if (params.options.force_compact) {
        LogInfo(HgLog::LEVELDB, "Starting database compaction of %s\n", fs::PathToString(params.path));
        DBContext().pdb->CompactRange(nullptr, nullptr);
        LogInfo(HgLog::LEVELDB, "Finished database compaction of %s\n", fs::PathToString(params.path));
    }
    ...
    if (!key_exists && params.obfuscate && IsEmpty()) {
        ...
        LogInfo(HgLog::LEVELDB, "Wrote new obfuscate key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
    }

    LogInfo(HgLog::LEVELDB, "Using obfuscation key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscate_key));
}
...
{
    const bool log_memory = LogAcceptCategory(HgLog::LEVELDB, HgLog::Level::Debug);
    double mem_before = 0;
    if (log_memory) {
    ...
    if (log_memory) {
        ...
        LogDebug(HgLog::LEVELDB, "WriteBatch memory usage: db=%s, before=%.1fMiB, after=%.1fMiB\n",
...
size_t CDBWrapper::DynamicMemoryUsage() const
{
    ...
    if (!DBContext().pdb->GetProperty("leveldb.approximate-memory-usage", &memory) || !(parsed = ToIntegral<size_t>(memory))) {
        LogDebug(HgLog::LEVELDB, "Failed to get approximate-memory-usage property\n");
        return 0;
    }
    return parsed.value();
}
...
std::optional<std::string> CDBWrapper::ReadImpl(Span<const std::byte> key) const
{
    ...
    if (!status.ok()) {
        if (status.IsNotFound())
            return std::nullopt;
        LogInfo(HgLog::LEVELDB, "LevelDB read failure: %s\n", status.ToString());
        HandleError(status);
    }
    return strValue;
}
