...
#ifndef QUICKSILVER_CHECKQUEUE_H
#define QUICKSILVER_CHECKQUEUE_H
...
public:
    ...
    {
        LogInfo(HgLog::LEDGER, "ready script_threads=%d\n", worker_threads_num);
        m_worker_threads.reserve(worker_threads_num);
        for (int n = 0; n < worker_threads_num; ++n) {
            // Spawn through util::TraceThread, as every other long-lived thread
            // in the tree does. An exception escaping a thread entry point calls
            // std::terminate() with no diagnostic -- on Windows a __fastfail
            // exiting 0xC0000409, which is also what a /GS stack-cookie failure
            // reports. TraceThread logs the exception before rethrowing, and
            // renames the thread, which is why the worker no longer calls
            // ThreadRename itself.
            //
            ...
            m_worker_threads.emplace_back([this, n] {
                util::TraceThread(strprintf("scriptch.%i", n), [this] {
...
#endif // QUICKSILVER_CHECKQUEUE_H
