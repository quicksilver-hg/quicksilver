...
static bool InitHTTPAllowList()
{
    ...
    LogDebug(HgLog::HTTP, "Allowing HTTP connections from: %s\n", strAllowed);
    return true;
}
...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    if (!ClientAllowed(hreq->GetPeer())) {
        LogDebug(HgLog::HTTP, "HTTP request from %s rejected: Client network is not allowed RPC access\n",
    ...
    if (hreq->GetRequestMethod() == HTTPRequest::UNKNOWN) {
        LogDebug(HgLog::HTTP, "HTTP request from %s rejected: Unknown HTTP request method\n",
                 hreq->GetPeer().ToStringAddrPort());
        hreq->WriteReply(HTTP_BAD_METHOD);
        return;
    }

    LogDebug(HgLog::HTTP, "Received a %s request for %s from %s\n",
    ...
    if (i != iend) {
        ...
        if (g_work_queue->Enqueue(item.get())) {
            ...
            LogInfo(HgLog::HTTP, "WARNING: request rejected because http work queue depth exceeded, it can be increased with the -rpcworkqueue= setting\n");
...
{
    LogDebug(HgLog::HTTP, "Rejecting request while shutting down\n");
...
{
    LogDebug(HgLog::HTTP, "Entering http event loop\n");
    event_base_dispatch(base);
    // Event loop will be interrupted by InterruptHTTPServer()
    LogDebug(HgLog::HTTP, "Exited http event loop\n");
}
...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    if (gArgs.GetArgs("-rpcallowip").empty() || gArgs.GetArgs("-rpcbind").empty()) { // Default to loopback if not allowing external IPs
        ...
        if (!gArgs.GetArgs("-rpcallowip").empty()) {
            LogInfo(HgLog::HTTP, "WARNING: option -rpcallowip was specified without -rpcbind; this doesn't usually make sense\n");
        }
        if (!gArgs.GetArgs("-rpcbind").empty()) {
            LogInfo(HgLog::HTTP, "WARNING: option -rpcbind was ignored because -rpcallowip was not specified, refusing to allow everyone to connect\n");
        }
    ...
    for (std::vector<std::pair<std::string, uint16_t> >::iterator i = endpoints.begin(); i != endpoints.end(); ++i) {
        LogInfo(HgLog::HTTP, "Binding RPC on address %s port %i\n", i->first, i->second);
        ...
        if (bind_handle) {
            ...
            if (i->first.empty() || (addr.has_value() && addr->IsBindAny())) {
                LogInfo(HgLog::HTTP, "WARNING: the RPC server is not safe to expose to untrusted networks such as the public internet\n");
            }
            ...
            if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (sockopt_arg_type)&one, sizeof(one)) == SOCKET_ERROR) {
                LogInfo(HgLog::HTTP, "WARNING: Unable to set TCP_NODELAY on RPC server socket, continuing anyway\n");
            }
            boundSockets.push_back(bind_handle);
        } else {
            LogInfo(HgLog::HTTP, "Binding RPC on address %s port %i failed.\n", i->first, i->second);
        }
    }
...
static void libevent_log_cb(int severity, const char *msg)
{
    ...
    case EVENT_LOG_DEBUG:
        level = HgLog::Level::Debug;
        break;
    case EVENT_LOG_MSG:
        level = HgLog::Level::Info;
        break;
    case EVENT_LOG_WARN:
        level = HgLog::Level::Warning;
        break;
    default: // EVENT_LOG_ERR and others are mapped to error
        level = HgLog::Level::Error;
        break;
    }
    LogPrintLevel(HgLog::LIBEVENT, level, "%s\n", msg);
}
...
bool InitHTTPServer(const util::SignalInterrupt& interrupt)
{
    ...
    UpdateHTTPServerLogging(LogInstance().WillLogCategory(HgLog::LIBEVENT));
    ...
    if (!http) {
        LogInfo(HgLog::HTTP, "couldn't create evhttp. Exiting.\n");
        return false;
    }
    ...
    if (!HTTPBindAddresses(http)) {
        LogInfo(HgLog::HTTP, "Unable to bind any endpoint for RPC server\n");
        return false;
    }

    LogDebug(HgLog::HTTP, "Initialized HTTP server\n");
    int workQueueDepth = std::max((long)gArgs.GetIntArg("-rpcworkqueue", DEFAULT_HTTP_WORKQUEUE), 1L);
    LogDebug(HgLog::HTTP, "creating work queue of depth %d\n", workQueueDepth);
...
void StartHTTPServer()
{
    ...
    LogInfo(HgLog::HTTP, "Starting HTTP server with %d worker threads\n", rpcThreads);
    // Spawn through util::TraceThread, as every other long-lived thread in the
    // tree does. An exception escaping a thread entry point calls
    // std::terminate() with no diagnostic at all -- on Windows a __fastfail
    // exiting 0xC0000409, which is also what a /GS stack-cookie failure
    // reports, so the two cannot be told apart afterwards. TraceThread logs the
    // exception before rethrowing, and renames the thread, which is why neither
    // entry point does so itself any more.
    g_thread_http = std::thread(&util::TraceThread, "http",
                                [base = eventBase] { ThreadHTTP(base); });

    for (int i = 0; i < rpcThreads; i++) {
        // Capture the name in this lambda rather than passing a strprintf
        // temporary as a string_view argument to std::thread: TraceThread
        // takes string_view, and the temporary would dangle once
        // emplace_back returns.
        g_thread_http_workers.emplace_back([i, queue = g_work_queue.get()] {
            util::TraceThread(strprintf("httpworker.%i", i), [queue] { queue->Run(); });
        });
    }
}
...
{
    LogDebug(HgLog::HTTP, "Interrupting HTTP server\n");
    if (eventHTTP) {
...
void StopHTTPServer()
{
    LogDebug(HgLog::HTTP, "Stopping HTTP server\n");
    if (g_work_queue) {
        LogDebug(HgLog::HTTP, "Waiting for HTTP worker threads to exit\n");
    ...
    boundSockets.clear();
    {
        if (const auto n_connections{g_requests.CountActiveConnections()}; n_connections != 0) {
            LogDebug(HgLog::HTTP, "Waiting for %d connections to stop HTTP server\n", n_connections);
        }
    ...
    if (eventBase) {
        LogDebug(HgLog::HTTP, "Waiting for HTTP event thread to exit\n");
        if (g_thread_http.joinable()) g_thread_http.join();
        event_base_free(eventBase);
        eventBase = nullptr;
    }
    g_work_queue.reset();
    LogDebug(HgLog::HTTP, "Stopped HTTP server\n");
}
...
HTTPRequest::~HTTPRequest()
{
    if (!replySent) {
        ...
        LogInfo(HgLog::HTTP, "%s: Unhandled request\n", __func__);
...
{
    LogDebug(HgLog::HTTP, "Registering HTTP handler for %s (exactmatch %d)\n", prefix, exactMatch);
    LOCK(g_httppathhandlers_mutex);
...
void UnregisterHTTPHandler(const std::string &prefix, bool exactMatch)
{
    ...
    if (i != iend)
    {
        LogDebug(HgLog::HTTP, "Unregistering HTTP handler for %s (exactmatch %d)\n", prefix, exactMatch);
