...
void AddrManImpl::Serialize(Stream& s_) const
{
    ...
    /**
     * Serialized format.
     * * format version byte (@see `Format`); this tree writes and reads FILE_FORMAT only
     * * lowest compatible format version byte; equal to FILE_FORMAT, so a later
     *   incompatible format can reject this file without attempting to parse it
     * * nKey
     * * nNew
     * * nTried
     ...
     * * asmap checksum
     *
     * 2**30 is xorred with the number of buckets as an on-disk consistency marker.
     *
    ...
    // the previous one.
    static constexpr uint8_t lowest_compatible = FILE_FORMAT;
    ...
    // Store asmap checksum after bucket entries.
...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (format < FILE_FORMAT) {
        throw InvalidAddrManVersionError(strprintf(
            "Unsupported pre-Quicksilver addrman database format: %u. The oldest supported format is %u.",
            format, FILE_FORMAT));
    }
    if (lowest_compatible > FILE_FORMAT) {
        throw InvalidAddrManVersionError(strprintf(
            ...
            format, lowest_compatible, CLIENT_NAME, FILE_FORMAT));
    }
    ...
    if (!restore_bucketing) {
        LogDebug(HgLog::PEERBOOK, "Bucketing method was updated, re-bucketing addrman entries from disk\n");
    }
    ...
    if (nLost + nLostUnk > 0) {
        LogDebug(HgLog::PEERBOOK, "addrman lost %i new and %i tried addresses due to collisions or invalid addresses\n", nLostUnk, nLost);
    }
...
void AddrManImpl::ClearNew(int nUBucket, int nUBucketPos)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != -1) {
        ...
        LogDebug(HgLog::PEERBOOK, "Removed %s from new[%i][%i]\n", infoDelete.ToStringAddrPort(), nUBucket, nUBucketPos);
...
void AddrManImpl::MakeTried(AddrInfo& info, nid_type nId)
{
    ...
    if (vvTried[nKBucket][nKBucketPos] != -1) {
        ...
        LogDebug(HgLog::PEERBOOK, "Moved %s from tried[%i][%i] to new[%i][%i] to make space\n",
...
bool AddrManImpl::AddSingle(const CAddress& addr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (vvNew[nUBucket][nUBucketPos] != nId) {
        ...
        if (fInsert) {
            ...
            LogDebug(HgLog::PEERBOOK, "Added %s%s to new[%i][%i]\n",
...
bool AddrManImpl::Good_(const CService& addr, bool test_before_evict, NodeSeconds time)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        LogDebug(HgLog::PEERBOOK, "Collision with %s while attempting to move %s to tried table. Collisions=%d\n",
        ...
        MakeTried(info, nId);
        const auto mapped_as{m_netgroupman.GetMappedAS(addr)};
        LogDebug(HgLog::PEERBOOK, "Moved %s%s to tried[%i][%i]\n",
...
bool AddrManImpl::Add_(const std::vector<CAddress>& vAddr, const CNetAddr& source, std::chrono::seconds time_penalty)
{
    ...
    if (added > 0) {
        LogDebug(HgLog::PEERBOOK, "Added %i addresses (of %i) from %s: %i tried, %i new\n", added, vAddr.size(), source.ToStringAddr(), nTried, nNew);
    }
    return added > 0;
}
...
std::pair<CAddress, NodeSeconds> AddrManImpl::Select_(bool new_only, const std::unordered_set<Network>& networks) const
{
    ...
    while (1) {
        ...
        if (insecure_rand.randbits<30>() < chance_factor * info.GetChance() * (1 << 30)) {
            LogDebug(HgLog::PEERBOOK, "Selected %s from %s\n", info.ToStringAddrPort(), search_tried ? "tried" : "new");
...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Network> network, const bool filtered) const
{
    ...
    LogDebug(HgLog::PEERBOOK, "GetAddr returned %d random addresses\n", addresses.size());
    return addresses;
}
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                if (current_time - info_old.m_last_success < ADDRMAN_REPLACEMENT) {
                    ...
                    if (current_time - info_old.m_last_try > 60s) {
                        LogDebug(HgLog::PEERBOOK, "Replacing %s with %s in tried table\n", info_old.ToStringAddrPort(), info_new.ToStringAddrPort());
                    ...
                    LogDebug(HgLog::PEERBOOK, "Unable to test; replacing %s with %s in tried table anyway\n", info_old.ToStringAddrPort(), info_new.ToStringAddrPort());
                    Good_(info_new, false, current_time);
                    erase_collision = true;
                }
...
void AddrManImpl::Check() const
{
    ...
    if (err) {
        LogInfo(HgLog::PEERBOOK, "ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        assert(false);
    }
}
...
int AddrManImpl::CheckAddrman() const
{
    ...
    LOG_TIME_MILLIS_WITH_CATEGORY_MSG_ONCE(
        strprintf("new %i, tried %i, total %u", nNew, nTried, vRandom.size()), HgLog::PEERBOOK, HgLog::Level::Debug);
