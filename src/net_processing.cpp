 ...
 *  Limits the impact of transaction-inventory floods. */
...
struct Peer {
    ...
    struct TxRelay {
         ...
         *  relaypool to sort transactions in dependency order before relay, so
         *  this does not have to be sorted. */
        std::set<uint256> m_tx_inventory_to_send GUARDED_BY(m_tx_inventory_mutex);
        /** The next time after which we will send an `inv` message containing
         *  transaction announcements to this peer. */
        std::chrono::microseconds m_next_inv_send_time GUARDED_BY(m_tx_inventory_mutex){0};
        /** The relaypool sequence num at which we sent the last `inv` message to this peer.
         *  Can relay txs with lower sequence numbers than this (see CTxRelayPool::info_for_relay). */
...
public:
    PeerManagerImpl(CConnman& connman, AddrMan& addrman,
                    ...
                    CTxRelayPool& pool, node::Warnings& warnings, Options opts);
...
private:
    ...
    /** Retrieve unbroadcast transactions from the relaypool and reattempt sending to peers */
    ...
    /** Handle a transaction whose result was not RelayPoolAcceptResult::ResultType::VALID.
    ...
    /** Handle a transaction whose result was RelayPoolAcceptResult::ResultType::VALID.
     ...
     */
    void ProcessPackageResult(const node::PackageToValidate& package_to_validate, const PackageRelayPoolAcceptResult& package_result)
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, g_msgproc_mutex, m_tx_download_mutex);

    /**
     * Reconsider orphan transactions after a parent has been accepted to the relaypool.
     *
    ...
    */
     ...
     */
    Mutex m_tx_download_mutex ACQUIRED_BEFORE(m_relaypool.cs);
...
bool PeerManagerImpl::BlockRequested(NodeId nodeid, const CBlockIndex& block, std::list<QueuedBlock>::iterator** pit)
{
    ...
    std::list<QueuedBlock>::iterator it = state->vBlocksInFlight.insert(state->vBlocksInFlight.end(),
            {&block, std::unique_ptr<PartiallyDownloadedBlock>(pit ? new PartiallyDownloadedBlock(&m_relaypool) : nullptr)});
...
void PeerManagerImpl::MaybeSetPeerAsAnnouncingHeaderAndIDs(NodeId nodeid)
{
    ...
    // relaypool will not contain the transactions necessary to reconstruct the
    // compact block.
...
void PeerManagerImpl::PushNodeVersion(CNode& pnode, const Peer& peer)
{
    ...
    if (fLogIPs) {
        LogDebug(HgLog::MESH, "send version message: version %d, blocks=%d, them=%s, txrelay=%d, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addr_you.ToStringAddrPort(), tx_relay, nodeid);
    } else {
        LogDebug(HgLog::MESH, "send version message: version %d, blocks=%d, txrelay=%d, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, tx_relay, nodeid);
    }
}
...
{
    std::set<uint256> unbroadcast_txids = m_relaypool.GetUnbroadcastTxs();

    for (const auto& txid : unbroadcast_txids) {
        CTransactionRef tx = m_relaypool.get(txid);

        if (tx != nullptr) {
            RelayTransaction(txid, tx->GetWitnessHash());
        } else {
            m_relaypool.RemoveUnbroadcastTx(txid, true);
        }
    }
...
void PeerManagerImpl::FinalizeNode(const CNode& node)
{
    ...
    LogDebug(HgLog::MESH, "Cleared nodestate for peer=%d\n", nodeid);
}
...
void PeerManagerImpl::Misbehaving(Peer& peer, const std::string& message)
{
    ...
    LogDebug(HgLog::MESH, "Misbehaving: peer=%d%s\n", peer.m_id, message_prefixed);
...
void PeerManagerImpl::MaybePunishNodeForTx(NodeId nodeid, const TxValidationState& state)
{
    ...
    case TxValidationResult::TX_RELAYPOOL_POLICY:
    case TxValidationResult::TX_NO_RELAYPOOL:
...
std::optional<std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    LogDebug(HgLog::MESH, "Requesting block %s from peer=%d\n",
...
{
    // Ensure relaypool mutex was released, otherwise deadlock may occur if another thread holding
    // m_tx_download_mutex waits on the relaypool mutex.
    AssertLockNotHeld(m_relaypool.cs);
    ...
    const CBlockIndex* pindex)
{
    ...
    if (stalling_timeout != BLOCK_STALLING_TIMEOUT_DEFAULT) {
        ...
        if (m_block_stalling_timeout.compare_exchange_strong(stalling_timeout, new_timeout)) {
            LogDebug(HgLog::MESH, "Decreased stalling timeout to %d seconds\n", count_seconds(new_timeout));
        }
    }
...
void PeerManagerImpl::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    m_connman.ForEachNode([this, pindex, &lazy_ser, &hashBlock](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
        ...
        if (state.m_requested_hb_cmpctblocks && !PeerHasHeader(&state, pindex) && PeerHasHeader(&state, pindex->pprev)) {
            ...
            LogDebug(HgLog::MESH, "%s sending header-and-ids %s to peer=%d\n", "PeerManager::NewPoWValidBlock",
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    if (need_activate_chain) {
        ...
        if (!m_chainman.ActiveChainstate().ActivateBestChain(state, a_recent_block)) {
            LogDebug(HgLog::MESH, "failed to activate chain (%s)\n", state.ToString());
        }
    }
    ...
    FlatFilePos block_pos{};
    {
        ...
        if (!BlockRequestAllowed(pindex)) {
            LogDebug(HgLog::MESH, "%s: ignoring request from peer=%i for old block that isn't in the main chain\n", __func__, pfrom.GetId());
            return;
        }
        ...
        if (m_connman.OutboundTargetReached(true) &&
            ...
            LogDebug(HgLog::MESH, "historical block serving limit reached, %s\n", pfrom.DisconnectMsg(fLogIPs));
        ...
        if (!pfrom.HasPermission(NetPermissionFlags::NoBan) && (
            ...
            LogDebug(HgLog::MESH, "Ignore block request below NODE_NETWORK_LIMITED threshold, %s\n", pfrom.DisconnectMsg(fLogIPs));
    ...
    if (a_recent_block && a_recent_block->GetHash() == pindex->GetBlockHash()) {
        ...
        if (!m_chainman.m_blockman.ReadRawBlock(block_data, block_pos)) {
            if (WITH_LOCK(m_chainman.GetMutex(), return m_chainman.m_blockman.IsBlockPruned(*pindex))) {
                LogDebug(HgLog::MESH, "Block was pruned before it could be read, %s\n", pfrom.DisconnectMsg(fLogIPs));
            } else {
                LogError(HgLog::MESH, "unlinked reason=block-read-failed peer=%d%s\n", pfrom.GetId(), pfrom.LogIP(fLogIPs));
            }
            pfrom.fDisconnect = true;
            return;
        }
    ...
    if (pblock) {
        if (inv.IsMsgBlk()) {
            ...
            // they won't have a useful relaypool to match against a compact block,
...
{
    // If a tx was in the relaypool prior to the last INV for this peer, permit the request.
    auto txinfo = m_relaypool.info_for_relay(gtxid, tx_relay.m_last_inv_sequence);
    if (txinfo.tx) {
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsGenTxMsg()) {
        ...
        if (tx) {
            ...
            m_relaypool.RemoveUnbroadcastTx(tx->GetHash());
        } else {
        ...
        const std::vector<CBlockHeader>& headers)
{
    ...
    if (MaybeSendGetHeaders(pfrom, GetLocator(best_header), peer)) {
        LogDebug(HgLog::MESH, "received header %s: missing prev block %s, sending getheaders (%d) to end (peer=%d)\n",
    ...
    if (peer.m_headers_sync) {
        ...
        if (result.request_more) {
            ...
            if (!locator.vHave.empty()) {
                ...
                LogDebug(HgLog::MESH, "more getheaders (from %s) to peer=%d\n",
...
bool PeerManagerImpl::TryLowWorkHeadersSync(Peer& peer, CNode& pfrom, const CBlockIndex* chain_start_header, std::vector<CBlockHeader>& headers)
{
    ...
    if (total_work < minimum_chain_work) {
        ...
        if (headers.size() == m_opts.max_headers_result) {
            ...
            LogDebug(HgLog::MESH, "Ignoring low-work chain (height=%u) from peer=%d\n", chain_start_header->nHeight + headers.size(), pfrom.GetId());
        }
...
void PeerManagerImpl::HeadersDirectFetchBlocks(CNode& pfrom, const Peer& peer, const CBlockIndex& last_header)
{
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork) {
        ...
        if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
            LogDebug(HgLog::MESH, "Large reorg, won't direct fetch to %s (%d)\n",
            ...
            if (vGetData.size() > 1) {
                LogDebug(HgLog::MESH, "Downloading blocks toward %s (%d) via headers direct fetch\n",
                         last_header.GetBlockHash().ToString(),
                         last_header.nHeight);
            }
            if (vGetData.size() > 0) {
        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    ...
    if (m_chainman.IsInitialBlockDownload() && !may_have_more_headers) {
        ...
        if (nodestate->pindexBestKnownBlock && nodestate->pindexBestKnownBlock->nChainWork < m_chainman.MinimumChainWork()) {
            ...
            if (pfrom.IsOutboundOrBlockRelayConn()) {
                LogInfo(HgLog::MESH, "unlinked reason=insufficient-headers-work peer=%d%s\n", pfrom.GetId(), pfrom.LogIP(fLogIPs));
                pfrom.fDisconnect = true;
            }
        }
    }
    ...
    if (!pfrom.fDisconnect && pfrom.IsFullOutboundConn() && nodestate->pindexBestKnownBlock != nullptr) {
        if (m_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
            LogDebug(HgLog::MESH, "Protecting outbound peer=%d from eviction\n", pfrom.GetId());
                                            ...
                                            bool via_compact_block)
{
    ...
    if (nCount == m_opts.max_headers_result && !have_headers_sync) {
        ...
        if (MaybeSendGetHeaders(pfrom, GetLocator(pindexLast), peer)) {
            LogDebug(HgLog::MESH, "more getheaders (%d) to end to peer=%d (startheight:%d)\n",
                                       ...
                                       bool first_time_failure)
{
    ...
    LogDebug(HgLog::RELAYPOOLREJ, "%s (wtxid=%s) from peer=%d was not accepted: %s\n",
        ptx->GetHash().ToString(),
        ptx->GetWitnessHash().ToString(),
        nodeid,
        state.ToString());

    const auto& [add_extra_compact_tx, unique_parents, package_to_validate] = m_txdownloadman.RelayPoolRejectedTx(ptx, state, nodeid, first_time_failure);
...
void PeerManagerImpl::ProcessValidTx(NodeId nodeid, const CTransactionRef& tx, const std::list<CTransactionRef>& replaced_transactions)
{
    ...
    m_txdownloadman.RelayPoolAcceptedTx(tx);

    LogDebug(HgLog::RELAYPOOL, "AcceptToMemoryPool: peer=%d: accepted %s (wtxid=%s) (poolsz %u txn, %u kB)\n",
             nodeid,
             tx->GetHash().ToString(),
             tx->GetWitnessHash().ToString(),
             m_relaypool.size(), m_relaypool.DynamicMemoryUsage() / 1000);
...
void PeerManagerImpl::ProcessPackageResult(const node::PackageToValidate& package_to_validate, const PackageRelayPoolAcceptResult& package_result)
{
    AssertLockNotHeld(m_peer_mutex);
    AssertLockHeld(g_msgproc_mutex);
    AssertLockHeld(m_tx_download_mutex);
    ...
    if (package_result.m_state.IsInvalid()) {
        m_txdownloadman.RelayPoolRejectedPackage(package);
    }
    ...
    while (package_iter != package.rend()) {
        ...
        if (it_result != package_result.m_tx_results.end()) {
            ...
            switch (tx_result.m_result_type) {
                ...
                {
                    // AlreadyHaveTx() should be catching transactions that are already in relaypool.
                    Assume(false);
                    break;
                }
            }
...
bool PeerManagerImpl::ProcessOrphanTx(Peer& peer)
{
    ...
    while (CTransactionRef porphanTx = m_txdownloadman.GetTxToReconsider(peer.m_id)) {
        const RelayPoolAcceptResult result = m_chainman.ProcessTransaction(porphanTx);
        const TxValidationState& state = result.m_state;
        const Txid& orphanHash = porphanTx->GetHash();
        const Wtxid& orphan_wtxid = porphanTx->GetWitnessHash();

        if (result.m_result_type == RelayPoolAcceptResult::ResultType::VALID) {
            LogDebug(HgLog::TXPACKAGES, "   accepted orphan tx %s (wtxid=%s)\n", orphanHash.ToString(), orphan_wtxid.ToString());
            ProcessValidTx(peer.m_id, porphanTx, result.m_replaced_transactions);
            return true;
        } else if (state.GetResult() != TxValidationResult::TX_MISSING_INPUTS) {
            LogDebug(HgLog::TXPACKAGES, "   invalid orphan tx %s (wtxid=%s) from peer=%d. %s\n",
                orphanHash.ToString(),
            ...
            if (Assume(state.IsInvalid() &&
                       ...
                       state.GetResult() != TxValidationResult::TX_NO_RELAYPOOL &&
                                                ...
                                                BlockFilterIndex*& filter_index)
{
    ...
    if (!supported_filter_type) {
        LogDebug(HgLog::MESH, "peer requested unsupported block filter type: %d, %s\n",
...

    {
        ...
        if (!stop_index || !BlockRequestAllowed(stop_index)) {
            LogDebug(HgLog::MESH, "peer requested invalid block hash: %s, %s\n",
    ...
    if (start_height > stop_height) {
        LogDebug(HgLog::MESH, "peer sent invalid getcfilters/getcfheaders with "
    ...
    if (stop_height - start_height >= max_height_diff) {
        LogDebug(HgLog::MESH, "peer requested too many cfilters/cfheaders: %d / %d, %s\n",
    ...
    if (!filter_index) {
        LogDebug(HgLog::MESH, "Filter index for supported type %s not found\n", BlockFilterTypeName(filter_type));
        return false;
    }
...
void PeerManagerImpl::ProcessGetCFilters(CNode& node, Peer& peer, DataStream& vRecv)
{
    ...
    if (!filter_index->LookupFilterRange(start_height, stop_index, filters)) {
        LogDebug(HgLog::MESH, "Failed to find block filter in index: filter_type=%s, start_height=%d, stop_hash=%s\n",
...
void PeerManagerImpl::ProcessGetCFHeaders(CNode& node, Peer& peer, DataStream& vRecv)
{
    ...
    if (start_height > 0) {
        ...
        if (!filter_index->LookupFilterHeader(prev_block, prev_header)) {
            LogDebug(HgLog::MESH, "Failed to find block filter header in index: filter_type=%s, block_hash=%s\n",
    ...
    if (!filter_index->LookupFilterHashRange(start_height, stop_index, filter_hashes)) {
        LogDebug(HgLog::MESH, "Failed to find block filter hashes in index: filter_type=%s, start_height=%d, stop_hash=%s\n",
                     BlockFilterTypeName(filter_type), start_height, stop_hash.ToString());
        return;
    }
...
void PeerManagerImpl::ProcessCompactBlockTxns(CNode& pfrom, Peer& peer, const BlockTransactions& block_transactions)
{
    ...
    bool fBlockRead{false};
    {
        ...
        if (!requested_block_from_this_peer) {
            LogDebug(HgLog::MESH, "Peer %d sent us block transactions for block we weren't expecting\n", pfrom.GetId());
            return;
        }
        ...
        if (status == READ_STATUS_INVALID) {
            ...
            if (first_in_flight) {
                ...
                LogDebug(HgLog::MESH, "Peer %d sent us a compact block but it failed to reconstruct, waiting on first download to complete\n", pfrom.GetId());
                return;
            }
        } else {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    LogDebug(HgLog::MESH, "received: %s (%u bytes) peer=%d\n", SanitizeString(msg_type), vRecv.size(), pfrom.GetId());
    ...
    if (msg_type == NetMsgType::VERSION) {
        if (pfrom.nVersion != 0) {
            LogDebug(HgLog::MESH, "redundant version message from peer=%d\n", pfrom.GetId());
            return;
        }
        ...
        {
            LogDebug(HgLog::MESH, "peer does not offer the expected services (%08x offered, %08x expected), %s\n",
                     nServices,
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION) {
            ...
            LogDebug(HgLog::MESH, "peer using obsolete version %i, %s\n", nVersion, pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
            return;
        }
        ...
        {
            LogInfo(HgLog::MESH, "unlinked reason=connected-to-self peer_addr=%s\n", pfrom.addr.ToStringAddrPort());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (!pfrom.IsBlockOnlyConn() && !pfrom.IsFeelerConn() && fRelay) {
            peer->SetTxRelay();
        ...
        LogDebug(HgLog::MESH, "receive version message: %s: version %d, blocks=%d, us=%s, txrelay=%d, peer=%d%s%s\n",
        ...
        if (pfrom.IsFeelerConn()) {
            LogDebug(HgLog::MESH, "feeler connection completed, %s\n", pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
        }
        return;
    }
    ...
    if (pfrom.nVersion == 0) {
        ...
        LogDebug(HgLog::MESH, "non-version message before version handshake. Message \"%s\" from peer=%d\n", SanitizeString(msg_type), pfrom.GetId());
        return;
    }

    if (msg_type == NetMsgType::VERACK) {
        if (pfrom.fSuccessfullyConnected) {
            LogDebug(HgLog::MESH, "ignoring redundant verack message from peer=%d\n", pfrom.GetId());
            return;
        }

        // Log successful connections unconditionally for outbound, but not for inbound as those
        // can be triggered by an attacker at high rate.
        if (!pfrom.IsInboundConn() || LogAcceptCategory(HgLog::MESH, HgLog::Level::Debug)) {
            const auto mapped_as{m_connman.GetMappedAS(pfrom.addr)};
            LogInfo(HgLog::MESH, "linked connection=%s transport=%s version=%d ledger_height=%d peer=%d%s%s\n",
        ...
        // Tell our peer we are willing to provide version 2 compact blocks.
        // We do not request new block announcements using compact blocks.
    ...
    if (msg_type == NetMsgType::WTXIDRELAY) {
        if (pfrom.fSuccessfullyConnected) {
            ...
            LogDebug(HgLog::MESH, "wtxidrelay received after verack, %s\n", pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
            return;
        }
        if (!peer->m_wtxid_relay) {
            ...
            LogDebug(HgLog::MESH, "ignoring duplicate wtxidrelay from peer=%d\n", pfrom.GetId());
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::SENDADDRV2) {
        if (pfrom.fSuccessfullyConnected) {
            ...
            LogDebug(HgLog::MESH, "sendaddrv2 received after verack, %s\n", pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
            return;
        }
    ...
    if (!pfrom.fSuccessfullyConnected) {
        LogDebug(HgLog::MESH, "Unsupported message \"%s\" prior to verack from peer=%d\n", SanitizeString(msg_type), pfrom.GetId());
        return;
    }
    ...
    if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
        ...
        if (!SetupAddressRelay(pfrom, *peer)) {
            LogDebug(HgLog::MESH, "ignoring %s message from %s peer=%d\n", msg_type, pfrom.ConnectionTypeAsString(), pfrom.GetId());
            return;
        }
        ...
        LogDebug(HgLog::MESH, "Received addr: %u addresses (%u processed, %u rate-limited) from peer=%d\n",
        ...
        if (pfrom.IsAddrFetchConn() && vAddr.size() > 1) {
            LogDebug(HgLog::MESH, "addrfetch connection completed, %s\n", pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                LogDebug(HgLog::MESH, "got inv: %s  %s peer=%d\n", inv.ToString(), fAlreadyHave ? "have" : "new", pfrom.GetId());
                ...
                if (reject_tx_invs) {
                    LogDebug(HgLog::MESH, "transaction (%s) inv sent in violation of protocol, %s\n", inv.hash.ToString(), pfrom.DisconnectMsg(fLogIPs));
                    pfrom.fDisconnect = true;
                    return;
                }
                ...
                LogDebug(HgLog::MESH, "Unknown inv type \"%s\" received from peer=%d\n", inv.ToString(), pfrom.GetId());
            }
        }
        ...
        if (best_block != nullptr) {
            ...
            if (state.fSyncStarted || (!peer->m_inv_triggered_getheaders_before_sync && *best_block != m_last_block_inv_triggering_headers_sync)) {
                if (MaybeSendGetHeaders(pfrom, GetLocator(m_chainman.m_best_header), *peer)) {
                    LogDebug(HgLog::MESH, "getheaders (%d) %s to peer=%d\n",
    ...
    if (msg_type == NetMsgType::GETDATA) {
        ...
        LogDebug(HgLog::MESH, "received getdata (%u invsz) peer=%d\n", vInv.size(), pfrom.GetId());

        if (vInv.size() > 0) {
            LogDebug(HgLog::MESH, "received getdata for: %s peer=%d\n", vInv[0].ToString(), pfrom.GetId());
        }
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        int nLimit = 500;
        LogDebug(HgLog::MESH, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, pfrom.GetId());
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                LogDebug(HgLog::MESH, "  getblocks stopping at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            if (m_chainman.m_blockman.IsPruneMode() && (!(pindex->nStatus & BLOCK_HAVE_DATA) || pindex->nHeight <= m_chainman.ActiveChain().Tip()->nHeight - nPrunedBlocksLikelyToHave)) {
                LogDebug(HgLog::MESH, " getblocks stopping, pruned or too old block at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            if (--nLimit <= 0) {
                ...
                LogDebug(HgLog::MESH, "  getblocks stopping at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    if (msg_type == NetMsgType::GETBLOCKTXN) {
        ...
        FlatFilePos block_pos{};
        {
            ...
            if (!pindex || !(pindex->nStatus & BLOCK_HAVE_DATA)) {
                LogDebug(HgLog::MESH, "Peer %d sent us a getblocktxn for a block we don't have\n", pfrom.GetId());
                return;
            }
        ...
        LogDebug(HgLog::MESH, "Peer %d sent us a getblocktxn for a block > %i deep\n", pfrom.GetId(), MAX_BLOCKTXN_DEPTH);
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogDebug(HgLog::MESH, "getheaders locator size %lld > %d, %s\n", locator.vHave.size(), MAX_LOCATOR_SZ, pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
            return;
        }

        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogDebug(HgLog::MESH, "Ignoring getheaders from peer=%d while importing/reindexing\n", pfrom.GetId());
            return;
        }

        LOCK(cs_main);

        // Requiring that our chainwork exceed the minimum chain work is a
        ...
        if (m_chainman.ActiveTip() == nullptr ||
            ...
            LogDebug(HgLog::MESH, "Ignoring getheaders from peer=%d because active chain has too little work; sending empty response\n", pfrom.GetId());
        ...
        if (locator.IsNull())
        {
            ...
            if (!BlockRequestAllowed(pindex)) {
                LogDebug(HgLog::MESH, "%s: ignoring request from peer=%i for old block header that isn't in the main chain\n", __func__, pfrom.GetId());
                return;
            }
        }
        else
        ...
        LogDebug(HgLog::MESH, "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), pfrom.GetId());
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
    ...
    if (msg_type == NetMsgType::TX) {
        if (RejectIncomingTxs(pfrom)) {
            LogDebug(HgLog::MESH, "transaction sent in violation of protocol, %s", pfrom.DisconnectMsg(fLogIPs));
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (!should_validate) {
            if (pfrom.HasPermission(NetPermissionFlags::ForceRelay)) {
                ...
                // permission, even if they were already in the relaypool, allowing
                // the node to function as a gateway for nodes hidden behind it.
                if (!m_relaypool.exists(GenTxid::Txid(tx.GetHash()))) {
                    LogInfo(HgLog::MESH, "rejected reason=not-in-relaypool tx=%s wtxid=%s source=forcerelay peer=%d\n",
                              tx.GetHash().ToString(), tx.GetWitnessHash().ToString(), pfrom.GetId());
                } else {
                    LogInfo(HgLog::MESH, "accepted tx=%s wtxid=%s source=forcerelay peer=%d\n",
                              tx.GetHash().ToString(), tx.GetWitnessHash().ToString(), pfrom.GetId());
                    RelayTransaction(tx.GetHash(), tx.GetWitnessHash());
                }
            }
            ...
            if (package_to_validate) {
                const auto package_result{ProcessNewPackage(m_chainman.ActiveChainstate(), m_relaypool, package_to_validate->m_txns, /*test_accept=*/false)};
                LogDebug(HgLog::TXPACKAGES, "package evaluation for %s: %s\n", package_to_validate->ToString(),
        ...
        const RelayPoolAcceptResult result = m_chainman.ProcessTransaction(ptx);
        const TxValidationState& state = result.m_state;
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogDebug(HgLog::MESH, "Unexpected cmpctblock message received from peer %d\n", pfrom.GetId());
            return;
        }
        ...
        if (!prev_block) {
            ...
            LogDebug(HgLog::MESH, "Ignoring low-work compact block from peer %d\n", pfrom.GetId());
            return;
        }
        ...
        if (received_new_header) {
            LogInfo(HgLog::MESH, "accepted header=%s source=cmpctblock peer=%d\n",
        ...
        if (pindex->nChainWork <= m_chainman.ActiveChain().Tip()->nChainWork || // We know something better
            ...
            if (requested_block_from_this_peer) {
                // We requested this block for some reason, but our relaypool will probably be useless
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && nodestate->vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!BlockRequested(pfrom.GetId(), *pindex, &queuedBlockIt)) {
                    if (!(*queuedBlockIt)->partialBlock)
                        (*queuedBlockIt)->partialBlock.reset(new PartiallyDownloadedBlock(&m_relaypool));
                    else {
                        // The block was already in flight using compact blocks from the same peer
                        LogDebug(HgLog::MESH, "Peer sent us compact block we were already syncing!\n");
                        return;
                    }
                }
                ...
                PartiallyDownloadedBlock tempBlock(&m_relaypool);
            ...
            if (requested_block_from_this_peer) {
                ...
                // relaypool will probably be useless - request the block normally
                std::vector<CInv> vInv(1);
                vInv[0] = CInv(MSG_BLOCK | GetFetchFlags(*peer), blockhash);
                MakeAndPushMessage(pfrom, NetMsgType::GETDATA, vInv);
                return;
    ...
    if (msg_type == NetMsgType::BLOCKTXN)
    {
        ...
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogDebug(HgLog::MESH, "Unexpected blocktxn message received from peer %d\n", pfrom.GetId());
            return;
        }
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogDebug(HgLog::MESH, "Unexpected headers message received from peer %d\n", pfrom.GetId());
            return;
        }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        if (m_chainman.m_blockman.LoadingBlocks()) {
            LogDebug(HgLog::MESH, "Unexpected block message received from peer %d\n", pfrom.GetId());
            return;
        }
        ...
        LogDebug(HgLog::MESH, "received block %s peer=%d\n", pblock->GetHash().ToString(), pfrom.GetId());
        ...
        if (prev_block && IsBlockMutated(/*block=*/*pblock,
            ...
            LogDebug(HgLog::MESH, "Received mutated block from peer=%d\n", peer->m_id);
    ...
    if (msg_type == NetMsgType::GETADDR) {
        ...
        if (!pfrom.IsInboundConn()) {
            LogDebug(HgLog::MESH, "Ignoring \"getaddr\" from %s connection. peer=%d\n", pfrom.ConnectionTypeAsString(), pfrom.GetId());
            return;
        }
        ...
        if (peer->m_getaddr_recvd) {
            LogDebug(HgLog::MESH, "Ignoring repeated \"getaddr\". peer=%d\n", pfrom.GetId());
            return;
        }
    ...
    if (msg_type == NetMsgType::PING) {
        uint64_t nonce = 0;
        vRecv >> nonce;
        // Echo the nonce so peers can check liveness and measure latency
        // without confusing overlapping pings.
    ...
    if (msg_type == NetMsgType::PONG) {
        ...
        if (!(sProblem.empty())) {
            LogDebug(HgLog::MESH, "pong peer=%d: %s, %x expected, %x received, %u bytes\n",
                pfrom.GetId(),
                sProblem,
    ...
    LogDebug(HgLog::MESH, "Unknown command \"%s\" from peer=%d\n", SanitizeString(msg_type), pfrom.GetId());
    return;
}
...
bool PeerManagerImpl::MaybeDiscourageAndDisconnect(CNode& pnode, Peer& peer)
{
    ...
    if (pnode.HasPermission(NetPermissionFlags::NoBan)) {
        ...
        LogWarning(HgLog::MESH, "exempted reason=noban from=punishment peer=%d\n", peer.m_id);
        return false;
    }

    if (pnode.IsManualConn()) {
        // We never disconnect or discourage manual peers for bad behavior
        LogWarning(HgLog::MESH, "exempted reason=manual-connection from=punishment peer=%d\n", peer.m_id);
        return false;
    }
    ...
    if (pnode.addr.IsLocal()) {
        ...
        LogDebug(HgLog::MESH, "Warning: disconnecting but not discouraging %s peer %d!\n",
    ...
    LogDebug(HgLog::MESH, "Disconnecting and discouraging peer %d!\n", peer.m_id);
...
bool PeerManagerImpl::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    try {
        ...
        LogDebug(HgLog::MESH, "%s(%s, %u bytes): Exception '%s' (%s) caught\n", __func__, SanitizeString(msg.m_type), msg.m_message_size, e.what(), typeid(e).name());
    } catch (...) {
        LogDebug(HgLog::MESH, "%s(%s, %u bytes): Unknown exception caught\n", __func__, SanitizeString(msg.m_type), msg.m_message_size);
    }
    ...
    if (!state.m_chain_sync.m_protect && pto.IsOutboundOrBlockRelayConn() && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogInfo(HgLog::MESH, "unlinked reason=stale-chain best_known_block=%s peer=%d%s\n", state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", pto.GetId(), pto.LogIP(fLogIPs));
                ...
                MaybeSendGetHeaders(pto,
                        ...
                        peer);
                LogDebug(HgLog::MESH, "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n", pto.GetId(), state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", state.m_chain_sync.m_work_header->GetBlockHash().ToString());
    ...
    if (m_connman.GetExtraBlockRelayCount() > 0) {
        ...
        m_connman.ForNode(to_disconnect, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            ...
            if (node_state == nullptr ||
                ...
                pnode->fDisconnect = true;
                LogDebug(HgLog::MESH, "disconnecting extra block-relay-only peer=%d (last block received at time %d)\n",
                         pnode->GetId(), count_seconds(pnode->m_last_block_time));
                return true;
            } else {
                LogDebug(HgLog::MESH, "keeping block-relay-only peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n",
        ...
        if (worst_peer != -1) {
            bool disconnected = m_connman.ForNode(worst_peer, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
                ...
                if (now - pnode->m_connected > MINIMUM_CONNECT_TIME && state.vBlocksInFlight.empty()) {
                    LogDebug(HgLog::MESH, "disconnecting extra outbound peer=%d (last block announcement received at time %d)\n", pnode->GetId(), oldest_block_announcement);
                    pnode->fDisconnect = true;
                    return true;
                } else {
                    LogDebug(HgLog::MESH, "keeping outbound peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n",
...
void PeerManagerImpl::CheckForStaleTipAndEvictPeers()
{
    ...
    if (now > m_stale_tip_check_time) {
        ...
        if (!m_chainman.m_blockman.LoadingBlocks() && m_connman.GetNetworkActive() && m_connman.GetUseAddrmanOutgoing() && TipMayBeStale()) {
            LogInfo(HgLog::MESH, "stale tip=1 last_update_s=%d action=extra-outbound-peer\n",
...
void PeerManagerImpl::MaybeSendPing(CNode& node_to, Peer& peer, std::chrono::microseconds now)
{
        ...
        now > peer.m_ping_start.load() + TIMEOUT_INTERVAL)
    {
        ...
        LogDebug(HgLog::MESH, "ping timeout: %fs, %s", 0.000001 * count_microseconds(now - peer.m_ping_start.load()), node_to.DisconnectMsg(fLogIPs));
...
namespace {
class CompareInvRelayPoolOrder
{
    CTxRelayPool* mp;
    bool m_wtxid_relay;
public:
    explicit CompareInvRelayPoolOrder(CTxRelayPool *_relaypool, bool use_wtxid)
    {
        mp = _relaypool;
    ...
    bool operator()(std::set<uint256>::iterator a, std::set<uint256>::iterator b)
    {
         ...
         * fewest ancestors/highest surplus-work rank to sort later. */
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
    ...
    if (pto->IsAddrFetchConn() && current_time - pto->m_connected > 10 * AVG_ADDRESS_BROADCAST_INTERVAL) {
        LogDebug(HgLog::MESH, "addrfetch connection timeout, %s\n", pto->DisconnectMsg(fLogIPs));
        pto->fDisconnect = true;
        return true;
    }
...

    {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !m_chainman.m_blockman.LoadingBlocks()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header->Time() > NodeClock::now() - 24h) {
                ...
                if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
                    LogDebug(HgLog::MESH, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, pto->GetId(), peer->m_starting_height);
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
                    ...
                    if (vHeaders.size() > 1) {
                        LogDebug(HgLog::MESH, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                vHeaders.size(),
                                vHeaders.front().GetHash().ToString(),
                                vHeaders.back().GetHash().ToString(), pto->GetId());
                    } else {
                        LogDebug(HgLog::MESH, "%s: sending header %s to peer=%d\n", __func__,
                                vHeaders.front().GetHash().ToString(), pto->GetId());
                    }
            ...
            if (fRevertToInv) {
                ...
                if (!peer->m_blocks_for_headers_relay.empty()) {
                    ...
                    // Just log for now.
                    if (m_chainman.ActiveChain()[pindex->nHeight] != pindex) {
                        LogDebug(HgLog::MESH, "Announcing block %s not on main chain (tip=%s)\n",
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogDebug(HgLog::MESH, "%s: sending inv peer=%d hash=%s\n", __func__,
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                if (fSendTrickle) {
                    ...
                    // Topologically sort the inventory we send for privacy and dependency order.
                    // A heap is used so that not all items need sorting if only a few are being sent.
                    CompareInvRelayPoolOrder compareInvRelayPoolOrder(&m_relaypool, peer->m_wtxid_relay);
                    std::make_heap(vInvTx.begin(), vInvTx.end(), compareInvRelayPoolOrder);
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < broadcast_max) {
                        ...
                        std::pop_heap(vInvTx.begin(), vInvTx.end(), compareInvRelayPoolOrder);
                        ...
                        // Not in the relaypool anymore? don't bother sending it.
                        auto txinfo = m_relaypool.info(ToGenTxid(inv));
                        if (!txinfo.tx) {
                            continue;
                        }
                        // Send
                    ...
                    LOCK(m_relaypool.cs);
                    tx_relay->m_last_inv_sequence = m_relaypool.GetSequence();
                }
        }
        if (!vInv.empty())
            MakeAndPushMessage(*pto, NetMsgType::INV, vInv);
        ...
        if (state.m_stalling_since.count() && state.m_stalling_since < current_time - stalling_timeout) {
            ...
            LogInfo(HgLog::MESH, "unlinked reason=block-download-stalling peer=%d%s\n", pto->GetId(), pto->LogIP(fLogIPs));
            pto->fDisconnect = true;
            // Increase timeout for the next peer so that we don't disconnect multiple peers if our own
            // bandwidth is insufficient.
            const auto new_timeout = std::min(2 * stalling_timeout, BLOCK_STALLING_TIMEOUT_MAX);
            if (stalling_timeout != new_timeout && m_block_stalling_timeout.compare_exchange_strong(stalling_timeout, new_timeout)) {
                LogDebug(HgLog::MESH, "Increased stalling timeout temporarily to %d seconds\n", count_seconds(new_timeout));
            }
            return true;
        }
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (current_time > state.m_downloading_since + std::chrono::seconds{consensusParams.nPowTargetSpacing} * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                LogInfo(HgLog::MESH, "unlinked reason=block-download-timeout block=%s peer=%d%s\n", queuedBlock.pindex->GetBlockHash().ToString(), pto->GetId(), pto->LogIP(fLogIPs));
                pto->fDisconnect = true;
                return true;
            }
        }
            ...
            if (m_chainman.m_best_header->Time() <= NodeClock::now() - 24h) {
                if (current_time > peer->m_headers_sync_timeout && nSyncStarted == 1 && (m_num_preferred_download_peers - state.fPreferredDownload >= 1)) {
                    ...
                    if (!pto->HasPermission(NetPermissionFlags::NoBan)) {
                        LogInfo(HgLog::MESH, "unlinked reason=headers-download-timeout peer=%d%s\n", pto->GetId(), pto->LogIP(fLogIPs));
                        pto->fDisconnect = true;
                        return true;
                    } else {
                        LogInfo(HgLog::MESH, "exempted reason=noban from=headers-download-timeout peer=%d%s\n", pto->GetId(), pto->LogIP(fLogIPs));
        ...
        if (CanServeBlocks(*peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(*peer)) || !m_chainman.IsInitialBlockDownload()) && state.vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (const CBlockIndex *pindex : vToDownload) {
                ...
                LogDebug(HgLog::MESH, "Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                    pindex->nHeight, pto->GetId());
            }
            if (state.vBlocksInFlight.empty() && staller != -1) {
                if (State(staller)->m_stalling_since == 0us) {
                    State(staller)->m_stalling_since = current_time;
                    LogDebug(HgLog::MESH, "Stall started peer=%d\n", staller);
                }
            }
        }
