...
std::vector<bool> DecodeAsmap(fs::path path)
{
    ...
    if (file.IsNull()) {
        LogInfo(HgLog::INIT, "Failed to open asmap file from disk\n");
        return bits;
    }
    file.seek(0, SEEK_END);
    int length = file.tell();
    LogInfo(HgLog::INIT, "Opened asmap file %s (%d bytes) from disk\n", fs::quoted(fs::PathToString(path)), length);
    ...
    if (!SanityCheckASMap(bits, 128)) {
        LogInfo(HgLog::INIT, "Sanity check of asmap file %s failed\n", fs::quoted(fs::PathToString(path)));
        return {};
    }
    return bits;
}
