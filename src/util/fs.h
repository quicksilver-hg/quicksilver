...
#ifndef QUICKSILVER_UTIL_FS_H
#define QUICKSILVER_UTIL_FS_H
...
/**
 * Keep this overload deleted so callers cannot bypass the symlink workaround
 * above by selecting the error_code variant. If the overload is needed later,
 * add the same symlink guard before enabling it.
 */
bool create_directories(const std::filesystem::path& p, std::error_code& ec) = delete;

/**
 * Best-effort file removal for cleanup paths. Never throws.
 *
 * `using namespace std::filesystem` above means an unqualified `remove(p)` on
 * a path resolves by ADL to the THROWING std::filesystem::remove, not to C's
 * `::remove`, which is what the bare spelling suggests. A cleanup that runs
 * because something already failed must not be able to raise: on Windows the
 * file it is deleting is routinely still held by another process (Defender,
 * the search indexer) immediately after fclose(), so the removal itself fails.
 * That threw out of the scheduler thread and killed a node. POSIX unlinks a
 * held file happily, so Linux never saw it.
 *
 * Returns true if the file was removed. A failure -- including "it was not
 * there" -- is reported as false and otherwise ignored, which is what a
 * best-effort cleanup wants.
 */
static inline bool remove_quiet(const std::filesystem::path& p) noexcept
{
    std::error_code ec;
