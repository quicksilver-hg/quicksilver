...
LockResult LockDirectory(const fs::path& directory, const fs::path& lockfile_name, bool probe_only)
{
    ...
    if (!lock->TryLock()) {
        LogError(HgLog::INIT, "Error while attempting to lock directory %s: %s\n", fs::PathToString(directory), lock->GetReason());
...
bool FileCommit(FILE* file)
{
    if (fflush(file) != 0) { // harmless if redundantly called
        LogInfo(HgLog::INIT, "fflush failed: %s\n", SysErrorString(errno));
        return false;
    }
#ifdef WIN32
    HANDLE hFile = (HANDLE)_get_osfhandle(_fileno(file));
    if (FlushFileBuffers(hFile) == 0) {
        LogInfo(HgLog::INIT, "FlushFileBuffers failed: %s\n", Win32ErrorString(GetLastError()));
        return false;
    }
#elif defined(__APPLE__) && defined(F_FULLFSYNC)
    if (fcntl(fileno(file), F_FULLFSYNC, 0) == -1) { // Manpage says "value other than -1" is returned on success
        LogInfo(HgLog::INIT, "fcntl F_FULLFSYNC failed: %s\n", SysErrorString(errno));
        return false;
    }
#elif HAVE_FDATASYNC
    if (fdatasync(fileno(file)) != 0 && errno != EINVAL) { // Ignore EINVAL for filesystems that don't support sync
        LogInfo(HgLog::INIT, "fdatasync failed: %s\n", SysErrorString(errno));
        return false;
    }
#else
    if (fsync(fileno(file)) != 0 && errno != EINVAL) {
        LogInfo(HgLog::INIT, "fsync failed: %s\n", SysErrorString(errno));
        return false;
    }
#endif
    return true;
...
fs::path GetSpecialFolderPath(int nFolder, bool fCreate)
{
    ...
    LogInfo(HgLog::INIT, "SHGetSpecialFolderPathW() failed, could not obtain requested path.\n");
    return fs::path("");
}
#endif
