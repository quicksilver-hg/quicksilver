...
std::partial_ordering CompareChunks(Span<const WorkFrac> chunks0, Span<const WorkFrac> chunks1)
{
    ...
    const std::array<Span<const WorkFrac>, 2> chunk = {chunks0, chunks1};
    /** How many elements we have processed in each input. */
    size_t next_index[2] = {0, 0};
    /** Accumulated work/sizes in diagrams, up to next_index[i] - 1. */
    WorkFrac accum[2];
    ...
    do {
        ...
        // compute the slopes of line AB and of line AP, and compare them. These slopes are work per size,
        // and can thus be expressed as WorkFracs.
        const WorkFrac& point_p = next_point(unproc_side);
        const WorkFrac& point_a = prev_point(!unproc_side);

        const auto slope_ap = point_p - point_a;
        Assume(slope_ap.size > 0);
        std::weak_ordering cmp = std::weak_ordering::equivalent;
        if (done_0 || done_1) {
            // If a single side has no points left, act as if AB has slope tail_workrate(of 0).
            Assume(!(done_0 && done_1));
            cmp = WorkRateCompare(slope_ap, WorkFrac(0, 1));
        } else {
            // If both sides have points left, compute B, and the slope of AB explicitly.
            const WorkFrac& point_b = next_point(!unproc_side);
            const auto slope_ab = point_b - point_a;
            Assume(slope_ab.size >= slope_ap.size);
            cmp = WorkRateCompare(slope_ap, slope_ab);
