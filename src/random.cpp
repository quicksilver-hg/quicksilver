...
{
    LogError(HgLog::RAND, "Failed to read randomness, aborting\n");
    std::abort();
}
...
void ReportHardwareRand()
{
    ...
    if (g_rdseed_supported) {
        LogInfo(HgLog::RAND, "Using RdSeed as an additional entropy source\n");
    }
    if (g_rdrand_supported) {
        LogInfo(HgLog::RAND, "Using RdRand as an additional entropy source\n");
    }
}
...
void SeedPeriodic(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    LogDebug(HgLog::RAND, "Feeding %i bytes of dynamic environment data into RNG\n", hasher.Size() - old_size);
...
void SeedStartup(CSHA512& hasher, RNGState& rng) noexcept
{
    ...
    LogDebug(HgLog::RAND, "Feeding %i bytes of environment data into RNG\n", hasher.Size() - old_size);
