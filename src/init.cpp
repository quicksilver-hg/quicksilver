...
using node::DEFAULT_PERSIST_RELAYPOOL;
...
using node::ShouldPersistRelayPool;
 ...
 */
static const char* QUICKSILVER_PID_FILENAME = "quicksilverd.pid";
/**
...
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", QUICKSILVER_PID_FILENAME));
}
...
static void RemovePidFile(const ArgsManager& args)
{
    ...
    if (std::error_code error; !fs::remove(pid_path, error)) {
        ...
        LogWarning(HgLog::INIT, "down pidfile=%s removed=0 detail=%s\n", fs::PathToString(pid_path), msg);
    }
}
...
void Interrupt(NodeContext& node)
{
    ...
    if (node.mining_service) node.mining_service->Interrupt();
}
...
void Shutdown(NodeContext& node)
{
    ...
    LogInfo(HgLog::INIT, "down stage=begin\n");
    Assert(node.args);
    ...
    if (node.relaypool) node.relaypool->AddTransactionsUpdated(1);
    ...
    // Quicksilver: stop the background miner before tearing down the chainstate it
    // uses. HTTP/RPC are already stopped, so no new startmining call can race this.
    if (node.mining_service) {
        node.mining_service->Stop();
        node.mining_service.reset();
    }
    ...
    StopTorControl();
    // After StopTorControl, never before: the controller holds a connection to
    // this Tor, and killing the process out from under it is a shutdown that
    // logs errors on its way out.
    tor::StopBundledTor();
    ...
    if (node.relaypool && node.relaypool->GetLoadTried() && ShouldPersistRelayPool(*node.args)) {
        DumpRelayPool(*node.relaypool, RelayPoolPath(*node.args));
    }
    ...
    // would too. The only reason to do the above flushes is to let the vault catch
    ...
    LogInfo(HgLog::INIT, "down stage=complete\n");
}
...
static BOOL WINAPI consoleCtrlHandler(DWORD dwCtrlType)
{
    if (!(*Assert(g_shutdown))()) {
        LogError(HgLog::INIT, "down ok=0 reason=shutdown-signal-failed source=ctrl-c\n");
        return false;
    }
    Sleep(INFINITE);
    return true;
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    const auto publictestBaseParams = CreateBaseChainParams(ChainType::PUBLIC_TEST);
    const auto sandboxBaseParams = CreateBaseChainParams(ChainType::SANDBOX);
    const auto defaultChainParams = CreateChainParams(argsman, ChainType::MAIN);
    const auto publictestChainParams = CreateChainParams(argsman, ChainType::PUBLIC_TEST);
    const auto sandboxChainParams = CreateChainParams(argsman, ChainType::SANDBOX);
...
#endif
    argsman.AddArg("-assumevalid=<hex>", strprintf("If this block is in the chain assume that it and its ancestors are valid and potentially skip their script verification (0 to verify all, default: %s, publictest: %s)", defaultChainParams->GetConsensus().defaultAssumeValid.GetHex(), publictestChainParams->GetConsensus().defaultAssumeValid.GetHex()), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location (only useable from command line, not configuration file) (default: %s)", QUICKSILVER_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-datadir=<dir>", "Specify data directory", ArgsManager::ALLOW_ANY | ArgsManager::DISALLOW_NEGATION, OptionsCategory::OPTIONS);
    argsman.AddArg("-dbbatchsize", strprintf("Maximum database write batch size in bytes (default: %u)", nDefaultDbBatchSize), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::OPTIONS);
    argsman.AddArg("-dbcache=<n>", strprintf("Maximum database cache size <n> MiB (minimum %d, default: %d). Make sure you have enough RAM. In addition, unused memory allocated to the relay pool is shared with this cache (see -maxrelaypool).", MIN_DB_CACHE >> 20, DEFAULT_DB_CACHE >> 20), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-includeconf=<file>", "Specify additional configuration file, relative to the -datadir path (only useable from configuration file, not command line)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-loadblock=<file>", "Imports blocks from external file on startup", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-maxrelaypool=<n>", strprintf("Keep the relay pool below <n> megabytes (default: %u)", DEFAULT_MAX_RELAYPOOL_SIZE_MB), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-maxorphantx=<n>", strprintf("Keep at most <n> unconnectable transactions in memory (default: %u)", DEFAULT_MAX_ORPHAN_TRANSACTIONS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-relaypoolexpiry=<n>", strprintf("Do not keep transactions in the relay pool longer than <n> hours (default: %u)", DEFAULT_RELAYPOOL_EXPIRY_HOURS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-minimumchainwork=<hex>", strprintf("Minimum work assumed to exist on a valid chain in hex (default: %s, publictest: %s)", defaultChainParams->GetConsensus().nMinimumChainWork.GetHex(), publictestChainParams->GetConsensus().nMinimumChainWork.GetHex()), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::OPTIONS);
    argsman.AddArg("-par=<n>", strprintf("Set the number of script verification threads (0 = auto, up to %d, <0 = leave that many cores free, default: %d)",
        MAX_SCRIPTCHECK_THREADS, DEFAULT_SCRIPTCHECK_THREADS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-persistrelaypool", strprintf("Whether to save the relay pool on shutdown and load on restart (default: %u)", DEFAULT_PERSIST_RELAYPOOL), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", QUICKSILVER_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-prune=<n>", strprintf("Reduce storage requirements by enabling pruning (deleting) of old blocks. This allows the pruneblockchain RPC to be called to delete specific blocks and enables automatic pruning of old blocks if a target size in MiB is provided. This mode is incompatible with -txindex. "
            "Warning: Reverting this setting requires re-downloading the entire blockchain. "
            "(default: 0 = disable pruning blocks, 1 = allow manual pruning via RPC, >=%u = automatically prune block files to stay under the specified target size in MiB)", MIN_DISK_SPACE_FOR_BLOCK_FILES / 1024 / 1024), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-reindex", "If enabled, wipe chain state and ledger index, and rebuild them from blk*.dat files on disk. Also wipe and rebuild other optional indexes that are active.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", QUICKSILVER_CONF_FILENAME, QUICKSILVER_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
    ...
    argsman.AddArg("-bind=<addr>[:<port>][=onion]", strprintf("Bind to given address and always listen on it (default: 0.0.0.0). Use [host]:port notation for IPv6. Append =onion to tag any incoming connections to that address and port as incoming Tor connections (default: 127.0.0.1:%u=onion, publictest: 127.0.0.1:%u=onion, sandbox: 127.0.0.1:%u=onion)", defaultChainParams->OnionServicePort(), publictestChainParams->OnionServicePort(), sandboxChainParams->OnionServicePort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-bundledtor", strprintf("Start and supervise a Tor process for this node rather than connecting to one that is already running. The desktop defaults to 1; the daemon defaults to %u.", DEFAULT_BUNDLED_TOR), ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    argsman.AddArg("-bundledtorpath=<path>", "Path to the tor executable used by -bundledtor. By default 'tor' beside the Quicksilver executable, then the first one on PATH.", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    ...
    g_vault_init_interface.AddVaultOptions(argsman);
    ...
    argsman.AddArg("-checkblockindex", strprintf("Do a consistency check for the block tree, chainstate, and other validation data structures every <n> operations. Use 0 to disable. (default: %u, sandbox: %u)", defaultChainParams->DefaultConsistencyChecks(), sandboxChainParams->DefaultConsistencyChecks()), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-checkaddrman=<n>", strprintf("Run addrman consistency checks every <n> operations. Use 0 to disable. (default: %u)", DEFAULT_ADDRMAN_CONSISTENCY_CHECKS), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-checkrelaypool=<n>", strprintf("Run relay pool consistency checks every <n> transactions. Use 0 to disable. (default: %u, sandbox: %u)", defaultChainParams->DefaultConsistencyChecks(), sandboxChainParams->DefaultConsistencyChecks()), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-stopafterblockimport", strprintf("Stop running after importing blocks from disk (default: %u)", DEFAULT_STOPAFTERBLOCKIMPORT), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-stopatheight", strprintf("Stop running after reaching the given height in the main chain (default: %u)", DEFAULT_STOPATHEIGHT), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-limitancestorcount=<n>", strprintf("Do not accept transactions if number of in-relay-pool ancestors is <n> or more (default: %u)", DEFAULT_ANCESTOR_LIMIT), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-limitancestorsize=<n>", strprintf("Do not accept transactions whose size with all in-relay-pool ancestors exceeds <n> kilobytes (default: %u)", DEFAULT_ANCESTOR_SIZE_LIMIT_KVB), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-limitdescendantcount=<n>", strprintf("Do not accept transactions if any ancestor would have <n> or more in-relay-pool descendants (default: %u)", DEFAULT_DESCENDANT_LIMIT), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-limitdescendantsize=<n>", strprintf("Do not accept transactions if any ancestor would have more than <n> kilobytes of in-relay-pool descendants (default: %u).", DEFAULT_DESCENDANT_SIZE_LIMIT_KVB), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-datacarriersize",
                   strprintf("Relay and mine transactions whose data-carrying output script "
    ...
    argsman.AddArg("-rpcport=<port>", strprintf("Listen for JSON-RPC connections on <port> (default: %u, publictest: %u, sandbox: %u)", defaultBaseParams->RPCPort(), publictestBaseParams->RPCPort(), sandboxBaseParams->RPCPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::RPC);
...
void InitParameterInteraction(ArgsManager& args)
{
    ...
    if (!args.GetArgs("-bind").empty()) {
        if (args.SoftSetBoolArg("-listen", true))
            LogInfo(HgLog::INIT, "overrode option=-listen value=1 reason=bind-set\n");
    }
    if (!args.GetArgs("-whitebind").empty()) {
        if (args.SoftSetBoolArg("-listen", true))
            LogInfo(HgLog::INIT, "overrode option=-listen value=1 reason=whitebind-set\n");
    }
    ...
    if (!args.GetArgs("-connect").empty() || args.IsArgNegated("-connect") || args.GetIntArg("-maxconnections", DEFAULT_MAX_PEER_CONNECTIONS) <= 0) {
        ...
        if (args.SoftSetBoolArg("-dnsseed", false))
            LogInfo(HgLog::INIT, "overrode option=-dnsseed value=0 reason=connect-or-maxconnections-zero\n");
        if (args.SoftSetBoolArg("-listen", false))
            LogInfo(HgLog::INIT, "overrode option=-listen value=0 reason=connect-or-maxconnections-zero\n");
    }
    ...
    if (proxy_arg != "" && proxy_arg != "0") {
        ...
        if (args.SoftSetBoolArg("-listen", false))
            LogInfo(HgLog::INIT, "overrode option=-listen value=0 reason=proxy-set\n");
        // to protect privacy, do not map ports when a proxy is set. The user may still specify -listen=1
        // to listen locally, so don't rely on this happening through -listen below.
        if (args.SoftSetBoolArg("-natpmp", false)) {
            LogInfo(HgLog::INIT, "overrode option=-natpmp value=0 reason=proxy-set\n");
        }
        // to protect privacy, do not discover addresses by default
        if (args.SoftSetBoolArg("-discover", false))
            LogInfo(HgLog::INIT, "overrode option=-discover value=0 reason=proxy-set\n");
    }

    if (!args.GetBoolArg("-listen", DEFAULT_LISTEN)) {
        // do not map ports or try to retrieve public IP when not listening (pointless)
        if (args.SoftSetBoolArg("-natpmp", false)) {
            LogInfo(HgLog::INIT, "overrode option=-natpmp value=0 reason=listen-disabled\n");
        }
        if (args.SoftSetBoolArg("-discover", false))
            LogInfo(HgLog::INIT, "overrode option=-discover value=0 reason=listen-disabled\n");
        if (args.SoftSetBoolArg("-listenonion", false))
            LogInfo(HgLog::INIT, "overrode option=-listenonion value=0 reason=listen-disabled\n");
        if (args.SoftSetBoolArg("-i2pacceptincoming", false)) {
            LogInfo(HgLog::INIT, "overrode option=-i2pacceptincoming value=0 reason=listen-disabled\n");
        }
    }
    ...
    if (!args.GetArgs("-externalip").empty()) {
        ...
        if (args.SoftSetBoolArg("-discover", false))
            LogInfo(HgLog::INIT, "overrode option=-discover value=0 reason=externalip-set\n");
    }

    if (args.GetBoolArg("-blocksonly", DEFAULT_BLOCKSONLY)) {
        // Reduce default relaypool size in blocksonly mode to avoid unexpected resource usage
        if (args.SoftSetArg("-maxrelaypool", ToString(DEFAULT_BLOCKSONLY_MAX_RELAYPOOL_SIZE_MB)))
            LogInfo(HgLog::INIT, "overrode option=-maxrelaypool value=%d reason=blocksonly\n", DEFAULT_BLOCKSONLY_MAX_RELAYPOOL_SIZE_MB);
    }
    ...
    if (!onlynets.empty()) {
        ...
        if (!clearnet_reachable && args.SoftSetBoolArg("-dnsseed", false)) {
            LogInfo(HgLog::INIT, "overrode option=-dnsseed value=0 reason=onlynet-excludes-ipv4-ipv6\n");
        }
    }
...
[[noreturn]] static void new_handler_terminate()
{
    ...
    LogError(HgLog::INIT, "halted reason=out-of-memory\n");

    // The log was successful, terminate now.
    std::terminate();
};

// Quicksilver: portable overwrite-set of an environment variable. POSIX setenv()
// is unavailable under MSVC, which provides _putenv_s() instead.
...
{
    // Quicksilver: expose the optional external GPU Cuckatoo solver to the
    // mining path (the cuckatoo lib reads CUCKATOO_GPU_SOLVER). Mining/test only;
    // returned proofs are self-verified before use. Done early, before any mining.
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    if (!g_vault_init_interface.ParameterInteraction()) return false;
    ...
    if (!test_options.empty()) {
        if (chainparams.GetChainType() != ChainType::SANDBOX) {
            return InitError(Untranslated("-test=<option> can only be used with sandbox"));
        }
    ...
    // Also report errors from parsing before daemonization
    {
        ...
        auto relaypool_result{ApplyArgsManOptions(args, chainparams, relaypool_opts)};
        if (!relaypool_result) {
            return InitError(util::ErrorString(relaypool_result));
        }
    }
...
bool AppInitInterfaces(NodeContext& node)
{
    ...
    // Construct vault interfaces during base initialization so the desktop can
    // open the vault runtime before consensus startup is requested.
    g_vault_init_interface.Construct(node);
    return true;
}
    ...
    const ArgsManager& args)
{
    ...
    Assert(ApplyArgsManOptions(args, chainparams, relaypool_opts)); // no error can happen, already checked in AppInitParameterInteraction
    bilingual_str relaypool_error;
    node.relaypool = std::make_unique<CTxRelayPool>(relaypool_opts, relaypool_error);
    if (!relaypool_error.empty()) {
        return {ChainstateLoadStatus::FAILURE_FATAL, relaypool_error};
    }
    LogInfo(HgLog::INIT, "ready cache=utxo size_mib=%.1f relaypool_spare_mib=%.1f\n", cache_sizes.coins * (1.0 / 1024 / 1024), relaypool_opts.max_size_bytes * (1.0 / 1024 / 1024));
    ...
    try {
        ...
        LogError(HgLog::INIT, "halted reason=blockdb-open-failed detail=%s\n", e.what());
    ...
    options.relaypool = Assert(node.relaypool.get());
    ...
    uiInterface.InitMessage(_("Loading ledger index…"));
    auto catch_exceptions = [](auto&& f) -> ChainstateLoadResult {
        try {
            return f();
        } catch (const std::exception& e) {
            LogError(HgLog::INIT, "halted reason=chainstate-load-failed detail=%s\n", e.what());
            return std::make_tuple(node::ChainstateLoadStatus::FAILURE, _("Error loading databases"));
        }
    };
    auto [status, error] = catch_exceptions([&] { return LoadChainstate(chainman, cache_sizes, options); });
    if (status == node::ChainstateLoadStatus::SUCCESS) {
        uiInterface.InitMessage(_("Verifying ledger blocks…"));
        if (chainman.m_blockman.m_have_pruned && options.check_blocks > MIN_BLOCKS_TO_KEEP) {
            LogWarning(HgLog::INIT, "ready checkblocks_capped=%d reason=pruned-datadir\n",
                       MIN_BLOCKS_TO_KEEP);
        }
        std::tie(status, error) = catch_exceptions([&] { return VerifyLoadedChainstate(chainman, options); });
        if (status == node::ChainstateLoadStatus::SUCCESS) {
            LogInfo(HgLog::INIT, "loaded ledger_index=1 chainstate=1");
        }
    }
    return {status, error};
};
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogWarning(HgLog::INIT, "ready datadir=%s relative_to=%s warn=fragile-relative-datadir\n",
    ...
    scheduler.scheduleEvery([&args, &node]{
        ...
        if (!CheckDiskSpace(args.GetBlocksDirPath(), min_disk_space)) {
            LogError(HgLog::INIT, "halted reason=insufficient-disk-space\n");
            if (!(Assert(node.shutdown_request))()) {
                LogError(HgLog::INIT, "down ok=0 reason=shutdown-signal-failed source=disk-space-check\n");
            }
        }
    }, std::chrono::minutes{5});

    if (args.GetBoolArg("-logratelimit", HgLog::DEFAULT_LOGRATELIMIT)) {
        LogInstance().SetRateLimiting(HgLog::LogRateLimiter::Create(
            [&scheduler](auto func, auto window) { scheduler.scheduleEvery(std::move(func), window); },
            HgLog::RATELIMIT_MAX_BYTES,
            HgLog::RATELIMIT_WINDOW));
    } else {
        LogInfo(HgLog::INIT, "ready log_ratelimit=disabled");
    }
    ...
    // ********************************************************* Step 5: verify vault database integrity
    for (const auto& client : node.chain_clients) {
        if (!client->verify()) {
            return false;
        }
...

    {
        ...
        if (args.IsArgSet("-asmap") && !args.IsArgNegated("-asmap")) {
            ...
            LogInfo(HgLog::INIT, "ready bucketing=asmap asmap_version=%s\n", asmap_version.ToString());
        } else {
            LogInfo(HgLog::INIT, "ready bucketing=ipv4-prefix-16\n");
        }
        ...
        // Initialize addrman
        assert(!node.addrman);
        uiInterface.InitMessage(_("Loading peer signals…"));
    ...
    // Bundled Tor, if enabled, before anything reads a proxy setting: it forces
    // -torcontrol, and StartTorControl() below then runs UNCHANGED -- it
    // authenticates with the cookie whose path Tor reports in PROTOCOLINFO,
    ...
    if (args.GetBoolArg("-bundledtor", DEFAULT_BUNDLED_TOR)) {
        if (!args.GetBoolArg("-listenonion", DEFAULT_LISTEN_ONION)) {
            ...
            // anything the user typed -- -connect and -listen=0 both switch it
            // off through InitParameterInteraction above, and refusing to start
            // would turn an ordinary -connect into a startup failure.
            LogInfo(HgLog::TOR, "tor: not starting a bundled tor reason=listenonion-disabled\n");
        } else {
            const auto control_endpoint{tor::StartBundledTor(args.GetDataDirNet(), args.GetPathArg("-bundledtorpath"))};
            if (!control_endpoint) return InitError(util::ErrorString(control_endpoint));
            // gArgs, not `args`: TorControlThread reads the global, and `args`
            // is a const reference here.
            gArgs.ForceSetArg("-torcontrol", *control_endpoint);
        }
    }
    ...
    LogInfo(HgLog::INIT, "ready stage=cache-config");
    LogInfo(HgLog::INIT, "ready cache=block_index size_mib=%.1f", kernel_cache_sizes.block_tree_db * (1.0 / 1024 / 1024));
    if (args.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
        LogInfo(HgLog::INIT, "ready cache=tx_index size_mib=%.1f", index_cache_sizes.tx_index * (1.0 / 1024 / 1024));
    }
    for (BlockFilterType filter_type : g_enabled_filter_types) {
        LogInfo(HgLog::INIT, "ready cache=filter_index filter_type=%s size_mib=%.1f",
                  BlockFilterTypeName(filter_type), index_cache_sizes.filter_index * (1.0 / 1024 / 1024));
    }
    LogInfo(HgLog::INIT, "ready cache=chainstate_db size_mib=%.1f", kernel_cache_sizes.coins_db * (1.0 / 1024 / 1024));
    ...
    if (status == ChainstateLoadStatus::FAILURE && !do_reindex && !ShutdownRequested(node)) {
        ...
        if (!Assert(node.shutdown_signal)->reset()) {
            LogError(HgLog::INIT, "halted reason=shutdown-signal-reset-failed\n");
        }
    ...
    if (ShutdownRequested(node)) {
        LogInfo(HgLog::INIT, "down reason=shutdown-requested\n");
        return false;
    }
    ...
    // after any vault rescanning has taken place.
    if (chainman.m_blockman.IsPruneMode()) {
        ...
        LogInfo(HgLog::INIT, "ready services=node_network reason=not-pruned\n");
    ...
    node.background_init_thread = std::thread(&util::TraceThread, "initload", [=, &chainman, &args, &node] {
        ...
        if (args.GetBoolArg("-stopafterblockimport", DEFAULT_STOPAFTERBLOCKIMPORT)) {
            LogInfo(HgLog::INIT, "down reason=stopafterblockimport\n");
            if (!(Assert(node.shutdown_request))()) {
                LogError(HgLog::INIT, "down ok=0 reason=shutdown-signal-failed source=block-import\n");
            }
            return;
        }
        ...
        // Load relaypool from disk
        if (auto* pool{chainman.ActiveChainstate().GetRelayPool()}) {
            LoadRelayPool(*pool, ShouldPersistRelayPool(args) ? RelayPoolPath(args) : fs::path{}, chainman.ActiveChainstate(), {});
    ...
    int64_t best_block_time{};
    {
        ...
        LogInfo(HgLog::INIT, "loaded block_tree_size=%u\n", chainman.BlockIndex().size());
    ...
    LogInfo(HgLog::INIT, "ready height=%d\n", chain_active_height);
    ...
    connOptions.m_implicit_whitelist_relay = !args.GetBoolArg("-blocksonly", DEFAULT_BLOCKSONLY);

    // Port to bind to if `-bind=addr` is provided without a `:port` suffix.
    const uint16_t default_bind_port =
        static_cast<uint16_t>(args.GetIntArg("-port", Params().GetDefaultPort()));

    // Loopback port the onion service forwards to. Upstream derives it as P2P+1;
    // that collides with the RPC port on publictest and sandbox, so chainparams
    // names it per network instead. An explicit -port still shifts it: the operator
    // has moved the whole node, and P2P+1 is then theirs to reason about.
    const uint16_t default_bind_port_onion =
        args.IsArgSet("-port") ? static_cast<uint16_t>(default_bind_port + 1)
                               : Params().OnionServicePort();
    ...
    // The default onion target is only bound when an onion service is actually
    // wanted. It used to be pushed unconditionally, above this check, so
    // -listenonion=0 did not avoid the bind — verified.
    if (args.GetBoolArg("-listenonion", DEFAULT_LISTEN_ONION)) {
    ...
    if (!connect.empty() || args.IsArgNegated("-connect")) {
        ...
        if (!connOptions.m_specified_outgoing.empty() && !connOptions.vSeedNodes.empty()) {
            LogInfo(HgLog::INIT, "ignored option=-seednode reason=connect-set\n");
        }

        if (args.IsArgSet("-dnsseed") && args.GetBoolArg("-dnsseed", DEFAULT_DNSSEED) && args.IsArgSet("-proxy")) {
            LogInfo(HgLog::INIT, "ignored option=-dnsseed reason=connect-set-and-proxy-set\n");
        }
    }
    ...
    if (args.GetBoolArg("-mine", false)) {
        ...
        if (!IsValidDestination(dest)) {
            return InitError(strprintf(_("Invalid -mineaddress: %s"), mine_addr));
        }
        node.mining_service->Start(GetScriptForDestination(dest), mine_addr);
        LogInfo(HgLog::INIT, "up miner=background payout=%s\n", mine_addr);
    }
