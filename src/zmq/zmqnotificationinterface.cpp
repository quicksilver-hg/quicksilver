...
bool CZMQNotificationInterface::Initialize()
{
    ...
    LogDebug(HgLog::ZMQ, "version %d.%d.%d\n", major, minor, patch);

    LogDebug(HgLog::ZMQ, "Initialize notification interface\n");
    assert(!pcontext);
    ...
    for (auto& notifier : notifiers) {
        if (notifier->Initialize(pcontext)) {
            LogDebug(HgLog::ZMQ, "Notifier %s ready (address = %s)\n", notifier->GetType(), notifier->GetAddress());
        } else {
            LogDebug(HgLog::ZMQ, "Notifier %s failed (address = %s)\n", notifier->GetType(), notifier->GetAddress());
            return false;
        }
    }
...
{
    LogDebug(HgLog::ZMQ, "Shutdown notification interface\n");
    if (pcontext)
    {
        for (auto& notifier : notifiers) {
            LogDebug(HgLog::ZMQ, "Shutdown notifier %s at %s\n", notifier->GetType(), notifier->GetAddress());
            notifier->Shutdown();
        }
...
void CZMQNotificationInterface::TransactionAddedToRelayPool(const CTransactionRef& ptx, uint64_t relaypool_sequence)
{
    const CTransaction& tx = *ptx;

    TryForEachAndRemoveFailed(notifiers, [&tx, relaypool_sequence](CZMQAbstractNotifier* notifier) {
        return notifier->NotifyTransaction(tx) && notifier->NotifyTransactionAcceptance(tx, relaypool_sequence);
    });
}

void CZMQNotificationInterface::TransactionRemovedFromRelayPool(const CTransactionRef& ptx, RelayPoolRemovalReason reason, uint64_t relaypool_sequence)
{
    // Called for all non-block inclusion reasons
    const CTransaction& tx = *ptx;

    TryForEachAndRemoveFailed(notifiers, [&tx, relaypool_sequence](CZMQAbstractNotifier* notifier) {
        return notifier->NotifyTransactionRemoval(tx, relaypool_sequence);
    });
}
