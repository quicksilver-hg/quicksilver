...
bool CZMQAbstractPublishNotifier::Initialize(void *pcontext)
{
    ...
    if (i==mapPublishNotifiers.end())
    {
        ...
        LogDebug(HgLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
    ...
    {
        LogDebug(HgLog::ZMQ, "Reusing socket for address %s\n", address);
        LogDebug(HgLog::ZMQ, "Outbound message high water mark for %s at %s is %d\n", type, address, outbound_message_high_water_mark);
...
void CZMQAbstractPublishNotifier::Shutdown()
{
    ...
    if (count == 1)
    {
        LogDebug(HgLog::ZMQ, "Close socket at address %s\n", address);
        int linger = 0;
...
bool CZMQPublishHashBlockNotifier::NotifyBlock(const CBlockIndex *pindex)
{
    ...
    LogDebug(HgLog::ZMQ, "Publish hashblock %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
...
bool CZMQPublishHashTransactionNotifier::NotifyTransaction(const CTransaction &transaction)
{
    ...
    LogDebug(HgLog::ZMQ, "Publish hashtx %s to %s\n", hash.GetHex(), this->address);
    uint8_t data[32];
    for (unsigned int i = 0; i < 32; i++) {
        data[31 - i] = hash.begin()[i];
    }
...
{
    LogDebug(HgLog::ZMQ, "Publish rawblock %s to %s\n", pindex->GetBlockHash().GetHex(), this->address);
...
{
    uint256 hash = transaction.GetHash();
    LogDebug(HgLog::ZMQ, "Publish rawtx %s to %s\n", hash.GetHex(), this->address);
    DataStream ss;
...
{
    uint256 hash = pindex->GetBlockHash();
    LogDebug(HgLog::ZMQ, "Publish sequence block connect %s to %s\n", hash.GetHex(), this->address);
...
{
    uint256 hash = pindex->GetBlockHash();
    LogDebug(HgLog::ZMQ, "Publish sequence block disconnect %s to %s\n", hash.GetHex(), this->address);
    return SendSequenceMsg(*this, hash, /* Block (D)isconnect */ 'D');
}

bool CZMQPublishSequenceNotifier::NotifyTransactionAcceptance(const CTransaction &transaction, uint64_t relaypool_sequence)
{
    uint256 hash = transaction.GetHash();
    LogDebug(HgLog::ZMQ, "Publish hashtx relay pool acceptance %s to %s\n", hash.GetHex(), this->address);
    return SendSequenceMsg(*this, hash, /* RelayPool (A)cceptance */ 'A', relaypool_sequence);
}

bool CZMQPublishSequenceNotifier::NotifyTransactionRemoval(const CTransaction &transaction, uint64_t relaypool_sequence)
{
    uint256 hash = transaction.GetHash();
    LogDebug(HgLog::ZMQ, "Publish hashtx relay pool removal %s to %s\n", hash.GetHex(), this->address);
    return SendSequenceMsg(*this, hash, /* RelayPool (R)emoval */ 'R', relaypool_sequence);
}
