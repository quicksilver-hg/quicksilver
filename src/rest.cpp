...
/**
 * Get the node context relaypool.
 *
 * @param[in]  req The HTTP request, whose status code will be set if node
 *                 context relaypool is not found.
 * @returns        Pointer to the relaypool or nullptr if no relaypool found.
 */
static CTxRelayPool* GetRelayPool(const std::any& context, HTTPRequest* req)
{
    auto node_context = util::AnyPtr<NodeContext>(context);
    if (!node_context || !node_context->relaypool) {
        RESTERR(req, HTTP_NOT_FOUND, "Relay pool disabled or instance not found");
        return nullptr;
    }
    return node_context->relaypool.get();
}
...
static bool rest_relaypool(const std::any& context, HTTPRequest* req, const std::string& str_uri_part)
{
    if (!CheckWarmup(req))
        return false;
    ...
    if (param != "contents" && param != "info") {
        return RESTERR(req, HTTP_BAD_REQUEST, "Invalid URI format. Expected /rest/relaypool/<info|contents>.json");
    }

    const CTxRelayPool* relaypool = GetRelayPool(context, req);
    if (!relaypool) return false;
    ...
    case RESTResponseFormat::JSON: {
        ...
        if (param == "contents") {
            ...
            std::string raw_relaypool_sequence;
            try {
                raw_relaypool_sequence = req->GetQueryParameter("relaypool_sequence").value_or("false");
            } catch (const std::runtime_error& e) {
                return RESTERR(req, HTTP_BAD_REQUEST, e.what());
            }
            if (raw_relaypool_sequence != "true" && raw_relaypool_sequence != "false") {
                return RESTERR(req, HTTP_BAD_REQUEST, "The \"relaypool_sequence\" query parameter must be either \"true\" or \"false\".");
            }
            const bool verbose{raw_verbose == "true"};
            const bool relaypool_sequence{raw_relaypool_sequence == "true"};
            if (verbose && relaypool_sequence) {
                return RESTERR(req, HTTP_BAD_REQUEST, "Verbose results cannot contain relay pool sequence values. (hint: set \"verbose=false\")");
            }
            str_json = RelayPoolToJSON(*relaypool, verbose, relaypool_sequence).write() + "\n";
        } else {
            str_json = RelayPoolInfoToJSON(*relaypool).write() + "\n";
        }
...
static bool rest_tx(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    const CTransactionRef tx{GetTransaction(/*block_index=*/nullptr, node->relaypool.get(), *hash, hashBlock, node->chainman->m_blockman)};
    if (!tx) {
        return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");
    }
...
static bool rest_getutxos(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    if (uriParts.size() > 0)
    {
        //inputs is sent over URI scheme (/rest/getutxos/checkrelaypool/txid1-n/txid2-n/...)
        if (uriParts[0] == "checkrelaypool") fCheckRelayPool = true;

        for (size_t i = (fCheckRelayPool) ? 1 : 0; i < uriParts.size(); i++)
        {
    ...
    uint256 active_hash;
    {
        auto process_utxos = [&vOutPoints, &outs, &hits, &active_height, &active_hash, &chainman](const CCoinsView& view, const CTxRelayPool* relaypool) EXCLUSIVE_LOCKS_REQUIRED(chainman.GetMutex()) {
            for (const COutPoint& vOutPoint : vOutPoints) {
                auto coin = !relaypool || !relaypool->isSpent(vOutPoint) ? view.GetCoin(vOutPoint) : std::nullopt;
        ...
        if (fCheckRelayPool) {
            const CTxRelayPool* relaypool = GetRelayPool(context, req);
            if (!relaypool) return false;
            // use db+relaypool as cache backend in case user likes to query relaypool
            LOCK2(cs_main, relaypool->cs);
            CCoinsViewCache& viewChain = chainman.ActiveChainstate().CoinsTip();
            CCoinsViewRelayPool viewRelayPool(&viewChain, *relaypool);
            process_utxos(viewRelayPool, relaypool);
        } else {
            LOCK(cs_main);
    ...
    case RESTResponseFormat::JSON: {
        ...
        for (const CCoin& coin : outs) {
            ...
            utxo.pushKV("output_script", std::move(o));
