...
public:
    ...
    std::optional<OutputType> GetOutputType() const override { return OutputType::BASE58; }
    bool IsSingleType() const final { return true; }
...
public:
    ...
    std::optional<int64_t> MaxSatisfactionWeight(bool use_max_sig) const override {
        // The maximum over every spending path, not the key path alone. Upstream
        // assumed the key path and documented that it "can lead to very large
        // underestimations" -- for a 999-key multi_a leaf the real witness is tens of
        // kilobytes against an assumed 66 bytes. That is tolerable when the number
        // only informs coin selection; it is not tolerable when a sender grinds
        // proof-of-work sized from it, because an underestimate produces a
        // transaction the network rejects.
        //
        // Key path: one signature, pushed. 65 rather than 64 is the worst case, a
        // schnorr signature carrying an explicit sighash byte.
        int64_t max_sat = 1 + 65;
        for (size_t pos = 0; pos < m_depths.size(); ++pos) {
            const auto sat = m_subdescriptor_args[pos]->MaxSatSize(use_max_sig);
            const auto script_size = m_subdescriptor_args[pos]->ScriptSize();
            if (!sat || !script_size) return {};
            // A script path also reveals the leaf script and a control block, each as
            // its own witness stack element. The control block grows by one 32-byte
            // hash per level of depth, so the tree shape is part of the bound.
            const int64_t control_size = int64_t{TAPROOT_CONTROL_BASE_SIZE} +
                                         int64_t{TAPROOT_CONTROL_NODE_SIZE} * m_depths[pos];
            const int64_t path_sat = *sat
                                   + GetSizeOfCompactSize(*script_size) + *script_size
                                   + GetSizeOfCompactSize(control_size) + control_size;
            max_sat = std::max(max_sat, path_sat);
        }
        return max_sat;
    }

    std::optional<int64_t> MaxSatisfactionElems() const override {
        int64_t max_elems = 1; // key path: the signature alone
        for (const auto& sub : m_subdescriptor_args) {
            const auto elems = sub->MaxSatisfactionElems();
            if (!elems) return {};
            max_elems = std::max(max_elems, *elems + 2); // + leaf script + control block
        }
        return max_elems;
    }
...
std::vector<std::unique_ptr<DescriptorImpl>> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    if (ctx == ParseScriptContext::TOP && Func("wpkh", expr)) {
        ...
        error = "Can only have wpkh() at top level";
        return {};
    }
...
std::unique_ptr<DescriptorImpl> InferScript(const CScript& script, ParseScriptContext ctx, const SigningProvider& provider)
{
    ...
    if (txntype == TxoutType::WITNESS_V0_KEYHASH && ctx == ParseScriptContext::TOP) {
        uint160 hash(data[0]);
        CKeyID keyid(hash);
        CPubKey pubkey;
        if (provider.GetPubKey(keyid, pubkey)) {
    ...
    if (txntype == TxoutType::WITNESS_V0_SCRIPTHASH && ctx == ParseScriptContext::TOP) {
