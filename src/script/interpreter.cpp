...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (witversion == 0) {
        ...
        // BIP341 Taproot: native 32-byte witness v1 program (which encodes a P2C-tweaked pubkey)
        ...
        // Other version/size combinations return true for future softfork compatibility.
        return true;
    }
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash())
    {
        ...
        // Quicksilver launches with native witness addresses. The P2SH wrapper
        // only served pre-SegWit address compatibility on older chains, so it
        // is not part of this chain's script language.
        if (pubKey2.IsWitnessProgram(witnessversion, witnessprogram)) {
            return set_error(serror, SCRIPT_ERR_P2SH_WITNESS);
        }
