                     ...
                     std::vector<valtype>& ret, TxoutType& whichTypeRet, SigVersion sigversion, SignatureData& sigdata)
{
    ...
    case TxoutType::MULTISIG: {
        ...
        for (size_t i = 1; i < vSolutions.size() - 1; ++i) {
            ...
            // possible signatures that we can create. This will allow further PSQT
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    if (solved && whichType == TxoutType::SCRIPTHASH)
    {
        ...
        solved = !subscript.IsWitnessProgram(witness_version, witness_program) &&
...
// Extracts signatures and scripts from incomplete scriptSigs. Please do not extend this, use PSQT instead
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const CTxOut& txout)
{
    ...
    if (!p2sh && script_type == TxoutType::WITNESS_V0_SCRIPTHASH && !stack.witness.empty() && !stack.witness.back().empty()) {
...
bool IsSegWitOutput(const CScript& script)
{
    int version;
    valtype program;
