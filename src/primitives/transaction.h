...
#ifndef QUICKSILVER_PRIMITIVES_TRANSACTION_H
#define QUICKSILVER_PRIMITIVES_TRANSACTION_H
...
void UnserializeTransaction(TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s >> tx.nLockTime;
    // Quicksilver (#5c-1): anchor height, then the per-tx Cuckatoo proof tail (txid-bound).
    s >> tx.nAnchorHeight;
    s >> tx.nPowNonce;
    for (auto& edge : tx.nCycle) s >> edge;
}
...
void SerializeTransaction(const TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    s << tx.nAnchorHeight;
    s << tx.nPowNonce;
    for (const auto& edge : tx.nCycle) s << edge;
}
...
public:
    ...
    //! Quicksilver (#5c-1): anchor height — see CMutableTransaction::nAnchorHeight.
    const uint32_t nAnchorHeight;
    //! Quicksilver: per-tx Cuckatoo proof. Part of the base serialization, so the
    //! proof is bound into the txid. nPowNonce is the grind field (trailing pre-image).
    const std::array<uint32_t, 42> nCycle;
    const uint32_t nPowNonce;
...
public:
    ...
    //! Quicksilver: the per-tx PoW pre-image — the bytes the SipHash keys derive
    //! from. Commits to tx intent (prevouts, sequences, outputs, locktime, anchor
    //! height) AND the anchor block hash, but NOT to scriptSig/witness or to nCycle
    //! itself. nPowNonce is the trailing 4 bytes (LE), so the grinder mutates them in
    //! place. anchor_hash is the hash of the active-chain block at nAnchorHeight
    //! (resolved by the caller via CheckTxAnchor); committing it makes the proof
    //! impossible to grind before the anchor block exists (precompute resistance).
    std::vector<unsigned char> PowPreimage(const uint256& anchor_hash) const;
...
struct CMutableTransaction
{
    ...
    uint32_t version;
    uint32_t nLockTime;
    //! Quicksilver (#5c-1): height of the recent active-chain block this tx's per-tx
    //! PoW is anchored to. The PoW preimage commits that block's hash, so the proof
    //! cannot be ground before the block exists (precompute resistance) and validity
    //! is deterministic. Serialized before the proof tail.
    uint32_t nAnchorHeight{0};
    std::array<uint32_t, 42> nCycle{};
    uint32_t nPowNonce{0};
...
#endif // QUICKSILVER_PRIMITIVES_TRANSACTION_H
