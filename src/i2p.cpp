...
bool Session::Listen(Connection& conn)
{
    try {
        ...
        LogPrintLevel(HgLog::I2P, HgLog::Level::Error, "Couldn't listen: %s\n", e.what());
        CheckControlSock();
    }
    return false;
}
...
bool Session::Accept(Connection& conn)
{
    ...
    if (*m_interrupt) {
        LogPrintLevel(HgLog::I2P, HgLog::Level::Debug, "Accept was interrupted\n");
    } else {
        LogPrintLevel(HgLog::I2P, HgLog::Level::Debug, "Error accepting%s: %s\n", disconnect ? " (will close the session)" : "", errmsg);
    }
    if (disconnect) {
        LOCK(m_mutex);
        Disconnect();
...
bool Session::Connect(const CService& to, Connection& conn, bool& proxy_error)
{
    ...
    if (to.GetPort() != I2P_SAM31_PORT) {
        LogPrintLevel(HgLog::I2P, HgLog::Level::Debug, "Error connecting to %s, connection refused due to arbitrary port %s\n", to.ToStringAddrPort(), to.GetPort());
        proxy_error = false;
        return false;
    }
    ...
    try {
        ...
        LogPrintLevel(HgLog::I2P, HgLog::Level::Debug, "Error connecting to %s: %s\n", to.ToStringAddrPort(), e.what());
        CheckControlSock();
        return false;
    }
}
...
void Session::CheckControlSock()
{
    ...
    if (m_control_sock && !m_control_sock->IsConnected(errmsg)) {
        LogPrintLevel(HgLog::I2P, HgLog::Level::Debug, "Control socket error: %s\n", errmsg);
        Disconnect();
    }
}
...
void Session::CreateIfNotCreatedAlready()
{
    ...
    LogPrintLevel(HgLog::I2P, HgLog::Level::Debug, "Creating %s SAM session %s with %s\n", session_type, session_id, m_control_host.ToString());
    ...
    LogPrintLevel(HgLog::I2P, HgLog::Level::Info, "%s SAM session %s created, my address=%s\n",
...
void Session::Disconnect()
{
    if (m_control_sock) {
        if (m_session_id.empty()) {
            LogPrintLevel(HgLog::I2P, HgLog::Level::Info, "Destroying incomplete SAM session\n");
        } else {
            LogPrintLevel(HgLog::I2P, HgLog::Level::Info, "Destroying SAM session %s\n", m_session_id);
        }
