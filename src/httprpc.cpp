...
static void JSONErrorReply(HTTPRequest* req, UniValue objError, const JSONRPCRequest& jreq)
{
    ...
    std::optional<UniValue> id{jreq.id};
    if (!id.has_value()) id = UniValue::VNULL;
...
static bool HTTPReq_JSONRPC(const std::any& context, HTTPRequest* req)
{
    ...
    if (!RPCAuthorized(authHeader.second, jreq.authUser)) {
        LogInfo(HgLog::RPC, "ThreadRPCServer incorrect password attempt from %s\n", jreq.peerAddr);
    ...
    try {
        ...
        if (!user_has_whitelist && g_rpc_whitelist_default) {
            LogInfo(HgLog::RPC, "RPC User %s not allowed to call any methods\n", jreq.authUser);
        ...
        // singleton request
            ...
            if (user_has_whitelist && !g_rpc_whitelist[jreq.authUser].count(jreq.strMethod)) {
                LogInfo(HgLog::RPC, "RPC User %s not allowed to call method %s\n", jreq.authUser, jreq.strMethod);
                req->WriteReply(HTTP_FORBIDDEN);
                return false;
            }

            reply = JSONRPCExec(jreq, /*catch_errors=*/true);
        ...
        // array of requests
            ...
            for (size_t i{0}; i < valRequest.size(); ++i) {
                ...
                jreq.id = UniValue::VNULL;
                try {
                    jreq.parse(valRequest[i]);
                    is_notification = jreq.IsNotification();
                    response = JSONRPCExec(jreq, /*catch_errors=*/true);
                } catch (UniValue& e) {
                    if (!jreq.id.has_value()) jreq.id = UniValue::VNULL;
                    response = JSONRPCReplyObj(NullUniValue, std::move(e), jreq.id);
                } catch (const std::exception& e) {
                    if (!jreq.id.has_value()) jreq.id = UniValue::VNULL;
                    response = JSONRPCReplyObj(NullUniValue, JSONRPCError(RPC_PARSE_ERROR, e.what()), jreq.id);
                }
                if (!is_notification) {
                    reply.push_back(std::move(response));
                }
            }
            // All-notification batches have no response.
            if (reply.size() == 0) {
                req->WriteReply(HTTP_NO_CONTENT);
                return true;
            }
...
static bool InitRPCAuthentication()
{
    if (gArgs.GetArg("-rpcpassword", "") == "")
    {
        ...
        if (cookie_perms_arg) {
            ...
            if (!perm_opt) {
                LogError(HgLog::RPC, "Invalid -rpccookieperms=%s; must be one of 'owner', 'group', or 'all'.", *cookie_perms_arg);
                return false;
            }
        ...
        if (strRPCUserColonPass.empty()) {
            LogInfo(HgLog::RPC, "RPC authentication cookie file generation is disabled.");
        } else {
            LogInfo(HgLog::RPC, "Using random cookie authentication.");
        }
    } else {
        LogInfo(HgLog::RPC, "Using rpcuser/rpcpassword authentication.\n");
        strRPCUserColonPass = gArgs.GetArg("-rpcuser", "") + ":" + gArgs.GetArg("-rpcpassword", "");
    }

    if (!gArgs.GetArgs("-rpcauth").empty()) {
        LogInfo(HgLog::RPC, "Using rpcauth authentication.\n");
        for (const std::string& rpcauth : gArgs.GetArgs("-rpcauth")) {
            ...
            if (fields.size() == 2 && salt_hmac.size() == 2) {
                ...
                LogInfo(HgLog::RPC, "Invalid -rpcauth argument.\n");
                return false;
            }
        }
    }
...
{
    LogDebug(HgLog::RPC, "Starting HTTP RPC server\n");
    if (!InitRPCAuthentication())
        return false;

    auto handle_rpc = [context](HTTPRequest* req, const std::string&) { return HTTPReq_JSONRPC(context, req); };
    RegisterHTTPHandler("/", true, handle_rpc);
    if (g_vault_init_interface.HasVaultSupport()) {
        RegisterHTTPHandler("/vault/", false, handle_rpc);
    }
...
{
    LogDebug(HgLog::RPC, "Interrupting HTTP RPC server\n");
}

void StopHTTPRPC()
{
    LogDebug(HgLog::RPC, "Stopping HTTP RPC server\n");
    UnregisterHTTPHandler("/", true);
    if (g_vault_init_interface.HasVaultSupport()) {
        UnregisterHTTPHandler("/vault/", false);
    }
