...
bool SerializeDB(Stream& stream, const Data& data)
{
    ...
    try {
        ...
        LogError(HgLog::PEERBOOK, "%s: Serialize or I/O error - %s\n", __func__, e.what());
        return false;
    }
...
bool SerializeFileDB(const std::string& prefix, const fs::path& path, const Data& data)
{
    ...
    if (fileout.IsNull()) {
        fileout.fclose();
        fs::remove_quiet(pathTmp);
        LogError(HgLog::PEERBOOK, "%s: Failed to open file %s\n", __func__, fs::PathToString(pathTmp));
        return false;
    }
    ...
    if (!fileout.Commit()) {
        fileout.fclose();
        fs::remove_quiet(pathTmp);
        LogError(HgLog::PEERBOOK, "%s: Failed to flush file %s\n", __func__, fs::PathToString(pathTmp));
        return false;
    }
    fileout.fclose();
    ...
    if (!RenameOver(pathTmp, path)) {
        fs::remove_quiet(pathTmp);
        LogError(HgLog::PEERBOOK, "%s: Rename-into-place failed\n", __func__);
        return false;
    }
...
bool CBanDB::Read(banmap_t& banSet)
{
    ...
    if (!common::ReadSettings(m_banlist_json, settings, errors)) {
        for (const auto& err : errors) {
            LogInfo(HgLog::PEERBOOK, "Cannot load banlist %s: %s\n", fs::PathToString(m_banlist_json), err);
        }
        return false;
    }
    ...
    try {
        ...
        LogInfo(HgLog::PEERBOOK, "Cannot parse banlist %s: %s\n", fs::PathToString(m_banlist_json), e.what());
        return false;
    }
...
util::Result<std::unique_ptr<AddrMan>> LoadAddrman(const NetGroupManager& netgroupman, const ArgsManager& args)
{
    ...
    try {
        ...
        LogInfo(HgLog::PEERBOOK, "Loaded %i addresses from peers.dat  %dms\n", addrman->Size(), Ticks<std::chrono::milliseconds>(SteadyClock::now() - start));
    } catch (const DbNotFoundError&) {
        // Addrman can be in an inconsistent state after failure, reset it
        addrman = std::make_unique<AddrMan>(netgroupman, deterministic, /*consistency_check_ratio=*/check_addrman);
        LogInfo(HgLog::PEERBOOK, "Creating peers.dat because the file was not found (%s)\n", fs::quoted(fs::PathToString(path_addr)));
        ...
        // Addrman can be in an inconsistent state after failure, reset it
        addrman = std::make_unique<AddrMan>(netgroupman, deterministic, /*consistency_check_ratio=*/check_addrman);
        LogInfo(HgLog::PEERBOOK, "Creating new peers.dat because the file version was not compatible (%s). Original backed up to peers.dat.bak\n", fs::quoted(fs::PathToString(path_addr)));
        DumpPeerAddresses(args, *addrman);
    } catch (const std::exception& e) {
...
{
    LOG_TIME_SECONDS_WITH_CATEGORY(strprintf("Flush %d outbound block-relay-only peer addresses to anchors.dat", anchors.size()), HgLog::PEERBOOK);
...
std::vector<CAddress> ReadAnchors(const fs::path& anchors_db_path)
{
    ...
    try {
        ...
        LogInfo(HgLog::PEERBOOK, "Loaded %i addresses from %s\n", anchors.size(), fs::quoted(fs::PathToString(anchors_db_path.filename())));
    } catch (const std::exception&) {
        anchors.clear();
    }

    fs::remove_quiet(anchors_db_path);
    return anchors;
}
