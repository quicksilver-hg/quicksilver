...
static std::vector<CAddress> ConvertSeeds(const std::vector<uint8_t> &vSeedsIn)
{
    ...
    while (!s.eof()) {
        ...
        LogDebug(HgLog::MESH, "Added hardcoded seed: %s\n", addr.ToStringAddrPort());
...
std::optional<CService> GetLocalAddrForPeer(CNode& node)
{
    ...
    if (addrLocal.IsRoutable()) {
        LogDebug(HgLog::MESH, "Advertising address %s to peer=%d\n", addrLocal.ToStringAddrPort(), node.GetId());
        return addrLocal;
    }
...
bool AddLocal(const CService& addr_, int nScore)
{
    ...
    LogInfo(HgLog::MESH, "ready local_addr=%s score=%i\n", addr.ToStringAddrPort(), nScore);
...
{
    LOCK(g_maplocalhost_mutex);
    LogInfo(HgLog::MESH, "unlinked local_addr=%s\n", addr.ToStringAddrPort());
...
static CService GetBindAddress(const Sock& sock)
{
    ...
    if (!sock.GetSockName((struct sockaddr*)&sockaddr_bind, &sockaddr_bind_len)) {
        ...
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "getsockname failed\n");
    }
    return addr_bind;
}
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, ConnectionType conn_type, bool use_v2transport)
{
    ...
    if (pszDest == nullptr) {
        ...
        if (pnode)
        {
            LogInfo(HgLog::MESH, "rejected reason=already-connected\n");
            return nullptr;
        }
    }

    LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "trying %s connection %s lastseen=%.1fhrs\n",
        ...
        if (!resolved.empty()) {
            ...
            for (const auto& r : resolved) {
                ...
                if (!addrConnect.IsValid()) {
                    LogDebug(HgLog::MESH, "Resolver returned invalid address %s for %s\n", addrConnect.ToStringAddrPort(), pszDest);
                    return nullptr;
                }
                ...
                CNode* pnode = FindNode(static_cast<CService>(addrConnect));
                if (pnode) {
                    LogInfo(HgLog::MESH, "rejected reason=already-connected dest=%s peer_addr=%s\n", pszDest, addrConnect.ToStringAddrPort());
                    return nullptr;
                }
    ...
    for (auto& target_addr: connect_to) {
        if (target_addr.IsValid()) {
            ...
            if (target_addr.IsI2P() && use_proxy) {
                ...
                LogPrintLevel(HgLog::PROXY, HgLog::Level::Debug, "Using proxy: %s to connect to %s\n", proxy.ToString(), target_addr.ToStringAddrPort());
...
void CNode::CloseSocketDisconnect()
{
    ...
    LOCK(m_sock_mutex);
    if (m_sock) {
        LogDebug(HgLog::MESH, "Resetting socket for peer=%d%s", GetId(), LogIP(fLogIPs));
        m_sock.reset();
...
void CConnman::AddWhitelistPermissionFlags(NetPermissionFlags& flags, const CNetAddr &addr, const std::vector<NetWhitelistPermissions>& ranges) const {
    ...
    if (NetPermissions::HasFlag(flags, NetPermissionFlags::Implicit)) {
        ...
        if (m_implicit_whitelist_relay) NetPermissions::AddFlag(flags, NetPermissionFlags::Relay);
...
int V1Transport::readHeader(Span<const uint8_t> msg_bytes)
{
    ...
    try {
        ...
        LogDebug(HgLog::MESH, "Header error: Unable to deserialize, peer=%d\n", m_node_id);
        return -1;
    }

    // Check start string, network magic
    if (hdr.pchMessageStart != m_magic_bytes) {
        LogDebug(HgLog::MESH, "Header error: Wrong MessageStart %s received, peer=%d\n", HexStr(hdr.pchMessageStart), m_node_id);
        return -1;
    }

    // reject messages larger than MAX_SIZE or MAX_PROTOCOL_MESSAGE_LENGTH
    if (hdr.nMessageSize > MAX_SIZE || hdr.nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
        LogDebug(HgLog::MESH, "Header error: Size too large (%s, %u bytes), peer=%d\n", SanitizeString(hdr.GetMessageType()), hdr.nMessageSize, m_node_id);
        return -1;
    }
...
CNetMessage V1Transport::GetReceivedMessage(const std::chrono::microseconds time, bool& reject_message)
{
    ...
    if (memcmp(hash.begin(), hdr.pchChecksum, CMessageHeader::CHECKSUM_SIZE) != 0) {
        LogDebug(HgLog::MESH, "Header error: Wrong checksum (%s, %u bytes), expected %s was %s, peer=%d\n",
        ...
        LogDebug(HgLog::MESH, "Header error: Invalid message type (%s, %u bytes), peer=%d\n",
 ...
 *
 * Assigned ids this node does not speak are empty slots so later short ids stay
 * put. Other messages get ignored anyway — whether we know how to decode them or not.
 */
const std::array<std::string, 33> V2_MESSAGE_IDS = {
    ...
    "", // BIP324 id 5; unused so later short ids stay stable
    "", // BIP324 id 6 ("filteradd"); BIP37 not spoken
    "", // BIP324 id 7 ("filterclear"); BIP37 not spoken
    "", // BIP324 id 8 ("filterload"); BIP37 not spoken
    ...
    NetMsgType::HEADERS,
    NetMsgType::INV,
    "", // BIP324 id 15; the BIP35 pool-request message is not spoken
    "", // BIP324 id 16 ("merkleblock"); BIP37 not spoken
    NetMsgType::NOTFOUND,
    NetMsgType::PING,
    NetMsgType::PONG,
...
bool V2Transport::ProcessReceivedKeyBytes() noexcept
{
    ...
    if (!m_initiating && m_recv_buffer.size() >= OFFSET + MATCH.size()) {
        if (std::equal(MATCH.begin(), MATCH.end(), m_recv_buffer.begin() + OFFSET)) {
            LogDebug(HgLog::MESH, "V2 transport error: V1 peer with wrong MessageStart %s\n",
...
bool V2Transport::ProcessReceivedGarbageBytes() noexcept
{
    ...
    if (m_recv_buffer.size() >= BIP324Cipher::GARBAGE_TERMINATOR_LEN) {
        if (std::ranges::equal(MakeByteSpan(m_recv_buffer).last(BIP324Cipher::GARBAGE_TERMINATOR_LEN), m_cipher.GetReceiveGarbageTerminator())) {
            ...
            LogDebug(HgLog::MESH, "V2 transport error: missing garbage terminator, peer=%d\n", m_nodeid);
            return false;
        } else {
...
bool V2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == BIP324Cipher::LENGTH_LEN) {
        ...
        if (m_recv_len > MAX_CONTENTS_LEN) {
            LogDebug(HgLog::MESH, "V2 transport error: packet too large (%u bytes), peer=%d\n", m_recv_len, m_nodeid);
            return false;
        }
        ...
        if (!ret) {
            LogDebug(HgLog::MESH, "V2 transport error: packet decryption failure (%u bytes), peer=%d\n", m_recv_len, m_nodeid);
            return false;
        }
...
CNetMessage V2Transport::GetReceivedMessage(std::chrono::microseconds time, bool& reject_message) noexcept
{
    ...
    if (msg_type) {
        ...
        LogDebug(HgLog::MESH, "V2 transport error: invalid message type (%u bytes contents), peer=%d\n", m_recv_decode_buffer.size(), m_nodeid);
        reject_message = true;
    }
...
void V2Transport::MarkBytesSent(size_t bytes_sent) noexcept
{
    ...
    if (m_send_state == SendState::AWAITING_KEY && m_send_pos == 0 && bytes_sent > 0) {
        LogDebug(HgLog::MESH, "start sending v2 handshake to peer=%d\n", m_nodeid);
    }
...
bool CConnman::AttemptToEvictConnection()
{
    ...
    for (CNode* pnode : m_nodes) {
        if (pnode->GetId() == *node_id_to_evict) {
            LogDebug(HgLog::MESH, "selected %s connection for eviction, %s", pnode->ConnectionTypeAsString(), pnode->DisconnectMsg(fLogIPs));
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if (!sock) {
        ...
        if (nErr != WSAEWOULDBLOCK) {
            LogWarning(HgLog::MESH, "rejected reason=accept-failed detail=%s\n", NetworkErrorString(nErr));
        }
        return;
    }
    ...
    if (!addr.SetSockAddr((const struct sockaddr*)&sockaddr, len)) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "Unknown socket family\n");
    } else {
                                            ...
                                            const CService& addr)
{
    ...
    if (!fNetworkActive) {
        LogDebug(HgLog::MESH, "connection from %s dropped: not accepting new connections\n", addr.ToStringAddrPort());
        return;
    }

    if (!sock->IsSelectable()) {
        LogInfo(HgLog::MESH, "rejected reason=non-selectable-socket peer_addr=%s\n", addr.ToStringAddrPort());
        return;
    }
    ...
    if (sock->SetSockOpt(IPPROTO_TCP, TCP_NODELAY, &on, sizeof(on)) == SOCKET_ERROR) {
        LogDebug(HgLog::MESH, "connection from %s: unable to set TCP_NODELAY, continuing anyway\n",
                 addr.ToStringAddrPort());
    }
    ...
    {
        LogDebug(HgLog::MESH, "connection from %s dropped (banned)\n", addr.ToStringAddrPort());
        return;
    }
    ...
    {
        LogDebug(HgLog::MESH, "connection from %s dropped (discouraged)\n", addr.ToStringAddrPort());
        return;
    }
    ...
    if (nInbound >= m_max_inbound)
    {
        if (!AttemptToEvictConnection()) {
            ...
            LogDebug(HgLog::MESH, "failed to find an eviction candidate - connection dropped (full)\n");
            return;
        }
    }
    ...
    LogDebug(HgLog::MESH, "connection from %s accepted\n", addr.ToStringAddrPort());
        ...
        if (!fNetworkActive) {
            ...
            for (CNode* pnode : m_nodes) {
                if (!pnode->fDisconnect) {
                    LogDebug(HgLog::MESH, "Network not active, %s\n", pnode->DisconnectMsg(fLogIPs));
                    pnode->fDisconnect = true;
                }
            }
        }
        ...
        for (CNode* pnode : nodes_copy)
        {
            if (pnode->fDisconnect)
            {
                ...
                if (pnode->m_transport->ShouldReconnectV1()) {
                    ...
                    LogDebug(HgLog::MESH, "retrying with v1 transport protocol for peer=%d\n", pnode->GetId());
                }
...
bool CConnman::InactivityCheck(const CNode& node) const
{
    ...
    if (!node.fSuccessfullyConnected) {
        if (node.m_transport->GetInfo().transport_type == TransportProtocolType::DETECTING) {
            LogDebug(HgLog::MESH, "V2 handshake timeout, %s\n", node.DisconnectMsg(fLogIPs));
        } else {
            LogDebug(HgLog::MESH, "version handshake timeout, %s\n", node.DisconnectMsg(fLogIPs));
        }
        return true;
    }
    ...
    for (CNode* pnode : nodes) {
        ...
        if (recvSet || errorSet)
        {
            ...
            else if (nBytes == 0)
            {
                ...
                if (!pnode->fDisconnect) {
                    LogDebug(HgLog::MESH, "socket closed, %s\n", pnode->DisconnectMsg(fLogIPs));
                }
                pnode->CloseSocketDisconnect();
            }
            else if (nBytes < 0)
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    if (!gArgs.GetArgs("-seednode").empty()) {
        ...
        LogInfo(HgLog::MESH, "bootstrapping source=seednode timeout_s=%d\n", SEEDNODE_TIMEOUT.count());
        while (!interruptNet) {
            ...
            if (NodeClock::now() > start + SEEDNODE_TIMEOUT) {
                LogInfo(HgLog::MESH, "bootstrapping source=dnsseed reason=seednode-insufficient-peers\n");
                break;
            }

            outbound_connection_count = GetFullOutboundConnCount();
            if (outbound_connection_count >= SEED_OUTBOUND_CONNECTION_THRESHOLD) {
                LogInfo(HgLog::MESH, "ready source=seednode\n");
                break;
            }
        }
    }
    ...
    // Proceed with dnsseeds if seednodes hasn't reached the target
    if (outbound_connection_count < SEED_OUTBOUND_CONNECTION_THRESHOLD || seeds_right_now) {
        ...
        for (const std::string& seed : seeds) {
            if (seeds_right_now == 0) {
                ...
                if (addrman.Size() > 0) {
                    LogInfo(HgLog::MESH, "bootstrapping source=dnsseed wait_s=%d\n", seeds_wait_time.count());
                    ...
                    while (to_wait.count() > 0) {
                        ...
                        if (GetFullOutboundConnCount() >= SEED_OUTBOUND_CONNECTION_THRESHOLD) {
                            if (found > 0) {
                                LogInfo(HgLog::MESH, "loaded source=dnsseed addresses=%d\n", found);
                                LogInfo(HgLog::MESH, "ready source=dnsseed\n");
                            } else {
                                LogInfo(HgLog::MESH, "ready source=cache reason=dnsseed-skipped\n");
                            }
                            return;
                        }
                    }
            ...
            if (!fNetworkActive) {
                LogInfo(HgLog::MESH, "halted stage=dnsseed reason=network-inactive\n");
                do {
                    if (!interruptNet.sleep_for(std::chrono::seconds{1})) return;
                } while (!fNetworkActive);
            }

            LogInfo(HgLog::MESH, "loading source=dnsseed seed=%s\n", seed);
        ...
        LogInfo(HgLog::MESH, "ready source=cache reason=enough-peers\n");
    }
}
...
void CConnman::DumpAddresses()
{
    ...
    LogDebug(HgLog::MESH, "Flushed %d addresses to peers.dat  %dms\n",
...
void CConnman::SetTryNewOutboundPeer(bool flag)
{
    ...
    LogDebug(HgLog::MESH, "setting try another outbound peer=%s\n", flag ? "true" : "false");
}

void CConnman::StartExtraBlockRelayPeers()
{
    LogDebug(HgLog::MESH, "enabling extra block-relay-only peers\n");
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect, Span<const std::string> seed_nodes)
{
    ...
    if (!add_fixed_seeds) {
        LogInfo(HgLog::MESH, "ready fixedseeds=disabled\n");
    }
    ...
    while (!interruptNet)
    {
        if (add_addr_fetch) {
            ...
            if (addrman.Size() == 0) {
                LogInfo(HgLog::MESH, "bootstrapping source=seednode seed=%s reason=empty-addrman\n", seed);
            } else {
                LogInfo(HgLog::MESH, "bootstrapping source=seednode seed=%s reason=addrman-connect-failed elapsed_s=%d\n", seed, ADD_NEXT_SEEDNODE.count());
            }
        }
        ...
        if (add_fixed_seeds && !fixed_seed_networks.empty()) {
            ...
            if (GetTime<std::chrono::seconds>() > start + std::chrono::minutes{1}) {
                ...
                LogInfo(HgLog::MESH, "bootstrapping source=fixedseeds reason=addrman-empty\n");
            }
            ...
            // Perform cheap checks before locking a mutex.
                ...
                if (m_added_node_params.empty()) {
                    add_fixed_seeds_now = true;
                    LogInfo(HgLog::MESH, "bootstrapping source=fixedseeds reason=dnsseed-disabled\n");
                }
            }
            ...
            if (add_fixed_seeds_now) {
                ...
                LogInfo(HgLog::MESH, "loaded source=fixedseeds addresses=%d\n", seed_addrs.size());
            }
        }
        ...
        while (!interruptNet)
        {
            if (anchor && !m_anchors.empty()) {
                ...
                addrConnect = addr;
                LogDebug(HgLog::MESH, "Trying to make an anchor connection to %s\n", addrConnect.ToStringAddrPort());
                break;
            }
            ...
            if (AddedNodesContain(addr)) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "Not making automatic %s%s connection to %s peer selected for manual (addnode) connection%s\n",
        ...
        if (addrConnect.IsValid()) {
            if (fFeeler) {
                ...
                LogDebug(HgLog::MESH, "Making feeler connection to %s\n", addrConnect.ToStringAddrPort());
            }

            if (preferred_net != std::nullopt) LogDebug(HgLog::MESH, "Making network specific connection to %s on %s.\n", addrConnect.ToStringAddrPort(), GetNetworkName(preferred_net.value()));
...
bool CConnman::BindListenPort(const CService& addrBind, bilingual_str& strError, NetPermissionFlags permissions)
{
    ...
    if (!addrBind.GetSockAddr((struct sockaddr*)&sockaddr, &len))
    {
        ...
        LogPrintLevel(HgLog::MESH, HgLog::Level::Error, "%s\n", strError.original);
        return false;
    }
    ...
    if (sock->SetSockOpt(SOL_SOCKET, SO_REUSEADDR, (sockopt_arg_type)&nOne, sizeof(int)) == SOCKET_ERROR) {
        const std::string sockopt_err{NetworkErrorString(WSAGetLastError())};
        strError = Untranslated(strprintf("Error setting SO_REUSEADDR on socket: %s, continuing anyway", sockopt_err));
        LogWarning(HgLog::MESH, "ready sockopt=so_reuseaddr ok=0 action=continue detail=%s\n", sockopt_err);
    }
    ...
    if (addrBind.IsIPv6()) {
        ...
        if (sock->SetSockOpt(IPPROTO_IPV6, IPV6_V6ONLY, (sockopt_arg_type)&nOne, sizeof(int)) == SOCKET_ERROR) {
            const std::string sockopt_err{NetworkErrorString(WSAGetLastError())};
            strError = Untranslated(strprintf("Error setting IPV6_V6ONLY on socket: %s, continuing anyway", sockopt_err));
            LogWarning(HgLog::MESH, "ready sockopt=ipv6_v6only ok=0 action=continue detail=%s\n", sockopt_err);
        }
#endif
#ifdef WIN32
        ...
        if (sock->SetSockOpt(IPPROTO_IPV6, IPV6_PROTECTION_LEVEL, (const char*)&nProtLevel, sizeof(int)) == SOCKET_ERROR) {
            const std::string sockopt_err{NetworkErrorString(WSAGetLastError())};
            strError = Untranslated(strprintf("Error setting IPV6_PROTECTION_LEVEL on socket: %s, continuing anyway", sockopt_err));
            LogWarning(HgLog::MESH, "ready sockopt=ipv6_protection_level ok=0 action=continue detail=%s\n", sockopt_err);
        }
#endif
    }
    ...
    LogInfo(HgLog::MESH, "ready bind_addr=%s\n", addrBind.ToStringAddrPort());
...
void Discover()
{
    ...
    for (const CNetAddr &addr: GetLocalAddresses()) {
        if (AddLocal(addr, LOCAL_IF))
            LogInfo(HgLog::MESH, "ready local_addr=%s source=discover\n", addr.ToStringAddr());
    }
}

void CConnman::SetNetworkActive(bool active)
{
    LogInfo(HgLog::MESH, "ready network_active=%d\n", active);
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (m_use_addrman_outgoing) {
        ...
        LogInfo(HgLog::MESH, "loaded anchors=%i\n", m_anchors.size());
    }

    if (m_client_interface) {
        m_client_interface->InitMessage(_("Starting signal threads…"));
    }
    ...
    if (!gArgs.GetBoolArg("-dnsseed", DEFAULT_DNSSEED))
        LogInfo(HgLog::MESH, "ready dnsseed=disabled\n");
    else
...
void CConnman::StopNodes()
{
    ...
    for (CNode* pnode : nodes) {
        LogDebug(HgLog::MESH, "unlinked %s", pnode->DisconnectMsg(fLogIPs));
        pnode->CloseSocketDisconnect();
        DeleteNode(pnode);
    }
...
bool CConnman::DisconnectNode(const std::string& strNode)
{
    ...
    if (CNode* pnode = FindNode(strNode)) {
        LogDebug(HgLog::MESH, "disconnect by address%s match, %s", (fLogIPs ? strprintf("=%s", strNode) : ""), pnode->DisconnectMsg(fLogIPs));
        pnode->fDisconnect = true;
        return true;
    }
    return false;
...
bool CConnman::DisconnectNode(const CSubNet& subnet)
{
    ...
    for (CNode* pnode : m_nodes) {
        if (subnet.Match(pnode->addr)) {
            LogDebug(HgLog::MESH, "disconnect by subnet%s match, %s", (fLogIPs ? strprintf("=%s", subnet.ToString()) : ""), pnode->DisconnectMsg(fLogIPs));
            pnode->fDisconnect = true;
            disconnected = true;
        }
    }
...
bool CConnman::DisconnectNode(NodeId id)
{
    ...
    for(CNode* pnode : m_nodes) {
        if (id == pnode->GetId()) {
            LogDebug(HgLog::MESH, "disconnect by id, %s", pnode->DisconnectMsg(fLogIPs));
            pnode->fDisconnect = true;
            return true;
        }
    }
      ...
      m_i2p_sam_session{std::move(node_opts.i2p_sam_session)}
{
    ...
    if (fLogIPs) {
        LogDebug(HgLog::MESH, "Added connection to %s peer=%d\n", m_addr_name, id);
    } else {
        LogDebug(HgLog::MESH, "Added connection peer=%d\n", id);
    }
}
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    LogDebug(HgLog::MESH, "sending %s (%d bytes) peer=%d\n", msg.m_type, nMessageSize, pnode->GetId());
