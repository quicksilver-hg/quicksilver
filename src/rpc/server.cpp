...
void StartRPC()
{
    LogDebug(HgLog::RPC, "Starting RPC\n");
    g_rpc_running = true;
}
...
void InterruptRPC()
{
    ...
    std::call_once(g_rpc_interrupt_flag, []() {
        LogDebug(HgLog::RPC, "Interrupting RPC\n");
...
void StopRPC()
{
    ...
    std::call_once(g_rpc_stop_flag, [&]() {
        LogDebug(HgLog::RPC, "Stopping RPC\n");
        WITH_LOCK(g_deadline_timers_mutex, deadlineTimers.clear());
        DeleteAuthCookie();
        LogDebug(HgLog::RPC, "RPC stopped.\n");
    });
}
    ...
    for (const auto& [argNamePattern, named_only]: argNames) {
        ...
        if (!options.empty() || fr != argsIn.end()) {
            for (int i = 0; i < hole; ++i) {
                // Fill gaps before a later named parameter with JSON nulls.
                // Trailing omissions require no placeholder.
...
void RPCRunLater(const std::string& name, std::function<void()> func, int64_t nSeconds)
{
    ...
    LogDebug(HgLog::RPC, "queue run of timer %s in %i seconds (using %s)\n", name, nSeconds, timerInterface->Name());
