...
UniValue blockheaderToJSON(const CBlockIndex& tip, const CBlockIndex& blockindex, const uint256 pow_limit)
{
    ...
    // Quicksilver: expose the Cuckatoo 42-cycle block proof.
    UniValue cuckatoo_cycle(UniValue::VARR);
    for (uint32_t edge : blockindex.nCycle) cuckatoo_cycle.push_back(static_cast<uint64_t>(edge));
    ...
    // math and the double below is lossy); "congestion_multiplier" is the human-readable
    // ratio, display-only and never consensus math.
    result.pushKV("congestion", static_cast<uint64_t>(blockindex.m_congestion));
    result.pushKV("congestion_multiplier",
                  double(blockindex.m_congestion) / double(CONGESTION_ONE));
...
UniValue blockToJSON(BlockManager& blockman, const CBlock& block, const CBlockIndex& tip, const CBlockIndex& blockindex, TxVerbosity verbosity, const uint256 pow_limit)
{
    ...
    switch (verbosity) {
        ...
        case TxVerbosity::SHOW_DETAILS_AND_PREVOUT:
            ...
            bool have_undo{verbosity == TxVerbosity::SHOW_DETAILS_AND_PREVOUT && is_not_pruned && WITH_LOCK(::cs_main, return blockindex.nStatus & BLOCK_HAVE_UNDO)};
...
static RPCHelpMan waitfornewblock()
{
    return RPCHelpMan{"waitfornewblock",
                ...
                "\nMake sure to use no RPC timeout (quicksilver-cli -rpcclienttimeout=0)",
                {
                ...
                },
                {
                    RPCResult{"for verbose = true",
                        RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::ARR, "cuckatoo_cycle", "The Cuckatoo 42-cycle block proof (edge indices)",
                            {
                                {RPCResult::Type::NUM, "", "An edge index"},
                            }},
                            {RPCResult::Type::STR_HEX, "bits", "nBits: compact representation of the block difficulty target"},
                            {RPCResult::Type::STR_HEX, "target", "The difficulty target"},
                            {RPCResult::Type::NUM, "difficulty", "The difficulty"},
                            {RPCResult::Type::NUM, "congestion", "Quicksilver header field nCongestion: the EIP-1559 congestion multiplier as the exact fixed-point integer (65536 = 1.0). Use this, not congestion_multiplier, to build or verify a block on top of this one"},
                            {RPCResult::Type::NUM, "congestion_multiplier", "Quicksilver per-tx PoW congestion multiplier as a ratio (1.0 = hardware floor); display only, lossy"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (chainHeight < chainman.GetParams().PruneAfterHeight()) {
        ...
        LogDebug(HgLog::RPC, "Attempt to prune blocks close to the tip.  Retaining the minimum number of blocks.\n");
...
{
    if (g_coin_stats_index && index_requested) {
        if (pindex) {
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::OBJ, "block_info", /*optional=*/true, "Info on amounts in the block at this block height (only available if coinstatsindex is used)",
                        {
                            ...
                            {RPCResult::Type::STR_AMOUNT, "coinbase", "Total coinbase output amount of this block"},
                            ...
                            {RPCResult::Type::OBJ, "unspendables", "Detailed view of the unspendable categories",
                            {
                                ...
                                {RPCResult::Type::STR_AMOUNT, "unclaimed_rewards", "Subsidy or mint allowance that miners did not claim in their coinbase transaction"},
                            }}
                        }},
                    }},
                RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const CBlockIndex* pindex{nullptr};
    const CoinStatsHashType hash_type{request.params[0].isNull() ? CoinStatsHashType::MUHASH : ParseHashType(request.params[0].get_str())};
    ...
    if (maybe_stats.has_value()) {
        ...
        if (hash_type == CoinStatsHashType::MUHASH) {
            ret.pushKV("muhash", stats.hash.GetHex());
        }
...
static RPCHelpMan gettxout()
{
    return RPCHelpMan{"gettxout",
        "\nReturns details about an unspent transaction output.\n",
        {
            ...
            {"include_relaypool", RPCArg::Type::BOOL, RPCArg::Default{true}, "Whether to include the relay pool. Note that an unspent output that is spent in the relay pool won't appear."},
        },
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fRelayPool = true;
    if (!request.params[2].isNull())
        fRelayPool = request.params[2].get_bool();
    ...
    if (fRelayPool) {
        ...
        LOCK(relaypool.cs);
        CCoinsViewRelayPool view(coins_view, relaypool);
        if (!relaypool.isSpent(out)) coin = view.GetCoin(out);
    } else {
    ...
    if (coin->nHeight == RELAYPOOL_HEIGHT) {
    ...
    ret.pushKV("output_script", std::move(o));
...
const std::vector<RPCResult> RPCHelpForDeployment{
    ...
    {RPCResult::Type::BOOL, "active", "true if the rules are enforced for the relay pool and the next block"},
...
static RPCHelpMan getblockstats()
{
    return RPCHelpMan{"getblockstats",
                "\nCompute per block statistics for a given window. All amounts are in cinnabar.\n"
            ...
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::NUM, "witness_total_size", /*optional=*/true, "Total size of all witness transactions"},
                {RPCResult::Type::NUM, "witness_total_weight", /*optional=*/true, "Total weight of all witness transactions"},
                {RPCResult::Type::NUM, "witness_txs", /*optional=*/true, "The number of witness transactions"},
                {RPCResult::Type::NUM, "time", /*optional=*/true, "The block time"},
                {RPCResult::Type::NUM, "total_out", /*optional=*/true, "Total amount in all non-coinbase outputs"},
                ...
                RPCExamples{
                    HelpExampleCli("getblockstats", R"('"00000000c937983704a73af28acdec37b049d214adbda81d7e2a3dd146f6ed09"' '["mintxsize","avgtxsize"]')") +
                    HelpExampleCli("getblockstats", R"(1000 '["mintxsize","avgtxsize"]')") +
                    HelpExampleRpc("getblockstats", R"("00000000c937983704a73af28acdec37b049d214adbda81d7e2a3dd146f6ed09", ["mintxsize","avgtxsize"])") +
                    HelpExampleRpc("getblockstats", R"(1000, ["mintxsize","avgtxsize"])")
                },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    ...
    if (!request.params[1].isNull()) {
        ...
        for (unsigned int i = 0; i < stats_univalue.size(); i++) {
            ...
            if (stat == "swtotal_size") {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Use witness_total_size instead of swtotal_size");
            }
            if (stat == "swtotal_weight") {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Use witness_total_weight instead of swtotal_weight");
            }
            if (stat == "swtxs") {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Use witness_txs instead of swtxs");
            }
            stats.insert(stat);
        }
    }
    ...
    const bool loop_inputs = do_all ||
        SetHasKeys(stats, "utxo_increase", "utxo_increase_actual", "utxo_size_inc", "utxo_size_inc_actual");
    const bool loop_outputs = do_all || loop_inputs || stats.count("total_out");
    const bool do_calculate_size = do_mediantxsize ||
        SetHasKeys(stats, "total_size", "avgtxsize", "mintxsize", "maxtxsize", "witness_total_size");
    const bool do_calculate_weight = do_all || SetHasKeys(stats, "total_weight", "witness_total_weight");
    const bool do_calculate_witness = do_all || SetHasKeys(stats, "witness_txs", "witness_total_size", "witness_total_weight");
    ...
    int64_t outputs = 0;
    int64_t witness_total_size = 0;
    int64_t witness_total_weight = 0;
    int64_t witness_txs = 0;
    ...
    for (size_t i = 0; i < block.vtx.size(); ++i) {
        ...
        if (loop_outputs) {
            for (const CTxOut& out : tx->vout) {
                ...
                // The genesis block does not change the UTXO set count.
        ...
        if (do_calculate_witness && tx->HasWitness()) {
            ++witness_txs;
            witness_total_size += tx_size;
            witness_total_weight += weight;
        }
        ...
        if (loop_inputs) {
            ...
            const CAmount value_delta = tx_total_in - tx_total_out;
            CHECK_NONFATAL(MoneyRange(value_delta));
        }
    }
    ...
    ret_all.pushKV("witness_total_size", witness_total_size);
    ret_all.pushKV("witness_total_weight", witness_total_weight);
    ret_all.pushKV("witness_txs", witness_txs);
...
{
    const std::string EXAMPLE_DESCRIPTOR_RAW = "addr(" + EXAMPLE_ADDRESS[0] + ")#pw2393jx";
    ...
    return RPCHelpMan{"scantxoutset",
        ...
        "    combo(<pubkey>)                      P2PK, P2PKH, and P2WPKH outputs for the given pubkey\n"
        ...
        "\nIn the above, <pubkey> either refers to a fixed public key in hexadecimal notation, or to a qpub/qprv optionally followed by one\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (action == "status") {
        ...
        for (const auto& it : coins) {
            ...
            unspent.pushKV("output_script", HexStr(txo.scriptPubKey));
...
static RPCHelpMan scanblocks()
{
    return RPCHelpMan{"scanblocks",
        ...
        "This call may take several minutes. Make sure to use no RPC timeout (quicksilver-cli -rpcclienttimeout=0)",
        {
            scan_action_arg_desc,
            scan_objects_arg_desc,
        ...
        RPCExamples{
            HelpExampleCli("scanblocks", "start '[\"addr(shg1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy4tf65p7)\"]' 300000") +
            HelpExampleCli("scanblocks", "start '[\"addr(shg1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy4tf65p7)\"]' 100 150 basic") +
            HelpExampleCli("scanblocks", "status") +
            HelpExampleRpc("scanblocks", "\"start\", [\"addr(shg1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy4tf65p7)\"], 300000") +
            HelpExampleRpc("scanblocks", "\"start\", [\"addr(shg1q4u4nsgk6ug0sqz7r3rj9tykjxrsl0yy4tf65p7)\"], 100, 150, \"basic\"") +
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params[0].get_str() == "status") {
        ...
        // The chain tip above may include blocks whose validation-interface
        // notifications have not reached the filter index yet. Drain those
        // notifications before looking up the snapshotted range.
        ...
        do {
            ...
            if (!index->LookupFilterRange(start_block, end_range, filters)) {
                throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to read block filters from index");
            }
...
static RPCHelpMan getdescriptoractivity()
{
    return RPCHelpMan{"getdescriptoractivity",
        ...
        "This call may take several minutes. If you encounter timeouts, try specifying no RPC timeout (quicksilver-cli -rpcclienttimeout=0)",
        {
            ...
            scan_objects_arg_desc,
            {"include_relaypool", RPCArg::Type::BOOL, RPCArg::Default{true}, "Whether to include unconfirmed activity"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "", {
        ...
        RPCExamples{
            HelpExampleCli("getdescriptoractivity", "'[\"000000000000000000001347062c12fded7c528943c8ce133987e2e2f5a840ee\"]' '[\"addr(hg1qzl6nsgqzu89a66l50cvwapnkw5shh23zumhgem)\"]'")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    UniValue ret(UniValue::VOBJ);
    ...
    bool search_relaypool = true;
    if (!request.params[2].isNull()) {
        search_relaypool = request.params[2].get_bool();
    }
    ...
    if (search_relaypool) {
        ...
        for (const CTxRelayPoolEntry& e : relaypool.entryAll()) {
            ...
            for (size_t vin_idx = 0; vin_idx < tx->vin.size(); ++vin_idx) {
                ...
                if (!coin) {
                    // If not found in the chain, check the relaypool. Likely, this is a
                    // child transaction of another transaction in the relaypool.
                    CTransactionRef prev_tx = CHECK_NONFATAL(relaypool.get(txin.prevout.hash));
