...
const std::string EXAMPLE_ADDRESS[2] = {"hg1q09vm5lfy0j5reeulh4x5752q25uqqvz34hufdl", "hg1q02ad21edsxd23d32dfgqqsz4vv4nmtfzuklhy3"};
...
std::string RpcExampleUrl()
{
    return "http://127.0.0.1:" + util::ToString(CreateBaseChainParams(ChainType::MAIN)->RPCPort()) + "/";
}
...
{
    return "> quicksilver-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> quicksilver-cli -named " + methodname;
...
std::string HelpExampleRpc(const std::string& methodname, const std::string& args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "]}' -H 'content-type: application/json' " + RpcExampleUrl() + "\n";
}
...
std::string HelpExampleRpcNamed(const std::string& methodname, const RPCArgList& args)
{
    ...
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"2.0\", \"id\": \"curltest\", "
           "\"method\": \"" + methodname + "\", \"params\": " + params.write() + "}' -H 'content-type: application/json' " + RpcExampleUrl() + "\n";
}
...
CPubKey AddrToPubKey(const FillableSigningProvider& keystore, const std::string& addr_in)
{
    ...
    if (!vchPubKey.IsFullyValid()) {
       throw JSONRPCError(RPC_INTERNAL_ERROR, "Vault contains an invalid public key");
    }
    return vchPubKey;
}
...
CTxDestination AddAndGetMultisigDestination(const int required, const std::vector<CPubKey>& pubkeys, OutputType type, FlatSigningProvider& keystore, CScript& script_out)
{
    ...
    // Check if any keys are uncompressed. If so, the type is base58.
    for (const CPubKey& pk : pubkeys) {
        if (!pk.IsCompressed()) {
            type = OutputType::BASE58;
            break;
        }
    }

    if (type == OutputType::BASE58 && script_out.size() > MAX_SCRIPT_ELEMENT_SIZE) {
...
RPCErrorCode RPCErrorFromPSQTError(PSQTError err)
{
    switch (err) {
...
UniValue JSONRPCPSQTError(PSQTError err)
{
    return JSONRPCError(RPCErrorFromPSQTError(err), PSQTErrorString(err).original);
}
