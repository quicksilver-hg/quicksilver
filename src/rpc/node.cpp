...
static RPCHelpMan setmocktime()
{
    return RPCHelpMan{"setmocktime",
        "\nSet the local time to given timestamp (-sandbox only)\n",
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    if (!Params().IsMockableChain()) {
        throw std::runtime_error("setmocktime is for regression testing (-sandbox mode) only");
    }

    // For now, don't change mocktime if we're in the middle of validation, as
    // this could have an effect on relaypool time-based eviction, as well as
...
static RPCHelpMan mockscheduler()
{
    return RPCHelpMan{"mockscheduler",
        "\nBump the scheduler into the future (-sandbox only)\n",
        {
...
{
    if (!Params().IsMockableChain()) {
        throw std::runtime_error("mockscheduler is for regression testing (-sandbox mode) only");
    }
...
static RPCHelpMan logging()
{
    return RPCHelpMan{"logging",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    HgLog::CategoryMask original_log_categories = LogInstance().GetCategoryMask();
    ...
    HgLog::CategoryMask updated_log_categories = LogInstance().GetCategoryMask();
    HgLog::CategoryMask changed_log_categories = original_log_categories ^ updated_log_categories;

    // Update libevent logging if HgLog::LIBEVENT has changed.
    if (changed_log_categories & HgLog::LIBEVENT) {
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "quicksilver-cli and the GUI. There is no server-side difference.",
        {
