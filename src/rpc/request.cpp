...
/**
 * Quicksilver uses JSON-RPC 2.0.
 * It will only return an HTTP error code if an actual HTTP error is encountered
 * such as the endpoint is not found (404) or the request is not formatted correctly (400).
...
bool GenerateAuthCookie(std::string* cookie_out, std::optional<fs::perms> cookie_perms)
{
    ...
    if (!file.is_open()) {
        LogWarning(HgLog::RPC, "Unable to open cookie authentication file %s for writing", fs::PathToString(filepath_tmp));
        return false;
    }
    file << cookie;
    file.close();
    ...
    if (!RenameOver(filepath_tmp, filepath)) {
        LogWarning(HgLog::RPC, "Unable to rename cookie authentication file %s to %s", fs::PathToString(filepath_tmp), fs::PathToString(filepath));
        return false;
    }
    if (cookie_perms) {
        std::error_code code;
        ...
        if (code) {
            LogWarning(HgLog::RPC, "Unable to set permissions on cookie authentication file %s", fs::PathToString(filepath));
            return false;
        }
    }

    g_generated_cookie = true;
    LogInfo(HgLog::RPC, "Generated RPC authentication cookie %s\n", fs::PathToString(filepath));
    LogInfo(HgLog::RPC, "Permissions used for cookie: %s\n", PermsToSymbolicString(fs::status(filepath).permissions()));
...
void DeleteAuthCookie()
{
    try {
        ...
        LogInfo(HgLog::RPC, "%s: Unable to remove random auth cookie file: %s\n", __func__, fsbridge::get_filesystem_error_message(e));
    }
}
...
void JSONRPCRequest::parse(const UniValue& valRequest)
{
    ...
    if (fLogIPs)
        LogDebug(HgLog::RPC, "ThreadRPCServer method=%s user=%s peeraddr=%s\n", SanitizeString(strMethod),
            this->authUser, this->peerAddr);
    else
        LogDebug(HgLog::RPC, "ThreadRPCServer method=%s user=%s\n", SanitizeString(strMethod), this->authUser);
