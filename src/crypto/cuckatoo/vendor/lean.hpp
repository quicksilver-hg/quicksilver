...
#endif
#include "cuckatoo.h"
#include <compat/endian.h>    // htole32_internal — the tree's single portable endian impl
#include "siphashxN.h"
#include "graph.hpp"
#include <stdio.h>
#include "barrier.hpp"
...
typedef struct {
  u32 id;
  std::thread thread;   // ported from pthread_t so the solver builds under MSVC
  cuckoo_ctx *ctx;
} thread_ctx;
...
void *worker(void *vp) {
  ...
  for (round=0; round < ctx->ntrims; round++) {
    ...
    for (u32 part = 0; part <= PART_MASK; part++) {
      ...
      if (ctx->barrier()) return nullptr;  // barrier aborted -> stop this worker
