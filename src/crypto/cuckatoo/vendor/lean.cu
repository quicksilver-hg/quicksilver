...
static inline u32 host_ffs64(const u64 x) {
  u64 v = x;
  u32 n = 1; // 1-based; callers only pass nonzero x
  while ((v & 1) == 0) {
    v >>= 1;
    ++n;
  }
...
static inline u32 host_popcount64(const u64 x) {
  u64 v = x;
  u32 n = 0;
  while (v != 0) {
    n += (u32)(v & 1);
...
static inline u32 host_htole32(const u32 x) {
...
public:
  ...
  void setheadernonce(char * const headernonce, const u32 len, const u32 nonce) {
    if (mutatenonce) {
      ((u32 *)headernonce)[len/sizeof(u32)-1] = host_htole32(nonce); // place nonce at end
    }
...
#if !defined(CUCKATOO_NO_MAIN) && !defined(_WIN32)
#include <unistd.h>
#endif
