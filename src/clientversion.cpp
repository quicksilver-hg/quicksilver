 ...
 * for both quicksilver-daemon and quicksilver, to make it harder for attackers to
 * target servers or GUI users specifically.
 */
const std::string UA_NAME("Quicksilver");


#include <quicksilver-build-info.h>
// The <quicksilver-build-info.h>, which is generated by the build environment (cmake/script/GenerateBuildInfo.cmake),
...
// A tagged checkout -- the published artifact -- reports the tag alone, so a
// release built from the public source still reads as a clean "v0.1.0".
#ifdef BUILD_GIT_TAG
...
/**
 * Format the subversion field according to BIP 14.
 */
...
//! strPrefix is the copyright marker alone, without any year: "Copyright (C) "
//! or the "\xc2\xA9 " glyph. Each holder line supplies its own year range,
//! because they differ -- Quicksilver forked Bitcoin Core in 2026 and holds no
//! copyright reaching back to 2009.
std::string CopyrightHolders(const std::string& strPrefix)
{
    const auto copyright_devs = strprintf(_(COPYRIGHT_HOLDERS), COPYRIGHT_HOLDERS_SUBSTITUTION).translated;
    std::string strCopyrightHolders = strPrefix + strprintf("%i ", COPYRIGHT_YEAR) + copyright_devs;

    // Make sure Bitcoin Core copyright is not removed by accident
    if (copyright_devs.find("Bitcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf("%i-%i ", 2009, COPYRIGHT_YEAR) + "The Bitcoin Core developers";
    }
...
{
    const std::string URL_SOURCE_CODE = "<" + std::string{CLIENT_URL} + ">";

    return CopyrightHolders(std::string{_("Copyright (C)")} + " ") + "\n" +
           "\n" +
