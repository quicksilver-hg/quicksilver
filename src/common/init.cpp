...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // quicksilver.conf file just ignores the other file.)
        const fs::path orig_datadir_path{args.GetDataDirBase()};
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", QUICKSILVER_CONF_FILENAME), /*net_specific=*/false)};
        ...
        if (!fs::exists(base_path)) {
            // When creating a *new* datadir, also create a "vaults" subdirectory
            // so vaults are listed separately from other datadir files. Vault
            // code uses that subdirectory when it exists and does not create it
            // itself.
            fs::create_directories(base_path / "vaults");
        }
        const auto net_path{args.GetDataDirNet()};
        if (!fs::exists(net_path)) {
            fs::create_directories(net_path / "vaults");
        }

        // Show an error or warn/log if there is a quicksilver.conf file in the
        // datadir that is being ignored.
        const fs::path base_config_path = base_path / QUICKSILVER_CONF_FILENAME;
        if (fs::exists(base_config_path)) {
            if (orig_config_path.empty()) {
                LogInfo(HgLog::INIT,
                    ...
                    fs::quoted(QUICKSILVER_CONF_FILENAME));
