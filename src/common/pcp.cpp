        ...
        std::function<bool(Span<const uint8_t>)> check_packet)
{
    ...
    for (int ntry = 0; !got_response && ntry < num_tries; ++ntry) {
        if (ntry > 0) {
            LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "%s: Retrying (%d)\n", protocol, ntry);
        }
        // Dispatch packet to gateway.
        if (sock.Send(request.data(), request.size(), 0) != static_cast<ssize_t>(request.size())) {
            LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "%s: Could not send request: %s\n", protocol, NetworkErrorString(WSAGetLastError()));
        ...
        while ((cur_time = time_point_cast<milliseconds>(MockableSteadyClock::now())) < deadline) {
            ...
            if (!sock.Wait(deadline - cur_time, Sock::RECV, &occurred)) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "%s: Could not wait on socket: %s\n", protocol, NetworkErrorString(WSAGetLastError()));
                return std::nullopt; // Network-level error, probably no use retrying.
            }
            if (!occurred) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "%s: Timeout\n", protocol);
                break; // Retry.
            }
            ...
            if (recvsz < 0) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "%s: Could not receive response: %s\n", protocol, NetworkErrorString(WSAGetLastError()));
                return std::nullopt; // Network-level error, probably no use retrying.
            }
            LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "%s: Received response of %d bytes: %s\n", protocol, recvsz, HexStr(Span(response, recvsz)));
    ...
    if (!got_response) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "%s: Giving up after %d tries\n", protocol, num_tries);
        return std::nullopt;
    }
...
std::variant<MappingResult, MappingError> NATPMPRequestPortMap(const CNetAddr &gateway, uint16_t port, uint32_t lifetime, int num_tries, std::chrono::milliseconds timeout_per_try)
{
    ...
    LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "natpmp: Requesting port mapping port %d from gateway %s\n", port, gateway.ToStringAddr());
    ...
    if (!sock) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Could not create UDP socket: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }

    // Associate UDP socket to gateway.
    if (sock->Connect((struct sockaddr*)&dest_addr, dest_addrlen) != 0) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Could not connect to gateway: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    if (sock->GetSockName((struct sockaddr*)&internal, &internal_addrlen) != 0) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Could not get sock name: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    auto recv_res = PCPSendRecv(*sock, "natpmp", request, num_tries, timeout_per_try,
        [&](const Span<const uint8_t> response) -> bool {
            if (response.size() < NATPMP_GETEXTERNAL_RESPONSE_SIZE) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Response too small\n");
            ...
            if (response[NATPMP_HDR_VERSION_OFS] != NATPMP_VERSION || response[NATPMP_HDR_OP_OFS] != (NATPMP_RESPONSE | NATPMP_OP_GETEXTERNAL)) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Response to wrong command\n");
                return false; // Wasn't response to what we expected, try receiving next packet.
            }
            return true;
        });
    ...
    if (recv_res) {
        ...
        if (result_code != NATPMP_RESULT_SUCCESS) {
            LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Getting external address failed with result %s\n", NATPMPResultString(result_code));
    ...
    recv_res = PCPSendRecv(*sock, "natpmp", request, num_tries, timeout_per_try,
        [&](const Span<const uint8_t> response) -> bool {
            ...
            if (internal_port != port) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Response port doesn't match request\n");
                return false; // Wasn't response to what we expected, try receiving next packet.
            }
            return true;
        });
    ...
    if (recv_res) {
        ...
        uint16_t result_code = ReadBE16(response.data() + NATPMP_RESPONSE_HDR_RESULT_OFS);
        if (result_code != NATPMP_RESULT_SUCCESS) {
            LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "natpmp: Port mapping failed with result %s\n", NATPMPResultString(result_code));
...
std::variant<MappingResult, MappingError> PCPRequestPortMap(const PCPMappingNonce &nonce, const CNetAddr &gateway, const CNetAddr &bind, uint16_t port, uint32_t lifetime, int num_tries, std::chrono::milliseconds timeout_per_try)
{
    ...
    LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "pcp: Requesting port mapping for addr %s port %d from gateway %s\n", bind.ToStringAddr(), port, gateway.ToStringAddr());
    ...
    if (!sock) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Could not create UDP socket: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    if (sock->Bind((struct sockaddr*)&bind_addr, bind_addrlen) != 0) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Could not bind to address: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }

    // Associate UDP socket to gateway.
    if (sock->Connect((struct sockaddr*)&dest_addr, dest_addrlen) != 0) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Could not connect to gateway: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    if (sock->GetSockName((struct sockaddr*)&internal_addr, &internal_addrlen) != 0) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Could not get sock name: %s\n", NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    CService internal;
    if (!internal.SetSockAddr((struct sockaddr*)&internal_addr, internal_addrlen)) return MappingError::NETWORK_ERROR;
    LogPrintLevel(HgLog::MESH, HgLog::Level::Debug, "pcp: Internal address after connect: %s\n", internal.ToStringAddr());
    ...
    auto recv_res = PCPSendRecv(*sock, "pcp", request, num_tries, timeout_per_try,
        [&](const Span<const uint8_t> response) -> bool {
            ...
            if (response.size() < (PCP_HDR_SIZE + PCP_MAP_SIZE)) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Response too small\n");
                return false; // Wasn't response to what we expected, try receiving next packet.
            }
            if (response[PCP_HDR_VERSION_OFS] != PCP_VERSION || response[PCP_HDR_OP_OFS] != (PCP_RESPONSE | PCP_OP_MAP)) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Response to wrong command\n");
                return false; // Wasn't response to what we expected, try receiving next packet.
            }
            ...
            if (!std::ranges::equal(response.subspan(PCP_HDR_SIZE + PCP_MAP_NONCE_OFS, PCP_MAP_NONCE_SIZE), nonce)) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Mapping nonce mismatch\n");
                return false; // Wasn't response to what we expected, try receiving next packet.
            }
            ...
            if (protocol != PCP_PROTOCOL_TCP || internal_port != port) {
                LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Response protocol or port doesn't match request\n");
                return false; // Wasn't response to what we expected, try receiving next packet.
            }
            return true;
        });
    ...
    if (result_code != PCP_RESULT_SUCCESS) {
        LogPrintLevel(HgLog::MESH, HgLog::Level::Warning, "pcp: Mapping failed with result %s\n", PCPResultString(result_code));
