...
const char * const QUICKSILVER_CONF_FILENAME = "quicksilver.conf";
const char * const QUICKSILVER_SETTINGS_FILENAME = "settings.json";
...
{
    KeyInfo result;
    // Split section name from key name for keys like "publictest.foo" or "sandbox.bar"
                                                  ...
                                                  unsigned int flags, std::string& error)
{
    ...
    if (key.negated) {
        ...
        if (value && !InterpretBool(*value)) {
            LogInfo(HgLog::INIT, "Warning: parsed potentially confusing double-negative -%s=%s\n", key.name, *value);
            return true;
        }
        return false;
    }
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ChainTypeToString(ChainType::SANDBOX),
        ChainTypeToString(ChainType::PUBLIC_TEST),
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //quicksilver-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", QUICKSILVER_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
bool ArgsManager::ReadSettingsFile(std::vector<std::string>* errors)
{
    ...
    for (const auto& setting : m_settings.rw_settings) {
        ...
        if (!GetArgFlags('-' + key.name)) {
            LogInfo(HgLog::INIT, "Ignoring unknown rw_settings value %s\n", setting.first);
        }
    }
    return true;
}
    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::RPC:
                ...
                break;
            case OptionsCategory::VAULT:
                usage += HelpMessageGroup("Vault options:");
                break;
            case OptionsCategory::VAULT_DEBUG_TEST:
                if (show_debug) usage += HelpMessageGroup("Vault debugging/testing options:");
                break;
...
{
    // Windows: C:\Users\Username\AppData\Local\Quicksilver
    // macOS: ~/Library/Application Support/Quicksilver
    // Unix-like: ~/.quicksilver
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Quicksilver";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Quicksilver";
#else
    // Unix-like
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fSandbox = get_net("-sandbox");
    const bool fPublicTest = get_net("-publictest");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fSandbox + (int)fPublicTest > 1) {
        throw std::runtime_error("Invalid combination of -sandbox, -publictest and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fSandbox) return ChainType::SANDBOX;
    if (fPublicTest) return ChainType::PUBLIC_TEST;
...
{
    std::string section_str = section.empty() ? "" : "section=" + section + " ";
    for (const auto& arg : args) {
        for (const auto& value : arg.second) {
            std::optional<unsigned int> flags = GetArgFlags('-' + arg.first);
            if (flags) {
                std::string value_str = (*flags & SENSITIVE) ? "****" : value.write();
                LogInfo(HgLog::INIT, "arg source=%s %s%s=%s\n", source, section_str, arg.first, value_str);
            }
        }
    }
}
...
void ArgsManager::LogArgs() const
{
    ...
    for (const auto& section : m_settings.ro_config) {
        logArgsPrefix("config-file", section.first, section.second);
    }
    for (const auto& setting : m_settings.rw_settings) {
        LogInfo(HgLog::INIT, "arg source=settings-file %s=%s\n", setting.first, setting.second.write());
    }
    logArgsPrefix("command-line", "", m_settings.command_line_options);
}
