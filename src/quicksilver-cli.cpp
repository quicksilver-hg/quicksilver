...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", QUICKSILVER_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: quicksilver-cli -generate 4 1000",
    ...
    argsman.AddArg("-getinfo", "Get general information from the remote server. Note that unlike server-side RPC calls, the output of -getinfo is the result of multiple non-atomic requests. Some entries in the output may represent results from different states (e.g. vault balance may be as of a different block from the chain state reported)", ArgsManager::ALLOW_ANY, OptionsCategory::CLI_COMMANDS);
    ...
    argsman.AddArg("-rpcport=<port>", strprintf("Connect to JSON-RPC on <port> (default: %u, publictest: %u, sandbox: %u)", defaultBaseParams->RPCPort(), publictestBaseParams->RPCPort(), sandboxBaseParams->RPCPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::OPTIONS);
...
{
    // Check IsArgNegated to return nullopt instead of "0" if -norpcvault is specified
    if (args.IsArgNegated("-rpcvault")) return std::nullopt;
    return args.GetArg("-rpcvault");
}
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The quicksilver-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                "\nIt can be used to query network information, manage vaults, create or broadcast transactions, and control the " CLIENT_NAME " server.\n"
                ...
                "\n"
                "Usage: quicksilver-cli [options] <command> [params]\n"
                "or:    quicksilver-cli [options] -named <command> [name=value]...\n"
                "or:    quicksilver-cli [options] help\n"
                "or:    quicksilver-cli [options] help <command>\n"
                "\n";
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        ...
        result.push_back(JSONRPCRequestObj("getvaultinfo", NullUniValue, ID_VAULTINFO));
    ...
    UniValue ProcessReply(const UniValue &batch_in) override
    {
        ...
        // getvaultinfo() and getbalances() are allowed to fail if there is no vault.
        ...
        if (!batch[ID_VAULTINFO]["result"].isNull()) {
            result.pushKV("has_vault", true);
            result.pushKV("keypoolsize", batch[ID_VAULTINFO]["result"]["keypoolsize"]);
            result.pushKV("vaultname", batch[ID_VAULTINFO]["result"]["vaultname"]);
            if (!batch[ID_VAULTINFO]["result"]["unlocked_until"].isNull()) {
                result.pushKV("unlocked_until", batch[ID_VAULTINFO]["result"]["unlocked_until"]);
            }
        }
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (ParseUInt8(args.at(0), &n)) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo level argument: %s\nFor more information, run: quicksilver-cli -netinfo help", args.at(0)));
            }
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    m_outbound_only_selected = true;
                } else if (n) {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: quicksilver-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: quicksilver-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    const std::string m_help_doc{
        ...
        "  txn      Time since last novel transaction received from the peer and accepted into our relay pool, in minutes\n"
        ...
        "  version  Peer version and subversion concatenated, e.g. \"70016/Quicksilver:1.0.0/\"\n\n"
        ...
        "> quicksilver-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> quicksilver-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> quicksilver-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "Full dashboard, but with outbound peers only\n"
        + strprintf("> quicksilver-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title quicksilver-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "See this help\n"
        "> quicksilver-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcvault = {})
{
    std::string host;
    ...
    // check if we should use a special vault endpoint
    std::string endpoint = "/";
    if (rpcvault) {
        char* encodedURI = evhttp_uriencode(rpcvault->data(), rpcvault->size(), false);
        if (encodedURI) {
            endpoint = "/vault/" + std::string(encodedURI);
            free(encodedURI);
        } else {
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the quicksilver-daemon server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"quicksilver-cli -help\" for more info.",
 ...
 * @param[in] rpcvault  Reference to const optional string vault name to forward to CallRPC.
 * @returns the RPC response as a UniValue object.
 * @throws a CConnectionFailed std::runtime_error if connection failed or RPC server still in warmup.
 */
static UniValue ConnectAndCallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcvault = {})
{
    ...
    do {
        try {
            response = CallRPC(rh, strMethod, args, rpcvault);
            if (fWait) {
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_VAULT_NOT_SPECIFIED) {
            strPrint += " Or for the CLI, specify the \"-rpcvault=<vaultname>\" option before the command";
            strPrint += " (run \"quicksilver-cli -h\" for help or \"quicksilver-cli listvaults\" to see which vaults are currently loaded).";
        }
    } else {
...
/**
 * GetVaultBalances calls listvaults; if more than one vault is loaded, it then
 * fetches mine.trusted balances for each loaded vault and pushes them to `result`.
 *
 * @param result  Reference to UniValue object the vault names and balances are pushed to.
 */
static void GetVaultBalances(UniValue& result)
{
    DefaultRequestHandler rh;
    const UniValue listvaults = ConnectAndCallRPC(&rh, "listvaults", /* args=*/{});
    if (!listvaults.find_value("error").isNull()) return;
    const UniValue& vaults = listvaults.find_value("result");
    if (vaults.size() <= 1) return;

    UniValue balances(UniValue::VOBJ);
    for (const UniValue& vault : vaults.getValues()) {
        const std::string& vault_name = vault.get_str();
        const UniValue getbalances = ConnectAndCallRPC(&rh, "getbalances", /* args=*/{}, vault_name);
        const UniValue& balance = getbalances.find_value("result")["mine"]["trusted"];
        balances.pushKV(vault_name, balance);
    }
...
static void ParseGetInfoResult(UniValue& result)
{
    ...
    if (!result["has_vault"].isNull()) {
        const std::string vaultname = result["vaultname"].getValStr();
        result_string += strprintf("%sVault: %s%s\n", MAGENTA, vaultname.empty() ? "\"\"" : vaultname, RESET);
    ...
    if (!result["balances"].isNull()) {
        ...
        for (const std::string& vault : result["balances"].getKeys()) {
            max_balance_length = std::max(result["balances"][vault].getValStr().length(), max_balance_length);
        }

        for (const std::string& vault : result["balances"].getKeys()) {
            result_string += strprintf("%*s %s\n",
                                       max_balance_length,
                                       result["balances"][vault].getValStr(),
                                       vault.empty() ? "\"\"" : vault);
        }
        result_string += "\n";
    }
...
{
    DefaultRequestHandler rh;
    return ConnectAndCallRPC(&rh, "getnewaddress", /* args=*/{}, RpcVaultName(gArgs));
}
...
static int CommandLineRPC(int argc, char *argv[])
{
    ...
    try {
        ...
        if (gArgs.GetBoolArg("-stdinvaultpassphrase", false)) {
            ...
            if (!std::getline(std::cin, vaultPass)) {
                throw std::runtime_error("-stdinvaultpassphrase specified but failed to read from standard input");
            }
            if (StdinTerminal()) {
                fputc('\n', stdout);
            }
        ...
        if (nRet == 0) {
            // Perform RPC call
            const std::optional<std::string> vault_name{RpcVaultName(gArgs)};
            const UniValue reply = ConnectAndCallRPC(rh.get(), method, args, vault_name);
            ...
            if (error.isNull()) {
                if (gArgs.GetBoolArg("-getinfo", false)) {
                    if (!vault_name) {
                        GetVaultBalances(result); // fetch multivault balances and append to result
                    }
