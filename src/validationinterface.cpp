...
void ValidationSignals::TransactionAddedToRelayPool(const CTransactionRef& tx, uint64_t relaypool_sequence)
{
    auto event = [tx, relaypool_sequence, this] {
        m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.TransactionAddedToRelayPool(tx, relaypool_sequence); });
    };
...
void ValidationSignals::TransactionRemovedFromRelayPool(const CTransactionRef& tx, RelayPoolRemovalReason reason, uint64_t relaypool_sequence) {
    auto event = [tx, reason, relaypool_sequence, this] {
        m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.TransactionRemovedFromRelayPool(tx, reason, relaypool_sequence); });
    };
...
void ValidationSignals::BlockConnected(const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindex) {
