...
FILE* FlatFileSeq::Open(const FlatFilePos& pos, bool read_only) const
{
    ...
    if (!file) {
        LogInfo(HgLog::LEDGER, "Unable to open file %s\n", fs::PathToString(path));
        return nullptr;
    }
    if (pos.nPos && fseek(file, pos.nPos, SEEK_SET)) {
        LogInfo(HgLog::LEDGER, "Unable to seek to position %u of %s\n", pos.nPos, fs::PathToString(path));
        fclose(file);
        return nullptr;
    }
    return file;
...
size_t FlatFileSeq::Allocate(const FlatFilePos& pos, size_t add_size, bool& out_of_space) const
{
    ...
    if (n_new_chunks > n_old_chunks) {
        ...
        if (CheckDiskSpace(m_dir, inc_size)) {
            FILE *file = Open(pos);
            if (file) {
                LogDebug(HgLog::LEDGER, "Pre-allocating up to position 0x%x in %s%05u.dat\n", new_size, m_prefix, pos.nFile);
...
bool FlatFileSeq::Flush(const FlatFilePos& pos, bool finalize) const
{
    ...
    if (finalize && !TruncateFile(file, pos.nPos)) {
        fclose(file);
        LogError(HgLog::LEDGER, "%s: failed to truncate file %d\n", __func__, pos.nFile);
        return false;
    }
    if (!FileCommit(file)) {
        fclose(file);
        LogError(HgLog::LEDGER, "%s: failed to commit file %d\n", __func__, pos.nFile);
        return false;
    }
