...
static void SetupQuicksilverTxArgs(ArgsManager &argsman)
{
    ...
    argsman.AddArg("outmultisig=VALUE:REQUIRED:PUBKEYS:PUBKEY1:PUBKEY2:....[:FLAGS]", "Add Pay To n-of-m Multi-sig output to TX. n = REQUIRED, m = PUBKEYS. "
        "Optionally add the \"W\" flag to produce a P2WSH output. "
        "Optionally add the \"S\" flag to wrap a non-witness output in P2SH. The flags are mutually exclusive.", ArgsManager::ALLOW_ANY, OptionsCategory::COMMANDS);
    argsman.AddArg("outpubkey=VALUE:PUBKEY[:FLAGS]", "Add pay-to-pubkey output to TX. "
        "Optionally add the \"W\" flag to produce a P2WPKH output. "
        "Optionally add the \"S\" flag to wrap a non-witness output in P2SH. The flags are mutually exclusive.", ArgsManager::ALLOW_ANY, OptionsCategory::COMMANDS);
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = "quicksilver-tx version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The quicksilver-tx tool is used for creating and modifying Quicksilver transactions.\n\n"
                "quicksilver-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded Quicksilver transaction, or with \"-create [commands]\" to create a hex-encoded Quicksilver transaction.\n"
                "\n"
                "Usage: quicksilver-tx [options] <hex-tx> [commands]\n"
                "or:    quicksilver-tx [options] -create [commands]\n"
                "\n";
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bSegWit && bScriptHash) {
        throw std::runtime_error("W and S output flags cannot be combined");
    }

    if (bSegWit) {
        if (!pubkey.IsCompressed()) {
            throw std::runtime_error("Uncompressed pubkeys are not useable for witness outputs");
        }
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
            ...
            if (prevOut.exists("scriptPubKey")) {
                throw std::runtime_error("Use output_script instead of scriptPubKey");
            }
            if (prevOut.exists("redeemScript")) {
                throw std::runtime_error("Use redeem_script instead of redeemScript");
            }
            if (prevOut.exists("witnessScript")) {
                throw std::runtime_error("Use witness_script instead of witnessScript");
            }

            std::map<std::string, UniValue::VType> types = {
                {"txid", UniValue::VSTR},
                {"vout", UniValue::VNUM},
                {"output_script", UniValue::VSTR},
            };
            ...
            std::vector<unsigned char> pkData(ParseHexUV(prevOut["output_script"], "output_script"));
            ...
            // if redeem_script given and private keys given,
            // add redeem_script to the tempKeystore so it can be signed:
            if ((scriptPubKey.IsPayToScriptHash() || scriptPubKey.IsPayToWitnessScriptHash()) &&
                prevOut.exists("redeem_script")) {
                UniValue v = prevOut["redeem_script"];
                std::vector<unsigned char> rsData(ParseHexUV(v, "redeem_script"));
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (amount == MAX_MONEY && !sigdata.scriptWitness.IsNull()) {
            throw std::runtime_error(strprintf("Missing amount for CTxOut with output script=%s", HexStr(prevPubKey)));
        }
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded Quicksilver transaction
