...
#ifndef QUICKSILVER_LOGGING_H
#define QUICKSILVER_LOGGING_H
...
static constexpr bool DEFAULT_LOGLEVELALWAYS = true;
...
namespace HgLog {
    ...
    enum LogFlags : CategoryMask {
        NONE        = CategoryMask{0},
        MESH        = (CategoryMask{1} <<  0),
        TOR         = (CategoryMask{1} <<  1),
        RELAYPOOL   = (CategoryMask{1} <<  2),
        HTTP        = (CategoryMask{1} <<  3),
        BENCH       = (CategoryMask{1} <<  4),
        ZMQ         = (CategoryMask{1} <<  5),
        VAULTDB     = (CategoryMask{1} <<  6),
        RPC         = (CategoryMask{1} <<  7),
        FORGE       = (CategoryMask{1} <<  8),
        PEERBOOK    = (CategoryMask{1} <<  9),
        SELECTINPUTS = (CategoryMask{1} << 10),
    ...
    private:
        ...
        std::atomic<CategoryMask> m_categories{HgLog::NONE};
        ...
        void LogPrintStr_(std::string_view str, std::source_location&& source_loc, HgLog::LogFlags category, HgLog::Level level, bool should_ratelimit)
    ...
    public:
        ...
        void LogPrintStr(std::string_view str, std::source_location&& source_loc, HgLog::LogFlags category, HgLog::Level level, bool should_ratelimit)
            EXCLUSIVE_LOCKS_REQUIRED(!m_cs);
         ...
         * Mostly intended for libquicksilver-kernel apps that don't want any logging.
        ...
        static std::string LogLevelToStr(HgLog::Level level);
...
static inline bool LogAcceptCategory(HgLog::LogFlags category, HgLog::Level level)
{
...
inline void LogPrintFormatInternal(std::source_location&& source_loc, HgLog::LogFlags flag, HgLog::Level level, bool should_ratelimit, util::ConstevalFormatString<sizeof...(Args)> fmt, const Args&... args)
{
...
// Log unconditionally, tagged with a mandatory category. These route through
// LogPrintLevel_ and are NEVER gated on LogAcceptCategory -- the category
// selects the [tag:level] prefix only. Only LogDebug/LogTrace are gated.
//
// Uses basic rate limiting to mitigate disk filling attacks. Be conservative
// when using functions that unconditionally log to the debug log! It should not
// be the case that an inbound peer can fill up a user's storage with entries.
#define LogInfo(category, ...) LogPrintLevel_((category), HgLog::Level::Info, /*should_ratelimit=*/true, __VA_ARGS__)
#define LogWarning(category, ...) LogPrintLevel_((category), HgLog::Level::Warning, /*should_ratelimit=*/true, __VA_ARGS__)
#define LogError(category, ...) LogPrintLevel_((category), HgLog::Level::Error, /*should_ratelimit=*/true, __VA_ARGS__)

// Use a macro instead of a function for conditional logging to prevent
// evaluating arguments when logging for the category is not enabled.

// Log by prefixing the output with the passed category name and severity level. This can either
// log conditionally if the category is allowed or unconditionally if level >= HgLog::Level::Info
...
#define LogPrintLevel(category, level, ...)                           \
    do {                                                              \
        if (LogAcceptCategory((category), (level))) {                 \
            bool rate_limit{level >= HgLog::Level::Info};             \
            LogPrintLevel_(category, level, rate_limit, __VA_ARGS__); \
        }                                                             \
    } while (0)

// Log conditionally, prefixing the output with the passed category name.
#define LogDebug(category, ...) LogPrintLevel(category, HgLog::Level::Debug, __VA_ARGS__)
#define LogTrace(category, ...) LogPrintLevel(category, HgLog::Level::Trace, __VA_ARGS__)

#endif // QUICKSILVER_LOGGING_H
