...
{
    return IsStandardTx(tx, std::nullopt, permit_bare_multisig, reason);
}

static bool IsStandardTx(const CTransaction& tx, std::string& reason)
{
    return IsStandardTx(tx, std::nullopt, /*permit_bare_multisig=*/true, reason) &&
           IsStandardTx(tx, std::nullopt, /*permit_bare_multisig=*/false, reason);
}
