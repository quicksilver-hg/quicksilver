...
BOOST_AUTO_TEST_CASE(parse_hex)
{
    ...
    // Leading space must be supported by callers that preserve shell-style fields.
...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    BOOST_CHECK_EQUAL(ParseMoney("0.00000001 ").value(), COIN/100000000);
    BOOST_CHECK_EQUAL(ParseMoney(" 0.00000001").value(), COIN/100000000);
    BOOST_CHECK_EQUAL(ParseMoney("100000000.00").value(), CAmount{100'000'000} * COIN);

    // Parsing amount that cannot be represented should fail
    BOOST_CHECK(!ParseMoney("1000000000.00000001"));
...
//! Outcomes the lock-directory stub reports, one character per attempt. Kept in
//! sync with src/test/lockdirstub.cpp.
namespace {
constexpr char kStubSuccess{'S'};
constexpr char kStubErrorWrite{'W'};
constexpr char kStubErrorLock{'L'};

//! Bounds a hang rather than expressing a performance expectation. A stub that
//! never reports fails the test instead of wedging the suite.
constexpr auto kStubTimeout{std::chrono::seconds(30)};

subprocess::Popen SpawnLockStub(const std::vector<std::string>& args)
{
    std::vector<std::string> argv{LOCK_DIR_STUB_PATH};
    argv.insert(argv.end(), args.begin(), args.end());
    return subprocess::Popen(argv);
}

bool AwaitStubFile(const fs::path& path)
{
...
{
    const fs::path dirname{m_args.GetDataDirBase() / "lock_dir"};
    const fs::path lockname{".lock"};
    const fs::path sync_dir{m_args.GetDataDirBase() / "lock_sync"};
    fs::create_directories(sync_dir);
    const std::string dir_arg{fs::PathToString(dirname)};
    const std::string lock_arg{fs::PathToString(lockname)};

    // A second process cannot lock a directory that does not exist either.
    const fs::path ready_missing{sync_dir / "ready-missing"};
    {
        auto child{SpawnLockStub({dir_arg, lock_arg, "try", fs::PathToString(ready_missing)})};
        BOOST_CHECK_EQUAL(StubOutcome(ready_missing), kStubErrorWrite);
    ...
    // THE CONTRACT: another process must NOT get the lock while we hold it.
    const fs::path ready_blocked{sync_dir / "ready-blocked"};
    {
        auto child{SpawnLockStub({dir_arg, lock_arg, "try", fs::PathToString(ready_blocked)})};
        BOOST_CHECK_EQUAL(StubOutcome(ready_blocked), kStubErrorLock);
        BOOST_CHECK_EQUAL(child.wait(), 0);
    }
    ...
    // ...and now the other process can take it, holds it against us, and can
    // hand it back by releasing without exiting.
    const fs::path ready_held{sync_dir / "ready-held"};
    const fs::path go_release{sync_dir / "go-release"};
    const fs::path released{sync_dir / "released"};
    const fs::path go_exit{sync_dir / "go-exit"};
    {
        auto child{SpawnLockStub({dir_arg, lock_arg, "hold-release",
                                  fs::PathToString(ready_held), fs::PathToString(go_release),
                                  fs::PathToString(released), fs::PathToString(go_exit)})};
        BOOST_CHECK_EQUAL(StubOutcome(ready_held), kStubSuccess);

        // While it holds the lock, we must not be able to probe it.
        BOOST_CHECK_EQUAL(util::LockDirectory(dirname, lockname, true), util::LockResult::ErrorLock);

        // An explicit release by the holder frees it without the holder exiting.
        TouchStubFile(go_release);
        BOOST_CHECK(AwaitStubFile(released));
        BOOST_CHECK_EQUAL(util::LockDirectory(dirname, lockname, true), util::LockResult::Success);

        TouchStubFile(go_exit);
        BOOST_CHECK_EQUAL(child.wait(), 0);
    }

    // Finally: a holder that exits WITHOUT releasing must still free the lock,
    // which is what makes a crashed or killed node's datadir reusable.
    const fs::path ready_dying{sync_dir / "ready-dying"};
    const fs::path go_die{sync_dir / "go-die"};
    {
        auto child{SpawnLockStub({dir_arg, lock_arg, "hold-exit",
                                  fs::PathToString(ready_dying), fs::PathToString(go_die)})};
        BOOST_CHECK_EQUAL(StubOutcome(ready_dying), kStubSuccess);
        BOOST_CHECK_EQUAL(util::LockDirectory(dirname, lockname, true), util::LockResult::ErrorLock);

        TouchStubFile(go_die);
        BOOST_CHECK_EQUAL(child.wait(), 0);
    }
    BOOST_CHECK_EQUAL(util::LockDirectory(dirname, lockname, true), util::LockResult::Success);
...
BOOST_AUTO_TEST_CASE(test_Capitalize)
{
    ...
    BOOST_CHECK_EQUAL(Capitalize("quicksilver"), "Quicksilver");
...
BOOST_AUTO_TEST_CASE(test_LogEscapeMessage)
{
    ...
    BOOST_CHECK_EQUAL(HgLog::LogEscapeMessage("Valid log message貓"), "Valid log message貓");
    // Newlines must pass through unaltered.
    BOOST_CHECK_EQUAL(HgLog::LogEscapeMessage("Message\n with newlines\n"), "Message\n with newlines\n");
    // Other control characters are escaped in C syntax.
    BOOST_CHECK_EQUAL(HgLog::LogEscapeMessage("\x01\x7f Corrupted log message\x0d"), R"(\x01\x7f Corrupted log message\x0d)");
    // Embedded NULL characters are escaped too.
    const std::string NUL("O\x00O", 3);
    BOOST_CHECK_EQUAL(HgLog::LogEscapeMessage(NUL), R"(O\x00O)");
}
...
BOOST_AUTO_TEST_CASE(message_sign)
{
    const std::array<unsigned char, 32> privkey_bytes = {
        ...
        // derived address from this private key: QQoR4YvgcH9NmeExYd6n3mRYEFa3FyQjEu
    ...
    const std::string expected_signature =
        "IFOUrt1HHR4/LRvDmRwQ+j3EpRHj1MtGDe1dHm/Qe8KeJEvzYoVhuob9DTKF8aeA2H62Tg69xhbOe8AsoqLh0Zo=";
...
BOOST_AUTO_TEST_CASE(message_verify)
{
    ...
    BOOST_CHECK_EQUAL(
        MessageVerify(
            "QQoR4YvgcH9NmeExYd6n3mRYEFa3FyQjEu",
            "INSr+2jtGBwIi0tj42yUpKQc+afRiVNH4iZsXWi8DUqZN3b9sD9XTSFqSpwwnOIIzcc16NaJpGqTjoRh9TdyKT4=",
            "Trust me"),
        MessageVerificationResult::OK);
}
...
BOOST_AUTO_TEST_CASE(util_WriteBinaryFile)
{
    ...
    std::string expected_text = "quicksilver";
