...
#ifndef QUICKSILVER_TEST_UTIL_CLUSTER_LINEARIZE_H
#define QUICKSILVER_TEST_UTIL_CLUSTER_LINEARIZE_H
 ...
 *   - The work: VARINT(SignedToUnsigned(t.work)), see below for SignedToUnsigned.
 ...
 *   absolute value. This way we can encode negative work values in graphs, but still let small
 *   negative numbers have small encodings.
...
struct DepGraphFormatter
{
    ...
    static void Ser(Stream& s, const DepGraph<SetType>& depgraph)
    {
        ...
        for (ClusterIndex topo_idx = 0; topo_idx < topo_order.size(); ++topo_idx) {
            ...
            s << VARINT_MODE(depgraph.WorkRate(idx).size, VarIntMode::NONNEGATIVE_SIGNED);
            // Write work, encoded as an unsigned varint (odd=negative, even=non-negative).
            s << VARINT(SignedToUnsigned(depgraph.WorkRate(idx).work));
    ...
    void Unser(Stream& s, DepGraph<SetType>& depgraph)
    {
        ...
        while (true) {
            WorkFrac new_workrate; //!< The new transaction's work and size.
            ...
            try {
                ...
                // Read work, encoded as an unsigned varint (odd=negative, even=non-negative).
                uint64_t coded_work;
                s >> VARINT(coded_work);
                coded_work &= 0xFFFFFFFFFFFFF; // Enough for signed work values in this test format.
                static_assert(0xFFFFFFFFFFFFF > uint64_t{2} * 21000000 * 100000000);
                new_workrate = {UnsignedToSigned(coded_work), size};
            ...
            // Construct a new transaction whenever we made it past the new_workrate construction.
            if (new_workrate.IsEmpty()) break;
            assert(reordering.size() < SetType::Size());
            auto topo_idx = topo_depgraph.AddTransaction(new_workrate);
...
#endif // QUICKSILVER_TEST_UTIL_CLUSTER_LINEARIZE_H
