...
#ifndef QUICKSILVER_TEST_UTIL_SETUP_COMMON_H
#define QUICKSILVER_TEST_UTIL_SETUP_COMMON_H
...
/**
 * Attach a deterministic, target-satisfying per-tx proof for sandbox unit tests
 * that opt into Consensus::Params::fTxPowNoCycle. This grinds only the cheap
 * Cuckatoo proof hash threshold; it does not solve a real Cuckatoo cycle.
 */
...
struct TestOpts {
    ...
    //! Byte cap for CTxRelayPool::Options::max_size_bytes. -maxrelaypool is in
    //! megabytes, so a unit test cannot reach the full-pool path without this.
    //! Ancestor/descendant size limits are clamped so Flatten() will accept it.
    std::optional<int64_t> relaypool_max_size_bytes{};
};
...
/** Identical to TestingSetup, but chain set to sandbox */
struct SandboxingSetup : public TestingSetup {
    SandboxingSetup()
        : TestingSetup{ChainType::SANDBOX} {}
};
...
/**
 * Testing fixture that pre-creates a 100-block SANDBOX-mode block chain
 */
struct TestChain100Setup : public TestingSetup {
    TestChain100Setup(
        const ChainType chain_type = ChainType::SANDBOX,
        TestOpts = {});
    ...
    */
    ...
    /**
     * Create a transaction and, optionally, submit to the relaypool.
     *
     * @param input_transactions   The transactions to spend
     * @param inputs               Outpoints with which to construct transaction vin.
     ...
     * @param outputs              Transaction vout.
     * @param submit               Whether or not to submit to relaypool
     */
    CMutableTransaction CreateValidRelayPoolTransaction(const std::vector<CTransactionRef>& input_transactions,
                                                      const std::vector<COutPoint>& inputs,
                                                      int input_height,
                                                      const std::vector<CKey>& input_signing_keys,
                                                      const std::vector<CTxOut>& outputs,
    ...
    /**
     * Create a 1-in-1-out transaction and, optionally, submit to the relaypool.
     *
     ...
     * @param submit             Whether or not to submit to relaypool
     */
    CMutableTransaction CreateValidRelayPoolTransaction(CTransactionRef input_transaction,
                                                      uint32_t input_vout,
                                                      int input_height,
     ...
     * relaypool consistency. All transactions will have some random number of inputs and outputs
     ...
     * @param[in]   submit      When true, submit transactions to the relaypool.
     *                          When false, return them but don't submit them.
     * @returns A vector of transactions that can be submitted to the relaypool.
     */
    std::vector<CTransactionRef> PopulateRelayPool(FastRandomContext& det_rand, size_t num_transactions, bool submit);
...
std::unique_ptr<T> MakeNoLogFileContext(const ChainType chain_type = ChainType::SANDBOX, TestOpts opts = {})
{
    opts.extra_args = Cat(
        {
            "-nodebuglogfile",
...
#endif // QUICKSILVER_TEST_UTIL_SETUP_COMMON_H
