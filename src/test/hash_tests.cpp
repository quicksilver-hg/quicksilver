...
#define T(expected, seed, data) BOOST_CHECK_EQUAL(MurmurHash3(seed, ParseHex(data)), expected)
    ...
    // rolling bloom filter tests - they would fail if MurmurHash3() had any
    // problems - but is useful for those trying to implement Quicksilver
    // libraries as a source of test data for their MurmurHash3() primitive
    // during development.
    //
    // The magic number 0xFBA4C795 is the per-hash-function seed stride used by
    // RollingBloomHash().
...
BOOST_AUTO_TEST_CASE(siphash)
{
    ...
    // Quicksilver: empty tx now serializes with the 176B per-tx PoW tail, shifting the hash.
    BOOST_CHECK_EQUAL(SipHashUint256(1, 2, ss.GetHash()), 0x5c0e32a9b65eda39ULL);
