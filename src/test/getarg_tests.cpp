...
void ResetArgs(ArgsManager& local_args, const std::string& strArg)
{
    ...
    vecArg.insert(vecArg.begin(), "testquicksilver");
...
BOOST_AUTO_TEST_CASE(patharg)
{
    ...
    const fs::path absolute_path{"/home/user/.quicksilver"};
    ResetArgs(local_args, "-dir=/home/user/.quicksilver");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);

    ResetArgs(local_args, "-dir=/root/../home/user/.quicksilver");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);
    ...
    const fs::path relative_path{"user/.quicksilver"};
    ResetArgs(local_args, "-dir=user/.quicksilver");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);

    ResetArgs(local_args, "-dir=somewhere/../user/.quicksilver");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);
    ...
    // line arguments or quicksilver.conf values. Currently the -dir= case cannot be
...
BOOST_AUTO_TEST_CASE(logargs)
{
    ...
    BOOST_CHECK(str.find("arg source=command-line okaylog-bool=\"\"") != std::string::npos);
    BOOST_CHECK(str.find("arg source=command-line okaylog-negbool=false") != std::string::npos);
    BOOST_CHECK(str.find("arg source=command-line okaylog=\"public\"") != std::string::npos);
