...
BOOST_AUTO_TEST_CASE(util_ReadConfigStream)
{
    ...
    // section takes priority for multiple values; last assignment in the section wins
...
{
    TestArgsManager test_args;
    const auto publictest = std::make_pair("-publictest", ArgsManager::ALLOW_ANY);
    const auto sandbox = std::make_pair("-sandbox", ArgsManager::ALLOW_ANY);
    test_args.SetupArgs({publictest, sandbox});

    const char* argv_publictest[] = {"cmd", "-publictest"};
    const char* argv_sandbox[] = {"cmd", "-sandbox"};
    const char* argv_publictest_no_reg[] = {"cmd", "-publictest", "-nosandbox"};
    const char* argv_both[] = {"cmd", "-publictest", "-sandbox"};

    // sandbox in the publictest section is ignored unless publictest is selected.
    const char* publictestconf = "publictest=1\nsandbox=0\n[publictest]\nsandbox=1";
    std::string error;

    BOOST_CHECK(test_args.ParseParameters(0, argv_publictest, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "main");

    BOOST_CHECK(test_args.ParseParameters(2, argv_publictest, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "publictest");

    BOOST_CHECK(test_args.ParseParameters(2, argv_sandbox, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "sandbox");

    BOOST_CHECK(test_args.ParseParameters(3, argv_publictest_no_reg, error));
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "publictest");
    ...
    BOOST_CHECK(test_args.ParseParameters(0, argv_publictest, error));
    test_args.ReadConfigString(publictestconf);
    BOOST_CHECK_EQUAL(test_args.GetChainTypeString(), "publictest");
    ...
    // check setting the network to publictest (and thus making
    // [publictest] sandbox=1 potentially relevant) doesn't break things.
    test_args.SelectConfigNetwork("publictest");
...
//
// - Testing "main" and "publictest" network values to make sure settings from network
//   sections are applied and to check for mainnet-specific behaviors like
//   inheriting settings from the default section.
//
// - Testing network-specific settings like "-vault", that may be ignored
            ...
            ForEachNoDup(conf_actions, SET, SECTION_NEGATE, [&] {
                for (bool soft_set : {false, true}) {
                    for (bool force_set : {false, true}) {
                        for (const std::string& section : {ChainTypeToString(ChainType::MAIN), ChainTypeToString(ChainType::PUBLIC_TEST)}) {
                            for (const std::string& network : {ChainTypeToString(ChainType::MAIN), ChainTypeToString(ChainType::PUBLIC_TEST)}) {
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions, bool soft_set, bool force_set,
        ...
        const std::string& name = net_specific ? "vault" : "server";
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_quicksilver --run_test=argsman_tests/util_ArgsMerge
    //
    ...
    BOOST_CHECK_EQUAL(out_sha_hex, "1738236c70b946934a26d66dac28b8798eec11e014ed963f0d8bcb58ca1b2454");
}
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    ForEachMergeSetup([&](const ActionList& arg_actions, const ActionList& conf_actions) {
        TestArgsManager parser;
        LOCK(parser.cs_args);
        parser.AddArg("-sandbox", "sandbox", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
        parser.AddArg("-publictest", "publictest", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);

        auto arg = [](Action action) { return action == ENABLE_TEST  ? "-publictest=1"   :
                                              action == DISABLE_TEST ? "-publictest=0"   :
                                              action == NEGATE_TEST  ? "-nopublictest=1" :
                                              action == ENABLE_REG   ? "-sandbox=1"   :
                                              action == DISABLE_REG  ? "-sandbox=0"   :
                                              action == NEGATE_REG   ? "-nosandbox=1" : nullptr; };
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_quicksilver --run_test=argsman_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
    //
    //   <input> || <output>
    BOOST_CHECK_EQUAL(out_sha_hex, "a5ccf7d0003272ee65c7939a4df30bfafd4352165a54ea47ae009d548a2b3acb");
}
