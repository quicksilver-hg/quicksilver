...
{
    CBlock block = getTestBlock9Tx();

    std::set<Txid> txids;

    // Last txn in block. (Derived from the block: Quicksilver's per-tx PoW tail is bound into
    // the txid.)
    Txid txhash1{block.vtx.back()->GetHash()};

    // Second txn in block.
    Txid txhash2{block.vtx[1]->GetHash()};
