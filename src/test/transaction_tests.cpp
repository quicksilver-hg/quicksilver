...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    vch.insert(vch.end(), 176, 0x00);  // Quicksilver: append the per-tx PoW tail (dummy) so the legacy tx hex deserializes
...
BOOST_AUTO_TEST_CASE(test_witness)
{
    ...
    // Signing does not solve a P2SH output whose redeem script is a witness program.
    SignatureData wrapped_witness_sigdata;
    BOOST_CHECK(!ProduceSignature(keystore, DUMMY_SIGNATURE_CREATOR,
                                  GetScriptForDestination(ScriptHash(destination_script_1)),
                                  wrapped_witness_sigdata));
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    // Quicksilver is feeless/exact-change: the dust machinery was removed in Class C.
    t.vout[0].nValue = 1;
    CheckIsStandard(t);
    t.vout.emplace_back(0, t.vout[0].scriptPubKey); // a zero-value output is no longer "dust"
    CheckIsStandard(t);
    t.vout.resize(1);
    t.vout[0].nValue = 90*CENT; // restore a normal output for the checks below
    ...
    // (Quicksilver removed the dust machinery; the odd-relay-fee dust-rounding checks no longer apply.)
    ...
    t.vin.clear();
    // Quicksilver: every tx carries a 176-byte per-tx PoW tail (=> 704 weight units), so the
    // input/output counts are re-derived to still land exactly on MAX_STANDARD_TX_WEIGHT.
    t.vin.resize(2434); // size per input (empty scriptSig): 41 bytes
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(7, 0); // output size: 18 bytes
    // tx header:                  12 bytes =>     48 weight units
    // 2434 inputs: 2434*41 = 99794 bytes => 399176 weight units
    //    1 output:                18 bytes =>     72 weight units
    // per-tx PoW tail:           176 bytes =>    704 weight units
    //                      ======================================
    ...
    // Quicksilver is feeless/exact-change: there is no per-output-type dust threshold. Verify each
    // standard output type is relayable even at a 1-atom value.
    // The dust machinery was removed in Class C.
    t.vout.resize(1);
    t.vout[0].nValue = 1;
    const std::vector<CScript> standard_spks{
        CScript() << std::vector<unsigned char>(33, 0x02) << OP_CHECKSIG,                                       // compressed P2PK
        CScript() << std::vector<unsigned char>(65, 0x04) << OP_CHECKSIG,                                       // uncompressed P2PK
        CScript() << OP_DUP << OP_HASH160 << std::vector<unsigned char>(20, 0) << OP_EQUALVERIFY << OP_CHECKSIG, // P2PKH
        CScript() << OP_HASH160 << std::vector<unsigned char>(20, 0) << OP_EQUAL,                               // P2SH
        CScript() << OP_0 << std::vector<unsigned char>(20, 0),                                                 // P2WPKH
        CScript() << OP_0 << std::vector<unsigned char>(32, 0),                                                 // P2WSH
        CScript() << OP_1 << std::vector<unsigned char>(32, 0),                                                 // P2TR
    };
    for (const CScript& spk : standard_spks) {
        t.vout[0].scriptPubKey = spk;
        CheckIsStandard(t);
    }

    // Future witness program versions are standard at any positive value too.
    for (int op = OP_1; op <= OP_16; op += 1) {
        t.vout[0].scriptPubKey = CScript() << (opcodetype)op << std::vector<unsigned char>(2, 0);
        CheckIsStandard(t);
    }

    // Anchor outputs remain recognized and standard.
...
BOOST_AUTO_TEST_CASE(tx_anchor_height_roundtrip_and_txid)
{
    CMutableTransaction mtx;
    mtx.version = 2;
    mtx.vin.resize(1);
    ...
    mtx.nLockTime = 0;
    mtx.nPowNonce = 7;
    mtx.nAnchorHeight = 12345;

    // Round-trips through serialization.
    DataStream ss;
    ss << TX_WITH_WITNESS(CTransaction(mtx));
    CTransaction tx2(deserialize, TX_WITH_WITNESS, ss);
    BOOST_CHECK_EQUAL(tx2.nAnchorHeight, 12345u);

    // The anchor is bound into the txid (consensus-committed).
    CMutableTransaction other = mtx;
    other.nAnchorHeight = 99999;
    BOOST_CHECK(CTransaction(mtx).GetHash() != CTransaction(other).GetHash());
}
