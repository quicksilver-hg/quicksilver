...
BOOST_FIXTURE_TEST_SUITE(net_tests, SandboxingSetup)

BOOST_AUTO_TEST_CASE(all_net_message_types_exclude_feefilter)
{
    // BIP133 is not a registered protocol message. BIP324 short-id 5 is an
    // unused slot tested in v2transport_test; this asserts the type itself is gone.
    BOOST_CHECK(std::ranges::find(ALL_NET_MESSAGE_TYPES, "feefilter") == ALL_NET_MESSAGE_TYPES.end());
}

BOOST_AUTO_TEST_CASE(all_net_message_types_exclude_sendtxrcncl)
{
    // BIP-330 was only ever a handshake upstream: negotiating it promised a
    // reconciliation round this node never performed. It carries no BIP324
    // short id, so unlike the id-5 slot nothing had to be reserved here.
    BOOST_CHECK(std::ranges::find(ALL_NET_MESSAGE_TYPES, "sendtxrcncl") == ALL_NET_MESSAGE_TYPES.end());
}
...
BOOST_AUTO_TEST_CASE(cnetaddr_serialize_v1)
{
    ...
    addr.SetInternal("a");
    s << ser_params(addr);
    BOOST_CHECK_EQUAL(HexStr(s), "fd585d14ac23ca978112ca1bbdcafac2");
    s.clear();
}
...
BOOST_AUTO_TEST_CASE(cnetaddr_serialize_v2)
{
    ...
    s << ser_params(addr);
    BOOST_CHECK_EQUAL(HexStr(s), "0210fd585d14ac23ca978112ca1bbdcafac2");
    s.clear();
}
...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    s << "02"                                    // network type (IPv6)
         "10"                                    // address length
         "fd585d14ac23ca978112ca1bbdcafac2"_hex; // address: 0xfd + sha256("quicksilver")[0:5] +
                                                 // sha256(name)[0:10]
    s >> ser_params(addr);
...
BOOST_AUTO_TEST_CASE(get_local_addr_for_peer_port)
{
    ...
    CNode peer_out{/*id=*/0,
                   /*sock=*/nullptr,
                   /*addrIn=*/CAddress{CService{peer_out_in_addr, 9555}, NODE_NETWORK},
                   /*nKeyedNetGroupIn=*/0,
                   /*nLocalHostNonceIn=*/0,
                   /*addrBindIn=*/CService{},
    ...
    // Without the fix peer_us:9555 is chosen instead of the proper peer_us:bind_port.
    ...
    CNode peer_in{/*id=*/0,
                  /*sock=*/nullptr,
                  /*addrIn=*/CAddress{CService{peer_in_in_addr, 9555}, NODE_NETWORK},
                  /*nKeyedNetGroupIn=*/0,
                  /*nLocalHostNonceIn=*/0,
                  /*addrBindIn=*/CService{},
                  /*addrNameIn=*/std::string{},
    ...
    // Without the fix peer_us:9555 is chosen instead of the proper peer_us:peer_us.GetPort().
...
BOOST_AUTO_TEST_CASE(initial_advertise_from_version_message)
{
    ...
    CNode peer{/*id=*/0,
               /*sock=*/nullptr,
               /*addrIn=*/CAddress{CService{peer_in_addr, 9555}, NODE_NETWORK},
               /*nKeyedNetGroupIn=*/0,
               /*nLocalHostNonceIn=*/0,
               /*addrBindIn=*/CService{},
               /*addrNameIn=*/std::string{},
...
BOOST_AUTO_TEST_CASE(advertise_local_address)
{
    ...
    CAddress addr_ipv4{Lookup("1.2.3.4", 9555, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv4.IsValid());
    BOOST_REQUIRE(addr_ipv4.IsIPv4());

    CAddress addr_ipv6{Lookup("1122:3344:5566:7788:9900:aabb:ccdd:eeff", 9555, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv6.IsValid());
    BOOST_REQUIRE(addr_ipv6.IsIPv6());

    CAddress addr_ipv6_tunnel{Lookup("2002:3344:5566:7788:9900:aabb:ccdd:eeff", 9555, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv6_tunnel.IsValid());
    BOOST_REQUIRE(addr_ipv6_tunnel.IsIPv6());
    BOOST_REQUIRE(addr_ipv6_tunnel.IsRFC3964());

    CAddress addr_teredo{Lookup("2001:0000:5566:7788:9900:aabb:ccdd:eeff", 9555, false).value(), NODE_NONE};
    ...
    CService service_cjdns{Lookup("fc00:3344:5566:7788:9900:aabb:ccdd:eeff", 9555, false).value(), NODE_NONE};
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        tester.SendMessage(uint8_t(5), {}); // BIP324 id 5 is unused
        ...
        BOOST_CHECK((*ret)[2] && (*ret)[2]->m_type.empty() && (*ret)[2]->m_recv.empty());
        BOOST_CHECK(!(*ret)[3]);
        BOOST_CHECK((*ret)[4] && (*ret)[4]->m_type == "foobar" && (*ret)[4]->m_recv.empty());
...
// Publishing the repository is the launch, so the shipped binary is the only
// thing a first user has. Quicksilver deliberately has no DNS seeds, which makes
// the fixed seeds the sole automatic path to a peer: with both empty a fresh node
// connects to nothing, forever, and reports nothing worse than zero peers.
BOOST_AUTO_TEST_CASE(published_networks_have_reachable_seeds)
{
    for (const auto chain : {ChainType::MAIN, ChainType::PUBLIC_TEST}) {
        const auto params{CreateChainParams(*m_node.args, chain)};
        BOOST_CHECK_MESSAGE(!params->FixedSeeds().empty(),
                            ChainTypeToString(chain) << " ships no fixed seeds: a fresh node "
                            "can never find a peer");
        BOOST_CHECK_MESSAGE(params->DNSSeeds().empty(),
                            ChainTypeToString(chain) << " ships a DNS seed; Quicksilver owns no "
                            "domain and must not depend on one");
    }

    // Sandbox is local by construction and must stay empty.
    const auto sandbox{CreateChainParams(*m_node.args, ChainType::SANDBOX)};
    BOOST_CHECK(sandbox->FixedSeeds().empty());
}

// "Non-empty" is a weak claim about a blob of bytes: a truncated or misgenerated
// chainparamsseeds.h still satisfies it, and the failure would only show up as a
// node that quietly reaches nobody. Decode the seeds the way net.cpp's
// ConvertSeeds does and check the endpoint that actually comes out.
BOOST_AUTO_TEST_CASE(fixed_seeds_decode_to_the_published_endpoint)
{
    const std::string kSeedOnion{"z2ban4msenn4at52o42wbaz5aws2fzwyafhme3hmzu2smnammken4sid.onion"};

    const std::map<ChainType, uint16_t> expected_ports{
        {ChainType::MAIN, 9555},
        {ChainType::PUBLIC_TEST, 19557},
    };

    for (const auto& [chain, port] : expected_ports) {
        const auto params{CreateChainParams(*m_node.args, chain)};
        const std::string name{ChainTypeToString(chain)};

        std::vector<CService> decoded;
        ParamsStream s{DataStream{params->FixedSeeds()}, CAddress::V2_NETWORK};
        while (!s.eof()) {
            CService endpoint;
            s >> endpoint;
            decoded.push_back(endpoint);
        }

        BOOST_REQUIRE_MESSAGE(decoded.size() == 1,
                              name << ": expected exactly one fixed seed, decoded "
                              << decoded.size());
        BOOST_CHECK_MESSAGE(decoded[0].IsTor(),
                            name << ": fixed seed is not an onion address ("
                            << decoded[0].ToStringAddrPort() << "); Quicksilver publishes "
                            "no clearnet seed and takes on no static-IP obligation");
        BOOST_CHECK_EQUAL(decoded[0].ToStringAddr(), kSeedOnion);
        BOOST_CHECK_EQUAL(decoded[0].GetPort(), port);
    }
}

// A node binds three loopback ports of its own: P2P, the onion service target, and
...
BOOST_AUTO_TEST_CASE(default_ports_do_not_collide)
{
    for (const auto chain : {ChainType::MAIN, ChainType::PUBLIC_TEST, ChainType::SANDBOX}) {
        ...
        const uint16_t p2p{params->GetDefaultPort()};
        const uint16_t onion{params->OnionServicePort()};
        const uint16_t rpc{base->RPCPort()};

        BOOST_CHECK_MESSAGE(onion != p2p,
                            name << ": onion service port " << onion << " is the P2P port");
        BOOST_CHECK_MESSAGE(onion != rpc,
                            name << ": onion service port " << onion << " is the RPC port; "
                            "the node cannot start on default ports");
        BOOST_CHECK_MESSAGE(p2p != rpc,
                            name << ": P2P port " << p2p << " is the RPC port");

        const uint32_t slip44{params->Slip44CoinType()};
        if (chain == ChainType::MAIN) {
            BOOST_CHECK_MESSAGE(slip44 != 0,
                                name << ": SLIP-44 coin type must not use type 0");
        } else {
            BOOST_CHECK_EQUAL(slip44, 1u);
        }
    }
}
