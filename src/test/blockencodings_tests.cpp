...
BOOST_FIXTURE_TEST_SUITE(blockencodings_tests, SandboxingSetup)
...
// (block + relaypool entry + relaypool txns_randomized + our copy from the GetSharedTx call)
...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest)
{
    ...
    AddToRelayPool(pool, entry.FromTx(block.vtx[2]));
    ...
    // Do a simple ShortTxIDs RT
    {
        ...
        pool.removeRecursive(*block.vtx[2], RelayPoolRemovalReason::REPLACED);
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest)
{
    ...
    LOCK2(cs_main, pool.cs);
    AddToRelayPool(pool, entry.FromTx(block.vtx[1]));
    BOOST_CHECK_EQUAL(pool.get(block.vtx[1]->GetHash()).use_count(), SHARED_TX_OFFSET + 0);

    uint256 txhash;

    // Test with pre-forwarding coinbase + tx 2 with tx 1 in relaypool
    {
        TestHeaderAndShortIDs shortIDs(block, rand_ctx);
...
BOOST_AUTO_TEST_CASE(ReceiveWithExtraTransactions) {
...

    {
        ...
        // and a tx from the block that's not in the relaypool:
