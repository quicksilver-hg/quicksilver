...
struct Dersig100Setup : public TestChain100Setup {
    Dersig100Setup()
        : TestChain100Setup{ChainType::SANDBOX, {.extra_args = {"-testactivationheight=dersig@102"}}}
    {
...
BOOST_FIXTURE_TEST_CASE(tx_relaypool_block_doublespend, Dersig100Setup)
{
    ...
    const auto ToRelayPool = [this](const CMutableTransaction& tx) {
        LOCK(cs_main);

        const RelayPoolAcceptResult result = m_node.chainman->ProcessTransaction(MakeTransactionRef(tx));
        return result.m_result_type == RelayPoolAcceptResult::ResultType::VALID;
    };
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vout[0].nValue = m_coinbase_txns[0]->vout[0].nValue;
    ...
    BOOST_CHECK(ToRelayPool(spends[0]));
    block = CreateAndProcessBlock(spends, scriptPubKey);
    {
        LOCK(cs_main);
        BOOST_CHECK(m_node.chainman->ActiveChain().Tip()->GetBlockHash() != block.GetHash());
    ...
    WITH_LOCK(m_node.relaypool->cs, m_node.relaypool->removeRecursive(CTransaction{spends[0]}, RelayPoolRemovalReason::CONFLICT));
    BOOST_CHECK_EQUAL(m_node.relaypool->size(), 0U);

    // Test 3: ... and should be rejected if spend2 is in the memory pool
    BOOST_CHECK(ToRelayPool(spends[1]));
    block = CreateAndProcessBlock(spends, scriptPubKey);
    {
        LOCK(cs_main);
        BOOST_CHECK(m_node.chainman->ActiveChain().Tip()->GetBlockHash() != block.GetHash());
    ...
    BOOST_CHECK_EQUAL(m_node.relaypool->size(), 1U);
    WITH_LOCK(m_node.relaypool->cs, m_node.relaypool->removeRecursive(CTransaction{spends[1]}, RelayPoolRemovalReason::CONFLICT));
    BOOST_CHECK_EQUAL(m_node.relaypool->size(), 0U);

    // Final sanity test: first spend in *m_node.relaypool, second in block, that's OK:
    ...
    // spends[1] should have been removed from the relaypool when the
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    spend_tx.vout[4].nValue = m_coinbase_txns[0]->vout[0].nValue - 44*CENT;
    spend_tx.vout[4].scriptPubKey = p2pk_scriptPubKey;
