...
struct TxDownloadTestSetup : TestChain100Setup {
    TxDownloadTestSetup()
    {
...
static std::map<TxValidationResult, Behaviors> expected_behaviors{
    ...
    {TxValidationResult::TX_RELAYPOOL_POLICY,          {                0,                 1,              0,               0,        1,            0,             1}},
    {TxValidationResult::TX_NO_RELAYPOOL,              {                0,                 1,              0,               0,        1,            0,             1}},
...
BOOST_FIXTURE_TEST_CASE(tx_rejection_types, TxDownloadTestSetup)
{
    ...
    for (const auto segwit_parent : {true, false}) {
        for (const auto segwit_child : {true, false}) {
            ...
            for (const auto& [result, expected_behavior] : expected_behaviors) {
                ...
                const auto& [keep, unique_txids, package_to_validate] = txdownload_impl.RelayPoolRejectedTx(ptx_parent, state, nodeid, /*first_time_failure=*/true);
                ...
                txdownload_impl.RelayPoolRejectedTx(ptx_child, state, nodeid, /*first_time_failure=*/true);
...
BOOST_FIXTURE_TEST_CASE(handle_missing_inputs, TxDownloadTestSetup)
{
    CTxRelayPool& pool = *Assert(m_node.relaypool);
    FastRandomContext det_rand{true};
    node::TxDownloadOptions DEFAULT_OPTS{pool, det_rand, DEFAULT_MAX_ORPHAN_TRANSACTIONS, true};
    ...
    // their parents.
    CKey vault_key = GenerateRandomKey();
    CScript destination = GetScriptForDestination(PKHash(vault_key.GetPubKey()));
    int test_chain_height{100};

    TxValidationState state_orphan;
    state_orphan.Invalid(TxValidationResult::TX_MISSING_INPUTS, "");

    // Transactions are not all submitted to relaypool. Conserve the number of m_coinbase_txns we
    // consume, and only increment this index number when we would conflict with an existing
    // relaypool transaction.
    size_t coinbase_idx{0};

    for (int decisions = 0; decisions < (1 << 4); ++decisions) {
        auto mtx_single_parent = CreateValidRelayPoolTransaction(m_coinbase_txns[coinbase_idx], /*input_vout=*/0, test_chain_height, coinbaseKey,
                                                               destination, m_coinbase_txns[coinbase_idx]->vout[0].nValue, /*submit=*/false);
        ProveTxPowAtTip(mtx_single_parent);
        auto single_parent = MakeTransactionRef(mtx_single_parent);

        auto mtx_orphan = CreateValidRelayPoolTransaction(single_parent, /*input_vout=*/0, test_chain_height, vault_key,
                                                        destination, single_parent->vout[0].nValue, /*submit=*/false);
        ProveTxPowAtTip(mtx_orphan);
        ...
        // may be both recently rejected and found to violate some relaypool policy when validated
        // in a 1p1c.
        ...
        const bool parent_in_relaypool((decisions >> 3) & 1);
        ...
        if (parent_in_relaypool) {
            const auto relaypool_result = WITH_LOCK(::cs_main, return m_node.chainman->ProcessTransaction(single_parent));
            BOOST_CHECK(relaypool_result.m_result_type == RelayPoolAcceptResult::ResultType::VALID);
            coinbase_idx += 1;
        ...
        const unsigned int expected_parents = parent_recent_rej || parent_recent_conf || parent_in_relaypool ? 0 : 1;
        ...
        const auto ret_1p1c = txdownload_impl.RelayPoolRejectedTx(orphan, state_orphan, nodeid, /*first_time_failure=*/true);
        std::string err_msg;
    ...
    // Orphan with multiple parents
    {
        ...
        CAmount total_parent_value{0};
        for (int32_t i = 0; i < num_parents; ++i) {
            assert(coinbase_idx < m_coinbase_txns.size());
            const auto coinbase_tx = m_coinbase_txns[coinbase_idx++];
            auto mtx_parent = CreateValidRelayPoolTransaction(coinbase_tx, /*input_vout=*/0, test_chain_height,
                                                            coinbaseKey, destination, coinbase_tx->vout[0].nValue, /*submit=*/false);
            ProveTxPowAtTip(mtx_parent);
            auto ptx_parent = MakeTransactionRef(mtx_parent);
            parents.emplace_back(ptx_parent);
            outpoints.emplace_back(ptx_parent->GetHash(), 0);
            total_parent_value += ptx_parent->vout[0].nValue;
        }

        // Send all coins to 1 output.
        auto mtx_orphan = CreateValidRelayPoolTransaction(parents, outpoints, test_chain_height, {vault_key}, {{total_parent_value, destination}}, /*submit=*/false);
        ProveTxPowAtTip(mtx_orphan);
        auto orphan = MakeTransactionRef(mtx_orphan);
        ...
        // 1 parent in RecentRejectsReconsiderableFilter, the rest are unknown
        {
            ...
            const auto ret_1p1c_parent_reconsiderable = txdownload_impl.RelayPoolRejectedTx(orphan, state_orphan, nodeid, /*first_time_failure=*/true);
            std::string err_msg;
        ...
        // 1 parent in RecentRejectsReconsiderableFilter, the rest are confirmed
        {
            ...
            const auto ret_1recon_conf = txdownload_impl.RelayPoolRejectedTx(orphan, state_orphan, nodeid, /*first_time_failure=*/true);
            std::string err_msg;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            const auto ret_2_problems = txdownload_impl.RelayPoolRejectedTx(orphan, state_orphan, nodeid, /*first_time_failure=*/true);
            std::string err_msg;
    ...
    // Orphan with multiple inputs spending from a single parent
    {
        assert(coinbase_idx < m_coinbase_txns.size());
        const auto coinbase_tx = m_coinbase_txns[coinbase_idx];
        const CAmount first_output{coinbase_tx->vout[0].nValue / 2};
        const CAmount second_output{coinbase_tx->vout[0].nValue - first_output};
        auto mtx_parent_2outputs = CreateValidRelayPoolTransaction({coinbase_tx}, {{coinbase_tx->GetHash(), 0}}, test_chain_height, {coinbaseKey},
                                                                 {{first_output, destination}, {second_output, destination}}, /*submit=*/false);
        ProveTxPowAtTip(mtx_parent_2outputs);
        auto parent_2outputs = MakeTransactionRef(mtx_parent_2outputs);

        auto mtx_orphan = CreateValidRelayPoolTransaction({parent_2outputs}, {{parent_2outputs->GetHash(), 0}, {parent_2outputs->GetHash(), 1}},
                                                        test_chain_height, {vault_key}, {{parent_2outputs->vout[0].nValue + parent_2outputs->vout[1].nValue, destination}}, /*submit=*/false);
        ProveTxPowAtTip(mtx_orphan);
        auto orphan = MakeTransactionRef(mtx_orphan);
        ...
        // should only counts as 1 parent in the filter.
        {
            ...
            const auto ret_1p1c_2reconsiderable = txdownload_impl.RelayPoolRejectedTx(orphan, state_orphan, nodeid, /*first_time_failure=*/true);
            std::string err_msg;
