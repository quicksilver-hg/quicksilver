...
BOOST_AUTO_TEST_CASE(GetTxSigOpCost)
{
    ...
    // Multisig script (legacy counting)
    {
        ...
        // Do not use a valid signature to avoid using vault operations.
    ...
    // P2SH-wrapped P2WPKH is rejected and contributes no witness sigops.
    {
        ...
        BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, scriptWitness);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), coins, flags) == 0);
        assert(VerifyWithFlag(CTransaction(creationTx), spendingTx, flags) == SCRIPT_ERR_P2SH_WITNESS);
    }
    ...
    // P2SH-wrapped P2WSH is rejected and contributes no witness sigops.
    {
        CScript witnessScript = CScript() << 1 << ToByteVector(pubkey) << ToByteVector(pubkey) << 2 << OP_CHECKMULTISIGVERIFY;
