...
BOOST_FIXTURE_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection, PeerTest)
{
    ...
    peerman = PeerManager::make(*connman, *m_node.addrman, nullptr, *m_node.chainman, *m_node.relaypool, *m_node.warnings, {});
    NodeId id{0};
    std::vector<CNode*> nodes;

    // Connect a localhost peer.
    {
        ASSERT_DEBUG_LOG(strprintf("Added connection to 127.0.0.1:%i peer=1", Params().GetDefaultPort()));
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG(strprintf("rejected reason=already-connected dest=localhost peer_addr=127.0.0.1:%i", Params().GetDefaultPort()));
