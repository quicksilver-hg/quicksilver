...
BOOST_AUTO_TEST_CASE(compress_amounts)
{
    ...
    // Quicksilver: the compressor domain now extends to MAX_MONEY = 1e9 COIN.
    // Round-trip the ceiling (enc value is derived, not asserted, to avoid a brittle constant).
    BOOST_CHECK_EQUAL(DecompressAmount(CompressAmount(CAmount{1'000'000'000} * COIN)),
                      CAmount{1'000'000'000} * COIN);
    BOOST_CHECK_EQUAL(DecompressAmount(CompressAmount(MAX_MONEY)), MAX_MONEY);
