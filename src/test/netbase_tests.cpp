...
BOOST_AUTO_TEST_CASE(netbase_splithost)
{
    ...
    BOOST_CHECK(TestSplitHost("::9555", "::9555", 0));
    BOOST_CHECK(TestSplitHost(":9555", "", 9555));
    BOOST_CHECK(TestSplitHost("[]:9555", "", 9555));
...
BOOST_AUTO_TEST_CASE(netbase_lookupnumeric)
{
    ...
    BOOST_CHECK(TestParse("[fd58:5d14:ac23:1:2:3:4:5]", "[::]:0"));
    // and that a one-off resolves correctly
    BOOST_CHECK(TestParse("[fd59:5d14:ac23:1:2:3:4:5]", "[fd59:5d14:ac23:1:2:3:4:5]:65535"));
}
...
BOOST_AUTO_TEST_CASE(netpermissions_purged_tokens_are_rejected)
{
    bilingual_str error;
    NetWhitebindPermissions whitebind;

    // BIP37 bloom filtering and the BIP35 relaypool wire command were removed, so
    // the permissions that gated them must not parse -- not even as silent no-ops.
    // A stale -whitebind entry has to fail loudly rather than grant nothing.
    for (const char* token : {"bloomfilter", "bloom", "relaypool", "mem" "pool"}) {
        BOOST_CHECK_MESSAGE(!NetWhitebindPermissions::TryParse(std::string{token} + "@1.2.3.4:32", whitebind, error),
                            std::string{"permission token should be rejected: "} + token);
...
BOOST_AUTO_TEST_CASE(netpermissions_test)
{
    ...
    BOOST_CHECK(NetWhitebindPermissions::TryParse("addr@1.2.3.4:32", whitebindPermissions, error));
    BOOST_CHECK_EQUAL(whitebindPermissions.m_flags, NetPermissionFlags::Addr);
    BOOST_CHECK(NetWhitebindPermissions::TryParse("@1.2.3.4:32", whitebindPermissions, error));
    BOOST_CHECK_EQUAL(whitebindPermissions.m_flags, NetPermissionFlags::None);
    ...
    BOOST_CHECK(NetWhitebindPermissions::TryParse("addr,forcerelay@1.2.3.4:32", whitebindPermissions, error));
    // forcerelay should also activate the relay permission
    BOOST_CHECK_EQUAL(whitebindPermissions.m_flags, NetPermissionFlags::Addr | NetPermissionFlags::ForceRelay | NetPermissionFlags::Relay);
    BOOST_CHECK(NetWhitebindPermissions::TryParse("addr,relay,noban@1.2.3.4:32", whitebindPermissions, error));
    BOOST_CHECK_EQUAL(whitebindPermissions.m_flags, NetPermissionFlags::Addr | NetPermissionFlags::Relay | NetPermissionFlags::NoBan);
    BOOST_CHECK(NetWhitebindPermissions::TryParse("addr,forcerelay,noban@1.2.3.4:32", whitebindPermissions, error));
    ...
    // Allow dups
    BOOST_CHECK(NetWhitebindPermissions::TryParse("addr,relay,noban,noban@1.2.3.4:32", whitebindPermissions, error));
    BOOST_CHECK_EQUAL(whitebindPermissions.m_flags, NetPermissionFlags::Addr | NetPermissionFlags::Relay | NetPermissionFlags::NoBan | NetPermissionFlags::Download); // "noban" implies "download"
    ...
    // Detect invalid flag
    BOOST_CHECK(!NetWhitebindPermissions::TryParse("addr,forcerelay,oopsie@1.2.3.4:32", whitebindPermissions, error));
    BOOST_CHECK(error.original.find("Invalid P2P permission") != std::string::npos);

    // Check netmask error
    BOOST_CHECK(!NetWhitelistPermissions::TryParse("addr,forcerelay,noban@1.2.3.4:32", whitelistPermissions, connection_direction, error));
    ...
    BOOST_CHECK(NetWhitelistPermissions::TryParse("addr,forcerelay,noban,relay@1.2.3.4/32", whitelistPermissions, connection_direction, error));
    BOOST_CHECK_EQUAL(whitelistPermissions.m_flags, NetPermissionFlags::Addr | NetPermissionFlags::ForceRelay | NetPermissionFlags::NoBan | NetPermissionFlags::Relay);
    BOOST_CHECK(error.empty());
