...
FUZZ_TARGET(utxo_total_supply)
{
    ...
    const auto mock_time{ConsumeTime(fuzzed_data_provider, /*min=*/1296688602)}; // sandbox genesis block timestamp
    ...
    const auto PrepareNextBlock = [&]() {
        // Use OP_FALSE to avoid the duplicate-output check from hitting early
        ...
        CallOneOf(
            ...
            [&] {
                ...
                if (!was_valid) {
                    ...
                    assert(prev_utxo_stats.hash == utxo_stats.hash);
                }
