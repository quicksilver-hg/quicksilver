...
static TxValidationResult TESTED_TX_RESULTS[] = {
    ...
    TxValidationResult::TX_RELAYPOOL_POLICY,
    // Skip TX_NO_RELAYPOOL
...
FUZZ_TARGET(txdownloadman, .init = initialize)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 10000)
    {
        ...
        CallOneOf(
            ...
            [&] {
                txdownloadman.RelayPoolAcceptedTx(rand_tx);
            },
            [&] {
                ...
                node::RejectedTxTodo todo = txdownloadman.RelayPoolRejectedTx(rand_tx, state, rand_peer, first_time_failure);
...
FUZZ_TARGET(txdownloadman_impl, .init = initialize)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 10000)
    {
        ...
        CallOneOf(
            ...
            [&] {
                txdownload_impl.RelayPoolAcceptedTx(rand_tx);
            },
            [&] {
                TxValidationState state;
                state.Invalid(fuzzed_data_provider.PickValueInArray(TESTED_TX_RESULTS), "");
                ...
                node::RejectedTxTodo todo = txdownload_impl.RelayPoolRejectedTx(rand_tx, state, rand_peer, first_time_failure);
                Assert(first_time_failure || !todo.m_should_add_extra_compact_tx);
            ...
            [&] {
                ...
                // Exclude reconsiderable rejects because an orphan package may make an
                // individually insufficient-surplus parent acceptable.
            ...
            [&] {
                ...
                if (ptx) {
                    ...
                    // orphanage longer. Later iterations might call RelayPoolAcceptedTx or
                    // RelayPoolRejectedTx with a different error.
                    TxValidationState state_missing_inputs;
                    state_missing_inputs.Invalid(TxValidationResult::TX_MISSING_INPUTS, "");
                    txdownload_impl.RelayPoolRejectedTx(ptx, state_missing_inputs, rand_peer, fuzzed_data_provider.ConsumeBool());
                }
            }
        );
