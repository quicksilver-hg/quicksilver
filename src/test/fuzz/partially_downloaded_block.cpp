...
FUZZ_TARGET(partially_downloaded_block, .init = initialize_pdb)
{
    ...
    // Set of available transactions (relaypool or extra_txn)
    ...
    for (size_t i = 1; i < block->vtx.size(); ++i) {
        ...
        bool add_to_relaypool{fuzzed_data_provider.ConsumeBool()};
        ...
        if (add_to_relaypool && !pool.exists(GenTxid::Txid(tx->GetHash()))) {
