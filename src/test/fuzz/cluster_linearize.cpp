        ...
        while (!queue.empty() && iterations_left) {
            ...
            for (auto split : und) {
                ...
                if (inc_none || inc.Overlaps(m_depgraph.Ancestors(split))) {
                    ...
                    if (new_inc.workrate > best.workrate) best = new_inc;
                    break;
                }
            }
        }
...
public:
    ...
    {
        // Best solution so far.
        SetInfo<SetType> best{m_todo, m_depgraph.WorkRate(m_todo)};
        ...
        for (uint64_t x = 1; x < limit; ++x) {
            ...
            if (cur.workrate > best.workrate) best = cur;
        }
        return best;
    }
};
...
FUZZ_TARGET(clusterlin_depgraph_sim)
{
     ...
     *  sim[i]->first is its individual workrate, and sim[i]->second is its set of ancestors. */
    std::array<std::optional<std::pair<WorkFrac, TestBitSet>>, TestBitSet::Size()> sim;
    ...
    auto check_fn = [&](ClusterIndex i) {
        ...
        if (sim[i].has_value()) {
            // Compare workrate.
            assert(real.WorkRate(i) == sim[i]->first);
    ...
    LIMITED_WHILE(provider.remaining_bytes() > 0, 1000) {
        ...
        if (num_tx_sim == 0 || ((command % 3) <= 0 && num_tx_sim < TestBitSet::Size())) {
            // AddTransaction.
            auto work = provider.ConsumeIntegralInRange<int64_t>(-0x8000000000000, 0x7ffffffffffff);
            auto size = provider.ConsumeIntegralInRange<int32_t>(1, 0x3fffff);
            WorkFrac workrate{work, size};
            // Apply to DepGraph.
            auto idx = real.AddTransaction(workrate);
            ...
            // Update sim.
            sim[idx] = {workrate, TestBitSet::Singleton(idx)};
            ++num_tx_sim;
            continue;
        }
...
FUZZ_TARGET(clusterlin_chunking)
{
    ...
    // Verify that chunk workrates are monotonically non-increasing.
    for (size_t i = 1; i < chunking.size(); ++i) {
        assert(!(chunking[i] >> chunking[i - 1]));
    }

    // Naively recompute the chunks (each is the highest-workrate prefix of what remains).
    auto todo = depgraph.Positions();
    for (const auto& chunk_workrate : chunking) {
        assert(todo.Any());
        SetInfo<TestBitSet> accumulator, best;
        for (ClusterIndex idx : linearization) {
            if (todo[idx]) {
                accumulator.Set(depgraph, idx);
                if (best.workrate.IsEmpty() || accumulator.workrate >> best.workrate) {
                    best = accumulator;
                }
            }
        }
        assert(chunk_workrate == best.workrate);
...
FUZZ_TARGET(clusterlin_ancestor_finder)
{
    ...
    while (todo.Any()) {
        ...
        assert(depgraph.WorkRate(best_anc.transactions) == best_anc.workrate);
        ...
        for (auto i : todo) {
            ...
            if (!real_best_anc.has_value() || info.workrate > real_best_anc->workrate) {
                real_best_anc = info;
            }
        }
...
FUZZ_TARGET(clusterlin_search_finder)
{
    ...
    while (todo.Any()) {
        ...
        assert(depgraph.WorkRate(found.transactions) == found.workrate);
        if (!init_best.workrate.IsEmpty()) assert(found.workrate >= init_best.workrate);
        // Check that it is topologically valid.
        ...
        if (iterations_done < max_iterations) {
            ...
            assert(found.workrate >= simple.workrate);
            ...
            assert(found.workrate >= anc.workrate);
            ...
            if (todo.Count() <= 12) {
                ...
                assert(exhaustive.workrate == found.workrate);
                ...
                assert(exhaustive.workrate >= simple.workrate);
                if (simple_iters < MAX_SIMPLE_ITERATIONS) {
                    assert(exhaustive.workrate == simple.workrate);
                }
            }
...
FUZZ_TARGET(clusterlin_linearization_chunking)
{
    ...
    while (todo.Any()) {
        ...
        // Verify that it matches the workrates of the chunks of chunking.
        assert(chunking.NumChunksLeft() == chunking_left.size());
        for (ClusterIndex i = 0; i < chunking.NumChunksLeft(); ++i) {
            assert(chunking.GetChunk(i).workrate == chunking_left[i]);
        }
        ...
        for (ClusterIndex i = 0; i < chunking.NumChunksLeft(); ++i) {
            ...
            // Chunk workrates must be monotonically non-increasing.
            if (i > 0) assert(!(chunk_info.workrate >> chunking.GetChunk(i - 1).workrate));
            // Chunks must be a subset of what is left of the linearization.
            assert(chunk_info.transactions.IsSubsetOf(todo));
            // Chunks' claimed workrates must match their transactions' aggregate workrate.
            assert(depgraph.WorkRate(chunk_info.transactions) == chunk_info.workrate);
            // Chunks must be the highest-workrate remaining prefix.
            SetInfo<TestBitSet> accumulator, best;
            ...
            assert(best.workrate == chunk_info.workrate);
        ...
        // - The claimed intersection workrate matches its transactions.
        assert(intersect.workrate == depgraph.WorkRate(intersect.transactions));
        // - The intersection may only be empty if its input is empty.
        assert(intersect.transactions.Any() == subset.transactions.Any());
        // - The intersection workrate must be as high as the input.
        assert(intersect.workrate >= subset.workrate);
        ...
        for (ClusterIndex i = 0; i < chunking.NumChunksLeft(); ++i) {
            ...
            if (!reintersect.workrate.IsEmpty()) {
                assert(reintersect.workrate <= intersect.workrate);
            }
        }
...
FUZZ_TARGET(clusterlin_postlinearize_moved_leaf)
{
    ...
    // increasing its work, and then post-linearizing, results in something as good as the
    // original. This guarantees that in a replacement that replaces a transaction with one of the same
    // size but higher work, applying the "remove conflicts, append new transaction, postlinearize"
    // process will never worsen linearization quality.

    // Construct an arbitrary graph and a work increment from the fuzz input.
    SpanReader reader(buffer);
    DepGraph<TestBitSet> depgraph;
    int32_t work_inc{0};
    try {
        uint64_t work_inc_code;
        reader >> Using<DepGraphFormatter>(depgraph) >> VARINT(work_inc_code);
        work_inc = work_inc_code & 0x3ffff;
    } catch (const std::ios_base::failure&) {}
    ...
    // Compare diagrams (applying the work delta after computing the old one).
    auto old_chunking = ChunkLinearization(depgraph, lin);
    depgraph.WorkRate(lin_leaf.back()).work += work_inc;
