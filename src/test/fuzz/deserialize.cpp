...
FUZZ_TARGET_DESERIALIZE(partially_signed_transaction_deserialize, {
    PartiallySignedQuicksilverTransaction partially_signed_transaction;
...
FUZZ_TARGET_DESERIALIZE(psqt_input_deserialize, {
    PSQTInput psqt_input;
    DeserializeFromFuzzingInput(buffer, psqt_input);
})
FUZZ_TARGET_DESERIALIZE(psqt_output_deserialize, {
    PSQTOutput psqt_output;
    DeserializeFromFuzzingInput(buffer, psqt_output);
})
...
FUZZ_TARGET(address_deserialize, .init = initialize_deserialize)
{
    ...
    // A CAddress in V1 mode will roundtrip in both network encodings and as V2 disk.
    if (ser_enc.enc == CNetAddr::Encoding::V1) {
        ...
        // A CAddress in V2 mode will roundtrip as V2 network/disk, and as V1 network
