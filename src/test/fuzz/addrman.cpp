...
{
    static const auto testing_setup = MakeNoLogFileContext<>(ChainType::SANDBOX);
...
void FillAddrman(AddrMan& addrman, FuzzedDataProvider& fuzzed_data_provider)
{
    ...
    for (size_t i = 0; i < num_sources; ++i) {
        ...
        for (size_t j = 0; j < num_addresses; ++j) {
            const auto addr = CAddress{CService{RandAddr(fuzzed_data_provider, fast_random_context), 9555}, NODE_NETWORK};
