...
FUZZ_TARGET(key, .init = initialize_key)
{
...

    {
        ...
        assert(IsValidDestination(CTxDestination{PKHash{pubkey}}));
        assert(IsValidDestination(CTxDestination{WitnessV0KeyHash{pubkey}}));
    }
...

    {
        ...
        assert(!IsSegWitOutput(tx_pubkey_script));
        assert(!IsSegWitOutput(tx_multisig_script));
