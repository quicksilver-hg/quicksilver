...
P ConsumeDeserializationParams(FuzzedDataProvider& fuzzed_data_provider) noexcept
{
    ...
    if constexpr (std::is_same_v<P, CAddress::SerParams>) {
        const auto fmt{PickValue(fuzzed_data_provider, ADDR_FORMATS)};
        const auto enc{fmt == CAddress::Format::Disk ? CNetAddr::Encoding::V2 : PickValue(fuzzed_data_provider, ADDR_ENCODINGS)};
        return P{{enc}, fmt};
    }
}
