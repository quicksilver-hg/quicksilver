...
BOOST_FIXTURE_TEST_CASE(coinstatsindex_requires_current_database_version, TestChain100Setup)
{
    {
        CoinStatsIndex index{interfaces::MakeChain(m_node), 1 << 20};
        BOOST_REQUIRE(index.Init());
    ...
    // A fresh index writes its semantic format version alongside its committed
    // best block. Removing the marker reproduces a database created before
    // Frame-B mint allowances were included in the cumulative reward totals.
    {
        const fs::path db_path{gArgs.GetDataDirNet() / "indexes" / "coinstats" / "db"};
        CDBWrapper db{DBParams{.path = db_path, .cache_bytes = 1 << 20}};
        uint8_t version{0};
        BOOST_REQUIRE(db.Read(uint8_t{'V'}, version));
        BOOST_CHECK_EQUAL(version, 2);
        BOOST_REQUIRE(db.Erase(uint8_t{'V'}));
    }

    {
        CoinStatsIndex old_index{interfaces::MakeChain(m_node), 1 << 20};
        BOOST_CHECK(!old_index.Init());
    }

    // The production -reindex path passes f_wipe=true and rebuilds the index
    // from block data using the current accounting rules.
    CoinStatsIndex rebuilt_index{interfaces::MakeChain(m_node), 1 << 20, false, true};
    BOOST_REQUIRE(rebuilt_index.Init());
    BOOST_REQUIRE(rebuilt_index.StartBackgroundSync());
    IndexWaitSynced(rebuilt_index, *Assert(m_node.shutdown_signal));
    rebuilt_index.Stop();
}
