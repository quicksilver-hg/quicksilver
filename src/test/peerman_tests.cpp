...
BOOST_FIXTURE_TEST_SUITE(peerman_tests, SandboxingSetup)
...
static void mineBlock(const node::NodeContext& node, std::chrono::seconds block_time)
{
    ...
    SolveBlockPoW(block, node.chainman->GetConsensus());
...
{
    std::unique_ptr<PeerManager> peerman = PeerManager::make(*m_node.connman, *m_node.addrman, nullptr, *m_node.chainman, *m_node.relaypool, *m_node.warnings, {});
