...
uint256 static SignatureHashOld(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    // Serialize and hash. Mirror the consensus signature serializer
    // (interpreter.cpp CTransactionSignatureSerializer): version, vin, vout, nLockTime —
    // and NOT the per-tx PoW tail (nAnchorHeight/nPowNonce/nCycle). The tail is ground
    // AFTER signing (nPowNonce is mutated to find the cycle), so it must be excluded from
    // the signature hash or every signature would be invalidated by grinding.
    HashWriter ss{};
    ss << txTmp.version << txTmp.vin << txTmp.vout << txTmp.nLockTime << nHashType;
    return ss.GetHash();
}
