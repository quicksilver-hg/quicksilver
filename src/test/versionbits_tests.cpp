...
BOOST_FIXTURE_TEST_CASE(versionbits_computeblockversion, BlockVersionTest)
{
    ...
    for (const auto& chain_type: {ChainType::MAIN, ChainType::PUBLIC_TEST, ChainType::SANDBOX}) {
    ...
    {
        // Use sandbox/testdummy to ensure we always exercise some
        // deployment that's not always/never active
        ArgsManager args;
        args.ForceSetArg("-vbparams", "testdummy:1199145601:1230767999"); // January 1, 2008 - December 31, 2008
        const auto chainParams = CreateChainParams(args, ChainType::SANDBOX);
        check_computeblockversion(vbcache, chainParams->GetConsensus(), Consensus::DEPLOYMENT_TESTDUMMY);
    }

    {
        // Use sandbox/testdummy to ensure we always exercise the
