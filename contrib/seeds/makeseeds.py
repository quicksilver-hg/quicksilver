...
PATTERN_AGENT = re.compile(r"^/Quicksilver:")
...
def parse_args():
    argparser = argparse.ArgumentParser(description='Generate a list of Quicksilver node seed ip addresses.')
...
def main():
    ...
    # Require a compatible protocol version.
    ips = [ip for ip in ips if ip['version'] >= MIN_PROTOCOL_VERSION]
    print(f'{ip_stats(ips):s} Require compatible protocol version', file=sys.stderr)
    ...
    # Require a Quicksilver user agent.
    ips = [ip for ip in ips if PATTERN_AGENT.match(ip['agent'])]
    print(f'{ip_stats(ips):s} Require a Quicksilver user agent', file=sys.stderr)
    # Sort by availability (and use last success as tie breaker)
    ips.sort(key=lambda x: (x['uptime'], x['lastsuccess'], x['ip']), reverse=True)
    # Filter out hosts with multiple Quicksilver ports, these are likely abusive
    ips = filtermultiport(ips)
    print(f'{ip_stats(ips):s} Filter out hosts with multiple Quicksilver ports', file=sys.stderr)
