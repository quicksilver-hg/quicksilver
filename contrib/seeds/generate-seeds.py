...
The output will be several data structures with the peers in binary format:
   ...
   static const uint8_t chainparams_seed_{main,publictest}[]={
   ...
   }
...
def main():
    ...
    g = sys.stdout
    indir = sys.argv[1]
    g.write('#ifndef QUICKSILVER_CHAINPARAMSSEEDS_H\n')
    g.write('#define QUICKSILVER_CHAINPARAMSSEEDS_H\n')
    g.write('/**\n')
    g.write(' * List of fixed seed nodes for the Quicksilver network\n')
    ...
    g.write('\n')
    with open(os.path.join(indir,'nodes_publictest.txt'), 'r', encoding="utf8") as f:
        process_nodes(g, f, 'chainparams_seed_publictest')
    g.write('#endif // QUICKSILVER_CHAINPARAMSSEEDS_H\n')
