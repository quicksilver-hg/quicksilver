...
# Quicksilver block headers are 252 bytes: the 84-byte pre-pow prefix (through
# nNonce, and including nCongestion) is followed by the 42-edge Cuckatoo cycle
# serialized as little-endian uint32 (see src/primitives/block.h). GetHash()
# covers the whole 252 bytes, so a tool that reads the wrong length both
# mis-hashes every block and mis-computes the remaining block length -- which
# manifests as this script never terminating, not as an error.
HEADER_LEN = 84 + 42 * 4
...
def read_xor_key(blocks_path):
    ...
    with open(xor_filename, "rb") as xor_file:
        xor_key = xor_file.read(NUM_XOR_BYTES)
    if len(xor_key) != NUM_XOR_BYTES:
        raise ValueError(f"Invalid XOR key length in {xor_filename}")
    return xor_key
...
class BlockDataCopier:
    ...
    def run(self):
        while self.blkCountOut < len(self.blkindex):
            ...
            inLen = su[0] - HEADER_LEN # length without header
            blk_hdr = self.read_xored(self.inF, HEADER_LEN)
...
if __name__ == '__main__':
    ...
    # Defaults are Quicksilver mainnet. Keep these in sync with CMainParams in
    # src/kernel/chainparams.cpp; a stale value here silently linearizes against
    # the wrong network instead of failing.
    if 'netmagic' not in settings:
        settings['netmagic'] = '48475153'
    if 'genesis' not in settings:
        settings['genesis'] = 'c9144acae20212e57e9e59f34a681bd25f55d81438001c424ebb95cd4869bcf3'
