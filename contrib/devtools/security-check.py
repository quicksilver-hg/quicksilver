...
def check_ELF_FORTIFY(binary) -> bool:
    ...
    # _FORTIFY_SOURCE only leaves an imported _chk symbol when the program calls
    # a libc function that the compiler can fortify. quicksilver-cli currently
    # has no such calls, so the binary has no runtime marker even though every
    # translation unit is compiled with _FORTIFY_SOURCE=3.
    if 'Quicksilver RPC client version ' in binary.strings:
        return True
