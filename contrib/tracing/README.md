...
The bpftrace examples contain a relative path to the `quicksilverd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `quicksilverd` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof quicksilverd)
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof quicksilverd)
```
...
```
$ ./build/bin/quicksilverd -publictest -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof quicksilverd)
```
